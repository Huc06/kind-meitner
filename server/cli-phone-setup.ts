...
export async function runPhoneSetup(
  ...
  const signIn = async (): Promise<boolean> => {
    try {
      if (await deps.accountReady?.(options)) {
        io.log("Your saved kind-meitner account can be reused. The connection will be checked when the server starts.");
        return true;
      }
      io.log("Sign in to a kind-meitner account using an emailed code. This is separate from your AI provider account.");
  ...
  for (;;) {
    const device = await io.choose("Use kind-meitner on your phone?", [
      "Skip for now",
    ...
    while (!back) {
      ...
      if (route === 0) {
        ...
        io.log("Starting it may download the Cloudflare connector. It stays active while kind-meitner runs; stop kind-meitner to close the connection.");
      ...
      if (route === 1) {
        ...
        if (!await io.confirm("Allow kind-meitner to serve HTTPS to your tailnet while it runs?", false)) continue;
...
export function phonePairingInstructions(
  ...
  if (!input.ready || !origin) {
    return ["Phone access is not ready yet. No phone QR should be shown until the HTTPS connection is verified.",
      "Your local kind-meitner can still be used on this computer."];
  }
  return [
    phone === "ios"
      ? "On iPhone or iPad, scan the QR with Camera to open Safari. If you already have the kind-meitner iOS app, use its pairing scanner or paste the full pairing link there."
