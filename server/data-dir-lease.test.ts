...
function tempDataDir(): string {
  const dir = mkdtempSync(join(tmpdir(), "kind-meitner-data-dir-lease-"));
  dirs.push(dir);
  return dir;
}
...
describe("kind-meitner data-directory lease", () => {
  it("holds one directory until its matching handle releases it", () => {
    const dir = tempDataDir();
    const lease = acquireDataDirLease(dir);
    const stored = JSON.parse(readFileSync(join(dir, "kind-meitner-server.lease"), "utf8"));
  ...
  it("will not release a lease whose owner token changed", () => {
    const dir = tempDataDir();
    const path = join(dir, "kind-meitner-server.lease");
    const lease = acquireDataDirLease(dir);
