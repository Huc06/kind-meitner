...
async function fixture(test: (f: any) => Promise<void>, fakeEnv: NodeJS.ProcessEnv = {}) {
  ...
  const cli = (...args: string[]) => runControlKindMeitner(args, { env: { KIND_MEITNER_URL: session.info.url } }) as Promise<any>;
...
it("returns a nested coordinated result after Claude retries a transient provider exit", async () => {
  const scratch = mkdtempSync(join(tmpdir(), "kind-meitner-coordination-retry-"));
  try {
