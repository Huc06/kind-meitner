...
// agent process; engine commands, session names, and credentials stay in kind-meitner.
...
export async function browserProxyRequest(
  ...
  if (message.method === "initialize") return {
    jsonrpc: "2.0", id,
    result: { protocolVersion: "2024-11-05", capabilities: { tools: {} }, serverInfo: { name: "kind-meitner-browser", version: "1" } },
  };
  ...
  try {
    ...
    if (!connection.token || url.protocol !== "http:" || !["127.0.0.1", "localhost", "[::1]"].includes(url.hostname) || url.username || url.password || url.pathname !== "/" || url.search || url.hash) {
      throw new Error("Browser connection is not configured. Start a new bot turn from kind-meitner.");
    }
...
function run(): void {
  const connection = { url: process.env.KIND_MEITNER_HARNESS_URL ?? "", token: process.env.KIND_MEITNER_BROWSER_TOKEN ?? "" };
