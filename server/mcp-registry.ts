...
/** Names used to route kind-meitner's built-in MCP proxies and their ephemeral
...
export function isHarnessOwnedMcpEnvName(name: string): boolean {
  return name === "ELECTRON_RUN_AS_NODE" || name.startsWith("KIND_MEITNER_") || name.startsWith("OGB_");
}
...
function environmentNameError(name: string): string | null {
  ...
  if (isHarnessOwnedMcpEnvName(name)) {
    return `Environment variable “${name}” is reserved by kind-meitner.`;
  }
  return null;
}
...
export function mcpServerNameError(name: string): string | null {
  ...
  if (RESERVED_MCP_NAMES.has(name)) return "That name is reserved by kind-meitner.";
  return null;
}
