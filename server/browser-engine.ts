...
export function browserRestoreKey(session: string): string {
  ...
  return `kind-meitner-${createHash("sha256").update(session).digest("hex")}`;
}
...
function managedBrowserConfigPath(env: NodeJS.ProcessEnv): string {
  return join(browserSessionsDirectory(env), "..", "kind-meitner-managed-config.json");
}
...
function packagedBrowser(options: BrowserLookupOptions) {
  const resources = (options.env ?? process.env).KIND_MEITNER_RESOURCES_PATH;
...
/** KIND_MEITNER_AGENT_BROWSER_PATH, then the complete desktop bundle, pinned download, then
...
export function resolveAgentBrowserBinary(options: BrowserLookupOptions = {}): string | null {
  ...
  const override = env.KIND_MEITNER_AGENT_BROWSER_PATH?.trim();
...
export function browserEngineStatus(options: BrowserLookupOptions = {}): BrowserEngineStatus {
  ...
  if (bundle && (options.exists ?? existsSync)(bundle.directory)) {
    return { kind: "unavailable", reason: "The desktop browser bundle is incomplete. Reinstall or update kind-meitner to repair it.", installable: false };
  }
  const platform = options.platform ?? process.platform;
...
export function agentBrowserIntegration(input: {
  ...
  const env: Record<string, string> = {
    ...
    // kind-meitner owns launch settings. A user's unrelated native CLI config must not
...
export function agentBrowserFrame(input: {
  ...
  const file = join(tmpdir(), `kind-meitner-browser-${randomUUID()}.png`);
