// `kind-meitner serve --domain maus.example.com`: HTTPS on your own domain
...
/** KIND_MEITNER_CADDY_PATH, then the pinned download, then a `caddy` on PATH. */
export function resolveCaddyBinary(options: { dataDir: string; env?: NodeJS.ProcessEnv; platform?: NodeJS.Platform; arch?: string; exists?: (p: string) => boolean } ): string | null {
  ...
  const override = env.KIND_MEITNER_CADDY_PATH?.trim();
...
export async function ensureCaddy(options: {
  ...
  const scratch = mkdtempSync(join(tmpdir(), "kind-meitner-caddy-"));
  try {
...
export function caddyfileFor(input: { domain: string; appPort: number; webhookPort: number }): string {
  return [
    "# Written by kind-meitner serve --domain. Edit the command, not this file.",
    "{",
    "\tadmin off",
    "\tlog {",
    "\t\tlevel WARN",
