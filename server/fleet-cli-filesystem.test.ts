...
vi.mock("node:child_process", async (importOriginal) => {
  ...
  return {
    ...
    execFileSync: (command: string, args: string[], options: ExecFileSyncOptionsWithStringEncoding) => {
      ...
      if (command === "/usr/bin/getent") return fixture.account || `kind-meitner-acme:x:${process.getuid!()}:${process.getgid!()}:fixture:${fixture.home}:/usr/sbin/nologin\n`;
...
describe.skipIf(process.platform === "win32" || process.getuid?.() === 0)("fleet tenant filesystem boundary in an isolated home", () => {
  ...
  beforeEach(() => {
    root = realpathSync(mkdtempSync(join(tmpdir(), "kind-meitner-fleet-files-")));
    fixture.home = join(root, "acme");
    fixture.account = "";
    fixture.calls = [];
    mkdirSync(fixture.home, { mode: 0o700 });
    data = join(fixture.home, ".kind-meitner");
  ...
  it("creates private tenant files and replaces them atomically through the real bounded child", () => {
    deps.mkdir(data, 0o700, "kind-meitner-acme");
    expect(deps.readText(file, "kind-meitner-acme")).toBeNull();
    deps.writeText(file, '{"signIn":{"admins":["a@example.test"]}}', 0o600, "kind-meitner-acme");
    const oldInode = statSync(file).ino;
    deps.writeText(file, '{"signIn":{"admins":["b@example.test"]}}', 0o600, "kind-meitner-acme");
    expect(deps.readText(file, "kind-meitner-acme")).toContain("b@example.test");
  ...
  it.each(["symbolic", "hard"])("refuses %s links without reading or modifying their target", (kind) => {
    ...
    expect(() => deps.readText(file, "kind-meitner-acme")).toThrow("could not read workspace file");
    expect(() => deps.writeText(file, "replacement", 0o600, "kind-meitner-acme")).toThrow("could not write workspace file");
  ...
  it("refuses a linked ancestor and never rewrites another directory", () => {
    ...
    expect(() => deps.readText(file, "kind-meitner-acme")).toThrow("could not read workspace file");
    expect(() => deps.writeText(file, "replacement", 0o600, "kind-meitner-acme")).toThrow("could not write workspace file");
    expect(() => deps.mkdir(data, 0o700, "kind-meitner-acme")).toThrow("could not mkdir workspace file");
  ...
  it("rejects special file types and oversized reads without echoing content", () => {
    ...
    expect(() => deps.readText(large, "kind-meitner-acme")).toThrow("check ownership, links, file size and permissions");
  });

  it("rejects root identities and paths outside the account home before launching file I/O", () => {
    fixture.account = `kind-meitner-acme:x:0:0:fixture:${fixture.home}:/bin/sh`;
    expect(() => deps.writeText(file, "secret", 0o600, "kind-meitner-acme")).toThrow("unsafe filesystem identity or path");
    fixture.account = "";
    expect(() => deps.writeText(join(root, "outside"), "secret", 0o600, "kind-meitner-acme")).toThrow("unsafe filesystem identity or path");
    expect(() => deps.readText(join(data, "..", "..", "outside"), "kind-meitner-acme")).toThrow("unsafe filesystem identity or path");
  ...
  it("updates managed models through the real tenant helper and refuses a symlink without touching its target", async () => {
    fixture.home = join(root, "var/lib/kind-meitner/acme");
    ...
    const files = new Map([[layout.registryFile, JSON.stringify(registry)], [join(layout.instancesDir, "acme.env"), "KIND_MEITNER_ADMIN_URL=https://admin.example.test\nKIND_MEITNER_ADMIN_WORKSPACE=acme\n"]]);
  ...
  it("summarizes the current month inside the actual helper without returning raw ledger rows", () => {
    ...
    const result = deps.usage(data, "kind-meitner-acme", now);
  ...
  it("ignores malformed, torn and out-of-date records, with inclusive UTC month/start and now boundaries", () => {
    ...
    const reference = summarizeUsage(readUsage(data, { from: new Date("2026-09-01T00:00:00Z"), to: now }), "bot").total;
    expect(deps.usage(data, "kind-meitner-acme", now)).toEqual({ turns: reference.turns, costUsd: reference.costUsd, billableUsd: null });
    expect(deps.usage(data, "kind-meitner-acme", now)).toEqual({ turns: 2, costUsd: 0.75, billableUsd: null });
  });
  ...
  it("keeps turns without valid prices unpriced and rejects numeric overflow without exposing rows", () => {
    ...
    writeFileSync(monthly, rows.map((row) => JSON.stringify(row)).join("\n"));
    expect(deps.usage(data, "kind-meitner-acme", now)).toEqual({ turns: 3, costUsd: null, billableUsd: null });
    ...
    expect(() => deps.usage(data, "kind-meitner-acme", now)).toThrow(/^could not usage workspace file as kind-meitner-acme: check ownership, links, file size and permissions$/);
  });
  ...
  it("returns empty totals for missing nested data, missing usage directory and missing month", () => {
    ...
    expect(deps.usage(join(fixture.home, "absent", "nested", ".kind-meitner"), "kind-meitner-acme", now)).toEqual(empty);
    expect(deps.usage(data, "kind-meitner-acme", now)).toEqual(empty);
    mkdirSync(data);
    expect(deps.usage(data, "kind-meitner-acme", now)).toEqual(empty);
    const monthly = usageFile();
    expect(deps.usage(data, "kind-meitner-acme", now)).toEqual(empty);
  ...
  it.each(["symlink", "fifo", "directory", "hardlink", "ancestor", "oversized"])("refuses an unsafe %s usage ledger promptly without exposing raw content", (kind) => {
    ...
    expect(() => deps.usage(targetData, "kind-meitner-acme", now)).toThrow(/^could not usage workspace file as kind-meitner-acme: check ownership, links, file size and permissions$/);
