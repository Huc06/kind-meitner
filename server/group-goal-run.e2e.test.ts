...
const completeReplies = [
  [
    "Scout should verify the draft.\n<kind-meitner-goal>{\"status\":\"continue\",",
    "\"next\":\"Scout\",\"instruction\":\"Verify the draft and report evidence\",\"detail\":\"Draft prepared\"}</kind-meitner-goal>",
  ],
  "The draft is accurate and the cited evidence checks out.",
  "The verified draft is ready to ship.\n<kind-meitner-goal>{\"status\":\"completed\",\"detail\":\"Draft produced and independently verified.\"}</kind-meitner-goal>",
];

const loopReplies = Array.from({ length: 13 }, (_, index) =>
  index % 2 === 0
    ? `More work is needed.\n<kind-meitner-goal>{"status":"continue","next":"Looper","instruction":"Try approach ${index / 2 + 1}","detail":"Still working"}</kind-meitner-goal>`
...
beforeAll(async () => {
  home = mkdtempSync(join(tmpdir(), "kind-meitner-goal-run-"));
    ...
    instances: {
      ...
      busyGoal: {
        ...
        environment: {
          ...
          FAKE_CLAUDE_REPLIES: JSON.stringify([
            ...
            "The queued team goal is complete.\n<kind-meitner-goal>{\"status\":\"completed\",\"detail\":\"Waited for the lead, then completed normally.\"}</kind-meitner-goal>",
          ]),
      ...
      busyWorkerLead: {
        ...
        environment: {
          FAKE_CLAUDE_MODE: "happy",
          FAKE_CLAUDE_REPLIES: JSON.stringify([
            "I am delegating the research.\n<kind-meitner-goal>{\"status\":\"continue\",\"next\":\"Busy specialist\",\"instruction\":\"Research the answer and report evidence\",\"detail\":\"Waiting for specialist research.\"}</kind-meitner-goal>",
            "The specialist's evidence resolves the goal.\n<kind-meitner-goal>{\"status\":\"completed\",\"detail\":\"Specialist research incorporated after their direct task finished.\"}</kind-meitner-goal>",
          ]),
      ...
      stopScopedLead: {
        ...
        environment: {
          ...
          FAKE_CLAUDE_REPLIES: JSON.stringify([
            "I am delegating this scheduled goal.\n<kind-meitner-goal>{\"status\":\"continue\",\"next\":\"Delayed worker\",\"instruction\":\"Finish the scheduled analysis\",\"detail\":\"Waiting for the delayed worker.\"}</kind-meitner-goal>",
            "This is unrelated direct work and should be stopped.",
            "The scheduled analysis is now complete.\n<kind-meitner-goal>{\"status\":\"completed\",\"detail\":\"Scheduled goal survived the coordinator's direct Stop.\"}</kind-meitner-goal>",
          ]),
      ...
      routineGoal: {
        ...
        environment: {
          ...
          FAKE_CLAUDE_REPLIES: JSON.stringify([
            "The scheduled review is complete.\n<kind-meitner-goal>{\"status\":\"completed\",\"detail\":\"Scheduled team review completed.\"}</kind-meitner-goal>",
          ]),
...
describe("goal-driven channel runs", () => {
  it("returns to the lead until the goal is completed and appends a clean receipt", async () => {
    ...
    expect(JSON.stringify(current.messages)).not.toContain("<kind-meitner-goal>");
  });
