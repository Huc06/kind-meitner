...
describe.skipIf(process.platform === "win32")("fleet agent over its socket", () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), "kind-meitner-fleet-agent-"));
  ...
  async function boot(m: ReturnType<typeof machine>) {
    const logs: string[] = [];
    const server = await startFleetAgent({ socketPath, node: "/usr/bin/node", script: "/usr/lib/node_modules/kind-meitner/cli.js", root: m.root, deps: m.deps, auditFile: join(m.root, "audit.jsonl"), licenseKey: "omb1.k", now: () => new Date("2026-09-10T12:00:00Z") }, { log: (line) => logs.push(line) });
  ...
  it("creates an OpenRouter-only workspace and updates its catalog through the real isolated socket", async () => {
    ...
    const configPath = join(root, "var/lib/kind-meitner/acme/.kind-meitner/config.json");
    ...
    const path = join(root, "var/lib/kind-meitner/acme/.config/opencode/opencode.json");
  ...
  it("creates, lists with this month's usage, edits users, suspends, deletes, and audits every action", async () => {
    ...
    expect(m.calls).toContain("useradd --system --create-home --home-dir " + join(root, "var/lib/kind-meitner/acme") + " --shell /usr/sbin/nologin --user-group kind-meitner-acme");
    expect(JSON.parse(m.files.get(join(root, "var/lib/kind-meitner/acme/.kind-meitner/config.json"))!)).toMatchObject({ anthropic: { key: "sk-ant-fixture", url: "https://admin.example.test/api/gateway/acme/anthropic" }, budgets: { monthlyUsd: 40 } });
    expect(m.files.get(join(root, "etc/kind-meitner/instances/acme.env"))).toContain("KIND_MEITNER_LICENSE_KEY=omb1.k");
    expect(m.files.get(join(root, "etc/kind-meitner/instances/acme.env"))).toContain("KIND_MEITNER_ADMIN_URL=https://admin.example.test");
    expect(m.files.get(join(root, "etc/kind-meitner/instances/acme.env"))).toContain("KIND_MEITNER_ADMIN_WORKSPACE=acme");
    ...
    const dataDir = join(root, "var/lib/kind-meitner/acme/.kind-meitner");
    ...
    expect(m.calls).toContain(`usage ${dataDir} as kind-meitner-acme`);
    ...
    expect(m.calls).toContain("systemctl disable --now kind-meitner@acme.service");
    ...
    expect(m.files.get(layout.fenceFile)).toContain("kind-meitner-acme");
  ...
  it("lists service status without reading tenant usage or mutating the fleet", async () => {
    ...
    expect(m.calls).toEqual(["systemctl is-active kind-meitner@alpha.service", "systemctl is-active kind-meitner@beta.service"]);
  ...
  it("refuses bad names and unknown operations, and reports a failed step without the tool's secrets", async () => {
    ...
    expect(m.calls).not.toContain("systemctl is-active kind-meitner@beta.service");
  });
  ...
  it("serializes complete mutations and exposes the reservation while creation is pending", async () => {
    ...
    const m = machine(root, { beforeRun: async (argv) => {
      if (argv[0] === "useradd" && argv.at(-1) === "kind-meitner-alpha") await blocked;
    } });
    const layout = fleetLayout(root);
    ...
    try {
      await vi.waitFor(() => expect(m.calls.some((call) => call.endsWith("kind-meitner-alpha"))).toBe(true));
      ...
      expect(m.calls.some((call) => call.endsWith("kind-meitner-beta"))).toBe(false);
      release();
      ...
      expect(m.files.get(layout.fenceFile)).toContain("kind-meitner-alpha");
      expect(m.files.get(layout.fenceFile)).toContain("kind-meitner-beta");
    } finally {
      release();
      await alpha;
    }
  ...
  it("keeps a failed account reservation and continues the queue after a filesystem exception", async () => {
    ...
    m.deps.writeText = (path, content, mode, owner) => {
      if (path.endsWith("/alpha/.kind-meitner/config.json")) throw new Error("fixture write refused");
  ...
  it("reports unsafe usage as unavailable without failing the list or exposing ledger content", async () => {
    ...
    m.deps.usage = (dataDir, owner, now) => {
      expect(owner).toBe(dataDir.includes("/alpha/") ? "kind-meitner-alpha" : "kind-meitner-beta");
      expect(now.toISOString()).toBe("2026-09-10T12:00:00.000Z");
      if (owner === "kind-meitner-alpha") throw new Error("unsafe fixture-secret ledger");
