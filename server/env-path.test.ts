...
describe("augmentedPath", () => {
  afterEach(() => {
    delete process.env.KIND_MEITNER_EXTRA_PATH;
  ...
  it("keeps the existing PATH entries first", () => {
    ...
    // KIND_MEITNER_EXTRA_PATH is unset here, so the inherited PATH leads
    expect(path.split(delimiter)[0]).toBe(firstExisting);
  });

  it("prepends KIND_MEITNER_EXTRA_PATH and dedupes", () => {
    process.env.KIND_MEITNER_EXTRA_PATH = ["/tmp/kind-meitner-extra", "/tmp/kind-meitner-extra"].join(delimiter);
    resetPathCacheForTests();
    const parts = augmentedPath().split(delimiter);
    expect(parts[0]).toBe("/tmp/kind-meitner-extra");
    expect(parts.filter((p) => p === "/tmp/kind-meitner-extra")).toHaveLength(1);
  });
  ...
  posixIt("makes a CLI in a known install dir spawnable despite a bare PATH", async () => {
    ...
    const fake = join(bin, "kind-meitner-fake-cli");
  ...
  posixIt("keeps the last login-shell PATH available during a rescan", async () => {
    ...
    writeFileSync(shell, `#!/bin/sh\nprintf '__KIND_MEITNER_PATH__%s' '${rcOnlyBin}'\n`);
  ...
  it.skipIf(process.platform !== "win32")("finds Antigravity installed after launch", () => {
    ...
    const localAppData = mkdtempSync(join(tmpdir(), "kind-meitner-localappdata-"));
    try {
...
winOnly("resolveCli (Windows)", () => {
  let dir: string;
  const onPath = () => {
    process.env.KIND_MEITNER_EXTRA_PATH = dir;
    resetPathCacheForTests();
  };
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "kind-meitner-shim-"));
  });
  afterEach(async () => {
    delete process.env.KIND_MEITNER_EXTRA_PATH;
    resetPathCacheForTests();
...
describe("resolveCli with wrapper commands", () => {
  ...
  posixIt("keeps an EXISTING unquoted spaced path whole — what the candidates list emits", () => {
    ...
    const spacedDir = join(bin, "kind-meitner space dir");
...
describe("registerPathDir", () => {
  ...
  it("puts an app-managed directory ahead of PATH once it exists, and survives a rescan", () => {
    const dir = mkdtempSync(join(tmpdir(), "kind-meitner-registered-path-"));
