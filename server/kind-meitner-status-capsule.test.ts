...
import { kindMeitnerStatusSystemPrompt, readKindMeitnerStatus } from "./kind-meitner-status-capsule.ts";
...
function successCapsule(options: {
  ...
  return sign({
    schema: "aos.kind_meitner_status.v1",
...
function cachePath(capsule: TestCapsule): string {
  const root = mkdtempSync(join(tmpdir(), "kind-meitner-status-"));
  roots.push(root);
  const parent = join(root, "kind-meitner");
...
posixOnly("readKindMeitnerStatus", () => {
  it("projects only fresh normalized two-VM capability data", () => {
    const capsule = successCapsule();
    // Cross-language receipt produced by scripts/aos_kind_meitner_status.py
    // for this exact normalized fixture.
    expect(capsule.receipt_sha256).toBe(
      "sha256:c213e66f55733450d9f1290441926123b52d880b4059307f0162299cc64d43ea",
    );
    const path = cachePath(capsule);

    const status = readKindMeitnerStatus({ cachePath: path, now: new Date(NOW.getTime() + 1_000) });
    ...
    const prompt = kindMeitnerStatusSystemPrompt({ cachePath: path, now: new Date(NOW.getTime() + 1_000) });
  ...
  it("rejects a signed success capsule whose installed and accepted hashes differ", () => {
    ...
    expect(readKindMeitnerStatus({ cachePath: path, now: NOW })).toMatchObject({
      freshness: "unknown",
      reason: "invalid",
  ...
  it("turns stale, future-skewed, failed, and receipt-tampered state into unknown", () => {
    const path = cachePath(successCapsule());
    expect(readKindMeitnerStatus({ cachePath: path, now: new Date("2026-08-22T06:35:00Z") })).toMatchObject({
      freshness: "stale", reason: "stale", runtimeState: "unknown", readyCount: 0,
      ui: { twoUp: false },
    });
    expect(readKindMeitnerStatus({ cachePath: path, now: new Date("2026-08-22T06:29:59Z") })).toMatchObject({
      freshness: "unknown", reason: "clock_skew", runtimeState: "unknown", readyCount: 0,
    });

    const failurePath = cachePath(failedCapsule());
    expect(readKindMeitnerStatus({ cachePath: failurePath, now: new Date(NOW.getTime() + 1_000) })).toMatchObject({
    ...
    expect(readKindMeitnerStatus({ cachePath: tamperedPath, now: new Date(NOW.getTime() + 1_000) })).toMatchObject({
  ...
  it("rejects signed extra fields, insecure modes, and same-path symlinks", () => {
    ...
    expect(readKindMeitnerStatus({ cachePath: extraPath, now: NOW }).reason).toBe("invalid");

    const insecurePath = cachePath(successCapsule());
    chmodSync(insecurePath, 0o644);
    expect(readKindMeitnerStatus({ cachePath: insecurePath, now: NOW }).reason).toBe("missing_or_insecure");

    const targetPath = cachePath(successCapsule());
    const linkPath = join(dirname(targetPath), "latest-link.json");
    symlinkSync(targetPath, linkPath);
    expect(readKindMeitnerStatus({ cachePath: linkPath, now: NOW }).reason).toBe("missing_or_insecure");
  });
  ...
  it("rejects a non-0700 parent and a symlinked parent", () => {
    ...
    expect(readKindMeitnerStatus({ cachePath: looseParentPath, now: NOW }).reason).toBe("missing_or_insecure");

    const targetPath = cachePath(successCapsule());
    const linkRoot = mkdtempSync(join(tmpdir(), "kind-meitner-status-parent-link-"));
    roots.push(linkRoot);
    const linkedParent = join(linkRoot, "kind-meitner");
    symlinkSync(dirname(targetPath), linkedParent, "dir");
    expect(
      readKindMeitnerStatus({ cachePath: join(linkedParent, "latest.json"), now: NOW }).reason,
...
it.skipIf(process.getuid !== undefined)(
  ...
  () => {
    ...
    expect(kindMeitnerStatusSystemPrompt({ cachePath: path, now: NOW })).toContain(
      "runtime_state=unknown",
    );
  },
);
