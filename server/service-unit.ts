// `kind-meitner service install`: keep the server running across reboots.
// Renders a systemd unit (Linux) or a launchd agent (macOS) that runs the
// same `kind-meitner serve …` the operator just used, and either installs it
...
export const SYSTEMD_UNIT_NAME = "kind-meitner.service";
export const LAUNCHD_LABEL = "com.kind-meitner.serve";
...
export function systemdUnit(spec: ServiceSpec): string {
  const lines = [
    "# Written by `kind-meitner service install`. Re-run it to change the options.",
    "[Unit]",
    `Description=kind-meitner${spec.label ? ` (${spec.label})` : ""}`,
    ...
    `Environment=KIND_MEITNER_DATA_DIR=${spec.dataDir}`,
...
export function unstableInstallWarning(script: string): string | null {
  ...
  if (/\/_npx\//.test(normalized) || /\/\.npm\/_npx\//.test(normalized)) {
    return `this command runs from an npx cache (${dirname(script)}), which npm may delete at any time. Install it permanently first (npm install -g kind-meitner) and run \`kind-meitner service install\` from that install.`;
  }
  return null;
}
...
export function currentUser(): string {
  try {
    ...
    return process.env.USER || process.env.USERNAME || "kind-meitner";
  }
}
