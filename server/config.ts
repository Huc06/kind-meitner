 ...
 * enables it by hand in `~/.kind-meitner/config.json`
...
// KIND_MEITNER_DATA_DIR isolates test/soak rigs from the user's real fleet.
export const DATA_DIR = process.env.KIND_MEITNER_DATA_DIR ?? join(homedir(), ".kind-meitner");
...
export function loadConfig(): AppConfig {
  ...
  if (process.env.KIND_MEITNER_ANTHROPIC_API_KEY !== undefined) cfg.anthropic.key = process.env.KIND_MEITNER_ANTHROPIC_API_KEY;
  if (process.env.KIND_MEITNER_ANTHROPIC_API_URL !== undefined) cfg.anthropic.url = process.env.KIND_MEITNER_ANTHROPIC_API_URL;
  ...
  cfg.tts = { ...cfg.tts };
  if (process.env.KIND_MEITNER_TTS_KEY !== undefined) cfg.tts.key = process.env.KIND_MEITNER_TTS_KEY;
  cfg.imageGen = { ...cfg.imageGen };
  if (process.env.KIND_MEITNER_OPENAI_IMAGE_KEY !== undefined) cfg.imageGen.key = process.env.KIND_MEITNER_OPENAI_IMAGE_KEY;
  if (process.env.KIND_MEITNER_CUSTOM_IMAGE_KEY !== undefined) cfg.imageGen.customApiKey = process.env.KIND_MEITNER_CUSTOM_IMAGE_KEY;
  ...
  if (process.env.KIND_MEITNER_SIGNIN_EMAILS !== undefined || process.env.KIND_MEITNER_SIGNIN_MEMBER_EMAILS !== undefined) {
    cfg.signIn = { ...cfg.signIn };
    if (process.env.KIND_MEITNER_SIGNIN_EMAILS !== undefined) cfg.signIn.admins = splitEmails(process.env.KIND_MEITNER_SIGNIN_EMAILS);
    if (process.env.KIND_MEITNER_SIGNIN_MEMBER_EMAILS !== undefined) cfg.signIn.members = splitEmails(process.env.KIND_MEITNER_SIGNIN_MEMBER_EMAILS);
  }
  return cfg;
}
...
export function syncCredentialEnv(patch: Partial<AppConfig>): void {
  const secrets: Array<[value: string | undefined, name: string]> = [
    ...
    [patch.anthropic?.key, "KIND_MEITNER_ANTHROPIC_API_KEY"],
    [patch.openaiCompat?.key, "OPENAI_COMPAT_API_KEY"],
    [patch.composio?.apiKey, "COMPOSIO_API_KEY"],
    [patch.box?.token, "BOX_TOKEN"],
    [patch.opencodeGo?.apiKey, "OPENCODE_API_KEY"],
    [patch.tts?.key, "KIND_MEITNER_TTS_KEY"],
    [patch.imageGen?.key, "KIND_MEITNER_OPENAI_IMAGE_KEY"],
    [patch.imageGen?.customApiKey, "KIND_MEITNER_CUSTOM_IMAGE_KEY"],
  ];
  ...
  const settings: Array<[value: string | undefined, name: string]> = [
    ...
    [patch.anthropic?.url, "KIND_MEITNER_ANTHROPIC_API_URL"],
...
export const WORKSPACE_CREDENTIAL_ENV = [
  "XAI_API_KEY",
  "KIND_MEITNER_ANTHROPIC_API_KEY",
  "KIND_MEITNER_ANTHROPIC_API_URL",
  ...
  "KIND_MEITNER_BROWSER_CONNECTION",
  "KIND_MEITNER_USER_DATA",
] as const;
...
/** Merge a partial config into ~/.kind-meitner/config.json (secrets never
