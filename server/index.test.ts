...
async function mintTestCapability(
  ...
  const response = await fetch(`${baseUrl}/api/testing/internal-capability`, {
    ...
    headers: {
      ...
      "x-kind-meitner-test-capability": TEST_CAPABILITY_KEY,
    },
...
const PHONE_SECRET_TEST_IDENTITY = {
  type: "kind-meitner:phone-secret-key",
  version: 1,
...
const managedBoxNameForFixture = (botId: string): string => {
  ...
  const environmentId = readFileSync(join(home, ".kind-meitner", "environment-id"), "utf8").trim();
  ...
  for (;;) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (health.app === "kind-meitner" && health.pid === serverChild.pid && health.static === true) return;
      }
    } catch {
      /* still starting */
    }
...
const storedMessageCount = (threadId: string): number => {
  const db = new DatabaseSync(join(home, ".kind-meitner", "messages.db"), { readOnly: true });
  try {
...
beforeAll(async () => {
  home = mkdtempSync(join(tmpdir(), "kind-meitner-api-test-"));
  ...
  fakeDockerFixture = join(home, ".kind-meitner", "fake-unmanaged-container");
  fakeDockerLog = join(home, ".kind-meitner", "fake-docker-calls.log");
  ...
  writeFileSync(join(staticDir, "index.html"), "<!doctype html><title>Packaged kind-meitner</title>");
  ...
  writeFileSync(
    join(home, ".kind-meitner", "groups.json"),
    JSON.stringify([
      {
        id: "test-dm",
  ...
  const linkedWorkspace = join(home, ".kind-meitner", "workspaces", "test-bot-a");
  const linkedFile = join(linkedWorkspace, "phone report.md");
  const linkedImage = join(linkedWorkspace, "preview.png");
  const privateAttachments = join(home, ".kind-meitner", "attachments");
  ...
  writeFileSync(
    join(home, ".kind-meitner", "messages-test-linked-file-room-thread.json"),
    JSON.stringify({
  ...
  writeFileSync(
    join(home, ".kind-meitner", "messages-test-goal-restart-thread.json"),
    JSON.stringify({
  ...
  writeFileSync(
    join(home, ".kind-meitner", "routines.json"),
    JSON.stringify({
      version: 1,
      routines: [],
      runs: [{
  ...
  writeFileSync(
    join(home, ".kind-meitner", "messages-test-stranded-room-thread.json"),
    JSON.stringify({
  ...
  writeFileSync(
    join(home, ".kind-meitner", "messages-test-cancel-room-thread.json"),
    JSON.stringify({
  ...
  const browserPrelude = `data:text/javascript,${encodeURIComponent(`
    ...
    childProcess.spawn = function(command, args, options) {
      if (command !== process.env.KIND_MEITNER_AGENT_BROWSER_PATH) return spawn(command, args, options);
  ...
  child = spawn(process.execPath, ["--import", browserPrelude, join(SERVER_DIR, "index.ts")], {
    ...
    env: {
      ...
      KIND_MEITNER_WEBHOOK_PORT: String(WEBHOOK_PORT),
      KIND_MEITNER_EXTRA_PATH: fakeDockerDir,
      KIND_MEITNER_BOX_API: `http://127.0.0.1:${boxStubPort}`,
      KIND_MEITNER_COMPOSIO_API: `http://127.0.0.1:${boxStubPort}/api/v3.1`,
      KIND_MEITNER_COMPOSIO_TOOLKITS_API: `http://127.0.0.1:${boxStubPort}/api/v3`,
      KIND_MEITNER_STATIC_DIR: staticDir,
      // The bots' browser engine: a stand-in binary the fake engine CLIs never
      // run; the turn only has to mount it.
      KIND_MEITNER_AGENT_BROWSER_PATH: join(home, "fake-agent-browser"),
      // Production uses 15s. Keep the real timer path while making the
      // browser-visible heartbeat assertion fast and deterministic.
      KIND_MEITNER_SSE_HEARTBEAT_MS: "50",
      ...
      KIND_MEITNER_TEST_INTERNAL_CAPABILITY_KEY: TEST_CAPABILITY_KEY,
    },
...
describe("harness HTTP API", () => {
  it("reconciles durable working goal cards with scheduler truth after a restart", async () => {
    ...
    expect(room.messages.find(
      ...
      text: "Goal failed: kind-meitner restarted before this goal finished.",
      goalRun: {
        status: "failed",
        detail: "kind-meitner restarted before this goal finished.",
        turnCount: 2,
  ...
  it("rejects non-loopback authorities while accepting IPv4 and IPv6 loopback forms", async () => {
    ...
    expect(probe.body).toEqual({ app: "kind-meitner" });
    ...
    expect(Reflect.get(Object(Reflect.get(Object(brand.body), "brand")), "name")).toBe("kind-meitner");
  ...
  it("identifies itself on /api/health", async () => {
    ...
    expect(body.app).toBe("kind-meitner");
  ...
  it("refuses a second live server that shares the same data directory", async () => {
    ...
    const contender = spawn(process.execPath, [join(SERVER_DIR, "index.ts")], {
      ...
      env: {
        ...(process.env.PATH ? { PATH: process.env.PATH } : {}),
        ...(process.env.SystemRoot ? { SystemRoot: process.env.SystemRoot } : {}),
        KIND_MEITNER_DATA_DIR: join(home, ".kind-meitner"),
        KIND_MEITNER_PORT: String(contenderPort),
        KIND_MEITNER_STATIC_DIR: staticDir,
      },
  ...
  it("serves packaged UI assets and preserves API 404s", async () => {
    ...
    expect(await root.text()).toContain("Packaged kind-meitner");
    ...
    expect(await spa.text()).toContain("Packaged kind-meitner");
  ...
  it("lets a Chief create operators from its direct and channel tasks but not from channels it cannot access", async () => {
    ...
    try {
      ...
      const dump = z.object({
        mcpConfig: z.object({
          mcpServers: z.object({
            agents: z.object({ env: z.object({ KIND_MEITNER_COMMS_TOKEN: z.string() }) }),
          }),
        }),
      }).parse(await readJsonFileWhenReady(fakeClaudeDump));
      expect(dump.mcpConfig.mcpServers.agents.env.KIND_MEITNER_COMMS_TOKEN).toMatch(/^[a-f0-9]{48}$/);
  ...
  it.each([
    ...
    const key = join(home, ".kind-meitner", "browser-engine-key");
    ...
    const journal = () => JSON.parse(readFileSync(join(home, ".kind-meitner", "browser-cleanups.json"), "utf8")) as Array<{ id: string; phase: string }>;
    try {
  ...
  it.skip("creates team computers only with consent, retains failed creates, and assigns safely", async () => {
    ...
    try {
      ...
      const persisted = JSON.parse(readFileSync(join(home, ".kind-meitner", "team-computers.json"), "utf8"));
      ...
      const saved = JSON.parse(readFileSync(join(home, ".kind-meitner", "team-computers.json"), "utf8"));
  ...
  it.skip("shares one team computer across direct and room turns without overriding explicit destinations", async () => {
    ...
    { timeout: 5_000 }).toBe(false);
    type ComputerDump = { mcpConfig: { mcpServers: { computer?: { env: { OGB_BOX_ID: string; KIND_MEITNER_CONTROL_URL: string; KIND_MEITNER_CONTROL_TOKEN: string } } } } };
    const gate = (computer: NonNullable<ComputerDump["mcpConfig"]["mcpServers"]["computer"]>) =>
      fetch(computer.env.KIND_MEITNER_CONTROL_URL, { headers: { authorization: `Bearer ${computer.env.KIND_MEITNER_CONTROL_TOKEN}` } });
    try {
      expect((await api("PUT", "/api/config", { box: { token: "box_route" } })).status).toBe(200);
      ...
      expect((await fetch(direct.env.KIND_MEITNER_CONTROL_URL.replace(botIds[0], botIds[1]), {
        headers: { authorization: `Bearer ${direct.env.KIND_MEITNER_CONTROL_TOKEN}` },
      })).status).toBe(403);
  ...
  it("keeps Full and Custom bots on Codex when the paired model route changes providers", async () => {
    const isolatedHome = mkdtempSync(join(tmpdir(), "kind-meitner-trusted-mode-model-"));
    const isolatedData = join(isolatedHome, ".kind-meitner");
    ...
    const isolatedChild = spawn(process.execPath, [join(SERVER_DIR, "index.ts")], {
      ...
      env: {
        ...
        KIND_MEITNER_PORT: String(isolatedPort),
        KIND_MEITNER_WEBHOOK_PORT: String(isolatedPort + 1),
        KIND_MEITNER_STATIC_DIR: isolatedStatic,
        FAKE_CLAUDE_MODE: "hang",
  ...
  it("exports every visible bot and imports the team without creating a room", async () => {
    ...
    expect(exported.body).toMatchObject({ format: "kind-meitner.team", version: 2, team: { name: "Field Team" } });
    ...
    expect((await api("POST", "/api/teams/export", {})).body.team.name).toBe("My KindMeitner Team");
  ...
  it("imports a team as a project: one room, on a folder", async () => {
    ...
    const folder = mkdtempSync(join(tmpdir(), "kind-meitner-project-"));
    ...
    try {
      for (const suffix of [3, 4]) {
        const imported = await api("POST", "/api/teams/import", {
          format: "kind-meitner.team", version: 2,
  ...
  it("the scout reads a folder, proposes an importable team, and creates nothing until the human imports", async () => {
    const folder = mkdtempSync(join(tmpdir(), "kind-meitner-scout-"));
  ...
  it("leaves a failed credential-card continuation on the card without buzzing twice", async () => {
    ...
    try {
      ...
      const dump = await readJsonFileWhenReady<{
        mcpConfig: { mcpServers: { agents: { env: { KIND_MEITNER_COMMS_TOKEN: string } } } };
      }>(fakeClaudeDump);
      const token = dump.mcpConfig.mcpServers.agents.env.KIND_MEITNER_COMMS_TOKEN;
      ...
      expect(directCard).toMatchObject({
        kind: "secret",
        text: "Securely provide the OpenAI API key from kind-meitner on your phone or computer. It is never added to chat.",
      });
      ...
      const developmentProvision = await fetch(
        `${BASE}/api/bots/${bot.id}/secret-cards/${messageId}/provide`,
        {
          ...
          headers: {
            "content-type": "application/json",
            "x-kind-meitner-companion": "1",
            "x-kind-meitner-companion-device": "phone-1",
          },
          body: JSON.stringify(encryptedEnvelope),
        },
      );
  ...
  it("keeps credential-card ownership stable while an encrypted phone save is in flight", async () => {
    const isolatedHome = mkdtempSync(join(tmpdir(), "kind-meitner-phone-secret-races-"));
    const isolatedData = join(isolatedHome, ".kind-meitner");
    const isolatedStatic = join(isolatedHome, "static");
    ...
    const desktopPrelude = `data:text/javascript,${encodeURIComponent(`
      ...
      Object.defineProperty(process, "parentPort", {
        value: {
          ...
          postMessage(message) {
            if (message?.type !== "kind-meitner:phone-secret-save") return;
            ...
            saves = saves.then(async () => {
              ...
              try {
                ...
                const response = await fetch(
                  "http://127.0.0.1:" + process.env.KIND_MEITNER_PORT + "/api/config?secretStorage=external",
                  {
                    method: "PUT",
                    headers: { "content-type": "application/json" },
                ...
                listener?.({ data: {
                  type: "kind-meitner:phone-secret-save-result",
      ...
      const provide = (botId: string, messageId: string, envelope: PhoneSecretContext) => fetch(
        `http://127.0.0.1:${isolatedPort}/api/bots/${botId}/secret-cards/${messageId}/provide`,
        {
          ...
          headers: {
            "content-type": "application/json",
            "x-kind-meitner-companion": "1",
            "x-kind-meitner-companion-device": deviceId,
          },
  ...
  it.skip("retires a journaled Box proven gone before clearing and later restoring credentials", async () => {
    ...
    try {
      ...
      expect((await api("PUT", "/api/config", { box: { token: "" } })).status).toBe(200);
      const journal = JSON.parse(readFileSync(join(home, ".kind-meitner", "box-create-requests.json"), "utf8"));
  ...
  it("manages and probes custom MCP servers without returning secret values", async () => {
    ...
    const disk = JSON.parse(readFileSync(join(home, ".kind-meitner", "config.json"), "utf8"));
  ...
  it("cards every ask when Claude's reviewer never started, says so once, and can hand the allow to Claude for the session", async () => {
    ...
    // Manual, and asks about everything. kind-meitner passes that through —
  ...
  it("mounts the verification skill into a real turn when its trigger appears", async () => {
    ...
    try {
      ...
      expect(system).toContain('<kind-meitner-skill id="create-verification-skill"');
  ...
  it("injects the bot's standing instructions (soul) into a real turn, directly after the persona", async () => {
    ...
    try {
      ...
      expect(system.startsWith("You are Kiwi, a personal bot in kind-meitner. Role: Tracker.")).toBe(true);
  ...
  it("does not coach an ordinary request even when the bot profile is blank", async () => {
    ...
    try {
      ...
      expect(system.startsWith("You are Blank, a personal bot in kind-meitner.")).toBe(true);
  ...
  it("never lets an out-of-band SOUL.md edit reach the prompt, and surfaces it as drift instead", async () => {
    ...
    try {
      ...
      writeFileSync(join(home, ".kind-meitner", "bots", bot.id, "SOUL.md"), "File text.");
      rmSync(fakeClaudeDump, { force: true });
      expect((await api("POST", `/api/bots/${bot.id}/messages`, { text: "hello" })).status).toBe(202);
      await expect.poll(() => existsSync(fakeClaudeDump), { timeout: 5_000 }).toBe(true);
      const seen = JSON.parse(readFileSync(fakeClaudeDump, "utf8"));
  ...
  it("mounts the verification skill only for the latest channel request", async () => {
    ...
    try {
      ...
      expect(system).toContain('<kind-meitner-skill id="create-verification-skill"');
      expect(system).toContain('<kind-meitner-skill id="phone-harness"');
      expect((await api("POST", `/api/groups/${room.id}/interrupt`, {})).status).toBe(200);
      await expect.poll(async () => {
        const state = (await api("GET", "/api/bots?messages=0")).body;
        return state.bots.find((candidate: { id: string }) => candidate.id === bot.id)?.busy;
      ...
      expect(system).not.toContain('<kind-meitner-skill id="create-verification-skill"');
      expect(system).toContain('<kind-meitner-skill id="phone-harness"');
    } finally {
      if (room) {
        expect((await api("POST", `/api/groups/${room.id}/interrupt`, {})).status).toBe(200);
  ...
  it("keeps skill authoring on by default and persists an explicit opt-out", async () => {
    ...
    const untouched = JSON.parse(readFileSync(join(home, ".kind-meitner", "config.json"), "utf8"));
  ...
  it.each(["direct", "room"] as const)("mounts the browser engine's MCP server and the sign-in policy in %s turns and preview", async (target) => {
    ...
    try {
      ...
      expect(browser.env.KIND_MEITNER_BROWSER_TOKEN).toEqual(expect.any(String));
      expect(browser.env.KIND_MEITNER_HARNESS_URL).toBe(BASE);
  ...
  it("reconciles a committed crash-stale bot reference before ACK and profile-id reuse", async () => {
    const isolatedHome = mkdtempSync(join(tmpdir(), "kind-meitner-browser-cleanup-restart-"));
    const isolatedData = join(isolatedHome, ".kind-meitner");
    const isolatedStatic = join(isolatedHome, "static");
    const isolatedPort = await freePortBlock([0, 1]);
    mkdirSync(join(isolatedStatic, "assets"), { recursive: true });
          ...
          postMessage(message) {
            if (message?.requestId && /browser-(?:bot|profile)-deleted/.test(message.type ?? "")) {
              queueMicrotask(() => listener?.({ data: {
                type: "kind-meitner:browser-lifecycle-result",
                requestId: message.requestId,
                ok: true,
              } }));
            }
  ...
  it("clears bot references when a named browser profile is removed", async () => {
    ...
    try {
      ...
      const config = JSON.parse(readFileSync(join(home, ".kind-meitner", "config.json"), "utf8"));
  ...
  it.skip("never removes an unmanaged container that squats on a bot's exact Local VM name", async () => {
    ...
    try {
      ...
      expect(removed.body.error).toMatch(/not created by kind-meitner.*remove it manually/i);
  ...
  it("tracks and cancels a queued channel credential continuation before provider dispatch", async () => {
    ...
    try {
      ...
      expect(firstDump.mcpConfig.mcpServers.agents.env.KIND_MEITNER_COMMS_TOKEN).toMatch(/^[a-f0-9]{48}$/);
  ...
  it("keeps a proposed profile change inert until its card is confirmed, then records history", async () => {
    const soulFileOf = (botId: string) => join(home, ".kind-meitner", "bots", botId, "SOUL.md");
  ...
  it("only enables the exact learned-skill proposal a current client reviewed", async () => {
    ...
    try {
      ...
      writeFileSync(
        join(home, ".kind-meitner", "skill-state", bot.id, "staged.json"),
  ...
  it.skipIf(process.platform === "win32")("stores the credentials file with owner-only permissions", () => {
    expect(statSync(join(home, ".kind-meitner", "config.json")).mode & 0o777).toBe(0o600);
  });
  ...
  it("creates an independent webhook, accepts a delivery, deduplicates it, and rotates its secret", async () => {
    ...
    if (process.platform !== "win32") {
      expect(statSync(join(home, ".kind-meitner", "webhooks.json")).mode & 0o777).toBe(0o600);
    }
  });
  ...
  it("keeps avatar providers and externally stored image credentials separate", async () => {
    try {
      ...
      for (const secret of ["openai-avatar-fixture", "custom-avatar-fixture"]) {
        ...
        expect(readFileSync(join(home, ".kind-meitner", "config.json"), "utf8")).not.toContain(secret);
      }
      const disk = JSON.parse(readFileSync(join(home, ".kind-meitner", "config.json"), "utf8"));
...
describe("bot memory API", () => {
  ...
  const workspaceOf = (botId: string) => join(home, ".kind-meitner", "workspaces", botId);
  ...
  it("refuses every coat of path traversal without reading the target", async () => {
    ...
    try {
      ...
      writeFileSync(join(home, ".kind-meitner", "secret.md"), "TOP-SECRET-MARKER sibling");
  ...
  it("round-trips soul through both PATCH routes and mirrors it to SOUL.md", async () => {
    ...
    expect(existsSync(join(home, ".kind-meitner", "bots", bot.id))).toBe(false);
  });
  ...
  it("keeps mixed-request runtime revocations effective when profile persistence fails", async () => {
    ...
    const botsFile = join(home, ".kind-meitner", "bots.json");
  ...
  it("refuses redacted history restores without changing SOUL and still restores exact safe text", async () => {
    ...
    const file = join(home, ".kind-meitner", "bots", bot.id, "history.ndjson");
...
describe("message pages", () => {
  ...
  it("downloads only a file linked by the exact stored bot message", async () => {
    ...
    const linkedFile = join(home, ".kind-meitner", "workspaces", "test-bot-a", "phone report.md");
    const response = await fetch(
    ...
    expect((await api(
      ...
      { path: join(home, ".kind-meitner", "workspaces", "test-bot-a", "other.md") },
    )).status).toBe(403);
  ...
  it("downloads an image rendered by the exact stored bot message", async () => {
    const threadId = "test-linked-file-room-thread";
    const linkedImage = join(home, ".kind-meitner", "workspaces", "test-bot-a", "preview.png");
  ...
  it("downloads an exact user attachment only from the private attachment store", async () => {
    const threadId = "test-linked-file-room-thread";
    const shared = join(home, ".kind-meitner", "attachments", "shared-notes.pdf");
    const response = await fetch(
    ...
    expect((await api(
      ...
      { path: join(home, ".kind-meitner", "attachments", "different.pdf") },
    )).status).toBe(403);
    expect((await api(
      "POST",
      `/api/threads/${threadId}/messages/user-outside-file-message/file`,
      { path: join(home, ".kind-meitner", "workspaces", "test-bot-a", "phone report.md") },
    )).status).toBe(403);
  });
