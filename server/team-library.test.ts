...
describe("team library", () => {
  it("validates catalog paths and adds the trusted repository URL", () => {
    ...
    expect(parsed.repositoryUrl).toBe("https://github.com/harrymove-ctrl/kind-meitner-teams");
  ...
  it("loads only the manifest selected by the trusted catalog", async () => {
    ...
    if (loaded.format !== "kind-meitner.team") throw new Error("expected a legacy team");
