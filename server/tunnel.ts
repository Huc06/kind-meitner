// Rung three of the hosting ladder: `kind-meitner serve --tunnel` gives a
// server a public HTTPS address (https://c-<id>.kind-meitner.com) with no
...
//     opens for it (KIND_MEITNER_TUNNEL_SOCKET), so to request-auth every public
...
export interface TunnelAccount {
  ...
  /** "" when KIND_MEITNER_CONTROL_PLANE_URL is set to something unusable. */
  controlPlane: string;
}

/** The desktop honours KIND_MEITNER_CONTROL_PLANE_URL only in development builds; a
...
export const FLEET_CREDENTIAL_ENV = "KIND_MEITNER_INSTALLATION_CREDENTIAL";
...
export async function fleetAccess(options: { credential: string; env?: NodeJS.ProcessEnv; fetchImpl?: typeof fetch }): Promise<ManagedTunnelAccess> {
  const env = options.env ?? process.env;
  const controlPlane = resolveCompanionControlPlaneURL({ isPackaged: true, environment: env });
  if (!controlPlane) throw new Error("KIND_MEITNER_CONTROL_PLANE_URL is set but is not an https address");
...
/** KIND_MEITNER_CLOUDFLARED_PATH, then the copy this command downloaded into the data
 * dir, then PATH. */
 ...
 * the tunnel at 8812; KIND_MEITNER_TUNNEL_ORIGIN_PORT exists for tests) and forwards
...
export function startTunnel(options: {
  ...
  const env = options.env ?? process.env;
  const originPort = Number(env.KIND_MEITNER_TUNNEL_ORIGIN_PORT || MANAGED_COMPANION_ORIGIN_PORT);
