...
it("gives a room turn the memory_update guidance, not the file-tools one", async () => {
  ...
  const env = { KIND_MEITNER_URL: fixture.info.url };
  ...
  try {
    // SAFETY: control-kind-meitner returns the created bot record under `bot`
    const { bot: lead } = await runControlKindMeitner(["new-bot", "--name", "Lead"], { env }) as { bot: { id: string } };
    // SAFETY: same shape as above
    const { bot: helper } = await runControlKindMeitner(["new-bot", "--name", "Helper"], { env }) as { bot: { id: string } };
    // A 1:1 turn first: it is the reference the room turn must match.
    await runControlKindMeitner(["send", "--bot", lead.id, "--text", "Remember that the fixture garden is watered on Mondays."], { env });
    await runControlKindMeitner(["wait", "--bot", lead.id, "--timeout", "30"], { env });
