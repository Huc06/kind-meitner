...
function fixture({
  ...
  const runner: VpsCommandRunner = async (args, options) => {
    ...
    if (command === "exec") {
      ...
      if (args.includes("kind-meitner-preview")) return { stdout: screenshotValid ? screenshot.toString("base64") : "not-an-image", stderr: "" };
...
describe("VPS computer", () => {
  it("uses a deterministic, bot-id-derived managed container name", () => {
    ...
    expect(vpsContainerName(BOT_ID)).toMatch(/^kind-meitner-vps-[a-z0-9-]+$/);
  });
  ...
  it("reports a ready container only when image, labels, limits, mounts, network, and Cua pass", async () => {
    ...
    expect(fake.calls.some(({ args }) => args.includes("kind-meitner-preview"))).toBe(false);
  ...
  it("lets explicit provisioning build and run the pinned container, but Auto only reuses", async () => {
    ...
    expect(run.find((arg) => arg.startsWith(`${VPS_ENVIRONMENT_LABEL}=`)))
      .toMatch(/^com\.kind-meitner\.environment=[0-9a-f-]{36}$/i);
  ...
  it("mounts the official Cua MCP server through the tiny remote exec bridge", () => {
    ...
    expect(vpsContainerMcpArgs("production-vps", vpsContainerName(BOT_ID))).toEqual([
      ...
      "/usr/local/libexec/kind-meitner/cua-driver",
      "mcp",
      "--socket",
      "/run/user/1000/kind-meitner-cua.sock",
    ]);
  });
  ...
  it("captures screenshots through Cua Driver and validates the returned image", async () => {
    ...
    const transfer = fake.calls.find(({ args }) => args.includes("kind-meitner-preview"))!.args;
    expect(transfer.slice(2)).toEqual([
      "exec", "-u", "cua", "-e", "HOME=/home/cua", CONTAINER_ID,
      "sh", "-c", expect.stringContaining('quality=70'), "kind-meitner-preview", "/tmp/kind-meitner-vps-preview.png",
    ]);
  ...
  it.skipIf(!hasPillow)("transfers real JPEG previews at quality 70, within 1280px without upscaling", async () => {
    ...
    const scratch = mkdtempSync(join(tmpdir(), "kind-meitner-vps-preview-image-"));
    try {
      for (const [width, height, expected] of [[2560, 1600, [1280, 800]], [1600, 2560, [800, 1280]], [320, 200, [320, 200]]] as const) {
        ...
        const encoded = execFileSync("sh", ["-c", script, "kind-meitner-preview", path], { encoding: "utf8", maxBuffer: 16 * 1024 * 1024 });
        ...
        const frame = await vpsComputerScreenshot(CONFIG, BOT_ID, async (args, options) => args.includes("kind-meitner-preview")
  ...
  it.skipIf(process.platform === "win32")("falls back to the original PNG when conversion is unavailable or fails", async () => {
    ...
    const scratch = mkdtempSync(join(tmpdir(), "kind-meitner-vps-preview-fallback-"));
    try {
      const path = join(scratch, "preview.png");
      ...
      for (const interpreter of [join(scratch, "missing-python"), "false"]) {
        ...
        const encoded = execFileSync("sh", ["-c", script, "kind-meitner-preview", path], { encoding: "utf8" });
  ...
  it("shares overlapping captures of the same target through cleanup", async () => {
    ...
    expect(fake.calls.filter(({ args }) => args.includes("kind-meitner-preview"))).toHaveLength(1);
  ...
  it("pins the capture alias and never shares its frame with a different VPS", async () => {
    ...
    for (const alias of ["first-preview-vps", "second-preview-vps"]) {
      ...
      expect(fake.calls.filter(({ args }) => args[1] === `ssh://${alias}` && args.includes("kind-meitner-preview"))).toHaveLength(1);
    }
  });
  ...
  it("bounds the complete slow preview, holds its lock through timed-out cleanup, and recovers", async () => {
    ...
    try {
      ...
      const runner: VpsCommandRunner = async (args, options) => {
        ...
        if (args.includes("kind-meitner-preview") || args.includes("rm")) {
      ...
      expect(calls.find(({ args }) => args.includes("kind-meitner-preview"))?.timeoutMs).toBe(14_000);
  ...
  it("never removes a container kind-meitner did not create", async () => {
