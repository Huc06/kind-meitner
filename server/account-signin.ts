...
// kind-meitner control plane (the account service the desktop companion and
// `kind-meitner login` already use), and this server decides who is welcome
...
// accounts.kind-meitner.com. The exchange happens server-side, so a browser
...
export const ADMIN_EMAILS_ENV = "KIND_MEITNER_SIGNIN_EMAILS";
export const MEMBER_EMAILS_ENV = "KIND_MEITNER_SIGNIN_MEMBER_EMAILS";
...
export function createEmailSignIn(options: {
  ...
  const controlPlane = (): ControlPlaneClient => {
    ...
    if (!url) throw new Error("KIND_MEITNER_CONTROL_PLANE_URL is set but is not an https address");
