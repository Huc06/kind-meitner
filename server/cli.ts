// `kind-meitner` on the command line: run the server anywhere and pair devices
// to it. One implementation for three homes — `npx kind-meitner` (the npm
// package), `node dist-server/cli.js` (the container image) and
// `pnpm kind-meitner` (a checkout) — because scripts/bundle-server.mjs bundles this
// file next to the server.
//
//   kind-meitner setup [--data-dir ~/.kind-meitner]
//   kind-meitner start [serve options]
//   kind-meitner serve [--port 8799] [--data-dir ~/.kind-meitner] [--label "cab mini"]
//                     [--public-url https://host] [--tailscale | --tunnel | --domain HOST] [--no-pair]
//   kind-meitner pair  [--label "My MacBook"] [--client] [--public-url https://host]
//   kind-meitner sessions [revoke <id>]
//   kind-meitner status
//   kind-meitner login [--email you@example.com]
//   kind-meitner logout
//
...
// `--tunnel` (after `login`) serves at a public https://….kind-meitner.com
// address through a Cloudflare tunnel: no domain, no proxy, no open port.
//
// This module only exports; kind-meitner.ts is the entry that runs main(), so
...
export function parseArgs(argv: string[], env: NodeJS.ProcessEnv = process.env): CliOptions | { error: string } {
  ...
  const options: CliOptions = {
    ...
    port: Number(env.KIND_MEITNER_PORT || 8799),
    dataDir: env.KIND_MEITNER_DATA_DIR || join(homedir(), ".kind-meitner"),
    tailscale: false,
    tunnel: false,
    client: false,
    pair: true,
...
export const USAGE = `kind-meitner — your team of AI bots, ready in a few steps

  kind-meitner                         set up once, then open your workspace
  kind-meitner setup [--data-dir DIR]
  kind-meitner start [the same options as serve]
  kind-meitner serve [--port 8799] [--data-dir DIR] [--label NAME]
                    [--public-url https://host] [--tailscale | --tunnel | --domain HOST] [--no-pair]
  kind-meitner pair  [--label NAME] [--client] [--public-url https://host]
  kind-meitner sessions [revoke ID]
  kind-meitner status
  kind-meitner login [--email you@example.com]
  kind-meitner logout
  kind-meitner access list | add EMAIL [--chat-only] | remove EMAIL
  kind-meitner service install [--domain HOST | --tunnel | --tailscale] [--port N] [--data-dir DIR] | uninstall
  kind-meitner browser install [--with-deps] | status
  kind-meitner fleet init --domain HOST [--operator USER] | create NAME --admin EMAIL [--member EMAIL] [--brand FILE]
...
start   same as kind-meitner: use your saved settings and open the workspace
...
login   signs this machine in to a kind-meitner account (an emailed code)
...
service keep the server running across reboots: writes a systemd unit
        ...
        permanently first (npm install -g kind-meitner).
...
--tunnel     serve at a public https://….kind-meitner.com address through a
             Cloudflare tunnel: no domain, no proxy, no open port. Run
             \`kind-meitner login\` once on this machine first.
...
Install once with \`npm install -g kind-meitner\`, then type \`kind-meitner\`.
Or run without a global install: \`npx kind-meitner\`. Node 24+ is required.
`;
...
async function serverUp(port: number, pid?: number): Promise<boolean> {
  try {
    ...
    return status === 200 && body?.app === "kind-meitner" && (pid === undefined || body.pid === pid);
  } catch {
    return false;
  }
}
...
export async function isWorkspaceRunning(options: CliOptions): Promise<boolean> {
  try {
    ...
    if (status !== 200 || body?.app !== "kind-meitner") return false;
    const expected = readFileSync(join(options.dataDir, "environment-id"), "utf8").trim();
    const descriptor = await api(options.port, "/.well-known/kind-meitner/environment");
...
export async function verifyPhoneEndpoint(port: number, origin: string): Promise<boolean> {
  ...
  try {
    const local = await api(port, "/.well-known/kind-meitner/environment");
    const remote = await fetch(`${origin}/.well-known/kind-meitner/environment`, { signal: AbortSignal.timeout(5000), redirect: "error" });
...
export function applyStartupPreferences(options: CliOptions, saved: AppConfig["cliStartup"]): CliOptions {
  ...
  if (saved.access === "public-url" && (!saved.publicUrl || !normalizePhoneOrigin(saved.publicUrl))) {
    throw new Error("The saved phone address is not a valid HTTPS origin. Run kind-meitner setup to correct it, or kind-meitner --local to start only on this computer.");
  }
  return {
    ...options,
...
async function showPhonePairing(options: CliOptions, origin: string | undefined, log: (line: string) => void): Promise<boolean> {
  ...
  if (!ready) {
    ...
    log("Check the HTTPS connection, then run kind-meitner pair again with the same --data-dir and --port.");
    return false;
  }
...
export async function runPair(options: CliOptions): Promise<number> {
  if (!(await serverUp(options.port))) {
    console.error(`no kind-meitner server on http://127.0.0.1:${options.port}; start one with \`kind-meitner serve\` or set KIND_MEITNER_PORT`);
    return 1;
  }
  if (process.stdin.isTTY && process.stdout.isTTY && !options.label && !options.client) {
    ...
    if (!origin || !normalizePhoneOrigin(origin)) {
      ...
      console.log("Stop the server, run kind-meitner setup and choose phone access, then start kind-meitner again.");
      return 1;
    }
...
export async function runSessions(options: CliOptions): Promise<number> {
  if (!(await serverUp(options.port))) {
    console.error(`no kind-meitner server on http://127.0.0.1:${options.port}`);
    return 1;
  }
  if (options.revoke) {
  ...
  if (!sessions.length) {
    console.log("no paired devices yet: run `kind-meitner pair`");
    return 0;
  }
...
export function formatSessions(sessions: Array<{ id: string; label: string; scopes: string[]; lastSeenAt: number; expiresAt: number }>, now = Date.now()): string {
  ...
  return [line(head), ...rows.map(line), "", "revoke one with: kind-meitner sessions revoke <id>"].join("\n");
}

export async function runStatus(options: CliOptions, io: CliIo = defaultIo()): Promise<number> {
  let code = 0;
  try {
    const res = await fetch(`http://127.0.0.1:${options.port}/.well-known/kind-meitner/environment`);
    const body: any = await res.json();
    io.log(options.json ? JSON.stringify(body, null, 2) : `${body.label} · kind-meitner ${body.version} on ${body.platform} · id ${body.environmentId}`);
  } catch {
    io.error(`no kind-meitner server on http://127.0.0.1:${options.port}`);
    code = 1;
  }
  if (!options.json) {
 ...
 * needs no restart. Environment variables (KIND_MEITNER_SIGNIN_EMAILS) win when set.
...
export async function runAccess(options: CliOptions, io: CliIo = defaultIo()): Promise<number> {
  ...
  const overridden = process.env.KIND_MEITNER_SIGNIN_EMAILS !== undefined || process.env.KIND_MEITNER_SIGNIN_MEMBER_EMAILS !== undefined;
  ...
  if (options.accessAction === "list") {
    if (!admins.length && !members.length) {
      io.log("nobody can sign in with an email yet; pairing codes only. Add someone with: kind-meitner access add you@example.com");
      return 0;
    }
    ...
    if (overridden) io.log("(KIND_MEITNER_SIGNIN_EMAILS / KIND_MEITNER_SIGNIN_MEMBER_EMAILS are set in the environment and win over this list while the server runs)");
    return 0;
  }
  ...
  if (options.accessAction === "remove") {
    ...
    io.log(`${entry} can no longer sign in (existing sessions stay until they expire or are revoked with \`kind-meitner sessions revoke\`)`);
    return 0;
  }
  ...
  if (overridden) io.log("note: KIND_MEITNER_SIGNIN_EMAILS / KIND_MEITNER_SIGNIN_MEMBER_EMAILS are set in the environment and win over this list while the server runs");
  return 0;
}
...
export async function runLogin(options: CliOptions, io: CliIo = defaultIo()): Promise<number> {
  ...
  if (!account.controlPlane) {
    io.error("KIND_MEITNER_CONTROL_PLANE_URL is set but is not an https address");
    return 1;
  }
  ...
  const email = (options.email ?? (await io.ask("Email for your kind-meitner account: "))).trim();
  if (!email) {
    io.error("an email address is needed: kind-meitner login --email you@example.com");
    return 1;
  }
  try {
  ...
  io.log("Serve there with:  kind-meitner serve --tunnel");
  return 0;
}
...
export async function runBrowser(options: CliOptions, io: CliIo = defaultIo()): Promise<number> {
  ...
  if (options.browserAction === "status") {
    ...
    if (status.kind !== "ready" && status.installable) io.log("install it with:  kind-meitner browser install");
  ...
  try {
    ...
    if (process.platform === "linux" && !options.withDeps) io.error("on Linux, install Chrome's system libraries with `sudo kind-meitner browser install --with-deps`, then retry `kind-meitner browser install` as the user running serve");
    return 1;
  }
  ...
  if (process.platform === "linux" && options.withDeps) io.log("if serve runs as another user, run `kind-meitner browser install` from that user's login shell too");
  return 0;
}
...
async function planTunnel(options: CliOptions, log: (line: string) => void): Promise<TunnelPlan | { error: string }> {
  ...
  if (credential) {
    ...
    if (!describeTunnelAccount(account.credentials.read()).email) {
      return { error: "no account on this machine yet: run `kind-meitner login` first, then `kind-meitner serve --tunnel`" };
    }
    ...
    if (!access) return { error: "this machine has no public address; run `kind-meitner login` again" };
  }
  let binary: string;
  try {
...
export async function runServe(options: CliOptions, log: (line: string) => void = console.log): Promise<number> {
  ...
  if (await serverUp(options.port)) {
    console.error(`something already answers on http://127.0.0.1:${options.port}; use \`kind-meitner pair\` against it, or --port for a second server`);
    return 1;
  }
  ...
  const env: NodeJS.ProcessEnv = {
    ...process.env,
    KIND_MEITNER_DATA_DIR: options.dataDir,
    KIND_MEITNER_PORT: String(options.port),
    KIND_MEITNER_WEBHOOK_PORT: process.env.KIND_MEITNER_WEBHOOK_PORT || String(options.port + 1),
  };
  if (options.local) delete env.KIND_MEITNER_PUBLIC_URL;
  if (entry.staticDir) env.KIND_MEITNER_STATIC_DIR = entry.staticDir;
  if (entry.skillsDir && !process.env.KIND_MEITNER_SKILLS_DIR) env.KIND_MEITNER_SKILLS_DIR = entry.skillsDir;
  if (options.label && !process.env.KIND_MEITNER_ENVIRONMENT_LABEL) env.KIND_MEITNER_ENVIRONMENT_LABEL = options.label;
  if (plan) env.KIND_MEITNER_TUNNEL_SOCKET = plan.origin.socketPath;
  ...
  try {
    ...
    if (startupCancelled) throw new SetupCancelled();
    if (publicUrl) env.KIND_MEITNER_PUBLIC_URL = publicUrl;
  ...
  try {
    ...
    if (exited !== null) {
      if (exited !== 0) log(`kind-meitner could not start.${logPath ? ` Details: ${logPath}` : " See the output above."}`);
      return exited;
    }
    if (stopping) return await childExit;
    if (!(await serverUp(options.port, child.pid))) {
      console.error(`kind-meitner did not become ready within a minute.${logPath ? ` Details: ${logPath}` : " See its output above."}`);
      await stop();
      return 1;
    }
    ...
    if (options.domain && caddyBinary) {
      try {
        caddy = await startCaddy({ binary: caddyBinary, dataDir: options.dataDir, domain: options.domain, appPort: options.port, webhookPort: Number(env.KIND_MEITNER_WEBHOOK_PORT), log });
    ...
    log("");
    log(`kind-meitner is running on http://127.0.0.1:${options.port}${publicUrl ? `, reachable at ${publicUrl}` : ""}`);
    if (options.guided) {
    ...
    if (options.pair && options.phone) {
      ...
      log("");
      log("another device later:  kind-meitner pair --label \"Kitchen iPad\"");
    }
    log(options.guided ? "\nKeep this terminal open while using your bots. Ctrl+C stops the server, not your saved work." : "stop with Ctrl+C");
    if (options.guided) log("Next time: kind-meitner · Change AI or phone setup: kind-meitner setup · Pair another phone: kind-meitner pair");
...
export async function runOnboardingCommand(
  ...
  if (options.command === "setup" && !interactive) {
    io.error("Setup needs an interactive terminal. Run `npx kind-meitner setup` in a terminal, then use `npx kind-meitner serve` for unattended starts.");
    return 1;
  }
  process.env.KIND_MEITNER_DATA_DIR = options.dataDir;
  ...
  try {
    if (options.command === "setup" || !(await isSetupComplete(options.dataDir))) {
      if (!interactive) {
        io.error("No completed setup was found. Run `npx kind-meitner setup` in an interactive terminal first, or use `npx kind-meitner serve` with an existing configuration.");
        return 1;
      }
      if (!(await runSetup({ dataDir: options.dataDir, port: options.port }))) {
        io.log("Setup cancelled. Run kind-meitner when you're ready.");
        return 130;
      }
    }
    ...
    if (options.command === "setup") {
      io.log("\nAll set. Start with: kind-meitner (or npx kind-meitner without a global install).");
      if (options.dataDir !== join(homedir(), ".kind-meitner") || options.port !== 8799) {
    ...
    if (!(error instanceof SetupCancelled)) throw error;
    io.log("\nSetup stopped. Any AI setup already saved is kept; no server was started. Run kind-meitner setup to continue.");
    return 130;
  }
}
    ...
    case "fleet":
      if (options.fleetAction === "agent") {
        ...
        await startFleetAgent({
          ...
          node: process.execPath,
          script: process.argv[1] ?? "",
          licenseKey: options.licenseKey ?? process.env.KIND_MEITNER_LICENSE_KEY,
        }, { log: (line) => console.log(line) });
