...
describe("commandSummary", () => {
  it("is the shell command, cut at 200", () => {
    expect(commandSummary({ command: "pnpm control:kind-meitner doctor --url http://127.0.0.1:1", url: "x" }))
      .toBe("pnpm control:kind-meitner doctor --url http://127.0.0.1:1");
  ...
  it("says nothing for a call that runs no command — a Read, an Edit, a fetch", () => {
    ...
    expect(commandSummary({ file_path: "scripts/control-kind-meitner.ts", old_string: "a", new_string: "b" })).toBeUndefined();
