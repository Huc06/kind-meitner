...
describe("connector MCP bridge", () => {
  it("turns agent connection requests into authenticated chat-card requests", async () => {
    ...
    const lines = start({
      KIND_MEITNER_HARNESS_URL: harness,
      KIND_MEITNER_CONNECTOR_TOKEN: "bridge-secret",
      // A simultaneously mounted agents proxy may define this different
      // value in Codex's flattened child environment.
      KIND_MEITNER_COMMS_TOKEN: "agents-secret",
  ...
  it("answers initialize after a bounded wait when the upstream stalls", async () => {
    ...
    const lines = start({ KIND_MEITNER_CONNECTOR_UPSTREAM_URL: upstream });
    child!.stdin.write(`${JSON.stringify({
      jsonrpc: "2.0",
      id: 11,
      method: "initialize",
  ...
  it("still opens the upstream MCP session on initialize without echoing secrets", async () => {
    ...
    const lines = start({
      KIND_MEITNER_CONNECTOR_UPSTREAM_URL: upstream,
      KIND_MEITNER_CONNECTOR_UPSTREAM_HEADERS: JSON.stringify({ authorization: "Bearer upstream-secret" }),
    });
    ...
    expect(reply.result.serverInfo).toEqual({ name: "kind-meitner-connectors", version: "1" });
