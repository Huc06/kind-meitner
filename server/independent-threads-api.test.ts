...
import { launchVerificationServer, runControlKindMeitner, type VerificationServer } from "../scripts/control-kind-meitner.ts";
...
describe("independent bot tasks through the isolated control surface", () => {
  ...
  const control = async (args: string[]) => {
    const result = await runControlKindMeitner([...args, "--url", session.info.url]);
  ...
  beforeEach(async () => {
    ...
    writeFileSync(wrapper, [
      ...
      'process.env.KIND_MEITNER_FIXTURE_CWD = process.cwd();',
  ...
  it("queues coordinated work behind a peer's approval and delivers it once without another user prompt", async () => {
    ...
    const token = (await dump(models[0])).mcpConfig.mcpServers.agents.env.KIND_MEITNER_COMMS_TOKEN;
  ...
  it("rejects blank memory replacements, caps new titles, and retains project files after deletion", async () => {
    ...
    const token = launched.mcpConfig.mcpServers.agents.env.KIND_MEITNER_COMMS_TOKEN;
  ...
  it("keeps A and B independent across selection, models, approvals, and stopping A", async () => {
    ...
    expect(launchedA.env.KIND_MEITNER_FIXTURE_CWD).toBe(realpathSync(join(session.info.dataDir, "task-workspaces", botId, taskA)));
    expect(launchedB.env.KIND_MEITNER_FIXTURE_CWD).toBe(realpathSync(join(session.info.dataDir, "task-workspaces", botId, taskB)));

    const tokenA = launchedA.mcpConfig.mcpServers.agents.env.KIND_MEITNER_COMMS_TOKEN;
    const tokenB = launchedB.mcpConfig.mcpServers.agents.env.KIND_MEITNER_COMMS_TOKEN;
  ...
  it("refuses a second engine in the same selected project folder until its owner stops", async () => {
    ...
    expect((await dump(models[0])).env.KIND_MEITNER_FIXTURE_CWD).toBe(realpathSync(cwd));
    ...
    expect((await dump(models[1])).env.KIND_MEITNER_FIXTURE_CWD).toBe(realpathSync(cwd));
  ...
  it.skipIf(process.platform !== "darwin")("claims the shared computer only on first use and keeps a sibling stop from releasing it", async () => {
    ...
    const descriptorDir = join(session.info.dataDir, "Library", "Application Support", "kind-meitner");
    ...
    const gate = (launched: any) => {
      ...
      const url = new URL(env.KIND_MEITNER_CONTROL_URL);
      expect(url.origin).toBe(session.info.url);
      return internal(env.KIND_MEITNER_CONTROL_TOKEN, "GET", `${url.pathname}${url.search}`);
    };
