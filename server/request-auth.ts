 ...
 * is there to forward traffic from elsewhere (`kind-meitner serve --tunnel`),
...
export function sessionCookieName(port: number, environmentId: string): string {
  return `kind_meitner_session_${port}_${environmentId.replace(/[^a-z0-9]/gi, "").slice(0, 12)}`;
}
...
const DESKTOP_OWNER_HEADER = "x-kind-meitner-desktop-owner";
...
export function resolveRequestAuth(req: IncomingMessage, options: ResolveOptions): RequestAuthResult {
  ...
  if (bearer?.startsWith("kind_meitner_sess_")) {
  ...
  if (loopback) {
    const companionToken = headerValue(req.headers["x-kind-meitner-companion-auth"]);
    if (companionToken && options.loopbackMutationToken !== undefined) {
      if (
        !secureTokenMatch(companionToken, options.companionMutationToken ?? "") ||
        req.headers["x-kind-meitner-companion"] !== "1" ||
        !/^[\w-]{1,128}$/.test(headerValue(req.headers["x-kind-meitner-companion-device"]) ?? "") ||
