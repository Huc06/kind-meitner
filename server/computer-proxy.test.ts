...
describe("computer proxy (fake box)", () => {
      ...
      if (url.pathname.endsWith("/commands")) {
        ...
        req.on("end", () => {
          ...
          const stdout = isWait
            ...
            : command.includes("kind-meitner-cdp.mjs snapshot")
              ? JSON.stringify({
                  title: "Account",
              ...
              : command.includes("kind-meitner-cdp.mjs click") || command.includes("kind-meitner-cdp.mjs fill")
                ? `GEOM 1920 1080\nHASH ${hash}\nSIZE ${size}\nB64 ${JPEG}\nSEM ok\n`
  ...
  it("uses fresh semantic browser refs without exposing URL secrets", async () => {
    ...
    expect(commands.length - before).toBe(1);
    expect(commands.at(-1)).toContain("kind-meitner-cdp.mjs fill");
  ...
  it("uses a private Chrome profile, strips URL credentials, and reports redirects", async () => {
    ...
    expect(issued[0]).toContain('profile="$HOME/.kind-meitner/chrome-profile"');
    ...
    expect(issued[0]).toContain('--user-data-dir="$HOME/.kind-meitner/chrome-profile"');
  ...
  beforeAll(async () => {
    ...
    proxy = spawn(process.execPath, ["--experimental-strip-types", PROXY], {
      env: {
        ...
        OGB_BOX_ID: "box-1",
        OGB_BOX_TOKEN: "t",
        KIND_MEITNER_CONTROL_URL: `http://127.0.0.1:${controlPort}/api/internal/computer-control?botId=b1`,
        KIND_MEITNER_CONTROL_TOKEN: "control-secret",
        // fast cadence so the wait tests measure logic, not wall-clock
        KIND_MEITNER_CONTROL_POLL_MS: "25",
        KIND_MEITNER_CONTROL_WAIT_MS: "1500",
      },
      stdio: ["pipe", "pipe", "pipe"],
    });
    let buf = "";
