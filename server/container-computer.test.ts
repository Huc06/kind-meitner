...
const readinessProbe =
  ...
  "--screenshot-out-file /tmp/kind-meitner-readiness.png";
const readinessRead = `docker exec ${CONTAINER} base64 -w0 /tmp/kind-meitner-readiness.png`;
...
describe("containerComputerStatus", () => {
  ...
  it.each(["exact", "legacy", "missing-effective", "missing-bounding", "extra-effective", "extra-bounding"])(
    ...
    const target: LocalVmTarget = {
      ...derived,
      workspaceDir: "C:\\Users\\light\\.kind-meitner\\vm-homes\\win-target",
    };
    const detail = JSON.parse(perBotReadyInspect("bot-win", 41629))[0];
    detail.Mounts[0].Source = "/mnt/c/Users/light/.kind-meitner/vm-homes/win-target";
    detail.HostConfig = {
      ...detail.HostConfig,
    ...
    const fake = runner({
      ...
      [`${targetDriverExec} call get_desktop_state {} --socket ${CUA_SOCKET} --screenshot-out-file /tmp/kind-meitner-readiness.png`]: "{}\n",
      [`podman exec ${target.containerName} base64 -w0 /tmp/kind-meitner-readiness.png`]: validPng.toString("base64"),
    });
  ...
  it("keeps per-bot identities, workspaces, and ephemeral viewer ports separate", async () => {
    ...
    const fake = runner({
      ...
      [`${targetDriverExec} call get_desktop_state {} --socket ${CUA_SOCKET} --screenshot-out-file /tmp/kind-meitner-readiness.png`]: "{}\n",
      [`docker exec ${target.containerName} base64 -w0 /tmp/kind-meitner-readiness.png`]: validPng.toString("base64"),
    });
  ...
  it("refuses a per-bot container carrying another target's label", async () => {
    ...
    expect(status.problem).toContain("not created by kind-meitner");
  });
...
describe("Cua integration", () => {
  ...
  it("builds an exact, checksum-verified Cua Driver 0.20.0 image", () => {
    ...
    expect(dockerfile).toContain("prepare-kind-meitner-workspace.sh");
  ...
  it("captures the preview through Cua Driver rather than xdotool or VNC", async () => {
    const screenshotCall =
      `${driverExec} call get_desktop_state {} --socket ${CUA_SOCKET} ` +
      "--screenshot-out-file /tmp/kind-meitner-preview.png";
    const png = validPng;
    const fake = runner({
      "/usr/bin/which docker": "docker\n",
      "/usr/bin/which podman": new Error("missing"),
      ...
      [screenshotCall]: "{}\n",
      [`docker exec ${CONTAINER} base64 -w0 /tmp/kind-meitner-preview.png`]: png.toString("base64"),
    });
...
describe("containerComputerAction", () => {
  it("never removes an exact-name container without kind-meitner ownership labels", async () => {
    const fake = runner({
      "/usr/bin/which docker": "docker\n",
      "/usr/bin/which podman": new Error("missing"),
      "docker info --format {{.ServerVersion}}": "29\n",
    ...
    await expect(containerComputerAction("remove", fake.run, "linux")).rejects.toThrow(
      /not created by kind-meitner.*remove it manually/i,
    );
    expect(fake.calls).not.toContain(`docker rm -f ${CONTAINER}`);
  });

  it("removes a verified kind-meitner container even when its version labels are stale", async () => {
    const fake = runner({
      "/usr/bin/which docker": "docker\n",
      "/usr/bin/which podman": new Error("missing"),
      "docker info --format {{.ServerVersion}}": "29\n",
...
describe("setupCommands", () => {
  ...
  it("uses an explicit local image name so Podman never resolves the managed build on Docker Hub", () => {
    expect(IMAGE).toMatch(/^localhost\/kind-meitner\/cua-local-vm:/);
    expect(setupCommands("podman", "darwin").run).toContain(IMAGE);
    expect(setupCommands("podman", "darwin").run).not.toContain("docker.io/kind-meitner");
  });
