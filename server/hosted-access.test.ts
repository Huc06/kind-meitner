...
beforeAll(async () => {
  home = mkdtempSync(join(tmpdir(), "kind-meitner-hosted-server-"));
  stateFile = join(home, "portal-fixture.json"); state();
  const data = join(home, ".kind-meitner");
  ...
  child = spawn(process.execPath, [join(ROOT, "server/index.ts")], { cwd: ROOT, env: fixtureEnv = {
    ...
    HOME: home, USERPROFILE: home, KIND_MEITNER_PORT: String(PORT), KIND_MEITNER_WEBHOOK_PORT: String(PORT + 1),
    KIND_MEITNER_STATIC_DIR: join(home, "static"), KIND_MEITNER_BROWSER_CONNECTION: join(home, "browser-connection.json"),
    KIND_MEITNER_ENTERPRISE_DIR: layer, KIND_MEITNER_LICENSE_KEY: "fixture-only", KIND_MEITNER_ADMIN_URL: "https://admin.example.test",
    KIND_MEITNER_ADMIN_WORKSPACE: "acme", KIND_MEITNER_PUBLIC_URL: `https://${HOST}`,
...
describe("hosted bridge in the full server", () => {
  it("loads the optional hook before listening, redirects hosted navigation, and disables legacy sign-in", async () => {
    ...
    expect((await call("/.well-known/kind-meitner/environment")).body.capabilities.emailSignIn).toBe(false);
  ...
  it("fails closed if a configured deployment loses its enterprise hook, with local owner access retained", async () => {
    await restart({ KIND_MEITNER_ENTERPRISE_DIR: join(home, "absent-layer"), KIND_MEITNER_ADMIN_MEMBERSHIP: "portal" });
    expect((await call("/api/health/hosted")).status).toBe(503);
  ...
  it("uses explicit portal membership without local allow-list synchronization and still revokes quiet streams", async () => {
    await waitForExit(child, { signal: "SIGTERM" });
    state();
    writeFileSync(join(home, ".kind-meitner", "config.json"), JSON.stringify({ signIn: { admins: [], members: [] }, instances: { fixture: { driver: "hosted-access-test-shadow" } } }));
    child = spawn(process.execPath, [join(ROOT, "server/index.ts")], { cwd: ROOT, env: { ...fixtureEnv, KIND_MEITNER_ADMIN_MEMBERSHIP: "portal" }, stdio: ["ignore", "pipe", "pipe"] });
    ...
    expect(readiness.body).toEqual({ ok: true, service: "kind-meitner", membershipAuthority: "portal", workspace: "acme", ...HOSTED_CONTRACT_METADATA });
  ...
  it.each([
    ["standalone", { KIND_MEITNER_ADMIN_URL: undefined, KIND_MEITNER_ADMIN_WORKSPACE: undefined, KIND_MEITNER_ADMIN_MEMBERSHIP: undefined }],
    ["incomplete hosted configuration", { KIND_MEITNER_ADMIN_URL: undefined, KIND_MEITNER_ADMIN_MEMBERSHIP: "portal" }],
    ["invalid workspace", { KIND_MEITNER_ADMIN_WORKSPACE: "../private", KIND_MEITNER_ADMIN_MEMBERSHIP: "portal" }],
    ["invalid membership mode", { KIND_MEITNER_ADMIN_MEMBERSHIP: "invalid" }],
    ["local membership mode", { KIND_MEITNER_ADMIN_MEMBERSHIP: "local" }],
  ...
  it.each([{ invalid: true }, { features: [] }])("does not attest portal readiness without valid admin entitlement (%j)", async (license) => {
    ...
    await restart({ KIND_MEITNER_ADMIN_MEMBERSHIP: "portal" });
    expect((await call("/api/health/hosted")).status).toBe(503);
    expect((await call("/api/health")).status).toBe(200);
  }, 25_000);
