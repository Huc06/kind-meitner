...
it("dispatches setup and canonical standing instructions through the real isolated app", async () => {
  ...
  try {
    const { bot } = await runControlKindMeitner(["new-bot", "--name", "Setup fixture"], { env }) as any;
    const turn = async (text: string) => {
      await runControlKindMeitner(["send", "--bot", bot.id, "--text", text], { env });
      await runControlKindMeitner(["wait", "--bot", bot.id, "--timeout", "30"], { env });
    ...
    const messages = await runControlKindMeitner(["messages", "--bot", bot.id, "--limit", "10"], { env });
