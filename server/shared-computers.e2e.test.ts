...
beforeAll(async () => {
  ...
  expect(pairing.token).toMatch(/^kind_meitner_sess_/);
  ...
  expect(agents.env.KIND_MEITNER_COMMS_TOKEN).toBeTruthy();
...
it("explicit edits and terminal work; old credentials and other sessions cannot impersonate the desktop", async () => {
  ...
  expect((await api("POST", `/api/shared-computers/${old.id}/lease`, {}, { ...auth, "x-kind-meitner-computer-secret": old.secret })).status).toBe(403);
  ...
  expect((await api("POST", `/api/shared-computers/${grant.id}/lease`, {}, { authorization: `Bearer ${other.token}`, "x-kind-meitner-computer-secret": grant.secret })).status).toBe(403);
  ...
  expect((await api("POST", `/api/shared-computers/${grant.id}/lease`, {}, { cookie, origin: "https://evil.invalid", "x-kind-meitner-computer-secret": grant.secret })).status).toBe(403);
...
it("disabling the local gate cancels a live job even while the remote workspace still permits sharing", async () => {
  ...
  expect((await api("GET", "/.well-known/kind-meitner/environment")).body.capabilities.sharedComputers).toBe(true);
...
it("withdrawing the workspace flag closes pending jobs and refuses a previously advertised tool", async () => {
  ...
  expect((await api("GET", "/.well-known/kind-meitner/environment")).body.capabilities).not.toHaveProperty("sharedComputers");
