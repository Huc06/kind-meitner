...
const gate = (c: any) => fetch(c.env.KIND_MEITNER_CONTROL_URL, { headers: { authorization: `Bearer ${c.env.KIND_MEITNER_CONTROL_TOKEN}` } });

beforeAll(async () => {
  fixtureHome = mkdtempSync(join(tmpdir(), "kind-meitner-group-vm-"));
  ...
  child = spawn(process.execPath, ["--import", pathToFileURL(join(ROOT, "server/testing/group-local-vm-hooks.mjs")).href, join(ROOT, "server/index.ts")], {
    cwd: ROOT, env: {
      ...
      HOME: fixtureHome, USERPROFILE: fixtureHome, KIND_MEITNER_DATA_DIR: data,
      ...
      KIND_MEITNER_PORT: String(port), KIND_MEITNER_WEBHOOK_PORT: String(port + 1), KIND_MEITNER_STATIC_DIR: ui, KIND_MEITNER_TEST_VM_STATE: stateFile,
...
describe("Group Local VM ownership on the real isolated server", () => {
  ...
  it("revokes the previous member and rejects cross-bot control after a shared desktop handoff", async () => {
    ...
    expect((await gate(first)).status).toBe(401);
    const impersonation = await fetch(second.env.KIND_MEITNER_CONTROL_URL.replace(bots[1].id, bots[0].id), {
      headers: { authorization: `Bearer ${second.env.KIND_MEITNER_CONTROL_TOKEN}` },
    });
