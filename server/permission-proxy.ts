...
// kind-meitner server and waits for the human's answer.
//
...
const dead = () => {
  for (const resolve of waiting.values()) {
    resolve({ behavior: "deny", message: "kind-meitner: permission broker unavailable — skip this action" });
  }
  waiting.clear();
};
conn.on("error", dead);
...
async function answerNativeQuestions(input: unknown): Promise<string> {
  ...
  if (!questions?.length) {
    return JSON.stringify({
      behavior: "deny",
      message:
        "kind-meitner: this AskUserQuestion call had no answerable question (each one needs question text), so nobody was shown it. Ask again with a well-formed call, or continue without it.",
    });
  }
  ...
  if (answer.behavior === "deny") {
    return JSON.stringify({ behavior: "deny", message: answer.message || "Denied from kind-meitner" });
  }
...
const TOOLS = [
  {
    name: "approve",
    description: "Ask the kind-meitner user whether a tool use is allowed",
    inputSchema: {
      type: "object",
      properties: {
  ...
  if (msg.method === "initialize") {
    return send({
      ...
      result: {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: "kind-meitner-permissions", version: "1" },
      },
    });
  }
  ...
  if (msg.method === "tools/call") {
    ...
    if (!isQuestion) {
      if (answer.behavior === "allow") {
        ...
        text = JSON.stringify({ behavior: "deny", message: answer.message || "Denied from kind-meitner" });
      }
    }
    return reply(text);
  }
