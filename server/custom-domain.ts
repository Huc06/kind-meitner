...
const DESCRIPTOR_PATH = "/.well-known/kind-meitner/environment";
export const CUSTOM_DOMAIN_CHALLENGE_PATH = "/.well-known/kind-meitner/domain-check/";
...
export function customDomainIpv4(
  ...
  configured = process.env.KIND_MEITNER_PUBLIC_IPV4,
): string | null {
  if (configured?.trim()) {
...
function requestJson(url: URL, address: LookupAddress, signal: AbortSignal): Promise<unknown> {
  return new Promise((resolve, reject) => {
    ...
    const request = httpsRequest(url, {
      ...
      headers: { Accept: "application/json", "User-Agent": "kind-meitner-domain-check" },
    }, (response) => {
      // Node's HTTPS client does not follow Location redirects.
      if (response.statusCode !== 200) {
        response.destroy();
        reject(new CustomDomainError("The domain did not return the kind-meitner verification page. Check your reverse proxy; redirects are not supported."));
        return;
      }
      const chunks: Buffer[] = [];
      let size = 0;
      ...
      response.on("data", (chunk: Buffer) => {
        ...
        if (size > MAX_RESPONSE_BYTES) {
          response.destroy();
          reject(new CustomDomainError("The domain returned an unexpected response. Check that it points to kind-meitner."));
          return;
        }
        chunks.push(chunk);
      });
      response.on("end", () => {
        ...
        catch { reject(new CustomDomainError("The domain did not return kind-meitner data. Check that it points to this server.")); }
      });
    });
    ...
    async verify(raw: string): Promise<{ origin: string; verifiedAt: string }> {
      ...
      try {
        ...
        if (!descriptor || typeof descriptor !== "object"
          ...
          throw new CustomDomainError("This domain does not point to this kind-meitner server. Check its DNS and reverse proxy configuration.");
        }
        active = {
        ...
        if (!proof || typeof proof !== "object" || Reflect.get(proof, "environmentId") !== dependencies.environmentId
          ...
          throw new CustomDomainError("The domain could not prove it reaches this server. Forward all kind-meitner routes through your reverse proxy and try again.");
        }
