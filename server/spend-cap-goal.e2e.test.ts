...
describe("spend cap inside a team goal run", () => {
  ...
  beforeEach(async () => {
    layerDir = mkdtempSync(join(tmpdir(), "kind-meitner-fake-layer-goal-"));
    ...
    session = await launchVerificationServer({
      ...
      FAKE_CLAUDE_REPLIES: JSON.stringify([
        'The plan is ready.\n<kind-meitner-goal>{"status":"continue","next":"Worker","instruction":"Do the thing","detail":"Plan ready"}</kind-meitner-goal>',
      ]),
