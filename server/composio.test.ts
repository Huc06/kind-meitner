...
beforeAll(async () => {
  api = createServer(async (req, res) => {
    ...
    if (req.method === "GET" && url.pathname === "/api/v3.1/tool_router/session/trs_legacy") {
      ...
      return res.end(JSON.stringify({
        ...
        config: { user_id: "kind_meitner_legacy" },
      }));
    }
  ...
  process.env.KIND_MEITNER_COMPOSIO_API = base;
  process.env.KIND_MEITNER_COMPOSIO_TOOLKITS_API = `${origin}/api/v3`;
});

afterAll(async () => {
  setManagedBrokerAccess(null);
  delete process.env.KIND_MEITNER_COMPOSIO_API;
  delete process.env.KIND_MEITNER_COMPOSIO_TOOLKITS_API;
  await new Promise<void>((resolve) => api.close(() => resolve()));
});

describe.sequential("Composio Sessions", () => {
  it("rejects broker URL components and invalid tokens from the environment", () => {
    process.env.KIND_MEITNER_COMPOSIO_BROKER_TOKEN = "a".repeat(64);
    try {
      for (const url of [
        ...
        process.env.KIND_MEITNER_COMPOSIO_BROKER_URL = url;
        expect(() => connectionMode({})).toThrow(/must not include/);
      }
      process.env.KIND_MEITNER_COMPOSIO_BROKER_URL = "http://[::1]:3210/root/";
      expect(connectionMode({})).toBe("managed");
      process.env.KIND_MEITNER_COMPOSIO_BROKER_TOKEN = "short";
      expect(() => connectionMode({})).toThrow(/token is invalid/);
    } finally {
      delete process.env.KIND_MEITNER_COMPOSIO_BROKER_URL;
      delete process.env.KIND_MEITNER_COMPOSIO_BROKER_TOKEN;
    }
  });
  ...
  it("ignores credential sync without access and clears only on explicit null", () => {
    const messageType = "kind-meitner:managed-composio";
    setManagedBrokerAccess({ url: "http://127.0.0.1:3210/", token: "a".repeat(64) });
  ...
  it("drops a managed MCP session when routing switches to a user project", async () => {
    ...
    try {
      ...
      const project = await relayMcp(
        { composio: { apiKey: "ak_test", userId: "kind_meitner_existing", sessionId: "trs_test" } },
  ...
  it("creates one stable per-installation session and reuses it", async () => {
    const created = await prepareProjectSession("ak_test", { userId: "kind_meitner_existing" });
    expect(created).toEqual({
      apiKey: "ak_test",
      userId: "kind_meitner_existing",
      sessionId: "trs_test",
    });
  ...
  it("names the project's own auth configs at creation and rebuilds a Session that predates them", async () => {
    ...
    sessionAuthConfigs = {};
    try {
      const current = { apiKey: "ak_test", userId: "kind_meitner_existing", sessionId: "trs_test" };
      const before = calls.length;
      ...
      expect(creates[0].body).toMatchObject({ user_id: "kind_meitner_existing", auth_configs: { twitter: "ac_twitter" } });
  ...
  it("rebuilds the Session and retries when a toolkit needs the project's own auth config", async () => {
    ...
    try {
      ...
      expect(cfg.composio).toMatchObject({ userId: "kind_meitner_existing", sessionId: "trs_test" });
    } finally {
      customAuthConfigs = [];
      sessionAuthConfigs = {};
    }
  ...
  it("does not offer Twitter through the official managed broker without an owned OAuth app", async () => {
    setManagedBrokerAccess({
      url: "https://broker.kind-meitner.test",
      token: "a".repeat(64),
    });
    try {
  ...
  it("mounts the Session MCP endpoint with the project key header", async () => {
    ...
    expect(integration).toMatchObject({
      ...
      env: {
        ...
        KIND_MEITNER_CONNECTOR_UPSTREAM_HEADERS: JSON.stringify({ authorization: "Bearer secret" }),
        KIND_MEITNER_HARNESS_URL: "http://127.0.0.1:8799",
        KIND_MEITNER_CONNECTOR_TOKEN: "secret",
        KIND_MEITNER_BOT_ID: "bot-1",
        KIND_MEITNER_THREAD_ID: "thread-1",
      },
    });
  });
