...
export type RoutineTarget = "bot" | "room-goal" | "okx-task";
...
export interface RoutineManagerOptions {
  ...
  okxTaskState?: () => "ready" | "busy" | "missing";
  startOkxTask?: (
    run: RoutineRun,
    prompt: string,
    onDispatchError: (message: string) => void,
...
function loadTarget(value: unknown): RoutineTarget {
  if (value === "okx-task") return "okx-task";
...
function sanitizeInput(input: RoutineInput, after: number): Omit<Routine, "id" | "createdAt" | "updatedAt" | "nextRunAt"> {
  ...
  if (target !== "bot" && target !== "room-goal" && target !== "okx-task") throw new Error("Choose a valid routine target");
...
export class RoutineManager {
  ...
  constructor(options: RoutineManagerOptions) {
    ...
    for (const run of this.runs) {
      if (run.status === "running" || run.status === "waiting") {
        ...
        run.error = "kind-meitner restarted while this routine was running";
    ...
    try {
      ...
      for (const id of this.runs.map((run) => run.id)) {
        ...
        try {
          ...
          const triggerSource = run.triggerSource ?? (run.manual ? "manual" : "schedule");
          if (run.target === "okx-task") {
            if (!this.options.startOkxTask) {
              this.failThread(task.threadId, "OKX task engine is unavailable");
              continue;
            }
            await this.options.startOkxTask(
              run,
              prompt,
              (message) => this.failThread(task.threadId, message),
            );
          } else if (run.target === "room-goal") {
  ...
  private targetState(target: Pick<RoutineRun, "target" | "groupId" | "botId">): "ready" | "busy" | "missing" {
    if (target.target === "okx-task") {
      if (this.options.okxTaskState) return this.options.okxTaskState();
  ...
  private missingTargetMessage(target: RoutineTarget): string {
    if (target === "okx-task") return "The assigned bot or OKX task engine is unavailable";
