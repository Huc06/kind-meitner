...
readline.createInterface({input: process.stdin}).on("line", (line) => {
  ...
  process.stdout.write(JSON.stringify({jsonrpc:"2.0",id:message.id,result:{
    ...
    tokenPresent:Boolean(process.env.KIND_MEITNER_CONTROL_TOKEN),
...
describe("local computer proxy (isolated child and control endpoint)", () => {
  it.each(["node", "electron"])("%s: keeps discovery lease-free, refuses contention and outages, and drains the final gated frame", async (runtime) => {
    ...
    const home = mkdtempSync(join(tmpdir(), "kind-meitner-cua-launch-"));
    ...
    try {
      if (runtime === "electron") {
        ...
        const bootstrap = spawnSync(electron, ["--input-type=module", "-e", `
          ...
          const { original, control } = JSON.parse(process.env.KIND_MEITNER_FIXTURE_CONNECTION);
          process.stdout.write(JSON.stringify(gatedLocalComputer(original, control)));
        `], {
          env: { ...process.env, HOME: home, USERPROFILE: home, ELECTRON_RUN_AS_NODE: "1", KIND_MEITNER_FIXTURE_CONNECTION: JSON.stringify({ original, control }) },
      ...
      child = spawn(connection.command, connection.args, { env: { ...process.env, ...connection.env,
        HOME: home, USERPROFILE: home, KIND_MEITNER_EXTRA_PATH: dirname(process.execPath), PATH: "",
  ...
  it("rejects malformed environment and missing authority without printing connection secrets", () => {
    for (const overrides of [
      { KIND_MEITNER_CUA_ARGS: "not-json-private-value" },
      { KIND_MEITNER_CUA_ARGS: '["mcp",7]' },
      { KIND_MEITNER_CONTROL_TOKEN: "" },
      { KIND_MEITNER_CONTROL_URL: "https://outside.example/control" },
    ]) {
      const result = spawnSync(process.execPath, ["--experimental-strip-types", SPAWNED_PROXIES.localComputer], {
        env: { ...process.env, KIND_MEITNER_CUA_COMMAND: process.execPath, KIND_MEITNER_CUA_ARGS: "[]", KIND_MEITNER_CONTROL_URL: "http://127.0.0.1:1/control", KIND_MEITNER_CONTROL_TOKEN: "private-fixture-token", ...overrides },
