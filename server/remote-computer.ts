...
// inside the VM and kind-meitner never exposes another inbound port.
...
export const REMOTE_CUA_SESSION = "kind-meitner";
export const REMOTE_CDP_HELPER = "/opt/ogb/kind-meitner-cdp.mjs";
...
export function remoteComputerBootstrapCommand(botName: string): string {
  ...
  const banner = Buffer.from(`  ▦ ${botName}'s computer — kind-meitner`).toString("base64");
