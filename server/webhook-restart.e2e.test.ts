...
it("does not run a delivery twice after a process dies between the queue and ingress commits", async () => {
  ...
  try {
    ...
    const wait = await runControlKindMeitner(["wait", "--bot", bot.id, "--task", run.threadId!, "--url", url]);
    const messages = await runControlKindMeitner(["messages", "--bot", bot.id, "--task", run.threadId!, "--url", url]);
