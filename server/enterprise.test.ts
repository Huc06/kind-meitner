...
function fakeLayer(registerSource: string, file = "index.ts"): string {
  const dir = mkdtempSync(join(tmpdir(), "kind-meitner-enterprise-"));
...
afterEach(async () => {
  await loadEnterpriseLayer({ dir: join(tmpdir(), "kind-meitner-enterprise-absent"), licenseKey: undefined });
...
describe("enterprise hook point", () => {
  it("does not create a hosted bridge unless opted in, and marks partial configuration as hosted", () => {
    ...
    expect(hostedWorkspaceConfigured({ KIND_MEITNER_PUBLIC_URL: "https://legacy.example.test" })).toBe(false);
    expect(hostedWorkspaceConfigured({ KIND_MEITNER_ADMIN_URL: "" })).toBe(true);
    expect(hostedWorkspaceConfigured({ KIND_MEITNER_ADMIN_WORKSPACE: "acme" })).toBe(true);
    expect(hostedWorkspaceConfigured({ KIND_MEITNER_ADMIN_MEMBERSHIP: "portal" })).toBe(true);
    ...
    expect(createWorkspaceAccess({ sessions, cookieName: "session", closeSessionStreams() {}, env: { KIND_MEITNER_ADMIN_WORKSPACE: "acme" } })).toBeNull();
  });
  it("requires complete valid hosted configuration before opting into portal membership", () => {
    const env = { KIND_MEITNER_ADMIN_URL: "https://admin.example.test", KIND_MEITNER_ADMIN_WORKSPACE: "acme", KIND_MEITNER_PUBLIC_URL: "https://acme.example.test" };
    expect(hostedWorkspaceConfiguration(env)?.portalMembership).toBe(false);
    expect(hostedWorkspaceConfiguration({ ...env, KIND_MEITNER_ADMIN_MEMBERSHIP: "portal" })?.portalMembership).toBe(true);
    expect(hostedWorkspaceConfiguration({ ...env, KIND_MEITNER_ADMIN_MEMBERSHIP: "local" })?.portalMembership).toBe(false);
    for (const patch of [{ KIND_MEITNER_ADMIN_URL: "" }, { KIND_MEITNER_PUBLIC_URL: "http://acme.example.test" }, { KIND_MEITNER_ADMIN_WORKSPACE: "" }, { KIND_MEITNER_ADMIN_MEMBERSHIP: "PORTAL" }]) {
      expect(hostedWorkspaceConfiguration({ ...env, KIND_MEITNER_ADMIN_MEMBERSHIP: "portal", ...patch })).toBeNull();
    }
    const dir = fakeLayer("export const fixture = true;");
    const invalid = { KIND_MEITNER_ADMIN_MEMBERSHIP: "portal" };
  ...
  it("passes the live admin entitlement to the optional dynamically loaded bridge", async () => {
    ...
    const sessions = new SessionRegistry({ file: join(dir, "sessions.json") });
    const bridge = createWorkspaceAccess({ sessions, cookieName: "session", closeSessionStreams() {}, env: { KIND_MEITNER_ADMIN_WORKSPACE: "acme" } });
  ...
  it("is the open-source edition when the folder is absent, and says so if a key was set anyway", async () => {
    const absent = join(tmpdir(), "kind-meitner-enterprise-absent");
    ...
    expect(status.notice).toContain("KIND_MEITNER_LICENSE_KEY is set but no enterprise layer exists");
    expect(entitled("whitelabel")).toBe(false);
    expect(describeEdition(status)).toContain("open-source edition (KIND_MEITNER_LICENSE_KEY is set");
  });
  ...
  it("registers the layer's entitlements from the key", async () => {
    ...
    expect(describeEdition(status)).toBe("kind-meitner enterprise edition for Acme until 2027-01-01: sso, whitelabel");
  });
  ...
  it("loads a compiled layer (server/index.js) the way an image ships it", async () => {
    ...
    expect(describeEdition(status)).toBe("kind-meitner enterprise edition for Built: admin");
  });
  ...
  it("tells the operator when the layer is present but no key is configured", async () => {
    ...
    expect(status).toEqual({ edition: "oss", features: [], notice: "enterprise layer present but KIND_MEITNER_LICENSE_KEY is not set" });
  });
