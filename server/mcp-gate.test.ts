...
describe("mcp-gate", () => {
  ...
  const start = (reply: unknown, env: Record<string, string> = {}) => {
    ...
    gate = spawn(process.execPath, ["--experimental-strip-types", "--no-warnings", GATE], {
      ...
      env: {
        ...process.env,
        KIND_MEITNER_GATE_NAME: "shop",
        KIND_MEITNER_GATE_SPILL_DIR: join(scratch, "spill"),
        KIND_MEITNER_GATE_UPSTREAM: JSON.stringify({
  ...
  beforeEach(() => {
    scratch = mkdtempSync(join(tmpdir(), "kind-meitner-gate-test-"));
    lines = [];
    waiting = [];
  });
  ...
  it("trims an oversized result and saves the whole thing for the bot to read", async () => {
    ...
    expect(text).toContain("kind-meitner trimmed this tool result");
  ...
  it("honours a budget the harness sets", async () => {
    const products = Array.from({ length: 300 }, (_, i) => ({ id: `p${i}`, blurb: "x".repeat(300) }));
    start({ content: [{ type: "text", text: JSON.stringify({ products }) }], structuredContent: undefined }, { KIND_MEITNER_GATE_BUDGET: "2000" });
  ...
  it("keeps the upstream server's own environment and hides the gate's", async () => {
    ...
    expect(JSON.parse(JSON.stringify(process.env.KIND_MEITNER_GATE_UPSTREAM ?? null))).toBe(null);
  });
  ...
  it("spawns an upstream named as a bare command on PATH", async () => {
    ...
    gate = spawn(process.execPath, ["--experimental-strip-types", "--no-warnings", GATE], {
      ...
      env: {
        ...process.env,
        KIND_MEITNER_GATE_NAME: "shop",
        KIND_MEITNER_GATE_UPSTREAM: JSON.stringify({ command: "node", args: [upstreamJs], env: { SCRIPT: script } }),
      },
    }) as ChildProcessWithoutNullStreams;
    createInterface({ input: gate.stdout }).on("line", (line) => {
      const next = waiting.shift();
