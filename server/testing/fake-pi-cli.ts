...
if (process.env.FAKE_PI_DUMP) {
  try {
    ...
    // KIND_MEITNER_MCP_CONFIG; read it here so a test can assert the mount contract
    ...
    if (process.env.KIND_MEITNER_MCP_CONFIG) {
      try {
        mcpConfig = JSON.parse(readFileSync(process.env.KIND_MEITNER_MCP_CONFIG, "utf8"));
      } catch {
