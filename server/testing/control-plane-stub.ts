// A stand-in for cloudflare/control-plane for tests of `kind-meitner login`
...
const newCredential = () => `kind_meitner_install_${randomBytes(16).toString("base64url")}.${randomBytes(32).toString("base64url")}`;
...
export async function startControlPlaneStub(options: { otp?: string; endpointUrl?: string } = {}): Promise<ControlPlaneStub> {
  ...
  const endpointUrl = options.endpointUrl ?? "https://c-stub.kind-meitner.invalid";
  ...
  const server = createServer(async (req, res) => {
    ...
    if (method === "GET" && path === "/healthz") return send(200, { ok: true, service: "kind-meitner-control-plane" });
