...
// Approve by creating HOME/.kind-meitner-fake-codex-login-approved; this never opens a
// network connection or creates real provider credentials.
import { existsSync, writeFileSync } from "node:fs";
import { isAbsolute, join } from "node:path";

const home = process.env.HOME;
if (process.env.KIND_MEITNER_DEVICE_AUTH_FIXTURE !== "1" || !home || !isAbsolute(home)) {
  process.stderr.write("This fake CLI requires KIND_MEITNER_DEVICE_AUTH_FIXTURE=1 and an isolated HOME.\n");
  process.exit(2);
}
const authenticated = join(home, ".kind-meitner-fake-codex-authenticated");
const approved = join(home, ".kind-meitner-fake-codex-login-approved");
...
if (args === "--version") {
  ...
  writeFileSync(join(home, ".kind-meitner-fake-codex-device.pid"), String(process.pid), { mode: 0o600 });
  process.stderr.write("Offline kind-meitner verification fixture — do not enter this code at OpenAI.\n");
