 ...
 * owner, the way `kind-meitner` on the server or a bootstrap script talks. */
...
beforeAll(async () => {
  ...
  home = mkdtempSync(join(tmpdir(), "kind-meitner-people-invite-"));
  ...
  writeFileSync(join(home, ".kind-meitner", "config.json"), JSON.stringify({ instances: { fixture: { driver: "people-invite-test-shadow" } } }));
  child = spawn(process.execPath, [join(SERVER_DIR, "index.ts")], {
    ...
    env: {
      ...
      KIND_MEITNER_PUBLIC_URL: `https://${HOST}`,
      KIND_MEITNER_ENVIRONMENT_LABEL: "acme",
...
describe("adding people to a hosted workspace", () => {
  ...
  it("offers no email sign-in until the owner names the first admin", async () => {
    expect((await remote("/.well-known/kind-meitner/environment")).body.capabilities.emailSignIn).toBe(false);
    ...
    expect((await remote("/.well-known/kind-meitner/environment")).body.capabilities.emailSignIn).toBe(true);
