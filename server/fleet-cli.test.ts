...
const base: FleetInput = {
  ...
  node: "/usr/bin/node", script: "/usr/lib/node_modules/kind-meitner/cli.js", root: "/",
};
const registryFile = "/etc/kind-meitner/fleet.json";
const withRegistry = (workspaces = {}) => ({ [registryFile]: JSON.stringify({ ...emptyRegistry("agentada.cc"), workspaces }) });

describe("kind-meitner fleet", () => {
  it("updates only managed OpenRouter models and rejects missing, malformed or redirected config", async () => {
    ...
    const path = "/var/lib/kind-meitner/acme/.config/opencode/opencode.json";
    const cfgPath = "/var/lib/kind-meitner/acme/.kind-meitner/config.json";
    ...
    const initialFiles = { ...withRegistry({ acme: workspace }), [path]: JSON.stringify(config), [cfgPath]: '{"defaultModelSelection":{"instanceId":"claude","model":"keep"}}', "/etc/kind-meitner/instances/acme.env": "KIND_MEITNER_ADMIN_URL=https://admin.example.test\nKIND_MEITNER_ADMIN_WORKSPACE=acme\n" };
    ...
    expect(m.calls).toEqual([`read ${path} as kind-meitner-acme`, `write ${path} 600 as kind-meitner-acme`]);
  ...
  it("prints the plan instead of acting when not root, and on --dry-run even as root", async () => {
    ...
    expect(out.join("\n")).toContain("cat > /etc/systemd/system/kind-meitner@.service <<'KIND_MEITNER_EOF'");
    expect(out.join("\n")).toContain("systemctl enable --now kind-meitner-fence.service");
    ...
    expect(dry.out.join("\n")).toContain("useradd --system --create-home --home-dir /var/lib/kind-meitner/acme");
  ...
  it("refuses an npx cache as the unit's script and asks for the domain", async () => {
    ...
    const bad = io();
    expect(await runFleetCommand({ ...base, action: "init", domain: "agentada.cc", script: "/root/.npm/_npx/abc/node_modules/kind-meitner/cli.js" }, bad.io, deps)).toBe(2);
  ...
  it("creates a workspace as root in order, and stops at the first failed step without repeating secrets", async () => {
    ...
    expect(calls.slice(0, 5)).toEqual([
      ...
      "run useradd --system --create-home --home-dir /var/lib/kind-meitner/acme --shell /usr/sbin/nologin --user-group kind-meitner-acme",
      `write ${registryFile} 600`,
      "mkdir /var/lib/kind-meitner/acme/.kind-meitner 700 as kind-meitner-acme",
      "write /var/lib/kind-meitner/acme/.kind-meitner/config.json 600 as kind-meitner-acme",
    ]);
    expect(JSON.parse(files.get("/var/lib/kind-meitner/acme/.kind-meitner/config.json")!)).toEqual({
      signIn: { admins: ["ada@example.test"], members: ["@acme.test"] }, anthropic: { key: "sk-ant-fixture" }, budgets: { monthlyUsd: 40 },
    });
    expect(files.get("/var/lib/kind-meitner/acme/.kind-meitner/brand.json")).toBe('{"name":"Acme"}');
    expect(files.get("/etc/kind-meitner/instances/acme.env")).toContain("KIND_MEITNER_LICENSE_KEY=omb1.k");
  ...
  it("edits a workspace's sign-in list in place, owned by the workspace, and lists what runs", async () => {
    const dataFile = "/var/lib/kind-meitner/acme/.kind-meitner/config.json";
    ...
    expect(ready.calls).toContain(`read ${dataFile} as kind-meitner-acme`);
    expect(ready.calls).toContain(`write ${dataFile} 600 as kind-meitner-acme`);
  ...
  it("reinitializes templates without losing the operator, reservations, ports or fence", async () => {
    ...
    expect(ready.files.get("/etc/kind-meitner/fence.nft")).toBe(fenceRules(Object.values(workspaces)));
    expect(ready.files.get("/etc/systemd/system/kind-meitner-fleet.service")).toContain("--group maus");
    expect(ready.calls).toContain("run nft -f /etc/kind-meitner/fence.nft");
  ...
  it.each(["/var/lib/kind-meitner/acme", "/etc/kind-meitner/instances/acme.env", "/etc/caddy/kind-meitner.d/acme.caddy", "/etc/systemd/system/kind-meitner@acme.service.d"])("refuses residual customer path %s before taking action", async (path) => {
  ...
  it.each(["provisioning", "error", "retained"])("lists a %s reservation accurately and rejects duplicate creation before residual paths", async (status) => {
    const ready = machine({ root: true, files: { ...withRegistry({ acme: { slug: "acme", host: "acme.agentada.cc", port: 8810, webhookPort: 8811, status, createdAt: "" } }), "/etc/kind-meitner/instances/acme.env": "retained" } });
    const listed = io();
