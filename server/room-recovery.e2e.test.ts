...
it("boots and reports an interrupted routine back to its source room", async () => {
  ...
  try {
    const { bot } = await runControlKindMeitner(["new-bot", "--name", "Recovery lead", "--url", url]) as any;
    ...
    expect(runs[0]).toMatchObject({ status: "failed", error: "kind-meitner restarted while this routine was running" });
