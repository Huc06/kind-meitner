...
beforeAll(async () => {
  ...
  expect((await runControlKindMeitner(["doctor"], { env }) as any).ok).toBe(true);
  const created = await runControlKindMeitner(["new-bot", "--name", "Browser profile fixture"], { env }) as any;
