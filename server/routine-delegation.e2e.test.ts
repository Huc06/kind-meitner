...
describe("routine delegation through the isolated harness", () => {
  ...
  const control = (args: string[]) => runControlKindMeitner([...args, "--url", fixture.info.url]);
  ...
  const delegate = async (threadId: string) => {
    ...
    const result = await api("POST", "/api/internal/delegate-bot", { toBotId: peer.id, message: "Produce the fixture report." }, launched.mcpConfig.mcpServers.agents.env.KIND_MEITNER_COMMS_TOKEN);
  ...
  beforeEach(async () => {
    ...
    writeFileSync(wrapper, [
      ...
      'const thread = at < 0 ? "probe" : JSON.parse(readFileSync(process.argv[at + 1], "utf8")).mcpServers?.agents?.env?.KIND_MEITNER_THREAD_ID ?? "probe";',
  ...
  it("coordinates a new user request on a completed routine's thread without changing its recorded result", async () => {
    ...
    }, launched.mcpConfig.mcpServers.agents.env.KIND_MEITNER_COMMS_TOKEN);
