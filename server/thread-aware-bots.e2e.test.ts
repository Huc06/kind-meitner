...
const liveToken = async (threadId: string): Promise<Record<string, string>> => {
  await expect.poll(() => dumpOf(threadId)?.mcpConfig?.mcpServers?.agents?.env?.KIND_MEITNER_COMMS_TOKEN, { timeout: 15_000 }).toBeTruthy();
  return { authorization: `Bearer ${dumpOf(threadId)!.mcpConfig.mcpServers.agents.env.KIND_MEITNER_COMMS_TOKEN}` };
};
...
const handoffs = (): any[] => JSON.parse(readFileSync(join(home, ".kind-meitner", "room-handoffs.json"), "utf8"));
...
beforeAll(async () => {
  ...
  home = mkdtempSync(join(tmpdir(), "kind-meitner-thread-aware-"));
  ...
  writeFileSync(gated, [
    ...
    "    for (const server of Object.values(servers)) thread ??= server?.env?.KIND_MEITNER_THREAD_ID ?? null;",
    "  } catch {}",
    "}",
