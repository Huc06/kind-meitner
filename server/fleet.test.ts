...
describe("rendered files", () => {
  it("renders one template unit for every workspace, hardened and parameterised by slug", () => {
    const unit = templateUnit({ node: "/usr/bin/node", script: "/usr/lib/node_modules/kind-meitner/cli.js", layout });
    expect(unit).toContain("User=kind-meitner-%i");
    expect(unit).toContain("EnvironmentFile=/etc/kind-meitner/instances/%i.env");
    expect(unit).toContain("ExecStart=/usr/bin/node /usr/lib/node_modules/kind-meitner/cli.js serve --port ${KIND_MEITNER_PORT} --data-dir ${KIND_MEITNER_DATA_DIR} --public-url ${KIND_MEITNER_PUBLIC_URL} --label %i --no-pair");
    for (const line of ["PrivateTmp=yes", "NoNewPrivileges=yes", "ProtectSystem=strict", "ReadWritePaths=/var/lib/kind-meitner/%i"]) expect(unit).toContain(line);
  ...
  it("requires the loopback fence to start successfully before every tenant instance", () => {
    const unit = templateUnit({ node: "/usr/bin/node", script: "/src/server/kind-meitner.ts", layout });
    const header = unit.split("[Service]")[0];
    expect(header).toContain("After=network-online.target kind-meitner-fence.service\n");
    expect(header).toContain("Requires=kind-meitner-fence.service\n");
    const fence = fenceUnit(layout);
    expect(fence).toContain("Type=oneshot\nRemainAfterExit=yes\nExecStart=/usr/sbin/nft -f /etc/kind-meitner/fence.nft\n");
    expect(fence).not.toContain("Before=kind-meitner@.service");
  });
  ...
  it("fences each workspace's loopback ports to its own user, Caddy and root", () => {
    ...
    expect(rules).toContain("add table inet kind-meitner\nflush table inet kind-meitner");
    expect(rules.indexOf("kind-meitner-acme")).toBeLessThan(rules.indexOf("kind-meitner-globex"));
    expect(rules).toContain("oif lo tcp dport { 8810, 8811 } meta skuid != { kind-meitner-acme, caddy, root } reject");
    ...
    expect(fenceRules([incomplete])).not.toContain("kind-meitner-pending");
    expect(fenceRules([{ ...incomplete, accountCreated: true }])).toContain("kind-meitner-pending");
    expect(fenceRules([{ ...incomplete, status: "error", accountCreated: true }])).toContain("kind-meitner-pending");
    expect(fenceRules([{ ...incomplete, status: "retained" }])).toContain("kind-meitner-pending");
  });
  ...
  it("writes the environment, the first config and the sign-in edits the server reads live", () => {
    ...
    expect(instanceEnv({ workspace, dataDir: "/var/lib/kind-meitner/acme/.kind-meitner", licenseKey: "omb1.k" })).toBe(
      "KIND_MEITNER_DATA_DIR=/var/lib/kind-meitner/acme/.kind-meitner\nKIND_MEITNER_PORT=8810\nKIND_MEITNER_WEBHOOK_PORT=8811\nKIND_MEITNER_PUBLIC_URL=https://acme.agentada.cc\nKIND_MEITNER_LICENSE_KEY=omb1.k\n",
    );
...
describe("plans", () => {
  it("initialises the server once: folders, registry, templates, fence, the Caddy import, and reloads", () => {
    const { steps, registry } = initPlan({ domain: "AgentAda.cc", node: "/usr/bin/node", script: "/usr/lib/node_modules/kind-meitner/cli.js", layout });
    expect(registry).toEqual({ version: 1, domain: "agentada.cc", nextPort: 8810, workspaces: {} });
    expect(writesOf(steps)).toEqual(["/etc/kind-meitner/fleet.json", "/etc/systemd/system/kind-meitner@.service", "/etc/kind-meitner/fence.nft", "/etc/systemd/system/kind-meitner-fence.service"]);
    expect(steps.find((step) => step.kind === "append-once")).toEqual({ kind: "append-once", path: "/etc/caddy/Caddyfile", line: "import /etc/caddy/kind-meitner.d/*.caddy" });
    expect(argvOf(steps)).toEqual(["systemctl daemon-reload", "systemctl enable --now kind-meitner-fence.service", "systemctl reload caddy"]);
    ...
    const withAgent = initPlan({ domain: "agentada.cc", node: "/usr/bin/node", script: "/usr/lib/node_modules/kind-meitner/cli.js", operator: "maus", layout });
    expect(withAgent.registry.operator).toBe("maus");
    const agent = withAgent.steps.find((step) => step.kind === "write" && step.path === "/etc/systemd/system/kind-meitner-fleet.service");
    expect(agent).toMatchObject({ content: expect.stringContaining("fleet agent --socket /run/kind-meitner/fleet.sock --group maus") });
    expect(agent).toMatchObject({ content: expect.stringContaining("RuntimeDirectory=kind-meitner") });
    expect(argvOf(withAgent.steps)).toContain("systemctl enable --now kind-meitner-fleet.service");
  ...
  it("creates a workspace as its own account with private data, a fenced port, a unit and a site", () => {
    ...
    expect(argvOf(plan.steps)).toEqual([
      "useradd --system --create-home --home-dir /var/lib/kind-meitner/acme --shell /usr/sbin/nologin --user-group kind-meitner-acme",
      "nft -f /etc/kind-meitner/fence.nft",
      "systemctl daemon-reload",
      "systemctl enable --now kind-meitner@acme.service",
      "systemctl reload caddy",
    ]);
    const config = plan.steps.find((step) => step.kind === "write" && step.path.endsWith("/acme/.kind-meitner/config.json"));
    expect(config).toMatchObject({ mode: 0o600, owner: "kind-meitner-acme" });
    const env = plan.steps.find((step) => step.kind === "write" && step.path === "/etc/kind-meitner/instances/acme.env");
  ...
  it("suspends, resumes, deletes and upgrades with the registry kept in step", () => {
    ...
    expect(argvOf(suspended.steps)).toEqual(["systemctl disable --now kind-meitner@acme.service", "systemctl reload caddy"]);
    ...
    expect(argvOf(resumed.steps)).toEqual(["systemctl enable --now kind-meitner@acme.service", "systemctl reload caddy"]);
    ...
    expect(argvOf(deletePlan({ registry: resumed.registry, slug: "acme", keepData: false, layout }).steps)).toContain("userdel --remove kind-meitner-acme");
    ...
    expect(fenceRules(Object.values(kept.registry.workspaces))).toContain("kind-meitner-acme");
    for (const status of ["provisioning", "error", "retained"] as const) {
    ...
    expect(argvOf(upgradePlan({ registry: suspendPlan({ registry: two, slug: "globex", layout }).registry }))).toEqual(["npm install -g kind-meitner@latest", "systemctl restart kind-meitner@acme.service"]);
  });
  ...
  it("seeds only the trusted portal gateway and reserves the portal hostname", () => {
    ...
    expect(env).toMatchObject({ mode: 0o600, content: expect.stringContaining("KIND_MEITNER_ADMIN_URL=https://admin.example.test\nKIND_MEITNER_ADMIN_WORKSPACE=acme") });
    expect(env).not.toHaveProperty("owner");
    const config = plan.steps.find((step) => step.kind === "write" && step.path.endsWith("/config.json"));
    expect(config).toMatchObject({ owner: "kind-meitner-acme", content: expect.stringContaining('"url": "https://admin.example.test/api/gateway/acme/anthropic"') });
  ...
  it("renders privileged steps but never recommends root writes through tenant paths", () => {
    const lines = describeSteps([
      { kind: "mkdir", path: "/var/lib/kind-meitner/acme/.kind-meitner", mode: 0o700, owner: "kind-meitner-acme" },
      { kind: "write", path: "/var/lib/kind-meitner/acme/.kind-meitner/config.json", content: "secret", mode: 0o600, owner: "kind-meitner-acme" },
      { kind: "write", path: "/etc/kind-meitner/instances/acme.env", content: "KIND_MEITNER_PORT=8810\n", mode: 0o600 },
      { kind: "run", argv: ["useradd", "--comment", "Acme Inc", "kind-meitner-acme"], why: "the account" },
      { kind: "append-once", path: "/etc/caddy/Caddyfile", line: "import /etc/caddy/kind-meitner.d/*.caddy" },
      { kind: "note", text: "done" },
    ]);
    expect(lines).toEqual([
      "# Create /var/lib/kind-meitner/acme/.kind-meitner mode 700 as kind-meitner-acme (use fleet --yes; never root chown on tenant paths)",
      "# Write /var/lib/kind-meitner/acme/.kind-meitner/config.json mode 600 as kind-meitner-acme (use fleet --yes; tenant content omitted)",
      "cat > /etc/kind-meitner/instances/acme.env <<'KIND_MEITNER_EOF'",
      "KIND_MEITNER_PORT=8810",
      "KIND_MEITNER_EOF",
      "chmod 600 /etc/kind-meitner/instances/acme.env",
      "useradd --comment 'Acme Inc' kind-meitner-acme   # the account",
      "grep -qxF 'import /etc/caddy/kind-meitner.d/*.caddy' /etc/caddy/Caddyfile || printf '\\n%s\\n' 'import /etc/caddy/kind-meitner.d/*.caddy' >> /etc/caddy/Caddyfile",
      "# done",
    ]);
  });
  ...
  it("seeds private agentful OpenRouter config, including an empty catalog for later assignments", () => {
    ...
    expect(config).toMatchObject({ owner: "kind-meitner-acme", mode: 0o600 });
    ...
    expect(plan.steps.filter((step) => step.kind === "mkdir" && step.path.includes("/.config"))).toEqual([
      { kind: "mkdir", path: "/var/lib/kind-meitner/acme/.config", mode: 0o700, owner: "kind-meitner-acme" },
      { kind: "mkdir", path: "/var/lib/kind-meitner/acme/.config/opencode", mode: 0o700, owner: "kind-meitner-acme" },
    ]);
