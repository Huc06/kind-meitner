...
describe("origin and cookies", () => {
  ...
  it("parses cookies and names the session cookie per port and environment", () => {
    expect(parseCookies("a=1; kind_meitner_session_8799_abc=tok; b = 2")).toEqual(new Map([["a", "1"], ["kind_meitner_session_8799_abc", "tok"], ["b", "2"]]));
    expect(parseCookies(undefined).size).toBe(0);
    expect(sessionCookieName(8799, "3f2a-uuid-like-id")).toBe("kind_meitner_session_8799_3f2auuidlike");
...
describe("resolveRequestAuth", () => {
  ...
  const cookieName = "kind_meitner_session_8799_env";
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "kind-meitner-auth-"));
  ...
  it("accepts authenticated relay mutations without exposing the desktop owner capability", () => {
    const headers = {
      ...
      "x-kind-meitner-companion-auth": "relay-secret",
    };
    ...
    const forged: Record<string, string>[] = [
      { "x-kind-meitner-companion-auth": "" },
      { "x-kind-meitner-companion-auth": "desktop-secret" },
      { "x-kind-meitner-companion-device": "" },
      { "x-kind-meitner-companion": "0" },
  ...
  it("requires the packaged desktop capability for public loopback mutations", () => {
    ...
    const desktop = resolveRequestAuth(
      request({
        host: "127.0.0.1:8799",
        "x-kind-meitner-desktop-owner": "owner-token-123",
      }, "POST"),
...
describe("an IPC listener (kind-meitner serve --tunnel) is remote by construction", () => {
  ...
  it("never grants loopback trust over the socket, even with a loopback Host and no forwarded headers; a session works", () => {
    const dir = mkdtempSync(join(tmpdir(), "kind-meitner-auth-ipc-"));
    try {
      const sessions = new SessionRegistry({ file: join(dir, "sessions.json") });
      const gate = { sessions, cookieName: "kind_meitner_session_test", streamPath: "/api/events", url: new URL("/api/bots", "http://x") };
      ...
      const admitted = resolveRequestAuth(overSocket({ host: "c-1.kind-meitner.com", authorization: `Bearer ${paired.token}` }), gate);
