...
beforeAll(async () => {
  home = mkdtempSync(join(tmpdir(), "kind-meitner-remote-test-"));
  ...
  writeFileSync(join(home, ".kind-meitner", "config.json"), JSON.stringify({
  ...
  child = spawn(process.execPath, [join(SERVER_DIR, "index.ts")], {
    ...
    env: {
      ...
      KIND_MEITNER_PUBLIC_URL: `${PUBLIC_URL}/`,
      KIND_MEITNER_APP_VERSION: "9.9.9-test",
      KIND_MEITNER_ENVIRONMENT_LABEL: "cab mini",
      KIND_MEITNER_BROWSER_CONNECTION: join(home, "browser-test-connection.json"),
      ...
      KIND_MEITNER_SSE_HEARTBEAT_MS: "4000",
    },
...
describe("before pairing", () => {
  it.each(["//%5B", "//[", "http://["])("rejects malformed request target %s and stays healthy", async (path) => {
    ...
    expect(health.body).toMatchObject({ app: "kind-meitner", pid: child.pid });
    expect(child.exitCode).toBeNull();
  });

  it("describes itself to anyone, but serves nothing else off-machine", async () => {
    const descriptor = await call("/.well-known/kind-meitner/environment", { headers: remote("10.0.0.1") });
...
describe("pairing", () => {
  ...
  it("exchanges a code exactly once, with a bearer token for native clients", async () => {
    ...
    expect(paired.status).toBe(200);
    expect(paired.body.token).toMatch(/^kind_meitner_sess_/);
    ...
    expect(ticket.body.ticket).toMatch(/^kind_meitner_tick_/);
  ...
  it("gives a browser a cookie that only works same-origin", async () => {
    ...
    expect(setCookie).toMatch(new RegExp(`^kind_meitner_session_${PORT}_[a-f0-9]{12}=kind_meitner_sess_`));
    ...
    const stranger = await call("/api/bots", { headers: remote("10.0.0.4", { cookie: `${cookie.split("=")[0]}=kind_meitner_sess_nope`, origin: `http://${REMOTE_HOST}` }) });
