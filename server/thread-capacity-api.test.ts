...
describe("per-bot thread capacity through an isolated HTTP fixture", () => {
  ...
  const capabilityStatus = async (launched: any) => {
    const response = await fetch(`${fixture.info.url}/api/internal/agents`, {
      headers: { authorization: `Bearer ${launched.mcpConfig.mcpServers.agents.env.KIND_MEITNER_COMMS_TOKEN}` },
    });
  ...
  beforeEach(async () => {
    ...
    writeFileSync(wrapper, [
      ...
      'process.env.KIND_MEITNER_FIXTURE_LAUNCHED_AT = String(Date.now());',
  ...
  it("defaults to three, runs ten real turns after raising the limit, and safely queues and cancels overflow", async () => {
    ...
    expect(new Set(launched.map((entry) => entry.env.KIND_MEITNER_FIXTURE_CWD)).size).toBe(10);
    ...
    evidence.push({ simultaneousProviderProcesses: launched.map((entry) => ({ pid: entry.pid, cwd: entry.env.KIND_MEITNER_FIXTURE_CWD })), model });
  ...
  it("counts approval waits, drains FIFO when raised or a turn finishes, and preserves active work when lowered", async () => {
    ...
    expect(Number(launchedSecond.env.KIND_MEITNER_FIXTURE_LAUNCHED_AT)).toBeLessThan(Number(launchedThird.env.KIND_MEITNER_FIXTURE_LAUNCHED_AT));
    expect(Number(launchedThird.env.KIND_MEITNER_FIXTURE_LAUNCHED_AT)).toBeLessThan(Number(launchedFourth.env.KIND_MEITNER_FIXTURE_LAUNCHED_AT));
    finish(fourth);
    await expect.poll(() => busyThreads(botId)).toEqual([]);
    for (const [index, threadId] of [second, third, fourth].entries()) {
