// The `kind-meitner service` command, kept separate from cli.ts so it can be
...
export function runServiceCommand(input: ServiceInstallInput, io: ServiceIo): number {
  ...
  if (!plan) {
    io.error("services are written for Linux (systemd) and macOS (launchd); on Windows, use Task Scheduler to run `kind-meitner serve` at startup");
    return 1;
  }
  ...
  io.log(`logs: ${platform === "darwin" ? `${input.dataDir}/logs/service.log` : "journalctl -u kind-meitner -f"}`);
  io.log(`change options later by running \`kind-meitner service install\` again with the new ones, then: ${platform === "darwin" ? plan.activate[1] : "sudo systemctl daemon-reload && sudo systemctl restart kind-meitner"}`);
  return 0;
}
