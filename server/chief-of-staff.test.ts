...
describe("chiefOfStaffSystemPrompt", () => {
  ...
  it("includes trusted KindMeitner status only when the Chief caller supplies it", () => {
    const status = "TRUSTED KIND_MEITNER STATUS\nfreshness=fresh; runtime_state=degraded";
    ...
    expect(ordinaryPrompt).not.toContain("TRUSTED KIND_MEITNER STATUS");
  });

  it("pins legacy delegation guidance and shared staffing independently of the bounded chat coordinator", () => {
    const prompt = chiefOfStaffSystemPrompt("chief", bots, true, "TRUSTED KIND_MEITNER STATUS\nfreshness=fresh");
