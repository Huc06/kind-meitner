...
describe("the managed Caddy", () => {
  let dir: string;
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "kind-meitner-caddy-"));
  });
  ...
  it("resolves KIND_MEITNER_CADDY_PATH, then the pinned download, then PATH", () => {
    ...
    expect(resolveCaddyBinary({ dataDir: dir, env: { KIND_MEITNER_CADDY_PATH: join(dir, "custom") }, platform: "linux", arch: "x64", exists })).toBe(join(dir, "custom"));
    expect(resolveCaddyBinary({ dataDir: dir, env: { KIND_MEITNER_CADDY_PATH: "relative/caddy" }, platform: "linux", arch: "x64", exists })).toBeNull();
