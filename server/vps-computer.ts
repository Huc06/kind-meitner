...
export const VPS_MANAGED_LABEL = "com.kind-meitner.vps";
export const VPS_CONTAINER_LABEL = "com.kind-meitner.container";
export const VPS_ENVIRONMENT_LABEL = "com.kind-meitner.environment";
export const VPS_VIEWER_LABEL = "com.kind-meitner.vps-viewer";
export const VPS_CONTAINER_PREFIX = "kind-meitner-vps";
...
const MANAGED_VPS_CONTAINER_NAME = /^kind-meitner-vps-[a-z0-9]{1,12}-[a-f0-9]{12}$/;
const IMAGE_ID = /^sha256:[a-f0-9]{64}$/i;
const PIDS_LIMIT = 512;
const SCREENSHOT_PATH = "/tmp/kind-meitner-vps-preview.png";
...
function statusProblem(status: VpsComputerStatus): string | null {
  ...
  if (!status.image) return `Prepare the pinned kind-meitner Cua image on the VPS (Driver ${CUA_DRIVER_VERSION})`;
  if (status.container === "missing") return "No kind-meitner container exists for this bot on the VPS";
  if (!status.imageMatches) return "The VPS container uses an incompatible or untrusted kind-meitner image";
  if (!status.managed) return "The VPS container name is occupied by a container kind-meitner did not create";
  ...
  if (status.security === "unsafe") return "The VPS container is missing kind-meitner safety limits";
  if (status.container === "stopped") return "The kind-meitner VPS container is stopped";
  ...
  const operation = async () => {
    ...
    try {
      ...
      if (action === "provision") {
        ...
        // never docker-rm a container kind-meitner did not create, even one
        // squatting on our name.
        if (before.container === "missing") return before;
        if (!before.managed) {
          throw Object.assign(
            new Error("The VPS container name is occupied by a container kind-meitner did not create — remove it on the VPS yourself"),
            { status: 409 },
          );
        }
