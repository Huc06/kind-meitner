...
const NAME = process.env.KIND_MEITNER_GATE_NAME || "mcp";
const SPILL_DIR = process.env.KIND_MEITNER_GATE_SPILL_DIR || "";
const BUDGET = Number(process.env.KIND_MEITNER_GATE_BUDGET) > 0 ? Number(process.env.KIND_MEITNER_GATE_BUDGET) : DEFAULT_RESULT_BUDGET;
...
const SPILL_HINT = process.env.KIND_MEITNER_GATE_SPILL_HINT === "1";

/** The gate's own settings never reach the upstream server's environment. */
const GATE_ENV_KEYS = ["KIND_MEITNER_GATE_NAME", "KIND_MEITNER_GATE_SPILL_DIR", "KIND_MEITNER_GATE_BUDGET", "KIND_MEITNER_GATE_UPSTREAM", "KIND_MEITNER_GATE_SPILL_HINT"];
...
function upstreamSpec(): Upstream {
  let parsed: unknown;
  try {
    parsed = JSON.parse(process.env.KIND_MEITNER_GATE_UPSTREAM ?? "");
  } catch {
    fail("KIND_MEITNER_GATE_UPSTREAM is not valid JSON");
  }
  const spec = parsed as Upstream | null;
  if (!spec || typeof spec !== "object" || typeof spec.command !== "string" || !spec.command) {
    fail("KIND_MEITNER_GATE_UPSTREAM needs a command");
  }
  return spec;
}
