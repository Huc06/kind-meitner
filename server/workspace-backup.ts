...
const MAGIC = Buffer.from("kind-meitner-WORKSPACE-1\n");
const HEADER_BYTES = MAGIC.length + 16 + 12;
const TAG_BYTES = 16;
const ID = /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/;
const EXCLUDED = new Set([
  ".backups", "tools", "cache", ".cache", "tmp", ".tmp", "dist-native", "tunnel-runtime",
  ".kind-meitner-server-child", "environment-id", "sessions.json", "tunnel-account.json",
  "team-computers.json",
  "kind-meitner-server.lease", "box-create-requests.lock", "messages.db-wal", "messages.db-shm",
]);
...
function excluded(name: string): boolean {
  return EXCLUDED.has(name) || excludedWorkspaceAuthPath(name) || name.startsWith("kind-meitner-server.lease.") || name.startsWith("box-create-requests.lock.") || /^perm-[A-Za-z0-9_-]+\.sock$/.test(name);
}
...
export async function createWorkspaceBackup(dataDir: string, options: CreateWorkspaceBackupOptions) {
  ...
  try {
    ...
    const summary: WorkspaceBackupSummary = {
      format: "kind-meitner.workspace-backup", version: 1, id: job.id, createdAt: new Date().toISOString(),
...
function validateManifest(value: unknown): Manifest {
  if (!record(value) || !record(value.summary) || value.summary.format !== "kind-meitner.workspace-backup" || value.summary.version !== 1 ||
...
async function inspectTar(path: string): Promise<Map<string, { type: string; size: number }>> {
  ...
  // parser can inflate an authenticated but malicious decompression bkind-meitner.
...
export async function stageWorkspaceBackup(dataDir: string, archivePath: string, options: { password: string; currentAppVersion?: string }) {
  ...
  try {
    ...
    if (versions[0] && versions[1]) {
      for (let i = 0; i < 3; i++) {
        if (versions[0][i] > versions[1][i]) throw new Error("This backup was made by a newer kind-meitner version. Update the app before restoring it.");
...
function finishRestore(dataDir: string, id: string, consumePending = true): LastWorkspaceRestore {
  ...
  const draftAttachments = clientState["kind-meitner-draft-attachments"];
  if (draftAttachments) {
    try {
      ...
      clientState["kind-meitner-draft-attachments"] = JSON.stringify(value);
