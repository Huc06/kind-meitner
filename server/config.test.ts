...
describe("configuration boundaries", () => {
  ...
  it("keeps avatar providers and credentials separate, normalizing a router endpoint", () => {
    ...
    const childEnv = { KIND_MEITNER_CUSTOM_IMAGE_KEY: "must-not-reach-bot" };
    stripWorkspaceCredentialEnv(childEnv);
    expect(childEnv).not.toHaveProperty("KIND_MEITNER_CUSTOM_IMAGE_KEY");
  });
...
describe("credential env preference", () => {
  ...
  it("prefers env over the config file for every credential", () => {
    ...
    process.env.KIND_MEITNER_TTS_KEY = "env-tts";
    process.env.KIND_MEITNER_OPENAI_IMAGE_KEY = "env-image";
  ...
  it("syncCredentialEnv keeps process.env in step with a credential save", () => {
    ...
    expect(process.env.KIND_MEITNER_TTS_KEY).toBeUndefined();
  });
...
describe("workspace credential env strip", () => {
  ...
  it("covers in-process secrets and private app-state paths", () => {
    ...
    expect(WORKSPACE_CREDENTIAL_ENV).toContain("KIND_MEITNER_TTS_KEY");
    expect(WORKSPACE_CREDENTIAL_ENV).toContain("KIND_MEITNER_OPENAI_IMAGE_KEY");
    expect(WORKSPACE_CREDENTIAL_ENV).toContain("KIND_MEITNER_BROWSER_CONNECTION");
    expect(WORKSPACE_CREDENTIAL_ENV).toContain("KIND_MEITNER_USER_DATA");
  });
});
