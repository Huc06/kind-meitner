...
function apiBase() {
  return (process.env.KIND_MEITNER_COMPOSIO_API ?? `${DEFAULT_BACKEND_ORIGIN}/api/v3.1`).replace(/\/$/, "");
}

function toolkitBase() {
  return (process.env.KIND_MEITNER_COMPOSIO_TOOLKITS_API ?? `${DEFAULT_BACKEND_ORIGIN}/api/v3`).replace(/\/$/, "");
}
...
export function applyManagedBrokerMessage(message: unknown): boolean {
  ...
  if (
    !parsed.success ||
    parsed.data.type !== "kind-meitner:managed-composio" ||
...
function brokerAccess(): { url: string; token: string } | null {
  ...
  const url = process.env.KIND_MEITNER_COMPOSIO_BROKER_URL?.trim();
  const token = process.env.KIND_MEITNER_COMPOSIO_BROKER_TOKEN?.trim();
...
/** Three answers, not two. The desktop shell sets KIND_MEITNER_CREDENTIAL_STORE to
...
export function connectorAvailability(
  cfg: AppConfig,
  storeState: string | undefined = process.env.KIND_MEITNER_CREDENTIAL_STORE,
...
export async function prepareProjectSession(
  ...
  if (trimmed === current?.apiKey && current.sessionId) {
    ...
    if (
      ...
      return {
        apiKey: trimmed,
        userId: existing.config?.user_id ?? current.userId ?? `kind_meitner_${randomUUID()}`,
  ...
  const userId = priorUserId ?? `kind_meitner_${randomUUID()}`;
...
export async function mcpIntegration(
  ...
  return {
    ...
    env: {
      ...
      KIND_MEITNER_CONNECTOR_UPSTREAM_URL: `${context.harnessUrl}/api/internal/connectors/mcp`,
      KIND_MEITNER_CONNECTOR_UPSTREAM_HEADERS: JSON.stringify({ authorization: `Bearer ${context.commsToken}` }),
      KIND_MEITNER_HARNESS_URL: context.harnessUrl,
      ...
      KIND_MEITNER_CONNECTOR_TOKEN: context.commsToken,
      KIND_MEITNER_BOT_ID: context.botId,
      KIND_MEITNER_THREAD_ID: context.threadId,
    },
  };
}
