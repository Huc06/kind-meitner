...
export interface BoxCreateRequest {
  ...
  createdAt: number;
  boxId?: string;
  /** The provider Box has its deterministic KindMeitner name. Until this is
...
function recoveryStateError(detail: string, cause?: unknown): Error & { status: number } {
  return Object.assign(
    new Error(
      ...
      + "Check ascii.dev for an unnamed Box before repairing kind-meitner's local state.",
    ),
    { status: 503, cause },
  );
}
  ...
  try {
    for (;;) {
      ...
      if (performance.now() >= deadline) {
        throw recoveryStateError("locked by another kind-meitner process");
      }
      if (reaped) continue;
