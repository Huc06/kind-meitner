...
describe("tunnel credentials: a private file, and unreadable is not empty", () => {
  let dir: string;
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "kind-meitner-tunnel-cred-"));
  });
...
describe("login and logout against the control plane", () => {
  ...
  beforeEach(async () => {
    dir = mkdtempSync(join(tmpdir(), "kind-meitner-tunnel-login-"));
  ...
  it("an unusable control-plane override is a clear error, not a silent default", async () => {
    vi.stubEnv("KIND_MEITNER_CONTROL_PLANE_URL", "ftp://nope");
    const io = fakeIo([]);
    expect(await runLogin(options(dir, { email: "a@b.test" }), io.io)).toBe(1);
    expect(io.err.join("\n")).toMatch(/KIND_MEITNER_CONTROL_PLANE_URL/);
  });
});
...
describe("where the pieces are", () => {
  it("names the platform the control plane's way and prefers the bundled guardian and downloader", () => {
    ...
    const bundled = mkdtempSync(join(tmpdir(), "kind-meitner-tunnel-bundled-"));
    try {
...
describe.skipIf(!posix)("startTunnel: guardian, gateway and connector, verified through the gateway", () => {
  it("forwards the public address to the harness socket (no peer address = remote) and tears everything down on stop", async () => {
    const dir = mkdtempSync(join(tmpdir(), "kind-meitner-tunnel-run-"));
    ...
    const harness = createServer((req, res) => {
      ...
      res.end(JSON.stringify({ app: "kind-meitner", url: req.url, peer: req.socket.remoteAddress ?? null }));
    });
    await new Promise<void>((done) => harness.listen(origin.socketPath, done));
    const originPort = 20000 + Math.floor(Math.random() * 20000);
    const endpoint = "https://c-stub.kind-meitner.invalid";
    ...
    const tunnel = startTunnel({
      ...
      binaryPath: fake,
      guardian: guardian ?? "",
      env: { ...process.env, KIND_MEITNER_TUNNEL_ORIGIN_PORT: String(originPort) },
    ...
    try {
      ...
      expect(viaGateway.app).toBe("kind-meitner");
...
describe("a fleet's credential in the environment", () => {
  it("gets the public address with no account file and no emailed code; a rejected credential is a clear error", async () => {
    ...
    try {
      const env = { ...process.env, KIND_MEITNER_CONTROL_PLANE_URL: stub.url };
      expect(fleetCredential({})).toBeNull();
      expect(fleetCredential({ KIND_MEITNER_INSTALLATION_CREDENTIAL: "   " })).toBeNull();
      const credential = stub.seedInstallation("box-1");
      expect(fleetCredential({ KIND_MEITNER_INSTALLATION_CREDENTIAL: ` ${credential} ` })).toBe(credential);
      ...
      await expect(fleetAccess({ credential: `kind_meitner_install_${"x".repeat(22)}.${"y".repeat(43)}`, env })).rejects.toThrow(/rejected/);
      await expect(fleetAccess({ credential, env: { ...env, KIND_MEITNER_CONTROL_PLANE_URL: "ftp://nope" } })).rejects.toThrow(/KIND_MEITNER_CONTROL_PLANE_URL/);
    } finally {
      await stub.close();
    }
  });
