...
// ~/.kind-meitner/attachments so every CLI engine can open them by path —
...
const PARTIAL_NAME = /^\.kind-meitner-upload-[0-9a-f-]+-[0-9a-f-]+\.partial$/i;
...
function cleanupAttachmentPartialsForUpload(uploadId: string): void {
  ensureAttachmentsDir();
  const prefix = `.kind-meitner-upload-${uploadId}-`;
  for (const entry of readdirSync(ATTACHMENTS_DIR, { withFileTypes: true })) {
...
export async function saveFile(
  ...
  return withUploadLock(uploadId, async () => {
    ...
    const partialPath = join(ATTACHMENTS_DIR, `.kind-meitner-upload-${id}-${randomUUID()}.partial`);
