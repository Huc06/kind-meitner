...
describe("spend cap and prices through real turns", () => {
  ...
  beforeEach(async () => {
    ...
    layerDir = mkdtempSync(join(tmpdir(), "kind-meitner-fake-layer-"));
  ...
  const control = (args: string[]) => runControlKindMeitner([...args, "--url", session.info.url]) as Promise<any>;
  ...
  it.each([1, 2])("blocks a goal after %i paid turn(s) without retrying or spending another turn", async (allowedTurns) => {
    ...
    session = await launchVerificationServer({
      ...
      FAKE_CLAUDE_REPLIES: JSON.stringify([
        'Delegating the check.<kind-meitner-goal>{"status":"continue","next":"Worker","instruction":"Check the draft"}</kind-meitner-goal>',
