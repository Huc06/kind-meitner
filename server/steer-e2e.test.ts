...
posixOnly("mid-turn steering e2e", () => {
  ...
  beforeAll(async () => {
    ...
    home = mkdtempSync(join(tmpdir(), "kind-meitner-steer-"));
    ...
    child = spawn(process.execPath, [join(SERVER_DIR, "index.ts")], {
      cwd: join(SERVER_DIR, ".."),
      env: { ...(process.env.PATH ? { PATH: process.env.PATH } : {}), HOME: home, USERPROFILE: home, KIND_MEITNER_PORT: String(PORT) },
  ...
  it("keeps two queued attachment messages as two native images in one follow-up turn", async () => {
    ...
    const attachments = join(home, ".kind-meitner", "attachments");
    ...
    const nativeRows = readFileSync(
      join(home, ".kind-meitner", "native", `${created.threadId}.ndjson`),
      "utf8",
    )
      .trim()
      .split("\n")
