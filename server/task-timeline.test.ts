...
describe("execution timeline", () => {
  ...
  it("shows recorded commands and redacts older unsanitized activity", () => {
    const events = timelineEvents([
      ...
      { id: "tool", role: "bot", kind: "activity", tool: { name: "Bash --password private-value", summary: "pnpm control:kind-meitner doctor --token fixture-secret", ok: false }, at: 2 },
      { id: "answer", role: "bot", kind: "text", text: "Private bot reply", at: 3 },
    ]);
    expect(events[1]).toMatchObject({ state: "failed", command: "pnpm control:kind-meitner doctor --token «redacted 14 chars»" });
