...
describe("permission proxy", () => {
  ...
  beforeEach(async () => {
    scratch = mkdtempSync(join(tmpdir(), "kind-meitner-perm-proxy-"));
    asks = [];
    conns = [];
    answerWith = () => null;
  ...
  it("files a timeout's own note as unanswered, not as the user's choice", async () => {
    ...
    answerWith = () => ({
      behavior: "answer",
      message: "kind-meitner: nobody answered in time. Use your best judgment and continue.",
      source: "timeout",
    });
    rpc({
      jsonrpc: "2.0",
  ...
  it("denies when the broker is gone, rather than reporting an empty answer", async () => {
    ...
    answerWith = () => ({ behavior: "deny", message: "kind-meitner: permission broker unavailable" });
    rpc({
      jsonrpc: "2.0",
      id: 2,
      method: "tools/call",
