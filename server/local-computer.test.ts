  ...
  return {
    ...
    mcp: {
      ...
      env: {
        ...
        CUA_DRIVER_HOST_BUNDLE_ID: "com.kind-meitner.app",
...
describe("local computer descriptor contract", () => {
  it("wraps only the transport while preserving descriptor identity and original driver environment", () => {
    ...
    expect(gated.env).toEqual({ ...connection.env, ELECTRON_RUN_AS_NODE: "1", KIND_MEITNER_CUA_COMMAND: connection.command, KIND_MEITNER_CUA_ARGS: JSON.stringify(connection.args), KIND_MEITNER_CONTROL_URL: "http://127.0.0.1:1234/control", KIND_MEITNER_CONTROL_TOKEN: "fixture-token" });
    expect(gated.args.join(" ")).not.toContain("fixture-token");
    expect(connection.env).not.toHaveProperty("KIND_MEITNER_CONTROL_TOKEN");
  });
...
function privateUserData(name: string) {
  ...
  const root = mkdtempSync(join(base, "kind-meitner-local-computer-"));
