// `kind-meitner fleet`: many client workspaces on one Linux server. The plans
...
export async function executePlan(steps: FleetStep[], deps: FleetDeps, io: FleetIo): Promise<number> {
  for (const step of steps) {
    switch (step.kind) {
      ...
      case "health":
        if (!(await deps.health(step.url, HEALTH_TIMEOUT_MS))) return failed(io, step.why, `${step.url} did not answer within ${HEALTH_TIMEOUT_MS / 1000}s; check journalctl -u kind-meitner@<slug>`);
        break;
      case "note":
        io.log(step.text);
        break;
...
export function loadRegistry(layout: FleetLayout, deps: FleetDeps): FleetRegistry {
  ...
  if (text === null) throw new Error(`no fleet on this server yet: run \`kind-meitner fleet init --domain your.domain\` first`);
...
export function planFleetAction(input: FleetInput, deps: FleetDeps): FleetStep[] {
  ...
  switch (input.action) {
    case "init": {
      ...
      if (kept) {
        ...
        return plan.steps.flatMap((step): FleetStep[] => {
          ...
          if (step.kind === "run" && step.argv.join(" ") === "systemctl enable --now kind-meitner-fence.service") return [step, { kind: "run", argv: ["nft", "-f", layout.fenceFile], why: "reapply the preserved workspace fence" }];
          return [step];
        });
      }
      return plan.steps;
    ...
    case "create": {
      ...
      for (const path of [workspaceHome(layout, input.slug), posix.join(layout.instancesDir, `${input.slug}.env`), posix.join(layout.caddyDir, `${input.slug}.caddy`), posix.join(posix.dirname(layout.unitFile), `kind-meitner@${input.slug}.service.d`)]) {
    ...
    case "providers": {
      ...
      const portal = /^KIND_MEITNER_ADMIN_URL=(.+)$/m.exec(env)?.[1];
      if (!portal || !env.split("\n").includes(`KIND_MEITNER_ADMIN_WORKSPACE=${input.slug}`)) throw new Error("workspace has no trusted portal configuration; operator recovery is required");
...
export async function runFleetCommand(input: FleetInput, io: FleetIo, deps: FleetDeps = defaultFleetDeps()): Promise<number> {
  ...
  try {
    if (input.action === "list") {
      ...
      if (!rows.length) {
        io.log(`no workspaces yet on ${registry.domain}; create one with: kind-meitner fleet create NAME --admin you@example.com`);
        return 0;
      }
      for (const workspace of rows) {
        const live = asRoot && (workspace.status === "running" || workspace.status === "suspended") ? (await deps.run(["systemctl", "is-active", `kind-meitner@${workspace.slug}.service`])).output.trim() : workspace.status;
