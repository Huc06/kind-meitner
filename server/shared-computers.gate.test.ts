...
const internal = async (method: string, path: string, body?: unknown): Promise<ApiResult> => {
  const minted = await fetch(`${BASE}/api/testing/internal-capability`, {
    method: "POST",
    headers: { "content-type": "application/json", "x-kind-meitner-test-capability": TEST_CAPABILITY_KEY },
...
const descriptorCapabilities = async (): Promise<Record<string, unknown>> =>
  ((await api("GET", "/.well-known/kind-meitner/environment")).body.capabilities ?? {}) as Record<string, unknown>;
...
beforeAll(async () => {
  ...
  home = mkdtempSync(join(tmpdir(), "kind-meitner-shared-computer-gate-"));
