...
beforeAll(async () => {
  home = mkdtempSync(join(tmpdir(), "kind-meitner-claude-account-api-"));
  dataDir = join(home, ".kind-meitner");
...
`, { mode: 0o755 });
  ...
  child = spawn(process.execPath, ["--import", offlinePrelude, join(SERVER_DIR, "index.ts")], {
    ...
    env: {
      ...
      HOME: home, USERPROFILE: home, KIND_MEITNER_DATA_DIR: dataDir,
      KIND_MEITNER_PORT: String(port), KIND_MEITNER_WEBHOOK_PORT: String(port + 1), KIND_MEITNER_SSE_HEARTBEAT_MS: "50",
    },
