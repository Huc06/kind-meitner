...
describe("Codex effective developer instructions", () => {
  it("preserves native rules after bot rules, including when bot rules are removed", () => {
    ...
    expect(codexDeveloperInstructions(config, "")).toBe("No kind-meitner bot-specific instructions remain.\n\nNative rules.");
...
describe("Codex instruction receipts", () => {
  it("does not repeat unchanged rules, but persists edits and removal", async () => {
    ...
    expect(request.mock.calls[1][1].items[0].content[0].text).toContain("No kind-meitner bot-specific instructions remain.");
  });
  ...
  it("keeps native sessions independent even for one kind-meitner task", async () => {
    const key = randomUUID();
    const request = vi.fn().mockResolvedValue({});
