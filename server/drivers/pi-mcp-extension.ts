...
// path is handed in through KIND_MEITNER_MCP_CONFIG and mounts every server described
// there as first-class pi tools (pi.registerTool). kind-meitner ships this file
// and the pi driver spawns it — the pi repo itself is never touched.
//
// Protocol: the kind-meitner proxies speak raw JSON-RPC 2.0 over stdio, one
 ...
 * Keeping it local avoids bundling Pi into kind-meitner; the extension is
...
/** A minimal stdio JSON-RPC 2.0 MCP client, matched to kind-meitner's
  ...
  async init(): Promise<void> {
    await this.call(
      "initialize",
      {
        ...
        capabilities: {},
        clientInfo: { name: "kind-meitner-pi", version: "1" },
      },
...
export default async function (pi: PiExtensionApi): Promise<void> {
  const configPath = process.env.KIND_MEITNER_MCP_CONFIG;
  ...
  const mounts = await Promise.all(
    serverEntries.map(async ([serverName, def]) => {
      ...
      try {
        ...
        process.stderr.write(`[kind-meitner-pi-mcp] ${serverName}: ${message}\n`);
        client?.dispose();
        return { serverName };
      }
    }),
  ...
  for (const mount of mounts) {
    ...
    for (const tool of tools) {
      if (!tool || typeof tool.name !== "string" || !tool.name.trim()) {
        process.stderr.write(`[kind-meitner-pi-mcp] ${serverName}: skipped a tool with no valid name\n`);
        continue;
      }
      ...
      try {
        ...
        const message = err instanceof Error ? err.message : String(err);
        process.stderr.write(`[kind-meitner-pi-mcp] ${serverName}:${toolName}: ${message}\n`);
      }
    }
