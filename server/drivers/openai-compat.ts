...
const idleTimeoutMs = () => {
  const raw = process.env.KIND_MEITNER_OPENAI_COMPAT_IDLE_TIMEOUT_MS;
...
export const OpenAICompatDriver: ProviderDriver<OpenAICompatConfig> = {
  ...
  install: {
    ...
    signInCommand:
      "add {\"openaiCompat\":{\"key\":\"sk-or-v1-…\"}} to ~/.kind-meitner/config.json (or set OPENAI_COMPAT_API_KEY)",
    command: {
      darwin:
        "Get a free key at https://openrouter.ai/keys (or https://console.groq.com) then add it to ~/.kind-meitner/config.json under openaiCompat.key",
      linux:
        "Get a free key at https://openrouter.ai/keys (or https://console.groq.com) then add it to ~/.kind-meitner/config.json under openaiCompat.key",
      win32:
        "Get a free key at https://openrouter.ai/keys (or https://console.groq.com) then add it to %USERPROFILE%\\.kind-meitner\\config.json under openaiCompat.key",
    },
  },
  decodeConfig,
