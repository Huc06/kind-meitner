...
describe("ClaudeDriver.decodeConfig", () => {
  ...
  it.skipIf(process.platform !== "win32")("names permission pipes per harness process", () => {
    expect(permissionSocketPath("thread-abc")).toMatch(
      new RegExp(`^\\\\\\\\\\.\\\\pipe\\\\kind-meitner-perm-${process.pid}-thre[0-9a-f]{4}$`),
    );
  });
  ...
  it("disposes its account controller while a logout is running", async () => {
    const home = mkdtempSync(join(tmpdir(), "kind-meitner-claude-logout-dispose-"));
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const dir = mkdtempSync(join(tmpdir(), "kind-meitner-broker-fallback-"));
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      const dir = mkdtempSync(join(tmpdir(), "kind-meitner-broker-unavailable-"));
...
describe("ClaudeDriver turns (fake CLI)", () => {
  ...
  beforeEach(() => {
    ...
    scratch = mkdtempSync(join(tmpdir(), "kind-meitner-claude-test-"));
  });
  ...
  afterEach(async () => {
    ...
    delete process.env.KIND_MEITNER_TTS_KEY;
    delete process.env.KIND_MEITNER_CLAUDE_SESSION_IDLE_MS;
    delete process.env.KIND_MEITNER_CLAUDE_SESSION_IDLE_MIN_MS;
    recorder?.stop();
    await instance?.dispose();
  ...
  it("keeps user and system prompts off argv and strips identity env vars", async () => {
    ...
    process.env.KIND_MEITNER_TTS_KEY = "tts-should-not-leak";
    ...
    expect(seen.env.KIND_MEITNER_TTS_KEY).toBeUndefined();
  });
  ...
  it.each(["ask", "auto"] as const)("pre-allows the agents comms proxy while retaining native %s approval", async (approvalMode) => {
    ...
    await instance.adapter.sendTurn({
      ...
      integrations: {
        agents: {
          ...
          env: { KIND_MEITNER_HARNESS_URL: "http://127.0.0.1:1", KIND_MEITNER_BOT_ID: "b1", KIND_MEITNER_COMMS_TOKEN: "tok", KIND_MEITNER_TURN_DEPTH: "0" },
        },
      },
    });
    await recorder.until((e) => e.type === "turn.completed");
    ...
    expect(seen.mcpConfig.mcpServers.agents).toMatchObject({
      alwaysLoad: true,
      args: ["/fake/agents-proxy.js"],
      env: { KIND_MEITNER_BOT_ID: "b1", KIND_MEITNER_COMMS_TOKEN: "tok" },
    });
  ...
  it("clamps a configured compaction window into the range the CLI accepts", () => {
    ...
    // turn, not degrade
    expect(autoCompactWindow({ KIND_MEITNER_CLAUDE_AUTOCOMPACT: "50000" })).toBe("100000");
    expect(autoCompactWindow({ KIND_MEITNER_CLAUDE_AUTOCOMPACT: "9000000" })).toBe("1000000");
    expect(autoCompactWindow({ KIND_MEITNER_CLAUDE_AUTOCOMPACT: "150000" })).toBe("150000");
    expect(autoCompactWindow({ KIND_MEITNER_CLAUDE_AUTOCOMPACT: "nonsense" })).toBe("200000");
    expect(autoCompactWindow({})).toBe("200000");
    expect(autoCompactWindow({ KIND_MEITNER_CLAUDE_AUTOCOMPACT: "auto" })).toBe("auto");
    expect(autoCompactWindow({ KIND_MEITNER_CLAUDE_AUTOCOMPACT: "off" })).toBe(null);
  });

  it("passes no compaction window when it is turned off", async () => {
    const dump = join(scratch, "compact-off.json");
    await create(undefined, { FAKE_CLAUDE_DUMP: dump, KIND_MEITNER_CLAUDE_AUTOCOMPACT: "off" });
  ...
  it("inherits the machine's configuration again when the escape hatch is set", async () => {
    ...
    await create(undefined, { FAKE_CLAUDE_DUMP: dump, KIND_MEITNER_CLAUDE_INHERIT_USER_CONFIG: "1" });
  ...
  it("preserves only the selected account's auth settings in a private file", async () => {
    ...
    mkdirSync(account);
    const settings = { apiKeyHelper: "echo synthetic-helper-key", env: { ANTHROPIC_BASE_URL: "http://127.0.0.1:9", ANTHROPIC_AUTH_TOKEN: "synthetic-token", KIND_MEITNER_TTS_KEY: "must-not-leak" }, hooks: { SessionStart: [{ command: "must-not-run" }] }, permissions: { defaultMode: "bypassPermissions" } };
  ...
  it("does not mix a personal helper/endpoint with an explicitly configured kind-meitner connection", async () => {
    const account = join(scratch, "account");
    mkdirSync(account);
  ...
  it("forwards the bot project's own .mcp.json, which strict mode would drop", async () => {
    ...
    expect(JSON.parse(seen.mcpConfig.mcpServers.shop.env.KIND_MEITNER_GATE_UPSTREAM)).toMatchObject({
      command: "npx",
  ...
  it("mounts a bot's own MCP server behind the result gate", async () => {
    ...
    expect(JSON.parse(shop.env.KIND_MEITNER_GATE_UPSTREAM)).toMatchObject({
      command: "npx",
      args: ["-y", "mcp-remote", "https://example.test/shop"],
      env: { SHOP_TOKEN: "secret" },
    });
    expect(shop.env.KIND_MEITNER_GATE_NAME).toBe("shop");
    expect(Number(shop.env.KIND_MEITNER_GATE_BUDGET)).toBeGreaterThan(0);
  ...
  it("mounts bot servers directly when the result budget is turned off", async () => {
    ...
    await create(undefined, { FAKE_CLAUDE_DUMP: dump, KIND_MEITNER_MCP_RESULT_BUDGET: "0" });
  ...
  it("mounts custom MCP servers without pre-allowing their tools", async () => {
    ...
    expect(JSON.parse(seen.mcpConfig.mcpServers.notes.env.KIND_MEITNER_GATE_UPSTREAM)).toMatchObject({
      command: "npx",
      args: ["-y", "@x/notes-mcp"],
      env: { NOTES_TOKEN: "tok-notes" },
    });
    expect(JSON.parse(seen.mcpConfig.mcpServers.constructor.env.KIND_MEITNER_GATE_UPSTREAM)).toMatchObject({ command: "fixture-constructor" });
    ...
    await instance.adapter.sendTurn({
      ...
      integrations: {
        composio: {
          ...
          env: { KIND_MEITNER_CONNECTOR_UPSTREAM_URL: "https://example.test/mcp" },
        },
      },
    });
    await recorder.until((e) => e.type === "turn.completed");
  ...
  it.each([
    ...
    expect(configPath).toMatch(/kind-meitner-mcp-/);
  ...
  it.each([false, true])("refuses steering and retires approvals after interrupt before process exit (retained=%s)", async (retained) => {
    ...
    try {
      ...
      await expect(lateAnswer).resolves.toMatchObject({ id: "after-stop", behavior: "deny", message: "kind-meitner: the turn ended" });
  ...
  it("denies late broker asks between retained turns without opening a zombie card", async () => {
    ...
    await expect(answer).resolves.toMatchObject({
      id: "ask-between",
      behavior: "deny",
      message: "kind-meitner: the turn ended",
    });
  ...
  it("closes an idle session after the configured window", async () => {
    process.env.KIND_MEITNER_CLAUDE_SESSION_IDLE_MIN_MS = "10";
    process.env.KIND_MEITNER_CLAUDE_SESSION_IDLE_MS = "50";
    await create();
  ...
  it("denies a colliding ask id on the same connection without orphaning the original", async () => {
    ...
    expect(await nextAnswer()).toMatchObject({
      id: "dup-1",
      behavior: "deny",
      message: "kind-meitner: duplicate ask id — skipping this request.",
    });
  ...
  it("drops a late question on an already-closed broker with an answer, not a deny (#211)", async () => {
    ...
    expect(await reply).toMatchObject({
      id: "q-late",
      behavior: "answer",
      message: "kind-meitner: the turn is ending — wrap up.",
    });
    expect(recorder.events.filter((e) => e.type === "request.opened")).toHaveLength(opensBefore);
    await expect(
  ...
  it("strips workspace credentials from generateText helper children", async () => {
    ...
    process.env.FAKE_CLAUDE_DUMP = dump;
    const names = ["XAI_API_KEY", "COMPOSIO_API_KEY", "BOX_TOKEN", "OPENCODE_API_KEY", "KIND_MEITNER_TTS_KEY"] as const;
...
describe("ClaudeDriver resume recovery (fake CLI)", () => {
  ...
  beforeEach(async () => {
    ensureDirs();
    chmodSync(FAKE_CLI, 0o755);
    scratch = mkdtempSync(join(tmpdir(), "kind-meitner-claude-recover-"));
