...
const INIT_TIMEOUT = envOr("KIND_MEITNER_ACP_INIT_TIMEOUT_MS", 300_000);
const SESSION_CONFIG_TIMEOUT = envOr("KIND_MEITNER_ACP_SESSION_CONFIG_TIMEOUT_MS", 300_000); // configureSession's per-request default
const NEW_SESSION_TIMEOUT = envOr("KIND_MEITNER_ACP_NEW_SESSION_TIMEOUT_MS", 300_000);
const LOAD_SESSION_TIMEOUT = envOr("KIND_MEITNER_ACP_LOAD_SESSION_TIMEOUT_MS", 120_000); // history replay on a long thread is slow
    ...
    async create(input: DriverCreateInput<AcpConfig>): Promise<ProviderInstance> {
        ...
        (async () => {
          try {
            const init = await request(
              ...
              {
                protocolVersion: 1,
                clientInfo: { name: "kind-meitner", version: "0.0.0" },
                clientCapabilities: {
                  fs: {
      ...
      return {
        ...
        customMcp: true,
            ...
            // kind-meitner supplies a per-bot approvalMode on every harness
