// Qwen Code — Alibaba's `qwen --acp` CLI. Custom-only in kind-meitner:
...
function envKeyFor(hostId: string): string {
  return `KIND_MEITNER_QWEN_${hostId.toUpperCase().replace(/[^A-Z0-9]+/g, "_")}_API_KEY`;
}
...
const support: AcpSupport = {
  ...
  // the complete route and confirms it before any prompt leaves kind-meitner.
  spawnArgs: () => ["--acp"],
