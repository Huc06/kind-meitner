...
import {
  ...
  HERMES_KIND_MEITNER_SCREENSHOT_COMPAT,
  HERMES_KIND_MEITNER_SCREENSHOT_COMPAT_MODEL,
...
describe("Hermes KindMeitner screenshot compatibility binding", () => {
  it("binds the exact leaf model for an injected local picker model", () => {
    const env = {
      [HERMES_KIND_MEITNER_SCREENSHOT_COMPAT]: undefined,
      [HERMES_KIND_MEITNER_SCREENSHOT_COMPAT_MODEL]: undefined,
    };

    bindHermesScreenshotCompat(env, "omlx::gemma-4-31b-it-bf16");

    expect(env[HERMES_KIND_MEITNER_SCREENSHOT_COMPAT]).toBe("1");
    expect(env[HERMES_KIND_MEITNER_SCREENSHOT_COMPAT_MODEL]).toBe("gemma-4-31b-it-bf16");
  });
  ...
  it.each([undefined, "", "anthropic/claude-opus-4.6", "unknown::model"])(
    ...
    (model) => {
      const env = {
        [HERMES_KIND_MEITNER_SCREENSHOT_COMPAT]: "1",
        [HERMES_KIND_MEITNER_SCREENSHOT_COMPAT_MODEL]: "stale/model",
      };

      bindHermesScreenshotCompat(env, model);

      expect(env[HERMES_KIND_MEITNER_SCREENSHOT_COMPAT]).toBeUndefined();
      expect(env[HERMES_KIND_MEITNER_SCREENSHOT_COMPAT_MODEL]).toBeUndefined();
    },
  );
});
...
describe("hermesConfiguredModel", () => {
  ...
  const home = (env: string, cfg?: string) => {
    const root = mkdtempSync(join(tmpdir(), "kind-meitner-hermes-"));
    dirs.push(root);
  ...
  it("returns null when there is no .env and no config.yaml, leaving local-only setups unchanged", () => {
    const root = mkdtempSync(join(tmpdir(), "kind-meitner-hermes-bare-"));
    dirs.push(root);
  ...
  it("offers the configured model when only config.yaml exists (Nous Portal OAuth)", () => {
    ...
    const root = mkdtempSync(join(tmpdir(), "kind-meitner-hermes-nous-"));
    dirs.push(root);
    const h = join(root, ".hermes");
    mkdirSync(h, { recursive: true });
...
`;
  ...
  function fakeCli(env: Record<string, string>): { cli: string; env: Record<string, string> } {
    const home = mkdtempSync(join(tmpdir(), "kind-meitner-hermes-probe-"));
    dirs.push(home);
  ...
  it("finds the catalog CLI on the supplied PATH, including a Windows npm shim", async () => {
    ...
    const command = "kind-meitner-hermes-probe";
