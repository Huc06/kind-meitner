...
function scratchConfig(toml: string): string {
  const dir = mkdtempSync(join(tmpdir(), "kind-meitner-grok-catalog-"));
...
describe("readGrokModelCatalog", () => {
  it("returns the static cloud pair when there is no config", () => {
    expect(readGrokModelCatalog({ HOME: join(tmpdir(), "kind-meitner-grok-missing-home") })).toEqual(STATIC_GROK_MODELS);
  });
...
`);
  ...
  it("honors GROK_HOME over HOME", () => {
    ...
    const grokHomeParent = mkdtempSync(join(tmpdir(), "kind-meitner-grok-home-"));
