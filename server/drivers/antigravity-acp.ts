...
const BROWSER_MARKER = "__KIND_MEITNER_ANTIGRAVITY_AUTH_URL__";
...
export async function prepareAntigravityProfile(input: {
  ...
  if (/\r|\n|\0|%s/u.test(executable) || executable.includes(delimiter)) {
    throw new Error("The kind-meitner runtime path cannot safely suppress Antigravity browser launches.");
  }
      ...
      const timer = setTimeout(() => {
        ...
        if (method === "initialize") {
          ...
          reject(new Error(
            ...
            "Retry setup. If it still fails, share this error and your kind-meitner version; do not paste Google sign-in links or tokens.",
          ));
...
export async function validateAntigravityRuntime(runtime: AntigravityRuntime, expectedVersion: string): Promise<void> {
  const profileDirectory = await mkdtemp(join(tmpdir(), "kind-meitner-antigravity-verify-"));
