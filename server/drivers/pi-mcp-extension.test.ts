...
const originalMcpConfig = process.env.KIND_MEITNER_MCP_CONFIG;
...
function tempDir(): string {
  const dir = mkdtempSync(join(tmpdir(), "kind-meitner-pi-mcp-ext-"));
  tempDirs.push(dir);
  return dir;
}
...
afterEach(() => {
  ...
  if (originalMcpConfig === undefined) delete process.env.KIND_MEITNER_MCP_CONFIG;
  else process.env.KIND_MEITNER_MCP_CONFIG = originalMcpConfig;
});
...
describe("Pi MCP extension registration", () => {
  it("keeps earlier tools alive when a later registration fails", async () => {
    ...
    process.env.KIND_MEITNER_MCP_CONFIG = config;
