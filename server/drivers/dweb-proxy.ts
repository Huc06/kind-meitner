...
// Exposes dweb's HTTP API (the local KindMeitner network daemon) as tools an
// agent can call to inspect the network and run model requests:
//
//   dweb_status          → ping dweb, summarize server + peer state
//   dweb_repo_status     → the kind-meitner repo the daemon is tracking
...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      "Report the state of the kind-meitner repository the dweb daemon is tracking: repo name, checked-out branch, last commit, file count, and checkout path.",
    inputSchema: { type: "object", properties: {} },
  },
  {
