...
describe("Codex server-owned device authentication", () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), "kind-meitner-device-auth-"));
  ...
  it("drives the reusable browser fixture only when its local approval marker is created", async () => {
    const controller = create("waiting", {
      ...
      environment: () => ({ ...process.env, HOME: home, CODEX_HOME: join(home, ".codex"), KIND_MEITNER_DEVICE_AUTH_FIXTURE: "1" }),
    });
    const start = await controller.start();
    expect(start.userCode).toBe("TEST-12345");
    expect(existsSync(join(home, ".kind-meitner-fake-codex-authenticated"))).toBe(false);
    writeFileSync(join(home, ".kind-meitner-fake-codex-login-approved"), "approve fixture only\n");
    await expect.poll(async () => (await controller.get(start.flowId!)).phase).toBe("succeeded");
    expect(readFileSync(join(home, ".kind-meitner-fake-codex-authenticated"), "utf8")).toContain("not a credential");
  });
