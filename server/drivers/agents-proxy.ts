...
//   KIND_MEITNER_HARNESS_URL  base URL of the harness (http://127.0.0.1:8799)
//   KIND_MEITNER_BOT_ID       the calling bot's id (excluded from list_bots; sender)
//   KIND_MEITNER_COMMS_TOKEN  shared secret for the localhost-only internal endpoints
//   KIND_MEITNER_TURN_DEPTH   this turn's comms depth (the harness refuses recursion)
...
const HARNESS = process.env.KIND_MEITNER_HARNESS_URL ?? "http://127.0.0.1:8799";
const BOT_ID = process.env.KIND_MEITNER_BOT_ID ?? "";
const THREAD_ID = process.env.KIND_MEITNER_THREAD_ID ?? "";
const TOKEN = process.env.KIND_MEITNER_COMMS_TOKEN ?? "";
const DEPTH = Number(process.env.KIND_MEITNER_TURN_DEPTH ?? "0") || 0;
const SKILL_AUTHORING_ENABLED = process.env.KIND_MEITNER_SKILL_AUTHORING_ENABLED === "1";
...
const SHARED_COMPUTERS_ENABLED = process.env.KIND_MEITNER_SHARED_COMPUTERS_ENABLED === "1";
...
const ROUTINE_FIELDS_SCHEMA = {
  ...
  run_on: {
    type: "string",
    enum: ["maus", "cloud"],
    description: "Where the routine runs. Defaults to maus (this kind-meitner setup).",
  },
  timeout_minutes: {
    type: "integer",
    minimum: 5,
...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      "Ask the user for a supported API key through kind-meitner's secure credential flow. The desktop app and a freshly QR-paired mobile app show a secure entry card; older mobile pairings show how to pair again or finish on the computer. Never claim a secure field opened unless this request succeeds, and never ask the user to paste a secret into chat. The secret is saved by the desktop app and is never returned to you. After calling this tool, end the turn; kind-meitner resumes the task after the user saves or declines.",
    inputSchema: {
      type: "object",
      properties: {
        credential_id: {
