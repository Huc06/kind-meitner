...
export function codexLocalProviderArgs(
  ...
  if (!host) return [];
  const envKey = `KIND_MEITNER_LOCAL_${host.id.toUpperCase().replace(/[^A-Z0-9]+/g, "_")}_API_KEY`;
...
export async function mergeLocalInject(
  ...
  const probe = env.KIND_MEITNER_PROBE_LOCAL_INJECT ?? process.env.KIND_MEITNER_PROBE_LOCAL_INJECT;
