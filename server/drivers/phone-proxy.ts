...
export function resolveAdbPath(env: NodeJS.ProcessEnv = process.env, platform = process.platform): string | null {
  ...
  const home = homedir();
  const candidates = [
    env.KIND_MEITNER_ADB_PATH,
    env.KIND_MEITNER_RESOURCES_PATH && join(env.KIND_MEITNER_RESOURCES_PATH, "android-platform-tools", platform, executable),
...
async function runAdb(args: string[], options: { binary?: boolean; timeoutMs?: number } = {}): Promise<Buffer> {
  ...
  if (!adb) throw new Error("Android platform tools are unavailable. Reopen kind-meitner or install adb.");
...
async function readNodes(serial: string) {
  ...
  const remotePath = "/data/local/tmp/kind-meitner-window.xml";
...
async function handle(message: Json) {
  ...
  if (method === "initialize") return ok(id, { protocolVersion: String(params.protocolVersion ?? "2024-11-05"), capabilities: { tools: {} }, serverInfo: { name: "kind-meitner-phone", version: "1" } });
