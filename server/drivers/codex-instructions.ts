...
export function codexDeveloperInstructions(config: unknown, botInstructions: string): string {
  ...
  return configured
    ? `${botInstructions || "No kind-meitner bot-specific instructions remain."}\n\n${configured}`
    : botInstructions;
}
    ...
    try {
      await request("thread/inject_items", {
        ...
        items: [{
          ...
          content: [{
            type: "input_text",
            text: "The following replaces the previous developer instruction block supplied by kind-meitner, including its native configured rules and bot-specific instructions. Other Codex instructions and permissions still apply.\n\n"
              + (instructions || "No kind-meitner bot-specific instructions remain."),
          }],
        }],
      });
    } catch (error) {
