...
function inheritsUserConfig(env: NodeJS.ProcessEnv): boolean {
  return env.KIND_MEITNER_CLAUDE_INHERIT_USER_CONFIG === "1";
}

/** Retain the selected CLI account's authentication without importing its
 * hooks, permissions, MCP servers or personal instructions. Explicit kind-meitner
 ...
 *
 * KIND_MEITNER_CLAUDE_AUTOCOMPACT takes a token count, "auto" to hand the decision
...
export function autoCompactWindow(env: NodeJS.ProcessEnv): string | null {
  const raw = (env.KIND_MEITNER_CLAUDE_AUTOCOMPACT ?? "").trim().toLowerCase();
...
export function claudeCliUpdate(version: string | null, cli: string): ProviderSnapshot["update"] | undefined {
  ...
  return {
    ...
    message:
      ...
      "no compaction window picked by kind-meitner" +
...
const DENY_TIMEOUT_NOTE =
  "kind-meitner: nobody answered this permission request in time. Skip this action and finish what you can without it.";
const QUESTION_TIMEOUT_NOTE = "kind-meitner: nobody answered in time. Use your best judgment and continue.";
const DUPLICATE_ASK_ID_NOTE = "kind-meitner: duplicate ask id — skipping this request.";
...
function systemEndedReply(kind: Ask["kind"]): { behavior: AskBehavior; message: string } {
  return kind === "question"
    ? { behavior: "answer", message: "kind-meitner: the turn is ending — wrap up." }
    : { behavior: "deny", message: "kind-meitner: the turn ended" };
}
...
export const ClaudeDriver: ProviderDriver<ClaudeConfig> = {
  ...
  async create(input: DriverCreateInput<ClaudeConfig>): Promise<ProviderInstance> {
    ...
    const configuredIdleMinimum = Number(process.env.KIND_MEITNER_CLAUDE_SESSION_IDLE_MIN_MS);
    const sessionIdleMinimum = Number.isFinite(configuredIdleMinimum) && configuredIdleMinimum > 0
      ? configuredIdleMinimum
      : 10_000;
    const SESSION_IDLE_MS = Math.max(sessionIdleMinimum, Number(process.env.KIND_MEITNER_CLAUDE_SESSION_IDLE_MS) || 10 * 60_000);
    ...
    const sendTurn = async (turn: SendTurnInput, logicalTurnId?: string) => {
      ...
      if (turn.integrations?.computer) {
        ...
        // Host tools always route through kind-meitner's permission broker.
      ...
      if (Object.keys(mcpServers).length) {
        mcpConfigPath = join(mkdtempSync(join(tmpdir(), "kind-meitner-mcp-")), "mcp.json");
      ...
      try {
        ...
        if (turn.system) {
          systemPromptPath = join(mkdtempSync(join(tmpdir(), "kind-meitner-system-")), "prompt.txt");
