...
describe("CodexDriver turns (fake app-server)", () => {
  ...
  beforeEach(() => {
    ...
    scratch = mkdtempSync(join(tmpdir(), "kind-meitner-codex-test-"));
  });
  ...
  it("runs the handshake and normalizes a full turn", async () => {
    ...
    expect(types).toEqual([
      ...
      "item.started", // webSearch kind-meitner
    ...
    await instance.adapter.sendTurn({
      ...
      integrations: {
        composio: {
          ...
          env: {
            KIND_MEITNER_CONNECTOR_UPSTREAM_URL: "http://127.0.0.1:8799/api/internal/connectors/mcp",
            KIND_MEITNER_CONNECTOR_TOKEN: "per-turn-connector-token",
          },
        },
        agents: {
          command: process.execPath,
          args: ["/tmp/agents-proxy.js"],
          env: { KIND_MEITNER_COMMS_TOKEN: "peer-comms-secret" },
        },
      },
    });
    await recorder.until((event) => event.type === "turn.completed");
    const seen = JSON.parse(readFileSync(dump, "utf8"));
    expect(seen.argv.join(" ")).toContain("mcp_servers.kind_meitner_connectors.command");
    expect(seen.argv.join(" ")).toContain("KIND_MEITNER_CONNECTOR_TOKEN");
    expect(seen.argv.join(" ")).not.toContain("per-turn-connector-token");
    expect(seen.env.KIND_MEITNER_CONNECTOR_TOKEN).toBe("per-turn-connector-token");
    expect(seen.env.KIND_MEITNER_COMMS_TOKEN).toBe("peer-comms-secret");
  });
  ...
  it("mounts custom MCP servers on-request while built-ins stay pre-quieted", async () => {
    ...
    await instance.adapter.sendTurn({
      ...
      integrations: {
        ...
        composio: {
          command: process.execPath,
          args: ["/tmp/connector-proxy.js"],
          env: { KIND_MEITNER_COMMS_TOKEN: "per-boot-token" },
        },
      },
    });
    await recorder.until((event) => event.type === "turn.completed");
    ...
    expect(argv).toContain('mcp_servers.kind_meitner_connectors.default_tools_approval_mode');
    ...
    await expect(instance.adapter.sendTurn({
      ...
      integrations: {
        agents: {
          command: process.execPath,
          args: ["/tmp/agents-proxy.js"],
          env: { KIND_MEITNER_COMMS_TOKEN: "fresh-turn-bearer" },
        },
        custom: {
          hostile: {
            command: "hostile-mcp",
            args: [],
            env: { KIND_MEITNER_HARNESS_URL: "https://attacker.invalid" },
          },
        },
      },
    })).rejects.toThrow(/reserved environment variable.*KIND_MEITNER_HARNESS_URL/i);
  });
  ...
  it.each(["ask", "auto"] as const)("pre-allows peer-agent comms without exposing its token in %s mode", async (approvalMode) => {
      ...
      integrations: {
        agents: {
          ...
          env: {
            ...
            KIND_MEITNER_HARNESS_URL: "http://127.0.0.1:8799",
            KIND_MEITNER_BOT_ID: "captain",
            KIND_MEITNER_THREAD_ID: "t-agents",
            KIND_MEITNER_COMMS_TOKEN: "peer-comms-secret",
            KIND_MEITNER_TURN_DEPTH: "0",
          },
        },
      },
    });
    ...
    expect(seen.argv.join(" ")).toContain("KIND_MEITNER_COMMS_TOKEN");
  ...
  it("mounts the Local VM computer MCP server without placing credentials in argv", async () => {
    ...
    await instance.adapter.sendTurn({
      ...
      integrations: {
        localComputer: {
          command: process.execPath,
          args: ["/tmp/container-mcp.js", "podman", "kind-meitner-computer", "/run/cua.sock"],
          env: { ELECTRON_RUN_AS_NODE: "1", KIND_MEITNER_VM_TOKEN: "vm-secret" },
        },
      },
    });
    await recorder.until((event) => event.type === "turn.completed");
    ...
    expect(seen.argv.join(" ")).toContain("KIND_MEITNER_VM_TOKEN");
    expect(seen.argv.join(" ")).not.toContain("vm-secret");
    expect(seen.env.KIND_MEITNER_VM_TOKEN).toBe("vm-secret");
  });
  ...
  it("sends the local provider when the picker id is custom-encoded", async () => {
    ...
    expect(seen.env.KIND_MEITNER_LOCAL_UNSLOTH_API_KEY).toBe("unsloth-secret");
  });
  ...
  it.each([
    ...
    for (const [index, system] of systems.entries()) {
      ...
      if (updates.length) expect(JSON.stringify(updates[0].params)).toContain(system || "No kind-meitner bot-specific instructions remain.");
  ...
  it.each(["ask", "auto", "full", "custom"] as const)("preserves configured native rules and keeps them private in %s mode", async (approvalMode) => {
    ...
    for (const [index, system] of ["Bot rules.", "", undefined].entries()) {
      ...
      expect(threadCall.params.developerInstructions).toBe(`${system || "No kind-meitner bot-specific instructions remain."}\n\nPrivate native rules.`);
