...
const mocks = vi.hoisted(() => ({
  ...
  fleetAccess: vi.fn(),
  FLEET_CREDENTIAL_ENV: "KIND_MEITNER_INSTALLATION_CREDENTIAL",
  denyLogOpen: false,
  spawn: vi.fn(),
...
beforeEach(() => {
  ...
  mocks.tunnelAccess.mockReturnValue({ endpoint: "https://fixture.kind-meitner.test" });
...
describe("CLI startup lifecycle", () => {
  ...
  it("does not start a tunnel after SIGINT during the final readiness response", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn(async (url: string | URL | Request) => {
      ...
      return Response.json({ app: "kind-meitner", pid: childPid });
    }));
  ...
  it("reopens a matching desktop workspace even when its server PID differs from the lease owner", async () => {
    ...
    writeFileSync(join(dataDir, "kind-meitner-server.lease"), JSON.stringify({
      version: 1, pid: process.pid, host: hostname(), token: workspaceId, createdAt: Date.now(),
    }));
    vi.stubEnv("KIND_MEITNER_DATA_DIR", process.env.KIND_MEITNER_DATA_DIR);
    ...
    vi.stubGlobal("fetch", vi.fn(async (url: string | URL | Request) => String(url).endsWith("/api/health")
      ? Response.json({ app: "kind-meitner", pid: childPid })
  ...
  it("does not mistake another workspace on the same port for this one", async () => {
    writeFileSync(join(dataDir, "environment-id"), workspaceId);
    vi.stubGlobal("fetch", vi.fn().mockResolvedValueOnce(Response.json({ app: "kind-meitner", pid: childPid }))
  ...
  it.each(["mismatched", "unreachable"])("never mints a phone code for a %s public endpoint", async (failure) => {
    ...
    vi.stubGlobal("fetch", vi.fn(async (url: string | URL | Request, init?: RequestInit) => {
      ...
      if (address.endsWith("/api/health")) {
        return ++healthRequests === 1 ? Response.json({}, { status: 503 }) : Response.json({ app: "kind-meitner", pid: childPid });
      }
    ...
    expect(requests).toContain("https://fixture.example.test/.well-known/kind-meitner/environment");
  });
  ...
  it("offers Android client pairing only after verifying the same workspace, without claiming it is connected", async () => {
    ...
    const fetcher = vi.fn(async (url: string | URL | Request, init?: RequestInit) => {
      ...
      expect(address).toMatch(/\/\.well-known\/kind-meitner\/environment$/);
      expect(init?.method).not.toBe("POST");
    ...
    expect(requests).toContain(`http://127.0.0.1:${options.port}/.well-known/kind-meitner/environment`);
    expect(requests).toContain(`${origin}/.well-known/kind-meitner/environment`);
    expect(requests.indexOf(`${origin}/.well-known/kind-meitner/environment`)).toBeLessThan(requests.indexOf(pairingRequest));
