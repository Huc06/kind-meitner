...
//
// kind-meitner owns only the sandbox boundary: image preparation, container
...
export const IMAGE_REPOSITORY = "localhost/kind-meitner/cua-local-vm";
export const IMAGE_LAYER_VERSION = "5";
export const IMAGE_LAYER_LABEL = "com.kind-meitner.image-layer";
export const IMAGE = `${IMAGE_REPOSITORY}:driver-${CUA_DRIVER_VERSION}-v${IMAGE_LAYER_VERSION}`;
export const CONTAINER = "kind-meitner-computer";
export const MANAGED_LABEL = "com.kind-meitner.local-vm";
export const DRIVER_LABEL = "com.kind-meitner.cua-driver";
export const BASE_IMAGE_LABEL = "com.kind-meitner.cua-base";
export const WORKSPACE_LABEL = "com.kind-meitner.workspace";
export const TARGET_LABEL = "com.kind-meitner.local-vm-target";
...
export const CUA_SOCKET = "/run/user/1000/kind-meitner-cua.sock";
export const CUA_EXECUTABLE = "/usr/local/libexec/kind-meitner/cua-driver";
...
RUN printf '%s\\n' \\
      ...
      > /usr/local/bin/prepare-kind-meitner-workspace.sh \\
    && chmod 0755 /usr/local/bin/prepare-kind-meitner-workspace.sh
RUN printf '%s\\n' \\
      '#!/bin/sh' \\
      '/usr/local/bin/prepare-kind-meitner-workspace.sh' \\
      'attempt=0' \\
      ...
      > /usr/local/bin/start-kind-meitner-cua-driver.sh \\
    && chmod 0755 /usr/local/bin/start-kind-meitner-cua-driver.sh
RUN printf '%s\\n' \\
      '' \\
      '[program:kind-meitner-cua-driver]' \\
      'command=/usr/local/bin/start-kind-meitner-cua-driver.sh' \\
      'user=cua' \\
...
function statusProblem(status: ContainerComputerStatus): string | null {
  ...
  if (!status.managed) return "The existing container was not created by kind-meitner; recreate it";
 ...
 * versions. An older kind-meitner container must stay removable (and eligible
...
export async function containerComputerStatus(
  ...
  try {
    ...
    // The prepared kind-meitner derivative has not been built yet.
  }
  ...
  if (canProbe) {
    try {
      ...
      const readinessShot = "/tmp/kind-meitner-readiness.png";
      await runner(
        status.runtime,
        cuaExecArgs([
          "call",
...
async function prepareManagedImage(runtime: Runtime, runner: CommandRunner): Promise<void> {
  ...
  const context = await mkdtemp(join(tmpdir(), "kind-meitner-cua-image-"));
  try {
...
export async function containerComputerAction(
  ...
  if (action === "remove" && !before.managed) {
    throw Object.assign(
      new Error(
        `The existing container named ${target.containerName} was not created by kind-meitner; remove it manually in ${runtime}`,
      ),
      { status: 409 },
    );
  }
...
export async function containerComputerFrame(
  ...
  try {
    const screenshot = "/tmp/kind-meitner-preview.png";
    await runner(
      status.runtime,
      cuaExecArgs([
        "call",
...
export function containerComputerMcp(
  ...
  return {
    ...
    env: {
      ...
      ...(control ? { KIND_MEITNER_CONTROL_URL: control.url, KIND_MEITNER_CONTROL_TOKEN: control.token } : {}),
    },
  };
}
...
/** Cloud boxes still use kind-meitner's high-latency REST adapter. Local VMs
