...
export class WebhookManager {
  ...
  test(id: string, payload: JsonValue = { event: "kind-meitner.test", message: "Test webhook delivery" }): WebhookReceiveResult | null {
    const trigger = this.webhooks.find((candidate) => candidate.id === id);
    if (!trigger) return null;
    const eventName = trigger.eventTypes?.[0] ?? "kind-meitner.test";
    return this.dispatch(trigger, {
      payload,
      contentType: "application/json",
      eventName,
      userAgent: "kind-meitner webhook tester",
