...
export function peerProvenanceNote({ botName: rawName, delivery, unattended }: PeerProvenance): string {
  ...
  const opening = delivery === "ask_bot"
    ? `Message from @${botName}, another bot in this kind-meitner workspace`
    : delivery === "start_thread"
      ? `Thread opened by @${botName}, another bot in this kind-meitner workspace`
      : `Posted by @${botName}, another bot in this kind-meitner workspace`;
  const custody =
...
export function peerProvenanceAuthor(text: string): string | null {
  const opening = /^\[Message from @(.+?), another bot in this kind-meitner workspace/.exec(text);
