...
export function loadBundledSkills(root = process.env.KIND_MEITNER_SKILLS_DIR || join(process.cwd(), "skills")): BundledSkill[] {
...
export function renderSkillInstructions(
  ...
  return selected.map(({ manifest, instructions, directory }) =>
    `\n\n<kind-meitner-skill id=${JSON.stringify(manifest.id)} version=${JSON.stringify(manifest.version)}${includeRoot ? ` root=${JSON.stringify(directory)}` : ""}>\n${instructions}\n</kind-meitner-skill>`,
  ).join("");
}
