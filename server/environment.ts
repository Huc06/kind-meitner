...
// /.well-known/kind-meitner/environment so a saved connection can check it is
 ...
 * an image sets KIND_MEITNER_APP_VERSION at build time. */
export function serverVersion(): string {
  const fromEnv = process.env.KIND_MEITNER_APP_VERSION?.trim();
...
export function environmentDescriptor(input: { environmentId: string; desktopManaged: boolean; emailSignIn?: boolean; sharedComputers?: boolean }): EnvironmentDescriptor {
  return {
    ...
    label: process.env.KIND_MEITNER_ENVIRONMENT_LABEL?.trim() || hostname(),
