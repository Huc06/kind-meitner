...
afterEach(() => {
  ...
  delete process.env.KIND_MEITNER_APP_VERSION;
  delete process.env.KIND_MEITNER_ENVIRONMENT_LABEL;
});

describe("environment identity", () => {
  it("creates the id once, owner-only, and keeps it across restarts", () => {
    const dir = mkdtempSync(join(tmpdir(), "kind-meitner-env-"));
    dirs.push(dir);
  ...
  it("rejects UUID-shaped garbage and an unreadable identity path", () => {
    const malformedDir = mkdtempSync(join(tmpdir(), "kind-meitner-env-"));
    ...
    const unreadableDir = mkdtempSync(join(tmpdir(), "kind-meitner-env-"));
  ...
  it("does not create the data directory merely by importing computer providers", async () => {
    const parent = mkdtempSync(join(tmpdir(), "kind-meitner-provider-import-"));
    dirs.push(parent);
  ...
  it("describes the server for clients without leaking anything secret", () => {
    process.env.KIND_MEITNER_APP_VERSION = "0.1.99";
    process.env.KIND_MEITNER_ENVIRONMENT_LABEL = "cab mini";
