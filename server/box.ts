...
const BOX_API = process.env.KIND_MEITNER_BOX_API || "https://ascii.dev/api/box/v1";
...
// into every new name so another kind-meitner installation using the same Box
 ...
 * carrying kind-meitner's exact deterministic name shape leave this boundary;
...
export async function listManagedBoxes(
  ...
  for (const candidate of listed.boxes) {
    ...
    if (!BOX_ID.test(boxId)) {
      return invalidInventory("ascii.dev returned an invalid id for an KindMeitner-managed cloud computer — refresh or repair it in ascii.dev");
    }
    const existing = ownedBoxByBot.get(owner.botId);
    if (existing && existing !== boxId) {
      return invalidInventory("ascii.dev returned conflicting cloud computers for one KindMeitner bot — repair them in ascii.dev before continuing");
    }
  ...
  for (const candidate of listed.boxes) {
    ...
    if (SCOPED_MANAGED_BOX_NAME.test(name)) {
      // A valid kind-meitner name for another environment is account-visible but not
    ...
    if ((boxIdCounts.get(boxId) ?? 0) !== 1 || seenBoxIds.has(boxId)) {
      return invalidInventory("ascii.dev returned a conflicting id for an KindMeitner-managed cloud computer — refresh or repair it in ascii.dev");
    }
    if (legacyOwner && owner && options.adoptLegacy !== false) {
      try {
        adoptResolvedBox(owner.botId, boxId);
      } catch {
        return invalidInventory("kind-meitner could not safely remember this legacy cloud computer's owner — repair it in ascii.dev before continuing");
      }
    }
...
async function revalidateManagedBox(
  ...
  if (!instance) {
    throw Object.assign(new Error("that KindMeitner-managed cloud computer no longer exists"), { status: 404 });
  }
  return instance;
}
...
export async function provisionBox(cfg: AppConfig, botId: string, botName: string) {
  cfg = snapshotBoxConfig(cfg);
  if (!boxConfigured(cfg)) {
    throw new Error('box provider not enabled — add {"box":{"token":"…"}} to ~/.kind-meitner/config.json');
  }
