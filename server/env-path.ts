...
export function augmentedPath(): string {
  if (cached === null) {
    cached = mergePaths([
      ...
      ...(process.env.KIND_MEITNER_EXTRA_PATH ? process.env.KIND_MEITNER_EXTRA_PATH.split(delimiter) : []),
...
function probeLoginShellPath(): void {
  ...
  execFile(
    shell,
    ["-l", "-i", "-c", 'printf "__KIND_MEITNER_PATH__%s" "$PATH"'],
    { timeout: 5000 },
    (err, stdout) => {
      if (err || !stdout) return;
      const m = /__KIND_MEITNER_PATH__([^\n]*)/.exec(stdout);
      if (!m || !m[1]) return;
      loginShellPath = m[1];
