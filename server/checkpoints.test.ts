...
// KIND_MEITNER_DATA_DIR at import time — so the env var must be set before the import
// is evaluated (same pattern as attachments.test.ts).
const DATA_ROOT = mkdtempSync(join(tmpdir(), "kind-meitner-checkpoints-"));
process.env.KIND_MEITNER_DATA_DIR = join(DATA_ROOT, "data");
...
let seq = 0;
function workspace() {
  const cwd = mkdtempSync(join(tmpdir(), "kind-meitner-ckpt-ws-"));
...
describe("refusals", () => {
  it("refuses the home folder, protected folders, and missing paths", async () => {
    ...
    expect(refusalReason(join(tmpdir(), "kind-meitner-ckpt-definitely-missing-xyz"))).not.toBeNull();
  ...
  it("lists nothing (and creates nothing) for a folder never snapshotted", async () => {
    ...
    const shadow = join(process.env.KIND_MEITNER_DATA_DIR!, "checkpoints", bot);
