...
beforeAll(async () => {
  dir = mkdtempSync(join(tmpdir(), "kind-meitner-webhook-ingress-"));
...
describe("webhook-only ingress", () => {
  ...
  it("exposes health but nothing from the main kind-meitner API", async () => {
    const health = await fetch(`${ingress.baseUrl}/health`);
    expect(health.status).toBe(200);
    expect(await health.json()).toEqual({ app: "kind-meitner-webhooks", ready: true });
