...
function fakeArchive() {
  ...
  const summary: WorkspaceBackupSummary = { format: "kind-meitner.workspace-backup", version: 1, id, createdAt: "2026-09-11T00:00:00Z", appVersion: "0.1.71", files: 1, directories: 0, bytes: 23, bots: 0, groups: 0, threads: 0, messages: 0, exclusions: [], warnings: [] };
  return { id, path, summary };
}

beforeEach(async () => {
  vi.resetAllMocks();
  dataDir = mkdtempSync(join(tmpdir(), "kind-meitner-backup-http-"));
...
it("accepts drafts between 1 and 2 MiB and enforces the preference cap in UTF-8 bytes", async () => {
  const clientState = { "kind-meitner-drafts": "a".repeat(1536 * 1024) };
  ...
  const tooLarge = { "kind-meitner-drafts": "é".repeat(1024 ** 2) };
