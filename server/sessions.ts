...
// session token (`kind_meitner_sess_…`, 256 bits) that lives 30 days, renewed on use
 ...
 * KIND_MEITNER_SESSION_TTL_DAYS overrides the 30-day default. */
export const SESSION_TTL_MS = daysMs(process.env.KIND_MEITNER_SESSION_TTL_DAYS, 30);
 ...
 * bounded life and every device re-pairs occasionally. KIND_MEITNER_SESSION_MAX_DAYS
 * overrides the 180-day default. */
export const SESSION_MAX_AGE_MS = daysMs(process.env.KIND_MEITNER_SESSION_MAX_DAYS, 180);
...
export class SessionRegistry {
  ...
  exchange(input: { code: string; label: string; source: string; fallbackLabel?: string; attemptId?: string }): ExchangeResult {
    ...
    const token = `kind_meitner_sess_${randomBytes(32).toString("base64url")}`;
  ...
  issueStreamTicket(sessionId: string): { ticket: string; expiresAt: number } {
    ...
    const ticket = `kind_meitner_tick_${randomBytes(24).toString("base64url")}`;
