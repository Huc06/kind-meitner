...
export function gatedLocalComputer(
  ...
  return {
    ...
    env: {
      ...
      // this the MCP client relaunches kind-meitner, whose single-instance handler
      // focuses the user's window, instead of starting the headless gate.
      ELECTRON_RUN_AS_NODE: "1",
      KIND_MEITNER_CUA_COMMAND: connection.command,
      KIND_MEITNER_CUA_ARGS: JSON.stringify(connection.args),
      KIND_MEITNER_CONTROL_URL: control.url,
      KIND_MEITNER_CONTROL_TOKEN: control.token,
    },
  };
}
...
export function decodeLinuxDescriptor(value: LinuxConnectionDescriptor): LocalComputerConnection | null {
  ...
  if (
    ...
    (mcp.env as Record<string, unknown>).CUA_DRIVER_HOST_BUNDLE_ID !== "com.kind-meitner.app" ||
...
export function readCuaConnection({
  ...
  userData = process.env.KIND_MEITNER_USER_DATA,
  home = homedir(),
  ...
  if (platform === "darwin") {
    ...
    for (const directory of ["kind-meitner", "kind-meitner", "OpenGrokBot", "opengrokbot"]) {
