  ...
  return async (req: IncomingMessage, res: ServerResponse, path: string, auth: RequestAuth): Promise<boolean> => {
    ...
    try {
      ...
      if (method === "POST" && path === `${PREFIX}/client-state`) {
        ...
        check(req, auth);
        if (!options.restored.restored || body.restoreId !== options.restored.id) throw failure("This restore has not completed. Restart kind-meitner first.", 409);
      ...
      if (method === "POST" && path === `${PREFIX}/export`) {
        ...
        json(res, 200, { id: result.id, filename: `kind-meitner-${result.summary.createdAt.slice(0, 10)}.ombbackup`, bytes: statSync(result.path).size, summary: result.summary });
        return true;
      }
      ...
      if (method === "GET" && download) {
        ...
        res.writeHead(200, {
          ...
          "content-disposition": `attachment; filename="kind-meitner-${artifact.summary!.createdAt.slice(0, 10)}.ombbackup"`,
