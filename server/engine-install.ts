...
function runNpm(args: string[], env: NodeJS.ProcessEnv, cwd: string, timeoutMs: number): Promise<{ code: number | null; output: string }> {
  return new Promise((resolveRun, rejectRun) => {
    ...
    try {
      ...
      rejectRun(new Error("npm could not start on this server. Install Node.js with npm for the user running kind-meitner, then try again."));
      return;
    }
    child.stdin.end();
    let output = "";
    ...
    child.once("error", (error: NodeJS.ErrnoException) => {
      ...
      rejectRun(new Error(error.code === "ENOENT"
        ? "npm is not installed on this server. Install Node.js with npm for the user running kind-meitner, then try again."
        : "npm could not start on this server. Check that Node.js is installed for the user running kind-meitner."));
    });
