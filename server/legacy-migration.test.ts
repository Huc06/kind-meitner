...
// everything in ~/.kind-meitner after the first boot. Anything that touches
...
beforeAll(async () => {
  home = mkdtempSync(join(tmpdir(), "kind-meitner-legacy-test-"));
...
describe("legacy data dir", () => {
  it("is renamed to the new name on first boot, with its contents and a fresh environment id", () => {
    const fresh = join(home, ".kind-meitner");
