...
it("recovers queued/due work without resurrecting an interrupted routine after restart", async () => {
  ...
  try {
    ...
    await runControlKindMeitner(["wait", "--bot", reusedBot.id, "--task", reusedBot.threadId, "--url", url]);
    ...
    Object.assign(env, {
      HOME: dataDir, USERPROFILE: dataDir, KIND_MEITNER_DATA_DIR: dataDir,
      ...
      KIND_MEITNER_PORT: new URL(url).port, KIND_MEITNER_WEBHOOK_PORT: String(Number(new URL(url).port) + 1),
    ...
    const wait = await runControlKindMeitner(["wait", "--bot", scheduledBot.id, "--task", scheduledRun.threadId!, "--url", url]);
    const messages = await runControlKindMeitner(["messages", "--bot", scheduledBot.id, "--task", scheduledRun.threadId!, "--url", url]);
