...
// The layer's only obligation is `register()`, which turns KIND_MEITNER_LICENSE_KEY into
...
export function hostedWorkspaceConfigured(env: NodeJS.ProcessEnv = process.env): boolean {
  return env.KIND_MEITNER_ADMIN_URL !== undefined || env.KIND_MEITNER_ADMIN_WORKSPACE !== undefined || env.KIND_MEITNER_ADMIN_MEMBERSHIP !== undefined;
}
...
export function hostedWorkspaceConfiguration(env: NodeJS.ProcessEnv = process.env): { admin: URL; tenant: URL; workspace: string; portalMembership: boolean } | null {
  try {
    ...
    if (!/^[a-z][a-z0-9-]{1,30}$/.test(env.KIND_MEITNER_ADMIN_WORKSPACE ?? "")) return null;
    if (env.KIND_MEITNER_ADMIN_MEMBERSHIP !== undefined && !["local", "portal"].includes(env.KIND_MEITNER_ADMIN_MEMBERSHIP)) return null;
    return { admin: origin(env.KIND_MEITNER_ADMIN_URL), tenant: origin(env.KIND_MEITNER_PUBLIC_URL), workspace: env.KIND_MEITNER_ADMIN_WORKSPACE!, portalMembership: env.KIND_MEITNER_ADMIN_MEMBERSHIP === "portal" };
  } catch { return null; }
}
...
export async function loadEnterpriseLayer(
  ...
  const dir = options.dir ?? process.env.KIND_MEITNER_ENTERPRISE_DIR ?? DEFAULT_DIR;
  const licenseKey = options.licenseKey ?? process.env.KIND_MEITNER_LICENSE_KEY;
  ...
  if (!entry) {
    return oss(
      licenseKey ? `KIND_MEITNER_LICENSE_KEY is set but no enterprise layer exists at ${dir}` : undefined,
    );
  }
  try {
    ...
    if (!licenseKey) return oss("enterprise layer present but KIND_MEITNER_LICENSE_KEY is not set");
...
export function editionStatus(now: number = Date.now()): EditionStatus {
  ...
  return {
    edition: "oss",
    features: [],
    notice: `enterprise layer disabled: KIND_MEITNER_LICENSE_KEY expired on ${current.expiresAt}; renew it to keep enterprise features`,
  };
}
...
export function describeEdition(status: EditionStatus): string {
  if (status.edition === "enterprise") {
    ...
    return `kind-meitner enterprise edition for ${status.customer}${until}: ${status.features.join(", ") || "no features"}`;
  }
  return `kind-meitner open-source edition${status.notice ? ` (${status.notice})` : ""}`;
}
