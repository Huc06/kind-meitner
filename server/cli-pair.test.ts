...
beforeEach(() => {
  ...
  vi.stubEnv("KIND_MEITNER_DATA_DIR", dataDir);
  ...
  fetchMock.mockImplementation(async (input, init) => {
    ...
    if (url === `${local}/api/health`) return Response.json({ app: "kind-meitner", pid: 12345 });
    ...
    if (url === `${local}/.well-known/kind-meitner/environment`) return Response.json({ environmentId: workspaceId });
    if ([advertisedOrigin, explicitOrigin].some((origin) => url === `${origin}/.well-known/kind-meitner/environment`)) {
...
function expectPhonePairing(origin: string): void {
  ...
  expect(String(probes[0]![0])).toBe(`${origin}/.well-known/kind-meitner/environment`);
