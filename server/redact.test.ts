...
describe("redactSecrets", () => {
  it("masks the tokens in an ACP session/new, keeping the shape", () => {
        ...
        mcpServers: [
          {
            ...
            env: [
              { name: "KIND_MEITNER_BOT_ID", value: "bot-123" },
              { name: "KIND_MEITNER_COMMS_TOKEN", value: "s3cret-comms-token-value" },
            ],
          },
          {
            name: "computer",
    ...
    expect(out).toContain("KIND_MEITNER_COMMS_TOKEN");
...
describe("redactSecretsInText", () => {
  ...
  it("masks an assignment to any name ending in KEY, whatever the value looks like", () => {
    ...
    expect(redactSecretsInText("X_KEY=value pnpm control:kind-meitner doctor")).toBe("X_KEY=«redacted 5 chars» pnpm control:kind-meitner doctor");
  ...
  it("is idempotent for structurally identified credentials", () => {
    const input = {
      ...
      env: [{ name: "KIND_MEITNER_COMMS_TOKEN", value: "abcdefghijklmnop" }],
    };
