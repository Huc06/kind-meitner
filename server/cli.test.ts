...
describe("kind-meitner command line", () => {
  it("parses commands and flags, and explains mistakes", () => {
    ...
    expect(parseArgs([], { KIND_MEITNER_PORT: "8123" })).toMatchObject({ command: "start", port: 8123 });
  ...
  it("prints a scannable block with the link, or says where to type the code", () => {
    ...
    const noUrl = pairingBlock({ code: "ABCD-EFGH-JKLM", url: null, expiresAt: Date.now(), hint: "set KIND_MEITNER_PUBLIC_URL" });
    expect(noUrl).toContain("/pair on the address you use");
    expect(noUrl).toContain("set KIND_MEITNER_PUBLIC_URL");
  ...
  it("serve: starts the server, prints the pairing link, and stops on SIGTERM", async () => {
    const home = mkdtempSync(join(tmpdir(), "kind-meitner-cli-serve-"));
    const port = 21000 + Math.floor(Math.random() * 9000);
    const child = spawn(process.execPath, ["--experimental-strip-types", join(SERVER_DIR, "kind-meitner.ts"), "serve", "--port", String(port), "--data-dir", join(home, "data"), "--label", "cli test", "--public-url", "https://mini.example"], {
      cwd: join(SERVER_DIR, ".."),
      env: { PATH: process.env.PATH ?? "", HOME: home, USERPROFILE: home, KIND_MEITNER_WEBHOOK_PORT: String(port + 1), KIND_MEITNER_BROWSER_CONNECTION: join(home, "browser-connection.json") },
    ...
    try {
      ...
      expect(out).toContain(`kind-meitner is running on http://127.0.0.1:${port}, reachable at https://mini.example`);
      expect(out).toMatch(/pairing code:  [A-Z2-9]{4}-[A-Z2-9]{4}-[A-Z2-9]{4}/);
      expect(out).toContain("open or scan:  https://mini.example/pair#code=");
      expect(out).toMatch(/[▀▄█]/);
      const descriptor: any = await (await fetch(`http://127.0.0.1:${port}/.well-known/kind-meitner/environment`)).json();
...
describe("terminal onboarding commands", () => {
  ...
  const preserveEnv = () => vi.stubEnv("KIND_MEITNER_DATA_DIR", process.env.KIND_MEITNER_DATA_DIR);
  ...
  it("runs explicit setup once and explains how to start without launching a server", async () => {
    ...
    setup.runSetup.mockImplementation(async () => {
      expect(process.env.KIND_MEITNER_DATA_DIR).toBe(options.dataDir);
      return true;
    });
    ...
    expect(output.log).toHaveBeenCalledWith(expect.stringContaining("Start with: kind-meitner"));
...
describe.skipIf(process.platform === "win32")("serve --tunnel", () => {
  const cli = (args: string[], env: NodeJS.ProcessEnv) =>
    spawn(process.execPath, ["--experimental-strip-types", join(SERVER_DIR, "kind-meitner.ts"), ...args], {
      cwd: join(SERVER_DIR, ".."),
  ...
  it("refuses without an account and says what to do; no local-only fallback", async () => {
    const home = mkdtempSync(join(tmpdir(), "kind-meitner-cli-tunnel-none-"));
    const port = 21000 + Math.floor(Math.random() * 9000);
    ...
    try {
      ...
      expect(err).toContain("run `kind-meitner login` first");
      let dead = false;
      try {
        await fetch(`http://127.0.0.1:${port}/api/health`);
      } catch {
  ...
  it("a fleet credential in the environment serves --tunnel with no account file and no code", async () => {
    const home = mkdtempSync(join(tmpdir(), "kind-meitner-cli-fleet-"));
    ...
    const fleetEnv = {
      ...
      KIND_MEITNER_BROWSER_CONNECTION: join(home, "browser-connection.json"),
      KIND_MEITNER_CONTROL_PLANE_URL: stub.url,
      KIND_MEITNER_CLOUDFLARED_PATH: fake,
      KIND_MEITNER_TUNNEL_ORIGIN_PORT: String(originPort),
    };
    ...
    const rejected = cli(["serve", "--tunnel", "--no-pair", "--port", String(port), "--data-dir", dataDir], {
      ...fleetEnv,
      KIND_MEITNER_INSTALLATION_CREDENTIAL: `kind_meitner_install_${"x".repeat(22)}.${"y".repeat(43)}`,
    });
    let err = "";
    ...
    const child = cli(["serve", "--tunnel", "--no-pair", "--port", String(port), "--data-dir", dataDir], {
      ...fleetEnv,
      KIND_MEITNER_INSTALLATION_CREDENTIAL: stub.seedInstallation("fleet box"),
    });
    let out = "";
    ...
    try {
      const deadline = Date.now() + 60_000;
      while (!out.includes("kind-meitner is running") && Date.now() < deadline && child.exitCode === null) await new Promise((r) => setTimeout(r, 200));
      expect(out).toContain("using the installation credential from KIND_MEITNER_INSTALLATION_CREDENTIAL");
      ...
      while (Date.now() < gatewayDeadline && status !== 200) {
        try {
          status = (await fetch(`${gateway}/.well-known/kind-meitner/environment`)).status;
        } catch {
          status = 0;
        }
  ...
  it("serves at the account's public address through the gateway, where every request is remote", async () => {
    const home = mkdtempSync(join(tmpdir(), "kind-meitner-cli-tunnel-"));
    const dataDir = join(home, "data");
    mkdirSync(dataDir, { recursive: true });
    const stub = await startControlPlaneStub();
    const fake = join(home, "cloudflared");
    ...
    vi.stubEnv("KIND_MEITNER_CONTROL_PLANE_URL", stub.url);
    ...
    try {
      const deadline = Date.now() + 60_000;
      while (!out.includes("open or scan:") && Date.now() < deadline && child.exitCode === null) await new Promise((r) => setTimeout(r, 200));
      expect(out).toContain(`kind-meitner is running on http://127.0.0.1:${port}, reachable at ${stub.endpointUrl}`);
      ...
      while (Date.now() < gatewayDeadline) {
        try {
          descriptor = await fetch(`${gateway}/.well-known/kind-meitner/environment`);
      ...
      expect(await (await fetch(`${gateway}/api/health`)).json()).toEqual({ app: "kind-meitner" });
...
describe("kind-meitner access", () => {
  it("edits the sign-in allow-list in config.json without a running server", async () => {
    const home = mkdtempSync(join(tmpdir(), "kind-meitner-cli-access-"));
    const dataDir = join(home, "data");
    const out: string[] = [];
    const err: string[] = [];
...
describe.skipIf(process.platform === "win32")("serve --domain", () => {
  it("runs a managed Caddy for the domain and serves the pairing link there", async () => {
    const home = mkdtempSync(join(tmpdir(), "kind-meitner-cli-domain-"));
    const dataDir = join(home, "data");
    mkdirSync(dataDir, { recursive: true });
    const fake = join(home, "fake-caddy");
    writeFileSync(fake, `#!/bin/sh\necho "$@" > "${join(home, "caddy-args.txt")}"\necho $$ > "${join(home, "caddy.pid")}"\nexec sleep 300\n`, { mode: 0o755 });
    const port = 21000 + Math.floor(Math.random() * 9000);
    const child = spawn(process.execPath, ["--experimental-strip-types", join(SERVER_DIR, "kind-meitner.ts"), "serve", "--domain", "kind-meitner.example.test", "--port", String(port), "--data-dir", dataDir], {
      cwd: join(SERVER_DIR, ".."),
      env: { PATH: process.env.PATH ?? "", HOME: home, USERPROFILE: home, KIND_MEITNER_WEBHOOK_PORT: String(port + 1), KIND_MEITNER_BROWSER_CONNECTION: join(home, "browser-connection.json"), KIND_MEITNER_CADDY_PATH: fake },
      stdio: ["ignore", "pipe", "pipe"],
    });
    let out = "";
    child.stdout?.on("data", (chunk) => (out += String(chunk)));
    ...
    try {
      const deadline = Date.now() + 60_000;
      while (!out.includes("open or scan:") && Date.now() < deadline && child.exitCode === null) await new Promise((r) => setTimeout(r, 200));
      expect(out).toContain(`kind-meitner is running on http://127.0.0.1:${port}, reachable at https://kind-meitner.example.test`);
      expect(out).toContain("https: Caddy serves https://kind-meitner.example.test");
      expect(out).toContain("open or scan:  https://kind-meitner.example.test/pair#code=");
      ...
      expect(caddyfile).toContain("kind-meitner.example.test {");
