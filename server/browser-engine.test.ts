...
describe("deleting one browser session's saved logins", () => {
  function fixture(closeCode = 0) {
    const home = mkdtempSync(join(tmpdir(), "kind-meitner-browser-cleanup-"));
    scratch.push(home);
  ...
  it("cannot launch a browser from close through inherited launch flags or native user config", async () => {
    ...
    expect(env?.AGENT_BROWSER_CONFIG).toBe(join(home, ".agent-browser", "kind-meitner-managed-config.json"));
...
describe("restoring exactly one browser profile", () => {
  function fixture() {
    const home = mkdtempSync(join(tmpdir(), "kind-meitner-browser-restore-"));
    scratch.push(home);
    const directory = join(home, ".agent-browser", "sessions");
  ...
  it("uses fixed-length restore identities even for overlapping legacy names", () => {
    ...
    expect(keys.every((key) => /^kind-meitner-[a-f0-9]{64}$/.test(key))).toBe(true);
  ...
  it("does not overwrite an unexpected managed config or start a browser with it", async () => {
    const { directory, options } = fixture();
    const path = join(directory, "..", "kind-meitner-managed-config.json");
...
describe("finding the browser engine", () => {
  it("pins the native-verified Windows revision in both download and desktop manifests", () => {
    ...
    expect(asset).toEqual({
      target: "win32-x64", version: "0.36.0-kind-meitner.1",
      asset: "agent-browser-win32-x64-0.36.0-kind-meitner.1.exe",
      url: "https://github.com/harrymove-ctrl/kind-meitner/releases/download/browser-engine-v0.36.0-kind-meitner.1/agent-browser-win32-x64-0.36.0-kind-meitner.1.exe",
      bytes: 13806080, sha256: "33bee834f6a6072ec8688b0914726e0262874d758f69f27e8baf7eaac6b5ed15",
    });
    expect(agentBrowserReleaseVersion(asset)).toBe("0.36.0-kind-meitner.1");
  ...
  it("does not reuse a pre-fix Windows managed install for a revised release", () => {
    const dataDir = join(tmpdir(), "kind-meitner-versioned-browser-fixture");
    const old = join(dataDir, "tools", "agent-browser", "0.36.0", "agent-browser.exe");
    const revised = pinnedBinaryPath(dataDir, "win32", "x64");
    expect(revised).toBe(join(dataDir, "tools", "agent-browser", "0.36.0-kind-meitner.1", "agent-browser.exe"));
    ...
    files.add(revised);
    expect(browserEngineStatus(options)).toMatchObject({ kind: "ready", binaryPath: revised, version: "0.36.0-kind-meitner.1" });
    expect(resolveAgentBrowserBinary({ ...options, env: { PATH: "", KIND_MEITNER_AGENT_BROWSER_PATH: old } })).toBe(old);
  });
  ...
  it("retains default upstream versions and permits a pinned platform-specific asset URL", () => {
    ...
    const revised = { ...official, version: "0.36.0-kind-meitner.1", url: "https://example.invalid/releases/download/fixed/fixture.exe" };
    expect(agentBrowserReleaseVersion(revised)).toBe("0.36.0-kind-meitner.1");
  ...
  it.each(SUPPORTED_BROWSER_TARGETS)("uses the complete %s desktop bundle before old downloaded engines", (target) => {
    ...
    const env = { KIND_MEITNER_RESOURCES_PATH: join(tmpdir(), "kind-meitner resources"), PATH: "" };
    const bundle = browserBundlePaths(join(env.KIND_MEITNER_RESOURCES_PATH, "browser-engine"), target);
    ...
    files.add(external);
    expect(resolveAgentBrowserBinary({ ...options, env: { ...env, KIND_MEITNER_AGENT_BROWSER_PATH: external } })).toBe(external);
  });

  it("mounts the bundled browser with no download and keeps explicit Chrome overrides", async () => {
    const resources = mkdtempSync(join(tmpdir(), "kind-meitner-browser-resources-"));
    scratch.push(resources);
    const env = { KIND_MEITNER_RESOURCES_PATH: resources, PATH: "" };
    ...
    expect(spec.env).not.toHaveProperty("KIND_MEITNER_RESOURCES_PATH");
  ...
  it("prefers the explicit path, then the pinned download, then PATH, and reports why when nothing is there", () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kind-meitner-engine-"));
    ...
    files.add(override);
    expect(resolveAgentBrowserBinary({ dataDir, env: { ...env, KIND_MEITNER_AGENT_BROWSER_PATH: override }, exists })).toBe(override);
    // an override that does not exist is an error, not a silent fallback
    expect(resolveAgentBrowserBinary({ dataDir, env: { ...env, KIND_MEITNER_AGENT_BROWSER_PATH: join(dataDir, "missing", name) }, exists })).toBeNull();
...
describe("installing the browser engine", () => {
  it("downloads the pinned asset, verifies size and digest, and only then names the file", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kind-meitner-engine-install-"));
    scratch.push(dataDir);
  ...
  it("refuses a download whose bytes do not match the pin, and leaves nothing behind", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kind-meitner-engine-bad-"));
    scratch.push(dataDir);
...
describe("what a bot gets", () => {
  it("forwards the explicit Chrome path without copying arbitrary environment or overriding session isolation", () => {
    ...
    expect(spec.env).toEqual({
      ...
      AGENT_BROWSER_CONFIG: expect.stringContaining("kind-meitner-managed-config.json"),
  ...
  it("makes one encryption key per data dir, private, and reuses it", () => {
    const dataDir = mkdtempSync(join(tmpdir(), "kind-meitner-engine-key-"));
    scratch.push(dataDir);
