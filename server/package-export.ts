...
export function createBotPackageExport(input: {
  ...
  const id = portableKey(input.name, "kind-meitner-package", new Set());
  ...
  const definition: BotPackageDefinition = {
    ...
    release: "1.0.0",
    name: input.name,
    tagline: `A portable kind-meitner setup with ${bots.length} ${bots.length === 1 ? "bot" : "bots"}.`,
    summary: "Exported from kind-meitner. Review the roles, rooms, playbooks, connector requirements, and paused routines before sharing or publishing.",
    category: "Community",
    author: { name: input.authorName?.trim() || "kind-meitner user" },
    license: "Unspecified",
