...
const SCHEMA = "aos.kind_meitner_status.v1";
...
export const KIND_MEITNER_STATUS_CACHE_PATH = join(
  homedir(),
  ".local/state/aos-session-bridge/kind-meitner/latest.json",
);

type KindMeitnerSlot = z.output<typeof slotSchema>;
type KindMeitnerUi = z.output<typeof uiSchema>;
type KindMeitnerCapsule = z.output<typeof capsuleSchema>;

export interface KindMeitnerStatusDigest {
...
export interface KindMeitnerStatusReadOptions {
  cachePath?: string;
  now?: Date;
}
...
function expectedUi(twoUp: boolean): KindMeitnerUi {
  return {
    two_up: twoUp,
...
function sameUi(left: KindMeitnerUi, right: KindMeitnerUi): boolean {
  return (
...
function unsignedDocument(capsule: KindMeitnerCapsule): JsonObject {
...
function validSlotState(slot: KindMeitnerSlot): boolean {
  return (
...
function validateCapsule(value: JsonValue): KindMeitnerCapsule | null {
...
function unknownDigest(
  reason: NonNullable<KindMeitnerStatusDigest["reason"]>,
  freshness: KindMeitnerStatusDigest["freshness"] = "unknown",
  receipt?: Pick<KindMeitnerStatusDigest, "observedAt" | "receiptSha256">,
): KindMeitnerStatusDigest {
  return {
    schema: SCHEMA,
    freshness,
    reason,
...
export function readKindMeitnerStatus(
  options: KindMeitnerStatusReadOptions = {},
): KindMeitnerStatusDigest {
  const raw = privateCache(options.cachePath ?? KIND_MEITNER_STATUS_CACHE_PATH);
  if (raw === null) return unknownDigest("missing_or_insecure");
  let capsule: KindMeitnerCapsule | null;
  try {
  ...
  const result: KindMeitnerStatusDigest = {
    schema: SCHEMA,
    freshness: "fresh",
    ...receipt,
...
export function kindMeitnerStatusSystemPrompt(options: KindMeitnerStatusReadOptions = {}): string {
  const status = readKindMeitnerStatus(options);
  const receipt = [
  ...
  const slots = status.slots.length
    ...
    : "none";
  return [
    "TRUSTED KIND_MEITNER STATUS (read-only, validated, no transcript or credential data):",
