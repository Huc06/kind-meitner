 ...
 * bot servers directly as before. `KIND_MEITNER_MCP_RESULT_BUDGET=0` is the escape
...
export function resultBudget(env: NodeJS.ProcessEnv = process.env): number {
  const raw = env.KIND_MEITNER_MCP_RESULT_BUDGET;
...
export function gateServer(input: {
  ...
  return {
    ...
    env: {
      ...input.nodeEnv,
      KIND_MEITNER_GATE_NAME: name,
      KIND_MEITNER_GATE_UPSTREAM: JSON.stringify({ command: spec.command, args: spec.args ?? [], env: spec.env ?? {} }),
      KIND_MEITNER_GATE_SPILL_DIR: spillDir(input.threadId),
      KIND_MEITNER_GATE_BUDGET: String(budget),
    },
  };
}
