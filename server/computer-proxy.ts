...
const CONTROL_POLL_MS = Math.max(Number(process.env.KIND_MEITNER_CONTROL_POLL_MS) || 1_500, 25);
const CONTROL_WAIT_MS = Math.max(Number(process.env.KIND_MEITNER_CONTROL_WAIT_MS) || 600_000, 100);
...
const CHROME_PROFILE = "$HOME/.kind-meitner/chrome-profile";
...
const CHROME_PROFILE_SETUP = [
  ...
  "      exit 1",
  "    fi",
  '    mv "$browser_dir" "$browser_dir.pre-kind-meitner-$(date +%s)-$$"',
  "  fi",
  ...
  if (msg.method === "initialize") {
    return send({
      ...
      result: {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: "kind-meitner-computer", version: "3" },
      },
    });
  }
