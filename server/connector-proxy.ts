...
// into first-class kind-meitner chat cards. The agent never authors an auth
...
const UPSTREAM = process.env.KIND_MEITNER_CONNECTOR_UPSTREAM_URL ?? "";
...
const TOKEN = process.env.KIND_MEITNER_CONNECTOR_TOKEN ?? process.env.KIND_MEITNER_COMMS_TOKEN ?? "";
...
function parsedHeaders(): Record<string, string> {
  try {
    const value: unknown = JSON.parse(process.env.KIND_MEITNER_CONNECTOR_UPSTREAM_HEADERS ?? "{}");
...
function initializeResult(id: unknown, protocolVersion: unknown): Json {
  return {
    ...
    result: {
      ...
      capabilities: { tools: {} },
      serverInfo: { name: "kind-meitner-connectors", version: "1" },
    },
  };
}
...
async function handle(message: Json): Promise<void> {
  ...
  if (method === "tools/call") {
    ...
    if (requests.length) {
      ...
      send(textResult(
        id,
        `kind-meitner showed the user a secure connection card for ${labels}. End this turn now. The app will continue the task automatically after the connection finishes.`,
      ));
      return;
    }
    if (/WAIT_FOR_CONNECTIONS$/i.test(name)) {
      send(textResult(id, "kind-meitner is handling connection completion and will continue the task automatically."));
      return;
    }
  }
  try {
