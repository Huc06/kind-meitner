...
const envelope = (payload: Record<string, string>) => `<kind-meitner-goal>${JSON.stringify(payload)}</kind-meitner-goal>`;
...
beforeAll(async () => {
  home = mkdtempSync(join(tmpdir(), "kind-meitner-goal-wait-cap-"));
  ...
  child = spawn(process.execPath, [join(SERVER_DIR, "index.ts")], {
    ...
    env: {
      ...
      KIND_MEITNER_GOAL_WAIT_MAX_MS: "1500",
    },
