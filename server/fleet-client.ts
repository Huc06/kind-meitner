...
export function fleetSocketPath(env: NodeJS.ProcessEnv = process.env): string {
  return env.KIND_MEITNER_FLEET_SOCKET?.trim() || "/run/kind-meitner/fleet.sock";
}
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on("error", (error: NodeJS.ErrnoException) => {
      reject(new Error(error.code === "ENOENT" || error.code === "ECONNREFUSED"
        ? "no fleet agent on this server: run `kind-meitner fleet init --domain … --operator <this user>` as root"
        : error.code === "EACCES"
          ? "this workspace's user may not open the fleet socket: re-run `kind-meitner fleet init` with --operator set to it"
