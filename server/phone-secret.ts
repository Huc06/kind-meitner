...
export const PHONE_SECRET_INFO = "kind-meitner phone credential v1";
...
function decodeSaveResult(raw: unknown): { requestId: string; ok: boolean; error?: string } | null {
  ...
  if (value.type !== "kind-meitner:phone-secret-save-result") return null;
...
export class PhoneSecretBridge {
  ...
  receive(raw: unknown): boolean {
    ...
    if (record.type === "kind-meitner:phone-secret-key") {
  ...
  private async provideWithinLimit(context: PhoneSecretContext): Promise<void> {
    if (!this.identity) {
      throw new PhoneSecretError(
        "Secure phone entry is not ready on this computer. Reopen kind-meitner and try again.",
        503,
      );
    }
