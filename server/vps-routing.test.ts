...
// KIND_MEITNER_EXTRA_PATH: the server runs in its own process, so injection happens
...
posixOnly("VPS turn routing e2e (fake ACP fleet + fake docker over SSH)", () => {
  ...
  beforeAll(async () => {
    ...
    home = mkdtempSync(join(tmpdir(), "kind-meitner-vps-routing-"));
...
`, { mode: 0o755 });
    ...
    const env: NodeJS.ProcessEnv = {
      ...
      KIND_MEITNER_EXTRA_PATH: fakeBin,
