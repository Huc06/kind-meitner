...
describe("steer-queue e2e (fake ACP fleet)", () => {
  ...
  beforeAll(async () => {
    ...
    home = mkdtempSync(join(tmpdir(), "kind-meitner-steer-test-"));
    ...
    evidencePath = join(tmpdir(), `kind-meitner-steer-evidence-${Date.now()}-${process.pid}.json`);
  ...
  it("retains a replacement receipt through stale completion and settles its own completion before ACK", async () => {
    ...
    const receipt = () => {
      const database = new DatabaseSync(join(home, ".kind-meitner", "messages.db"), { readOnly: true });
