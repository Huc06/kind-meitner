...
function directory(): string {
  const path = mkdtempSync(join(tmpdir(), "kind-meitner-workspace-backup-"));
  scratch.push(path);
  return path;
}
...
function encryptedPayload(root: string, plaintext: Buffer): string {
  ...
  const header = Buffer.concat([Buffer.from("kind-meitner-WORKSPACE-1\n"), salt, iv]);
...
describe("encrypted full workspace backups", () => {
  it("round-trips WAL conversations, binary files, drafts and IDs; preserves destination identity and connections", async () => {
    ...
    try {
      ...
      const exported = await createWorkspaceBackup(source, {
        ...
        clientState: { "kind-meitner-drafts": '{"thread":"unsent"}', "kind-meitner-draft-attachments": JSON.stringify({ thread: [{ kind: "file", path: join(source, "attachments", "image.png") }] }) },
      });
      ...
      writeFileSync(join(target, "kind-meitner-server.lease"), "live-lease");
      ...
      expect(readFileSync(join(target, "kind-meitner-server.lease"), "utf8")).toBe("live-lease");
      ...
      expect(JSON.parse(receipt.clientState!["kind-meitner-draft-attachments"]).thread[0].path).toBe(join(target, "attachments", "image.png"));
  ...
  it("leaves owned authentication stores out of the archive and retains only the destination copies", async () => {
    ...
    const authPaths = [
      ...
      "tmp/kind-meitner-mcp-123/mcp.json", ".tmp/secret",
    ];
