...
} from "../scripts/control-kind-meitner.ts";
import { installedChrome, UI_MUTATING } from "../scripts/testing/control-kind-meitner-ui.ts";
import { removeTempDir } from "./testing/cleanup.ts";

describe("control-kind-meitner command mapping", () => {
  ...
  it("keeps every ui verb off discovery: the launch handle is required, whatever the environment says", async () => {
    const env = { KIND_MEITNER_URL: "http://127.0.0.1:19999", KIND_MEITNER_PORT: "19999" };
    ...
    for (const verb of verbs) {
      await expect(runControlKindMeitner(["ui", verb], { env })).rejects.toMatchObject({
    ...
    expect(await runControlKindMeitner(["ui", "help"])).toBe(HELP_UI);
    await expect(runControlKindMeitner(["ui", "launch"])).rejects.toThrow("ui launch is available only from the executable CLI");
    await expect(runControlKindMeitner(["ui", "bogus"])).rejects.toThrow('unknown ui command "bogus"');
    await expect(runControlKindMeitner(["ui", "snapshot", "--ui", "/nowhere/ui.json"])).rejects.toThrow("could not read the ui handle");
  });

  it("refuses a handle whose launch has already been stopped", async () => {
    const dir = mkdtempSync(join(tmpdir(), "kind-meitner-ui-handle-"));
    try {
      const handle = join(dir, "ui.json");
      writeFileSync(handle, JSON.stringify({
        url: "http://127.0.0.1:19999", previewUrl: "http://127.0.0.1:5199/__threads.html", session: "kind-meitner-ui-19999",
        binary: "/fixture/agent-browser", home: join(dir, "gone"), botId: "bot-1", logPath: "/fixture/server.log", chrome: null,
      }));
      await expect(runControlKindMeitner(["ui", "snapshot", "--ui", handle])).rejects.toMatchObject({
        message: expect.stringContaining("data directory is gone"),
        hint: expect.stringContaining("ui launch"),
      });
      writeFileSync(handle, JSON.stringify({ url: "http://127.0.0.1:19999" }));
      await expect(runControlKindMeitner(["ui", "snapshot", "--ui", handle])).rejects.toThrow("lacks previewUrl");
    } finally {
  ...
  it("finds the newest Chrome for Testing that agent-browser install unpacked", () => {
    const dir = mkdtempSync(join(tmpdir(), "kind-meitner-ui-tools-"));
    try {
  ...
  it("runs directly under Node's strip-only TypeScript loader", () => {
    const result = spawnSync(process.execPath, [
      ...
      join(process.cwd(), "scripts", "control-kind-meitner.ts"),
      "help",
    ], { encoding: "utf8" });
    expect(result.status).toBe(0);
    expect(result.stdout).toContain("control-kind-meitner");
  });
  ...
  it("composes doctor from the shared health and model tools", async () => {
    const callTool = vi.fn(async (name: string) => name === "get_system_health"
      ? { status: "connected", app: "kind-meitner" }
      : {
          instances: [
    ...
    const result = await runControlKindMeitner(["doctor", "--url", "http://127.0.0.1:19999"], {
  ...
  it("rejects an available engine when the endpoint is not kind-meitner", async () => {
    const callTool = vi.fn(async (name: string) => name === "get_system_health"
  ...
  it("refuses to mutate a silently discovered live app", async () => {
    await expect(runControlKindMeitner(["new-bot", "--name", "Probe"], {
      callTool: vi.fn() as any,
      env: {},
    })).rejects.toMatchObject({
      message: "mutating commands require an explicit kind-meitner instance",
    });
  });
  ...
  it("maps bounded reads and dry-run actions without reimplementing them", async () => {
    ...
    const env = { KIND_MEITNER_URL: "http://127.0.0.1:19999" };
    await expect(runControlKindMeitner(["messages", "--channel", "room-1", "--limit", "20"], {
      callTool: callTool as any,
      env,
    })).resolves.toEqual({ name: "get_channel_messages", args: { channel_id: "room-1", limit: 20 } });

    await expect(runControlKindMeitner(["send", "--bot", "bot-1", "--text", "hello", "--dry-run"], {
      callTool: callTool as any,
      env: {},
  ...
  it("maps the rewind that makes the harness rebuild instead of resume", async () => {
    ...
    const callTool = vi.fn(async (name: string, args: Record<string, unknown>) => ({ name, args }));
    const env = { KIND_MEITNER_URL: "http://127.0.0.1:19999" };
    await expect(runControlKindMeitner(
  ...
  it("treats edit as mutating, so it cannot reach a silently discovered app", async () => {
    ...
    await expect(runControlKindMeitner(["edit", "--bot", "b", "--message", "m", "--text", "x"], {
      callTool: vi.fn() as any,
      env: {},
    })).rejects.toMatchObject({
      message: "mutating commands require an explicit kind-meitner instance",
  ...
  it("requires every part of an edit before calling the shared tool", async () => {
    ...
    for (const args of [
      ...
      await expect(runControlKindMeitner(args, { callTool: callTool as any, env })).rejects.toThrow(/is required/);
    }
  ...
  it("rejects invalid bounds before the shared tool is called", async () => {
    const callTool = vi.fn();
    await expect(runControlKindMeitner(["wait", "--bot", "bot-1", "--timeout", "0"], {
      callTool: callTool as any,
      env: {},
  ...
  it("forwards pinned task IDs for sends, reads, waits, interrupts, and model changes", async () => {
    const callTool = vi.fn(async (name: string, args: Record<string, unknown>) => ({ name, args }));
    const dependencies = { callTool: callTool as any, env: { KIND_MEITNER_URL: "http://127.0.0.1:19999" } };
    for (const [command, tool, extra] of [
      ...
      await expect(runControlKindMeitner([command, "--bot", "bot-1", "--task", "task-a", ...extra], dependencies))
    ...
    await expect(runControlKindMeitner(["set-model", "--bot", "bot-1", "--instance", "claude", "--model", "model-b"], {
      callTool: callTool as any, env: {},
    })).rejects.toThrow("explicit kind-meitner instance");
  });
});

describe("control-kind-meitner isolated verification loop", () => {
  it.each([
    "tcp://127.0.0.1:2375",
  ...
  it("launches, drives a real fake-engine turn, and removes only its test data", async () => {
    const parentEnv = {
      ...
      KIND_MEITNER_SKILLS_DIR: "/must/not/reach/the/fixture",
    ...
    const env = { KIND_MEITNER_URL: session.info.url };
    try {
      const doctor = await runControlKindMeitner(["doctor"], { env }) as any;
      expect(doctor.ok).toBe(true);
      expect(doctor.availableEngines).toEqual(["claude"]);

      const created = await runControlKindMeitner(["new-bot", "--name", "Verification Probe"], { env }) as any;
      const botId = created.bot.id as string;
      await runControlKindMeitner(["send", "--bot", botId, "--text", "hello from the verification test"], { env });
      const settled = await runControlKindMeitner(["wait", "--bot", botId, "--timeout", "20"], { env }) as any;
      expect(settled.status).toBe("settled");
      const transcript = await runControlKindMeitner(["messages", "--bot", botId, "--limit", "10"], { env }) as any;
      ...
      expect(fixtureEnv).not.toHaveProperty("KIND_MEITNER_SKILLS_DIR");
  ...
  it("scripts the fake engine's tool calls from the launcher's environment", async () => {
    const session = await launchVerificationServer({
      ...process.env,
      FAKE_CLAUDE_TOOL_CALLS: '[{"name":"Bash","input":{"command":"pnpm control:kind-meitner doctor","password":"fixture-secret"},"output":{"text":"fixture healthy","api_key":"fixture-output-secret"},"ok":true},{"name":"Bash","input":{"command":"false"},"output":"fixture command failed","ok":false}]',
    });
    const env = { KIND_MEITNER_URL: session.info.url };
    try {
      const created = await runControlKindMeitner(["new-bot", "--name", "Tool Script Probe"], { env }) as any;
      const botId = created.bot.id as string;
      await runControlKindMeitner(["send", "--bot", botId, "--text", "run both commands"], { env });
      const settled = await runControlKindMeitner(["wait", "--bot", botId, "--timeout", "20"], { env }) as any;
      expect(settled.status).toBe("settled");
      const transcript = await runControlKindMeitner(["messages", "--bot", botId, "--limit", "10"], { env }) as any;
      ...
      expect(recorded[0].tool?.input).toContain("pnpm control:kind-meitner doctor");
