...
it("survives a real server crash: queued sends keep receipts, cancellation and uncertain dispatch never replay", async () => {
  ...
  const restart = async () => {
    ...
    Object.assign(env, {
      ...
      KIND_MEITNER_PORT: new URL(url).port, KIND_MEITNER_WEBHOOK_PORT: String(Number(new URL(url).port) + 1), PATH: dirname(process.execPath),
    });
  ...
  try {
    ...
    const settled = await runControlKindMeitner(["wait", "--bot", bot.id, "--task", bot.threadId, "--url", url]);
    ...
    const channelSettled = await runControlKindMeitner(["wait", "--channel", channel.id, "--url", url]);
