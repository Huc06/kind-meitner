...
it("Clive reviews multi-provider teams once, continues after each decision, and preserves existing threads through setup and deletion", async () => {
  ...
  const control = async (...args: string[]) => {
    const result = await runControlKindMeitner([...args, "--url", fixture.info.url]) as any;
  ...
  try {
    ...
    const start = async (text: string) => {
      ...
      return dump.mcpConfig.mcpServers.agents.env.KIND_MEITNER_COMMS_TOKEN as string;
    };
    const finish = async () => {
    ...
    ] } }, 201, groupDump.mcpConfig.mcpServers.agents.env.KIND_MEITNER_COMMS_TOKEN);
