...
// own `kind-meitner@<slug>` service on its own loopback ports, its own data
...
export const FLEET_USER_PREFIX = "kind-meitner-";
...
export function fleetLayout(root = "/"): FleetLayout {
  ...
  return {
    etcDir: at("etc", "kind-meitner"),
    registryFile: at("etc", "kind-meitner", "fleet.json"),
    instancesDir: at("etc", "kind-meitner", "instances"),
    unitFile: at("etc", "systemd", "system", "kind-meitner@.service"),
    fenceFile: at("etc", "kind-meitner", "fence.nft"),
    fenceUnitFile: at("etc", "systemd", "system", "kind-meitner-fence.service"),
    caddyDir: at("etc", "caddy", "kind-meitner.d"),
    caddyfile: at("etc", "caddy", "Caddyfile"),
    homesDir: at("var", "lib", "kind-meitner"),
    agentUnitFile: at("etc", "systemd", "system", "kind-meitner-fleet.service"),
    socketPath: at("run", "kind-meitner", "fleet.sock"),
    auditFile: at("var", "log", "kind-meitner", "fleet.jsonl"),
  };
}
...
export function agentUnit(spec: { node: string; script: string; operator: string; layout?: FleetLayout }): string {
  ...
  return [
    "# Written by `kind-meitner fleet init`. The operator workspace creates and manages workspaces through this.",
    "[Unit]",
    "Description=kind-meitner fleet agent",
...
export function workspaceDataDir(layout: FleetLayout, slug: string): string {
  return join(workspaceHome(layout, slug), ".kind-meitner");
}
...
export function templateUnit(spec: { node: string; script: string; layout?: FleetLayout }): string {
  ...
  const strip = spec.script.endsWith(".ts") ? " --experimental-strip-types" : "";
  return [
    "# Written by `kind-meitner fleet init`. One unit for every workspace: %i is the slug.",
    "[Unit]",
    "Description=kind-meitner workspace %i",
    "After=network-online.target kind-meitner-fence.service",
    "Wants=network-online.target",
    "Requires=kind-meitner-fence.service",
    "",
    "[Service]",
    "Type=simple",
    ...
    `ExecStart=${spec.node}${strip} ${spec.script} serve --port \${KIND_MEITNER_PORT} --data-dir \${KIND_MEITNER_DATA_DIR} --public-url \${KIND_MEITNER_PUBLIC_URL} --label %i --no-pair`,
    "Restart=always",
    "RestartSec=3",
    "KillMode=mixed",
    "TimeoutStopSec=30",
...
export function fenceUnit(layout = fleetLayout()): string {
  return [
    "# Written by `kind-meitner fleet init`: keeps each workspace's loopback ports to its own user.",
    "[Unit]",
    "Description=kind-meitner per-workspace loopback fence",
    "",
    "[Service]",
    "Type=oneshot",
    "RemainAfterExit=yes",
...
export function fenceRules(workspaces: FleetWorkspace[]): string {
  const lines = ["#!/usr/sbin/nft -f", "# Written by `kind-meitner fleet`; regenerated on every create, suspend, resume and delete.", "add table inet kind-meitner", "flush table inet kind-meitner", "table inet kind-meitner {", "\tchain output {", "\t\ttype filter hook output priority 0; policy accept;"];
...
export function instanceEnv(input: { workspace: FleetWorkspace; dataDir: string; licenseKey?: string; portalUrl?: string }): string {
  const lines = [
    `KIND_MEITNER_DATA_DIR=${input.dataDir}`,
    `KIND_MEITNER_PORT=${input.workspace.port}`,
    `KIND_MEITNER_WEBHOOK_PORT=${input.workspace.webhookPort}`,
    `KIND_MEITNER_PUBLIC_URL=https://${input.workspace.host}`,
  ];
  if (input.licenseKey) lines.push(`KIND_MEITNER_LICENSE_KEY=${input.licenseKey}`);
  if (input.portalUrl) lines.push(`KIND_MEITNER_ADMIN_URL=${input.portalUrl}`, `KIND_MEITNER_ADMIN_WORKSPACE=${input.workspace.slug}`);
  return lines.join("\n") + "\n";
}
...
export const MANAGED_OPENROUTER = "kind-meitner-managed-openrouter";
...
export function initPlan(input: { domain: string; node: string; script: string; operator?: string; layout?: FleetLayout }): { steps: FleetStep[]; registry: FleetRegistry } {
  ...
  const steps: FleetStep[] = [
    ...
    { kind: "run", argv: ["systemctl", "enable", "--now", "kind-meitner-fence.service"], why: "apply the loopback fence now and at boot" },
    ...(input.operator ? [{ kind: "run" as const, argv: ["systemctl", "enable", "--now", "kind-meitner-fleet.service"], why: `start the fleet agent for ${input.operator}` }] : []),
...
export function createPlan(input: {
  ...
  const steps: FleetStep[] = [
    ...
    ...(input.memoryMax
      ? [
          { kind: "mkdir" as const, path: join(layout.unitFile.replace(/kind-meitner@\.service$/, ""), `kind-meitner@${workspace.slug}.service.d`), mode: 0o755 },
          { kind: "write" as const, path: join(layout.unitFile.replace(/kind-meitner@\.service$/, ""), `kind-meitner@${workspace.slug}.service.d`, "limits.conf"), content: `[Service]\nMemoryMax=${input.memoryMax}\n`, mode: 0o644 },
        ]
      : []),
    ...
    { kind: "run", argv: ["systemctl", "enable", "--now", `kind-meitner@${workspace.slug}.service`], why: "start the workspace now and at boot" },
...
export function suspendPlan(input: { registry: FleetRegistry; slug: string; layout?: FleetLayout }): { steps: FleetStep[]; registry: FleetRegistry } {
  ...
  return {
    registry,
    steps: [
      { kind: "run", argv: ["systemctl", "disable", "--now", `kind-meitner@${workspace.slug}.service`], why: "stop the workspace and keep it stopped at boot" },
      { kind: "write", path: join(layout.caddyDir, `${workspace.slug}.caddy`), content: caddySite(workspace), mode: 0o644 },
...
export function resumePlan(input: { registry: FleetRegistry; slug: string; layout?: FleetLayout }): { steps: FleetStep[]; registry: FleetRegistry } {
  ...
  return {
    registry,
    steps: [
      { kind: "run", argv: ["systemctl", "enable", "--now", `kind-meitner@${workspace.slug}.service`], why: "start the workspace again" },
      { kind: "health", url: `http://127.0.0.1:${workspace.port}/api/health`, why: "wait for the workspace to answer" },
      { kind: "write", path: join(layout.caddyDir, `${workspace.slug}.caddy`), content: caddySite(workspace), mode: 0o644 },
...
export function deletePlan(input: { registry: FleetRegistry; slug: string; keepData: boolean; layout?: FleetLayout }): { steps: FleetStep[]; registry: FleetRegistry } {
  ...
  const unitsDir = layout.unitFile.replace(/kind-meitner@\.service$/, "");
  return {
    registry,
    steps: [
      { kind: "run", argv: ["systemctl", "disable", "--now", `kind-meitner@${workspace.slug}.service`], why: "stop the workspace" },
      ...
      { kind: "remove", path: join(unitsDir, `kind-meitner@${workspace.slug}.service.d`, "limits.conf") },
      { kind: "write", path: layout.fenceFile, content: fenceRules(Object.values(registry.workspaces)), mode: 0o600 },
...
export function upgradePlan(input: { registry: FleetRegistry }): FleetStep[] {
  ...
  return [
    { kind: "run", argv: ["npm", "install", "-g", "kind-meitner@latest"], why: "install the release every workspace runs from" },
    ...running.map((workspace) => ({ kind: "run" as const, argv: ["systemctl", "restart", `kind-meitner@${workspace.slug}.service`], why: `restart ${workspace.slug} on the new release` })),
 ...
 * `kind-meitner access` writes; the server reads it live. */
  ...
  for (const step of steps) {
    switch (step.kind) {
      ...
      case "write":
        ...
        lines.push(`cat > ${shellQuote(step.path)} <<'KIND_MEITNER_EOF'`, step.content.replace(/\n$/, ""), "KIND_MEITNER_EOF", `chmod ${step.mode.toString(8)} ${shellQuote(step.path)}`);
        break;
      case "append-once":
