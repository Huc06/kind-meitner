...
const browserCleanupResultSchema = z.object({
  type: z.literal("kind-meitner:browser-lifecycle-result"),
  requestId: z.string().regex(REQUEST_ID),
  ok: z.boolean(),
}).strict();
...
export type BrowserCleanupWireRequest = {
  type: "kind-meitner:browser-bot-deleted" | "kind-meitner:browser-profile-deleted";
  requestId: string;
  botId?: string;
  partitionId?: string;
};
...
export function requireBrowserCleanupAcknowledged(ok: boolean, target: string): void {
  ...
  const error = Object.assign(new Error(
    `${target} was removed, but kind-meitner could not confirm its local browser data was erased. `
...
export class BrowserCleanupCoordinator {
  ...
  receive(message: BrowserCleanupIncomingMessage | undefined): boolean {
    if (message?.type !== "kind-meitner:browser-lifecycle-result") return false;
  ...
  async #attempt(request: BrowserCleanupRequest): Promise<boolean> {
    ...
    const sent = this.#send({
      type: current.kind === "bot"
        ? "kind-meitner:browser-bot-deleted"
        : "kind-meitner:browser-profile-deleted",
