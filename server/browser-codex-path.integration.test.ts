...
describe.skip("Codex browser turns with a minimal GUI PATH (Codex driver retired; no CLI in this job)", () => {
  ...
  beforeAll(async () => {
    home = mkdtempSync(join(tmpdir(), "kind-meitner-browser-codex-path-"));
    ...
    child = spawn(process.execPath, [join(SERVER_DIR, "index.ts")], {
      cwd: join(SERVER_DIR, ".."),
      env: {
        PATH: FINDER_PATH, HOME: home, KIND_MEITNER_DATA_DIR: data, KIND_MEITNER_PORT: String(port),
        KIND_MEITNER_AGENT_BROWSER_PATH: browser, VITEST: "1",
      },
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      child.stdout!.on("data", (chunk) => {
        output += chunk;
        if (output.includes(`kind-meitner server on ${base}`)) { clearTimeout(timer); resolve(); }
      });
    });
  ...
  it.each(["default", "absolute"])("keeps the browser mounted and completes a turn with the %s Codex CLI", async (instanceId) => {
    ...
    expect(dump.env.KIND_MEITNER_BROWSER_TOKEN).toBeTruthy();
