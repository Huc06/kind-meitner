...
describe("computer inventory request wiring", () => {
  ...
  it("builds the exact confirmed Local VM, Box, and VPS lifecycle requests", () => {
    ...
    const vpsName = "kind-meitner-vps-current-123456abcdef";
...
describe("Local VM inventory UI", () => {
  ...
  it("shows unmanaged containers without offering an unsafe delete action", () => {
    ...
    expect(markup).toContain("not managed by kind-meitner");
...
describe("cloud computer inventory UI", () => {
  ...
  it("keeps disconnected, unavailable, and empty states distinct", () => {
    ...
    expect(disconnected).not.toContain("No KindMeitner-managed cloud computers found");
    ...
    expect(unavailable).not.toContain("No KindMeitner-managed cloud computers found");
    ...
    expect(empty).toContain("No KindMeitner-managed cloud computers found");
  });
...
describe("VPS computer inventory UI", () => {
  const ownedVps: VpsComputerInventoryInstance = {
    name: "kind-meitner-vps-current-123456abcdef",
    state: "running",
    ownerBotId: "current-owner",
    ownerName: "Research",
    orphaned: false,
  ...
  it("shows the configured host, owners, orphans, and status without raw container details", () => {
    const orphanName = "kind-meitner-vps-deleted-abcdef123456";
    const markup = renderCard({
      instances: [
        ownedVps,
        {
  ...
  it("derives a stable identifier without exposing the bot-derived container name", () => {
    expect(vpsComputerShortId("kind-meitner-vps-deleted-abcdef123456")).toBe("ef123456");
  ...
  it("keeps disconnected, unavailable, and empty states distinct", () => {
    ...
    expect(renderCard()).toContain("No KindMeitner-managed VPS computers found");
  });
