...
vi.mock("../lib/brand", () => ({ brand: () => ({ name: "kind-meitner" }) }));
...
describe("UpdateBanner", () => {
  ...
  it.each(["ETIMEDOUT while staging", "native error ".repeat(30)])("preserves restart recovery for a nonretryable error: %s", (message) => {
    ...
    expect(html).toContain("Quit and reopen kind-meitner before trying the update again.");
    expect(html).not.toContain("Try again");
  ...
  it("shows the failure cause as well as the required restart without offering a retry", () => {
    const html = render({ status: "error", retryable: false,
      message: "Not enough disk space to prepare the update. Free some space, then try again. Quit and reopen kind-meitner before trying the update again.",
    });
    expect(html).toContain("Free some space");
    expect(html).toContain("Quit and reopen kind-meitner");
