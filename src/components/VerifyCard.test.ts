...
const steps: RunStep[] = [
  { id: "s1", label: "doctor", command: "pnpm control:kind-meitner doctor --url http://127.0.0.1:8799", status: "passed", dryRun: false, verified: true },
  { id: "s2", label: "send", command: "node --experimental-strip-types scripts/control-kind-meitner.ts send --bot x --text y", status: "failed", dryRun: false, verified: true },
...
describe("VerifyCard", () => {
  ...
  it("lists each step with its label, full command and status, tagging the verified ones, behind labelled controls", () => {
    ...
    expect(markup).toContain('title="pnpm control:kind-meitner doctor --url http://127.0.0.1:8799"');
