...
describe("Run log", () => {
  ...
  it("renders commands, failure and pending states without fabricating output", () => {
    const html = render(timelineEvents([
      { id: "a", at: 1, role: "bot", kind: "activity", tool: { name: "Bash", summary: "pnpm control:kind-meitner doctor", ok: true } },
      { id: "b", at: 2, role: "bot", kind: "activity", tool: { name: "Bash", summary: "pnpm control:kind-meitner ui click --name Missing", ok: false } },
      { id: "c", at: 3, role: "bot", kind: "activity", tool: { name: "Read" } },
    ]));
    expect(html).toContain("pnpm control:kind-meitner doctor");
