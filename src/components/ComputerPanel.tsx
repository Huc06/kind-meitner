...
const PANEL_WIDTH_KEY = "kind-meitner-computer-panel-width";
...
export function ComputerPanel({
  ...
  const [pending, setPending] = useState<"join" | null>(null);
  ...
  useEffect(() => {
    ...
    if (!bot.computer && capabilitiesReady && localSelectable) {
      setPhase("local");
      return;
    }
    setPhase("off");
  ...
  const frameSrc =
    phase === "local" && !isLinux
      ...
      : cloudPreviewReady || (bot.computer === "cloud" && phase === "starting")
        ...
        : null;
  const previewOpensDesktop = Boolean(frameSrc && cloudPreviewReady);
  ...
  } satisfies Record<Exclude<Phase, "ready" | "local">, string>;
