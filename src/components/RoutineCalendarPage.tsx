...
const BOT_DRAG_TYPE = "application/x-kind-meitner-bot";
const EVENT_DRAG_TYPE = "application/x-kind-meitner-calendar-event";
...
function EventEditor({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-label={existingRoutine || existingCall ? "Edit calendar event" : "Create calendar event"} tabIndex={-1} className="max-h-[94vh] w-full max-w-[760px] overflow-y-auto rounded-2xl border border-hairline/70 bg-panel/95 shadow-2xl backdrop-blur-md p-1.5">
        <div className="rounded-xl border border-hairline/50 bg-card overflow-hidden">
          <div className="sticky top-0 z-20 flex items-center justify-between border-b border-hairline/40 bg-card/95 px-6 py-4 backdrop-blur">
            <div>
              <h2 className="text-[15px] font-semibold tracking-tight text-ink">{existingRoutine || existingCall ? "Edit Schedule" : "New Schedule"}</h2>
              <p className="mt-0.5 text-[11.5px] text-ink-secondary">Configure repeating schedule rules, timeout limits, and execution target.</p>
            </div>
            <button onClick={onClose} className="rounded-lg p-1.5 text-ink-secondary hover:bg-raised hover:text-ink transition-colors" aria-label="Close"><X size={16} /></button>
          </div>
        ...
        <div className="sticky bottom-0 flex items-center justify-end gap-2 border-t border-hairline/40 bg-card/95 px-6 py-3.5 backdrop-blur">
          <button type="button" onClick={onClose} className="rounded-lg px-4 py-2 text-[12.5px] font-medium text-ink-secondary hover:bg-raised hover:text-ink transition-colors">Cancel</button>
          <button type="button" onClick={save} disabled={saving || attachmentPending || !valid} className="flex items-center gap-2 rounded-lg bg-accent px-5 py-2 text-[12.5px] font-semibold text-white shadow-xs hover:brightness-110 active:scale-[0.98] disabled:opacity-40 transition">{(saving || attachmentPending) && <Loader2 size={14} className="animate-spin" />}{attachmentPending ? "Attaching…" : existingRoutine || existingCall ? "Save" : kind === "call" ? "Schedule call" : isRoomGoal ? "Schedule team goal" : "Schedule routine"}</button>
        </div>
      </div>
    </div>
  </div>
...
function QuickComposer({
  ...
  const [durationMinutes, setDurationMinutes] = useState(kind === "routine" ? (seed.routine?.durationMinutes ?? 30) : seed.durationMinutes);
  const selectedBot = bots.find((b) => b.id === botIds[0]);
  ...
  const DURATION_CHIPS = [15, 30, 45, 60, 120];
  ...
  return (
    <div
      ...
      className={cn(
        "fixed z-50 max-h-[calc(100vh-24px)] w-[min(480px,calc(100vw-24px))] overflow-y-auto rounded-2xl border border-hairline/70 bg-panel/95 p-1.5 shadow-2xl backdrop-blur-md",
        !dialogPosition && "left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2",
      )}
    >
      <div className="rounded-xl border border-hairline/50 bg-card p-4 space-y-4">
        {/* Header */}
        <div className="flex items-start justify-between border-b border-hairline/40 pb-3">
          <div>
            <h3 className="text-[14px] font-semibold tracking-tight text-ink">
              {kind === "routine" ? "New Scheduled Task" : "New Scheduled Call"}
            </h3>
        ...
        <div className="space-y-3.5">
          ...
          {/* Assigned Agent (Attendee Chip style from scheduling-3) */}
          <div>
            <label className="text-[10.5px] font-semibold uppercase tracking-[0.06em] text-ink-secondary">
          ...
          {/* Schedule Time & Duration Presets (from scheduling-2 & scheduling-3) */}
          <div>
          ...
          <div>
            <label className="text-[10.5px] font-semibold uppercase tracking-[0.06em] text-ink-secondary">
              {kind === "routine" ? "Prompt & Deliverable" : "Description (optional)"}
            </label>
            <textarea
              value={description}
              ...
              placeholder={
                kind === "routine"
                  ? "What should the bot do? e.g. Analyze liquidity benchmarks and publish a markdown report."
                  : "Add notes or agenda for this call."
              }
          ...
          {error && (
            <div className="rounded-lg bg-danger/10 border border-danger/20 px-3 py-2 text-[11.5px] text-danger">
              {error}
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-between border-t border-hairline/40 pt-3">
          <button
            type="button"
            onClick={() =>
              onMore({ ...seed, at: scheduledAt, kind, botIds, name, description, durationMinutes, resultsThreadId })
            }
            title="Choose repeating schedules, timeout, and attachments"
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={onClose}
              className="rounded-lg px-3 py-1.5 text-[12px] font-medium text-ink-secondary hover:bg-raised transition-colors"
            >
              Cancel
            </button>
            <button
...
function CalendarEventCard({
  ...
  return (
    <button
      ...
      className={cn(
        "group absolute z-10 overflow-hidden rounded-lg border text-left shadow-xs transition-all hover:z-20 hover:shadow-md hover:scale-[1.01] focus:z-20 focus:outline-none focus:ring-2 focus:ring-accent",
        previewDuration < 30 ? "px-1.5 py-0.5" : "px-2.5 py-2",
        status === "cancelled" && "opacity-50",
        (status === "failed" || status === "missed") && "border-danger/60 bg-danger/10",
      )}
      style={{
        ...
        height: `${Math.max(previewDuration >= 30 ? 46 : 18, (previewDuration / 60) * HOUR_HEIGHT)}px`,
        background: `color-mix(in srgb, ${color} 15%, var(--color-card, #1c1c1e))`,
        borderColor: `color-mix(in srgb, ${color} 38%, var(--color-hairline, rgba(255,255,255,0.12)))`,
    ...
    >
      <div className="flex min-w-0 items-start gap-2">
        {previewDuration >= 30 && (
          isCall ? (
            <div className="mt-0.5 flex size-5 shrink-0 items-center justify-center rounded-full bg-accent/20 text-accent">
              <Video size={11} />
            </div>
          ) : primary ? (
            <div className="shrink-0 mt-0.5">
              <BotAvatar bot={primary} state={status ? statusState(status) : "idle"} size={compact ? 20 : 22} animated={status === "running" || status === "waiting"} />
            </div>
          ) : null
        )}
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            <span className={cn("truncate text-[11.5px] text-ink", previewDuration < 30 ? "font-medium leading-none" : "font-semibold leading-tight")}>
              {name}
            </span>
            {status && status !== "completed" && (
              <span className={cn(
                "inline-flex shrink-0 items-center rounded px-1 py-0.2 text-[8px] font-semibold uppercase tracking-wider",
                status === "failed" ? "bg-danger/20 text-danger" : status === "running" ? "bg-accent/20 text-accent animate-pulse" : "bg-warning/20 text-warning"
              )}>
                {status}
              </span>
            )}
          ...
          {previewDuration >= 30 && (
            <div className="mt-0.5 flex items-center gap-1.5 truncate text-[10px] text-ink-secondary">
              <span className="font-mono font-medium">{niceTime(item.at)}</span>
              {statusLabel && (
                <>
              ...
              {isCall && (
                <>
                  <span className="opacity-40">·</span>
                  <span>{ownerBots.length} {ownerBots.length === 1 ? "bot" : "bots"}</span>
                </>
              )}
              {isRoomGoal && room?.name && (
                <>
                  <span className="opacity-40">·</span>
        ...
        {previewDuration >= 30 && ownerBots.length > 1 && (
          <span className="rounded bg-black/20 px-1 py-0.5 text-[8px] font-medium text-ink-secondary shrink-0">
            +{ownerBots.length - 1}
          </span>
        )}
      </div>
      {isCall && (
        <div onPointerDown={beginResize} className="absolute inset-x-1 bottom-0 h-1.5 cursor-ns-resize rounded-full opacity-0 transition group-hover:opacity-100" aria-label="Resize event">
          <div className="mx-auto mt-0.5 h-0.5 w-5 rounded-full bg-white/55" />
        </div>
      )}
    </button>
  );
...
function CalendarGrid({
  ...
  return (
    <div ref={scrollRef} className="min-h-0 flex-1 overflow-auto border-l border-t border-hairline/40 bg-app">
      <div className="sticky top-0 z-30 grid border-b border-hairline/40 bg-card/90 backdrop-blur-md" style={{ gridTemplateColumns, minWidth }}>
        <div className="flex flex-col items-center justify-center border-r border-hairline/40 px-2 py-2 text-center">
          <span className="text-[9px] font-mono font-medium uppercase tracking-wider text-ink-secondary">
            {formatGmtOffset(-new Date(anchor).getTimezoneOffset())}
          </span>
        </div>
        {starts.map((start) => {
          ...
          return (
            <div
              ...
              role="columnheader"
              className={cn(
                "flex flex-col items-center justify-center border-r border-hairline/40 py-2.5 transition-colors last:border-r-0",
                isToday && "bg-accent/[0.04]",
              )}
            >
              <span className={cn("text-[11px] font-medium tracking-tight", isToday ? "text-accent font-semibold" : "text-ink-secondary")}>
                {DAY_NAMES[date.getDay()]}
              </span>
              <span
                className={cn(
                  "mt-0.5 inline-flex size-7 items-center justify-center rounded-full text-[13px] font-semibold tabular-nums transition-transform",
                  isToday ? "bg-accent text-white shadow-xs" : "text-ink hover:bg-raised",
                )}
              >
                {date.getDate()}
              </span>
      ...
      <div role="grid" aria-label="Routine and call calendar" onDragEnd={() => setDragPreview(null)} className="relative grid" style={{ height: HOUR_HEIGHT * 24, gridTemplateColumns, minWidth }}>
        <div className="relative border-r border-hairline/40 bg-card/10">
          {Array.from({ length: 24 }, (_, hour) => (
            <div
              key={hour}
              className="absolute right-2.5 -translate-y-1/2 text-[10px] font-mono tabular-nums text-ink-secondary/65"
              style={{ top: hour * HOUR_HEIGHT }}
            >
              {hour === 0 ? "" : new Date(2000, 0, 1, hour).toLocaleTimeString([], { hour: "numeric" }).toLowerCase()}
            </div>
          ))}
        </div>
        {starts.map((start) => {
          ...
          return (
            <div
              key={start}
              role="gridcell"
              aria-label={`${niceDate(start)} calendar`}
              onPointerDown={(event) => beginSelection(event, start)}
              onDragOver={(event) => previewDrop(event, start)}
              onDragLeave={(event) => {
                if (!(event.relatedTarget instanceof Node) || !event.currentTarget.contains(event.relatedTarget)) setDragPreview(null);
              }}
              onDrop={(event) => drop(event, start)}
              className={cn("relative border-r border-hairline/35 last:border-r-0", start === today && "bg-accent/[0.02]")}
            >
              {Array.from({ length: 48 }, (_, half) => (
                <div
                  key={half}
                  className={cn("pointer-events-none absolute inset-x-0 border-t", half % 2 === 0 ? "border-hairline/30" : "border-hairline/10")}
                  style={{ top: (half / 2) * HOUR_HEIGHT }}
                />
              ))}
              {start === today && (
                <div className="pointer-events-none absolute inset-x-0 z-20 flex items-center" style={{ top: nowTop }}>
                  <span className="-ml-1 size-2 rounded-full bg-danger shadow-xs" />
                  <span className="h-px flex-1 bg-danger/80" />
                </div>
              )}
              {selected && (
                <div
                  className="pointer-events-none absolute inset-x-1 z-10 rounded-lg border border-accent/70 bg-accent/15 backdrop-blur-xs"
                  style={{
                    top: ((new Date(selected.start).getHours() * 60 + new Date(selected.start).getMinutes()) / 60) * HOUR_HEIGHT,
                    height: Math.max(20, ((selected.end - selected.start) / 3_600_000) * HOUR_HEIGHT),
                  }}
                />
              )}
              {preview && (
                <div
                  className="pointer-events-none absolute inset-x-1 z-20 rounded-lg border border-accent/80 bg-accent/20 shadow-xs"
                  style={{
                    top: ((new Date(preview.at).getHours() * 60 + new Date(preview.at).getMinutes()) / 60) * HOUR_HEIGHT,
                    height: HOUR_HEIGHT / 2,
                  }}
                >
                  <div className="px-2 py-1 text-[10px] font-mono font-medium text-accent">{niceTime(preview.at)}</div>
                </div>
              )}
              {dayItems.map((item) => (
                <CalendarEventCard
                  ...
                  groups={groups}
                  compact={days === 7}
                  layout={collisionLayouts.get(item.id) ?? { column: 0, columns: 1 }}
                  onOpen={() => onOpen(item)}
                  onResize={(minutes) => onResize(item, minutes)}
                />
              ))}
            </div>
          );
  ...
  return (
    ...
    >
      <div
        ...
        className="w-full max-w-[540px] overflow-hidden rounded-2xl border border-hairline/70 bg-panel/95 p-1.5 shadow-2xl backdrop-blur-md"
      >
        <div className="rounded-xl border border-hairline/50 bg-card">
          <div className="flex items-start gap-3.5 border-b border-hairline/40 px-5 py-4">
            <span
              className={cn("mt-1.5 size-3.5 shrink-0 rounded-full", isCall ? "bg-[#6d7cff]" : "bg-accent")}
            />
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-1 flex flex-wrap items-center gap-2 text-[12px] text-ink-secondary">
                <span className="font-medium text-ink">{niceDate(item.at)}</span>
                <span className="opacity-40">·</span>
                <span className="font-mono">{niceTime(item.at)}{isCall ? ` – ${niceTime(item.at + item.durationMinutes * 60_000)}` : ""}</span>
                {(routine || call) && (
                  <>
                    <span className="opacity-40">·</span>
                    <span className="rounded bg-inset px-1.5 py-0.5 text-[11px] font-medium text-ink-secondary">
                      {scheduleLabel((routine ?? call)!.schedule)}
                    </span>
                  </>
                )}
              </div>
              {routine?.schedule.type === "cron" && (
                <div className="mt-2.5">
                  <CronSchedulePreview schedule={routine.schedule} paused={!routine.enabled} />
                </div>
              )}
            </div>
            <button
          ...
          <div className="max-h-[55vh] space-y-4 overflow-y-auto p-5">
            <div>
              <div className="text-[10.5px] font-semibold uppercase tracking-[0.06em] text-ink-secondary mb-1.5">
                {isCall ? "Attendees" : isRoomGoal ? "Lead Coordinator" : "Assigned Agent"}
              </div>
              <div className="flex flex-wrap gap-2">
                {invited.map((bot) => (
                  <div
                    key={bot.id}
                    className="flex items-center gap-2 rounded-lg border border-hairline/60 bg-inset py-1 pl-1.5 pr-2.5"
                  >
                    <BotAvatar bot={bot} state="idle" size={22} animated={false} />
                    <span className="text-[12px] font-medium text-ink">{bot.name}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {isRoomGoal && (
              <div className="rounded-lg border border-hairline/40 bg-inset/50 p-2.5">
                <div className="text-[10.5px] font-semibold uppercase tracking-[0.06em] text-ink-secondary">
                  Team Goal Group
                </div>
                <div className="mt-0.5 text-[12.5px] font-medium text-ink">{goalGroup?.name ?? "Group unavailable"}</div>
              </div>
            )}

            {description && (
              <div>
                <div className="text-[10.5px] font-semibold uppercase tracking-[0.06em] text-ink-secondary mb-1">
                  {isCall ? "Agenda & Notes" : "Prompt & Task Instructions"}
                </div>
                <div className="rounded-lg border border-hairline/50 bg-inset/60 p-3 text-[12.5px] leading-relaxed text-ink whitespace-pre-wrap">
                  {description}
                </div>
              </div>
            )}
            ...
            {!isCall && (
              <div className="flex items-center gap-2 text-[12px] text-ink-secondary">
                ...
                <span>Run limit: {safetyLimit == null ? "No time limit" : `Stops if still running after ${durationLabel(safetyLimit)}`}</span>
              </div>
            )}

            {run && (
              <div className="rounded-xl border border-hairline/60 bg-inset/70 p-3.5 space-y-2">
                <div className="flex items-center justify-between">
                ...
                {run.output && (
                  <div className="max-h-48 overflow-y-auto rounded-lg border border-hairline/40 bg-card p-2.5 font-mono text-[11px] leading-relaxed text-ink-secondary whitespace-pre-wrap">
                    {run.output}
                  </div>
                )}
                {run.error && (
                  <div className="rounded-lg bg-danger/10 p-2.5 text-[11.5px] text-danger">
                    {run.error}
                  </div>
                )}
              </div>
            ...
            {run?.attention && (
              <div className="flex items-start gap-2 rounded-xl border border-warning/30 bg-warning/10 px-3 py-2.5 text-warning">
                ...
                <div className="min-w-0 whitespace-pre-wrap text-[11.5px] leading-relaxed">{run.attention}</div>
              </div>
            )}

            {error && (
              <div className="rounded-lg bg-danger/10 border border-danger/20 p-2.5 text-[11.5px] text-danger">
                {error}
              </div>
            )}
          </div>

          <div className="flex flex-wrap items-center justify-between gap-2 border-t border-hairline/40 px-5 py-3.5 bg-inset/20">
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={deleteEvent}
                className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[12px] font-medium text-danger hover:bg-danger/10 transition-colors"
              >
                <Trash2 size={13} />
                Delete
              </button>
              {(routine || call) && (
                <button
                  type="button"
                  onClick={onEdit}
                  className="rounded-lg px-2.5 py-1.5 text-[12px] font-medium text-ink-secondary hover:bg-raised hover:text-ink transition-colors"
                >
                  Edit
                </button>
              )}
            ...
            <div className="flex items-center gap-2">
              {roomId && (
                <button onClick={() => onOpenRoom(roomId)} className="flex items-center gap-1.5 rounded-lg border border-hairline/60 bg-raised px-3 py-1.5 text-[12px] font-semibold text-ink hover:bg-raised/80">
                  <ExternalLink size={13} />Join group
                </button>
              )}
              {call && call.botIds.length > 1 && (
                <button onClick={joinRoom} disabled={working} className="flex items-center gap-1.5 rounded-lg border border-hairline/60 bg-raised px-3 py-1.5 text-[12px] font-semibold text-ink hover:bg-raised/80 disabled:opacity-50">
                  <ExternalLink size={13} />Join group
                </button>
              )}
              {isRoomGoal && goalGroup && !executionThreadId && (
                <button onClick={() => { onOpenRoom(goalGroup.id); onClose(); }} className="flex items-center gap-1.5 rounded-lg border border-hairline/60 bg-raised px-3 py-1.5 text-[12px] font-semibold text-ink hover:bg-raised/80">
              ...
              {routine && (
                <button
                  type="button"
                  onClick={() => { dispatch({ type: "showRoutines", section: "logs", routineId: routine.id, botId: routine.botId }); onClose(); }}
                  className="flex items-center gap-1.5 rounded-lg border border-hairline/60 bg-raised px-3 py-1.5 text-[12px] font-medium text-ink hover:bg-raised/80 transition-colors"
                >
                  <FileText size={13} />
                  Logs
                </button>
              ...
              {canOpenExecution && (
                ...
                >
                  ...
                  {isRoomGoal ? "Open group thread" : "Open thread"}
                </button>
              )}
              {canOpenResults && resultsThreadId && (
                <button
                  type="button"
                  onClick={() => { openNotificationTarget(dispatch, { botId: botIds[0], threadId: resultsThreadId }, state); onClose(); }}
                  className="flex items-center gap-1.5 rounded-lg border border-hairline/60 bg-raised px-3 py-1.5 text-[12px] font-medium text-ink hover:bg-raised/80 transition-colors"
                >
                  <ExternalLink size={13} />
                  {t("routines.results.open")}
                </button>
              )}
              {run && ["queued", "running", "waiting"].includes(run.status) && (
                <button onClick={() => void invoke(`/api/routine-runs/${run.id}/cancel`)} disabled={working} className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-[12px] text-ink-secondary hover:bg-raised hover:text-ink disabled:opacity-40">
              ...
              {routine && (
                <button
                  ...
                  disabled={working}
                  className="flex items-center gap-1.5 rounded-lg bg-accent px-4 py-1.5 text-[12px] font-semibold text-white shadow-xs hover:brightness-110 active:scale-[0.98] disabled:opacity-50 transition"
                >
...
function PausedList({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div role="dialog" aria-modal="true" aria-label="Paused routines" className="w-full max-w-[520px] rounded-2xl border border-hairline/70 bg-panel/95 p-1.5 shadow-2xl backdrop-blur-md">
        <div className="rounded-xl border border-hairline/50 bg-card overflow-hidden">
          <div className="flex items-center justify-between border-b border-hairline/40 px-5 py-4">
            <div>
              ...
              <div className="mt-0.5 text-[11.5px] text-ink-secondary">History is kept; no new tasks will run.</div>
            </div>
            <button onClick={onClose} className="rounded-lg p-1.5 text-ink-secondary hover:bg-raised hover:text-ink transition-colors" aria-label="Close">
              <X size={16} />
            </button>
          </div>
          <div className="max-h-[55vh] space-y-1.5 overflow-y-auto p-3">
            {routines.map((routine) => {
              ...
              const room = routine.target === "room-goal"
                ...
                : undefined;
              return (
                <div key={routine.id} className="flex items-center gap-3 rounded-lg border border-hairline/40 bg-inset/40 px-3 py-2.5 hover:bg-raised/60 transition-colors">
                  {room ? (
                    <span className="flex size-8 shrink-0 items-center justify-center rounded-full bg-accent/12 text-accent">
                      <UsersRound size={15} />
                    </span>
                  ) : (
                    bot && <BotAvatar bot={bot} state="sleeping" size={32} animated={false} />
                  )}
                  <div className="min-w-0 flex-1">
                  ...
                  {room && <button onClick={() => { onOpenRoom(room.id); onClose(); }} className="rounded-lg px-2.5 py-1 text-[11px] text-ink-secondary hover:bg-raised">Group</button>}
                  <button onClick={() => dispatch({ type: "updateRoutine", routineId: routine.id, patch: { enabled: true } })} className="rounded-lg bg-accent/15 px-2.5 py-1 text-[11px] font-medium text-accent hover:bg-accent/25">Resume</button>
                  <button onClick={() => onEdit(routine)} className="rounded-lg px-2.5 py-1 text-[11px] text-ink-secondary hover:bg-raised">Edit</button>
                </div>
              );
            })}
          </div>
...
export function RoutinesPage({ onBack, onOpenRoom }: { onBack: () => void; onOpenRoom: (id: string) => void }) {
  ...
  return (
    <main className="flex h-full min-w-0 flex-1 flex-col bg-app animate-workspace-in">
      <header
        className={cn("shrink-0 border-b border-hairline/35 bg-card/60 backdrop-blur-sm py-2.5 pr-4 transition-colors", macInset ? "pl-[86px]" : "pl-4")}
      ...
      >
        <div className="flex flex-wrap items-center gap-2.5">
          <button
            ...
            aria-label="Back"
            title="Back"
            className="flex size-8 shrink-0 cursor-pointer items-center justify-center rounded-lg text-ink-secondary hover:bg-raised hover:text-ink transition-colors"
            style={windowNoDragStyle}
          >
            <ArrowLeft size={16} />
          </button>

          <div data-tour="automations-page" className="mr-1 flex items-center gap-2">
            <div className="flex size-7 items-center justify-center rounded-lg bg-accent/15 text-accent">
              <CalendarDays size={16} />
            </div>
            <h1 className="text-[15px] font-semibold tracking-tight text-ink">Automations</h1>
          </div>

          {/* Section Switcher (Schedule | Logs | Webhooks) - Segmented Control Pill */}
          <div className="flex items-center rounded-lg border border-hairline/50 bg-inset/60 p-0.5" style={windowNoDragStyle} aria-label="Automation type">
            <button
              type="button"
              ...
              className={cn(
                ...
                section === "calendar" ? "bg-raised text-ink shadow-xs" : "text-ink-secondary hover:text-ink",
              )}
            >
              {routinesOnly ? "Scheduled routines" : "Schedule"}
            </button>
            <button
              type="button"
              ...
              className={cn(
                ...
                section === "logs" ? "bg-raised text-ink shadow-xs" : "text-ink-secondary hover:text-ink",
              )}
            >
              <FileText size={12} />
              {t("routines.logs")}
              {unseenFailures > 0 && <span className="rounded-full bg-danger/15 px-1.5 text-[9px] font-bold text-danger">{unseenFailures}</span>}
            </button>
            {!routinesOnly && (
              <button
                type="button"
                aria-pressed={section === "webhooks"}
                onClick={() => setSection("webhooks")}
                className={cn(
                  "flex items-center gap-1.5 rounded-md px-3 py-1.5 text-[12px] font-medium transition-all",
                  section === "webhooks" ? "bg-raised text-ink shadow-xs" : "text-ink-secondary hover:text-ink",
                )}
              >
                <Webhook size={12} />
                Webhooks
                {state.webhooks.length > 0 && <span className="rounded-full bg-accent/20 px-1.5 text-[9px] font-bold text-accent">{state.webhooks.length}</span>}
              </button>
            )}
          </div>
          ...
          <details ref={newMenuRef} className="group relative ml-auto" style={windowNoDragStyle}>
            <summary
              ...
              className="flex cursor-pointer list-none items-center gap-1.5 rounded-lg bg-accent px-3 py-1.5 text-[12.5px] font-semibold text-white shadow-xs transition hover:brightness-110 active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/60"
            >
              <Plus size={14} aria-hidden="true" />
              New
            </summary>
            <div role="group" aria-label="New automation" className="absolute right-0 top-full z-40 mt-1.5 w-[280px] rounded-xl border border-hairline/60 bg-card p-1.5 shadow-2xl backdrop-blur-md">
              <button
                type="button"
                aria-label="Create a scheduled task"
                onClick={() => { newMenuRef.current?.removeAttribute("open"); setSection("calendar"); openCreate({ kind: "routine" }); }}
                className="flex w-full items-start gap-3 rounded-lg px-3 py-2.5 text-left hover:bg-raised transition-colors"
              >
                <div className="flex size-7 shrink-0 items-center justify-center rounded-lg bg-accent/15 text-accent mt-0.5">
                  <Clock3 size={15} />
                </div>
                <span>
                  <span className="block text-[12.5px] font-medium text-ink">Scheduled task</span>
                  <span className="mt-0.5 block text-[11px] leading-relaxed text-ink-secondary">Ask a bot to do something later.</span>
                </span>
              </button>
              {!routinesOnly && (
                <button
                  type="button"
                  aria-label="Create a scheduled call"
                  onClick={() => { newMenuRef.current?.removeAttribute("open"); setSection("calendar"); openCreate({ kind: "call" }); }}
                  className="flex w-full items-start gap-3 rounded-lg px-3 py-2.5 text-left hover:bg-raised transition-colors"
                >
                  <div className="flex size-7 shrink-0 items-center justify-center rounded-lg bg-accent/15 text-accent mt-0.5">
                    <Video size={15} />
                  ...
                  <span>
                    <span className="block text-[12.5px] font-medium text-ink">Scheduled call</span>
                    <span className="mt-0.5 block text-[11px] leading-relaxed text-ink-secondary">Bring bots together at a set time.</span>
                  </span>
                </button>
              )}
              {!routinesOnly && (
                <button
                  ...
                  onClick={() => { newMenuRef.current?.removeAttribute("open"); setSection("webhooks"); setWebhookCreateRequest((request) => request + 1); }}
                  className="flex w-full items-start gap-3 rounded-lg px-3 py-2.5 text-left hover:bg-raised transition-colors disabled:cursor-not-allowed disabled:opacity-40"
                >
                  <div className="flex size-7 shrink-0 items-center justify-center rounded-lg bg-accent/15 text-accent mt-0.5">
                    <Webhook size={15} />
                  </div>
                  <span>
                    <span className="block text-[12.5px] font-medium text-ink">Webhook</span>
                    <span className="mt-0.5 block text-[11px] leading-relaxed text-ink-secondary">Start a task when another app sends an event.</span>
                  </span>
                </button>
              )}
            </div>
        ...
        {/* Sub-toolbar: Date Navigator, Range Heading, View Switchers from scheduling-6 */}
        {section !== "webhooks" && (
          <div className="mt-2.5 flex flex-wrap items-center gap-2 border-t border-hairline/25 pt-2" style={windowNoDragStyle}>
            {section === "calendar" && scheduleView === "calendar" && (
              <>
                <div className="flex items-center gap-1">
                  <div className="flex items-center rounded-lg border border-hairline/50 bg-inset/40 p-0.5">
                    <button
                      onClick={() => setAnchor((current) => addDays(current, -viewDays))}
                      className="inline-flex size-7 items-center justify-center rounded-md text-ink-secondary hover:bg-raised hover:text-ink transition-colors"
                  ...
                  <button
                    onClick={goToday}
                    className="inline-flex h-8 items-center rounded-lg border border-hairline/50 bg-inset/40 px-2.5 text-[12px] font-medium text-ink hover:bg-raised transition-colors active:scale-[0.98]"
                  >
                    Today
                  </button>
                </div>

                <h2 className="min-w-[180px] px-2 text-[13.5px] font-semibold tabular-nums text-ink">
            ...
            <div className="ml-auto flex items-center gap-2">
              {section === "calendar" && (
                <div className="flex items-center rounded-lg border border-hairline/50 bg-inset/60 p-0.5" aria-label="Schedule view">
                  <button
                    type="button"
                    ...
                    className={cn(
                      ...
                      scheduleView === "calendar" ? "bg-raised text-ink shadow-xs" : "text-ink-secondary hover:text-ink",
                    )}
                  >
                  ...
                  <button
                    ...
                    className={cn(
                      "flex items-center gap-1.5 rounded-md px-2.5 py-1 text-[11.5px] font-medium transition-all",
                      scheduleView === "list" ? "bg-raised text-ink shadow-xs" : "text-ink-secondary hover:text-ink",
                    )}
                  >
                    <List size={12} />
                    List
              ...
              {section === "calendar" && scheduleView === "calendar" && (
                <div className="hidden sm:flex items-center rounded-lg border border-hairline/50 bg-inset/60 p-0.5" aria-label="View days">
                  {([1, 3, 7] as const).map((daysOption) => (
                    <button
                      key={daysOption}
                      type="button"
                      onClick={() => setView(daysOption)}
                      className={cn(
                        "rounded-md px-2 py-1 text-[11.5px] font-medium transition-all",
                        viewDays === daysOption ? "bg-raised text-ink shadow-xs" : "text-ink-secondary hover:text-ink",
                      )}
                    >
                      {daysOption === 1 ? "Day" : daysOption === 3 ? "3 Days" : "Week"}
                    </button>
                  ))}
                </div>
              )}
              ...
              {running > 0 && (
                <span className="hidden items-center gap-1.5 rounded-full bg-accent/10 px-2.5 py-1 text-[10.5px] font-medium text-accent sm:flex">
              ...
              {unseenFailures > 0 && (
                <button
                  type="button"
                  onClick={() => dispatch({ type: "showRoutines", section: "logs" })}
                  className="hidden items-center gap-1.5 rounded-full bg-danger/10 px-2.5 py-1 text-[10.5px] font-medium text-danger sm:flex"
                  title="Open failed run logs"
                  aria-label="Open failed run logs"
                >
                  <CircleAlert size={11} />
                  {unseenFailures}
                </button>
              ...
              {paused.length > 0 && (
                <button
                  ...
                  aria-label="View paused routines"
                  className="hidden items-center gap-1.5 rounded-full border border-hairline/50 px-2.5 py-1 text-[10.5px] text-ink-secondary hover:bg-raised sm:flex"
                >
                  <Pause size={11} />
                  {paused.length}
                </button>
              ...
              <select
                ...
                className="max-w-[160px] rounded-lg border border-hairline/50 bg-inset/50 px-2.5 py-1.5 text-[11.5px] text-ink outline-none focus:border-accent"
              >
            ...
            {error && (
              <button onClick={() => setError("")} className="flex items-center gap-1.5 rounded-lg bg-danger/10 px-2.5 py-1.5 text-[10.5px] text-danger">
                <CircleAlert size={11} />
                {error}
                <X size={11} />
              </button>
