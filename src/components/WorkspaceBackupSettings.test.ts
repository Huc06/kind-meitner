...
describe("Settings full backups", () => {
  ...
  it("exports encrypted state by POST and downloads without putting the password in a URL or storage", async () => {
    await ready();
    storage.set("kind-meitner-drafts", "private draft"); storage.set("auth-token", "not exported"); storage.set("kind-meitner-webhook-credentials", "not exported either");
    let view = render();
    ...
    expect(JSON.parse(init.body)).toEqual({ password: "correct horse battery", clientState: { "kind-meitner-drafts": "private draft" } });
  ...
  it("uploads a raw file, validates it, and requires exact REPLACE with the staged ID", async () => {
    ...
    expect(storage.get("kind-meitner-pending-workspace-restore")).toBe("stage-id");
    expect(render().html).toContain("Fully quit kind-meitner");
  });
  ...
  it("does not restore client state before restart, then replaces only the initiating browser's allowlist", async () => {
    storage.set("kind-meitner-pending-workspace-restore", "stage-id"); storage.set("kind-meitner-drafts", "old"); storage.set("auth-token", "keep");
    ...
    expect(render(true).html).toContain("Fully quit kind-meitner");
    expect(render(true).html).not.toContain("Continue without restoring drafts");
    expect(fixture.api).toHaveBeenCalledOnce(); expect(storage.get("kind-meitner-drafts")).toBe("old");
    fixture.values = [];
    fixture.api.mockResolvedValueOnce({ busy: false, lastRestoreId: "stage-id" }).mockResolvedValueOnce({ clientState: { "kind-meitner-drafts": "restored" } });
    render(true); fixture.effects[0](); await flush();
    expect(storage.get("kind-meitner-drafts")).toBe("restored"); expect(storage.get("auth-token")).toBe("keep");
    expect(storage.has("kind-meitner-pending-workspace-restore")).toBe(false); expect(window.location.reload).toHaveBeenCalledOnce();
  });

  it("never imports a different restore's browser state", async () => {
    storage.set("kind-meitner-pending-workspace-restore", "my-stage"); storage.set("kind-meitner-drafts", "keep");
    ...
    expect(storage.get("kind-meitner-drafts")).toBe("keep");
    expect(storage.has("kind-meitner-pending-workspace-restore")).toBe(false);
    expect(window.location.reload).not.toHaveBeenCalled();
  });

  it("keeps the normal app gated and its old drafts intact on recovery failure", async () => {
    storage.set("kind-meitner-pending-workspace-restore", "stage-id"); storage.set("kind-meitner-drafts", "keep");
    ...
    expect(storage.get("kind-meitner-drafts")).toBe("keep"); expect(storage.get("kind-meitner-pending-workspace-restore")).toBe("stage-id");
    expect(window.location.reload).not.toHaveBeenCalled();
  });

  it.each([401, 403])("explicitly returns to normal bootstrap after a %s without changing browser state", async (status) => {
    storage.set("kind-meitner-pending-workspace-restore", "stage-id");
    storage.set("kind-meitner-drafts", "keep drafts"); storage.set("kind-meitner-draft-attachments", "keep attachments");
    storage.set("kind-meitner-skin", "keep preferences"); storage.set("auth-token", "keep session");
    ...
    before.delete("kind-meitner-pending-workspace-restore");
