...
export function TeamCanvas({ sections, canManage, onMove, onInstructions, onEditTeam, onDeleteTeam, isEmpty, onComputer, onComputerDrop, onTeamComputer, teamComputers = {}, connectedBotIds = [], onLogs, edges = [] }: {
  ...
  useEffect(() => {
    let active = true;
    void api("/.well-known/kind-meitner/environment", { signal: AbortSignal.timeout(5_000) }).then((environment) => {
      if (!active || typeof environment.environmentId !== "string") return;
      storageKey.current = `kind-meitner-team-canvas:${environment.environmentId}`;
      try {
