...
function LocalVmPane({
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      if (!bridge) {
        setError("The two-desktop workspace requires the kind-meitner desktop app.");
        return;
      }
      try {
        ...
        setError(
          cause instanceof Error && cause.message === "layout-unavailable"
            ...
            : "kind-meitner could not connect this Local VM viewer.",
        );
      }
    };
  ...
  const updateLayout = useCallback(() => {
    ...
    void bridge
      ...
      .catch(() => setError("kind-meitner could not position this Local VM viewer."));
...
export function LocalVmWorkspace({
  ...
  const handBack = useCallback(async () => {
    ...
    try {
      ...
      setControlError("kind-meitner could not hand control back. The workspace stayed open.");
      return false;
    } finally {
