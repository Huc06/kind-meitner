...
describe("desktop notifications", () => {
  ...
  it("shows a notification after permission is granted", () => {
    ...
    expect(notices[0]).toMatchObject({ title: frame.title, options: { body: frame.body, tag: `kind-meitner:${frame.botId}` } });
  });
  ...
  it("groups under the bot, not the thread", () => {
    ...
    expect(notices[0]?.options?.tag).toBe(`kind-meitner:${frame.botId}`);
    expect(notices[1]?.options?.tag).toBe(`kind-meitner:${frame.botId}`);
    showNotification({ ...frame, botId: "bot-2" }, vi.fn());
    expect(notices[2]?.options?.tag).toBe(`kind-meitner:bot-2`);
  });
