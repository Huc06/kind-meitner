...
describe("team import preview", () => {
  ...
  it("previews portable backups and validates their references before confirmation", () => {
    const backup = {
      format: "kind-meitner.backup", version: 1, name: "Saved team", exportedAt: 0,
  ...
  it("rejects unsupported and empty files", () => {
    expect(() => teamImportPreview({ format: "kind-meitner.team", version: 3, team: {} })).toThrow("not supported");
    expect(() =>
      teamImportPreview({ format: "kind-meitner.team", version: 2, team: { name: "Empty", members: [] } }),
    ).toThrow("no members");
  });
