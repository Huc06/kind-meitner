...
describe("composeMessage with images", () => {
  it("emits an attached-image tag carrying the server path and display name", () => {
    const prompt = composeMessage("what is this?", [image("/home/u/.kind-meitner/attachments/abc.png")]);
    expect(prompt).toBe(
      'what is this?\n\n<attached-image path="/home/u/.kind-meitner/attachments/abc.png" name="shot.png" />',
    );
  });
...
describe("splitTranscriptAttachments", () => {
  ...
  it("uses the saved basename for old file tags without a name", () => {
    const { display, files } = splitTranscriptAttachments(
      '<attached-file path="/home/me/.kind-meitner/attachments/report.pdf" />',
    );
    expect(display).toBe("");
    expect(files).toEqual([
      { path: "/home/me/.kind-meitner/attachments/report.pdf", name: "report.pdf" },
    ]);
  });

  it("marks only private-store UUID attachment names as actionable", () => {
    const privatePath = "/home/me/.kind-meitner/attachments/123e4567-e89b-42d3-a456-426614174000.pdf";
