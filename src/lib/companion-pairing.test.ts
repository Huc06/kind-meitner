...
describe("companionPairingLink", () => {
  const token = `kind_meitner_pair_${"a".repeat(43)}`;
  ...
  it("carries the dialable address, one-time token, fallback code, and display name", () => {
    ...
    expect(url.protocol).toBe("kind-meitner:");
  ...
  it("carries the ordered fallback hosts, comma-joined", () => {
    const link = companionPairingLink({
      ...
      code: "004209",
      token,
      hosts: ["macbook.tail1234.ts.net", "192.168.1.42", "kind-meitner-abcd1234.local"],
    });
    expect(new URL(link!).searchParams.get("hosts")).toBe(
      "macbook.tail1234.ts.net,192.168.1.42,kind-meitner-abcd1234.local",
    );
  });
  ...
  it("carries sorted typed endpoints as URL-safe base64 JSON while preserving legacy fields", () => {
    const link = companionPairingLink({
      ...
      code: "004209",
      token,
      hosts: ["192.168.1.42", "kind-meitner-abcd1234.local"],
      endpoints: [
        { url: "http://192.168.1.42:8810", kind: "lan", priority: 200 },
        { url: "https://Device-123.Companion.Example/", kind: "hosted", priority: 0 },
        { url: "http://kind-meitner-abcd1234.local:8810", kind: "bonjour", priority: 300 },
      ],
    });

    const url = new URL(link!);
    expect(url.searchParams.get("address")).toBe("192.168.1.42:8810");
    expect(url.searchParams.get("hosts")).toBe("192.168.1.42,kind-meitner-abcd1234.local");
  ...
  it("makes the automatic QR hosted-only even when Tailscale and LAN are advertised", () => {
    const endpoints = [
      { url: "https://device.kind-meitner.com", kind: "hosted" as const, priority: 0 },
    ...
    expect(route).toEqual({
      address: "device.kind-meitner.com",
      port: 443,
      hosts: ["device.kind-meitner.com"],
      endpoints: [endpoints[0]],
    });
    const link = companionPairingLink({ ...route!, code: "004209", token });
    const url = new URL(link!);
    expect(url.searchParams.get("address")).toBe("device.kind-meitner.com:443");
    expect(url.searchParams.get("hosts")).toBe("device.kind-meitner.com");
  ...
  it("refuses automatic pairing when hosted HTTPS is not ready", () => {
    const source = {
      ...
      tailnetName: "mac.tail1234.ts.net",
      lan: "192.168.1.42",
      hosts: ["mac.tail1234.ts.net", "192.168.1.42", "kind-meitner-aa.local"],
      endpoints: [
  ...
  it("pins the protected automatic transport instead of downgrading the live QR to LAN", () => {
    ...
    expect(pin?.protectedEndpoint).toEqual({
      url: "https://device.kind-meitner.com",
      kind: "hosted",
      priority: 0,
    });
  ...
  it("selects hosted HTTPS regardless of an unprotected endpoint's priority", () => {
    expect(companionPairingRoutePin({
      ...
      endpoints: [
        ...
        { url: "https://device.kind-meitner.com", kind: "hosted", priority: 100 },
      ],
  ...
  it("makes the explicitly selected LAN route first without losing protected upgrades", () => {
    const route = companionPairingRoute({
      ...
      hosts: ["mac.tail1234.ts.net", "192.168.1.42", "kind-meitner-aa.local"],
      endpoints: [
        { url: "https://device.kind-meitner.com", kind: "hosted", priority: 0 },
        { url: "http://mac.tail1234.ts.net:8810", kind: "tailnet", priority: 100 },
        { url: "http://192.168.1.42:8810", kind: "lan", priority: 200 },
        { url: "http://kind-meitner-aa.local:8810", kind: "bonjour", priority: 300 },
      ],
    }, "local");
    ...
    expect(decodedEndpoints(link!)).toEqual([
      { url: "http://192.168.1.42:8810", kind: "lan", priority: 0 },
      { url: "https://device.kind-meitner.com", kind: "hosted", priority: 100 },
      { url: "http://kind-meitner-aa.local:8810", kind: "bonjour", priority: 200 },
    ]);
  });
  ...
  it("uses an advertised Bonjour route when no LAN address is available", () => {
    const route = companionPairingRoute({
      port: 8810,
      hosts: ["mac.tail1234.ts.net", "kind-meitner-aa.local"],
      discovery: { advertising: true, name: "kind-meitner-aa.local" },
      endpoints: [
        { url: "https://device.kind-meitner.com", kind: "hosted", priority: 0 },
        { url: "http://kind-meitner-aa.local:8810", kind: "bonjour", priority: 300 },
      ],
    ...
    expect(route?.address).toBe("kind-meitner-aa.local");
    expect(route?.hosts?.[0]).toBe("kind-meitner-aa.local");
  ...
  it("does not treat an inactive synthetic Bonjour name as a reachable local route", () => {
    expect(companionPairingRoute({
      port: 8810,
      hosts: ["mac.tail1234.ts.net", "kind-meitner-aa.local"],
      discovery: { advertising: false, name: "kind-meitner-aa.local" },
      endpoints: [
        { url: "https://device.kind-meitner.com", kind: "hosted", priority: 0 },
        { url: "http://kind-meitner-aa.local:8810", kind: "bonjour", priority: 300 },
      ],
