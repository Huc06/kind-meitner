...
export function teamImportPreview(manifest: unknown): PendingTeamImport {
  ...
  if (root.format === "kind-meitner.backup") {
  ...
  if (root.format === "kind-meitner.package") return packagePreview(root, manifest);
  if (root.format !== "kind-meitner.team") throw new Error("This is not an KindMeitner backup, BotMRR playbook or legacy team.");
...
function markdownPackage(markdown: string): unknown {
  ...
  return { format: "kind-meitner.package", version: 1, package: pkg };
}
