...
// control:kind-meitner doctor`, a `control-<app>.mjs` script) are the verified ones,
 ...
 * control:kind-meitner doctor…") and must not become steps. */
...
// Position-based, not substring-based: `cat scripts/control-kind-meitner.ts` reads
...
/** `pnpm run control:kind-meitner`, `npm run control:kind-meitner -- doctor`, `yarn -s control:kind-meitner`. */
const PACKAGE_RUNNER = new Set(["pnpm", "npm", "yarn", "bun"]);
const PACKAGE_RUNNER_WORD = new Set(["run", "exec", "-s", "--silent", "-r", "--"]);
/** `node --experimental-strip-types scripts/control-kind-meitner.ts`, `npx tsx …`. */
...
/** The CLI token's basename when the token IS a control CLI: `control:kind-meitner`
