...
>(input: {
  ...
  if (action === "take" && syncNativeBrowser && !(await setNativeBrowserControl(true))) {
    throw new Error("kind-meitner could not pause this bot's browser safely");
  }
  const snap = await requestControl(action);
  if (snap.held !== (action === "take")) {
    throw new Error(`kind-meitner could not ${action === "take" ? "confirm" : "release"} computer control`);
  }
