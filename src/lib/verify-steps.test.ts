...
const stagedSkill = (): Message => ({
  ...
  card: { title: "Enable this skill?", options: [], skillRequest: { name: "verify-kind-meitner" } } as unknown as Message["card"],
});
...
const DOCTOR = "pnpm control:kind-meitner doctor --url http://127.0.0.1:8799";
const SEND = "node --experimental-strip-types scripts/control-kind-meitner.ts send --bot x --text y";
const PRESS = './node_modules/.bin/control-atlas.mjs press "Meta+K"';
const PUSH = "git push origin main";

describe("parseControlCommand", () => {
  it.each([
    ["pnpm control:kind-meitner doctor --url http://127.0.0.1:1", "doctor"],
    ["pnpm run control:kind-meitner doctor", "doctor"],
    ["npm run control:kind-meitner -- doctor", "doctor"],
    [SEND, "send"],
    [PRESS, "press"],
    ["cd repo && node scripts/control-kind-meitner.ts doctor", "doctor"],
    ["node scripts\\control-kind-meitner.ts doctor", "doctor"],
    ["FOO=1 pnpm control:kind-meitner wait --bot b", "wait"],
    ["timeout 30 npx tsx ./scripts/control-kind-meitner.ts screenshot > out.png 2>&1", "screenshot"],
    ["sudo -u maus pnpm control:kind-meitner doctor", "doctor"],
  ...
  it.each([
    "cat scripts/control-kind-meitner.ts",
    "sed -n 1,40p scripts/control-kind-meitner.ts",
    "git log -- scripts/control-kind-meitner.ts",
    "grep -n doctor scripts/control-kind-meitner.ts",
    "cat docs/access-control-kind-meitner.tsx",
    "npmcontrol:kind-meitner doctor",
    "pnpm typecheck",
  ...
  it("marks a dry run wherever the flag sits in the segment", () => {
    expect(parseControlCommand("pnpm control:kind-meitner send --bot b --dry-run")).toEqual({ subcommand: "send", dryRun: true });
    expect(parseControlCommand("pnpm control:kind-meitner --dry-run send")).toEqual({ subcommand: "send", dryRun: true });
    // the flag in another segment belongs to that command
    expect(parseControlCommand("pnpm control:kind-meitner doctor && pnpm release --dry-run")).toEqual({ subcommand: "doctor", dryRun: false });
  });

  it("falls back to the CLI token's basename when no subcommand follows", () => {
    expect(parseControlCommand("pnpm control:kind-meitner --help")?.subcommand).toBe("control:kind-meitner");
...
describe("parseRunCommand", () => {
  ...
  it.each([
    ...
    ["sudo -u maus systemctl restart kind-meitner", "systemctl"],
...
describe("runSteps", () => {
  it("lists every command from every driver shape, in order, reads left out, control-CLI steps verified", () => {
    const steps = runSteps([
      ...
      claude("cat scripts/control-kind-meitner.ts", true),
      claude(PUSH, true),
    ]);
