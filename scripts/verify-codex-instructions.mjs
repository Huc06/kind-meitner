...
const root = mkdtempSync(join(tmpdir(), 'kind-meitner-native-instructions-'));
process.env.KIND_MEITNER_DATA_DIR = join(root, 'kind-meitner');
...
async function start() {
    ...
    await rpc('initialize', { clientInfo: { name: 'kind-meitner-instruction-probe', version: '1' } });
...
try {
    ...
    if (!process.env.PROBE_BEFORE) {
        ...
        assert(text(8, 'developer').includes('No kind-meitner bot-specific instructions remain.'));
