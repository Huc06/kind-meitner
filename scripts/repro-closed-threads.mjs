...
// close_thread stay in the sidebar. Runs an isolated kind-meitner server on a
...
const home = mkdtempSync(join(tmpdir(), "kind-meitner-closed-threads-repro-"));
...
const child = spawn(process.execPath, ["--experimental-strip-types", join(ROOT, "server", "index.ts")], {
  ...
  env: {
    ...
    HOME: home, USERPROFILE: home, KIND_MEITNER_DATA_DIR: home,
    TMPDIR: home,
    KIND_MEITNER_PORT: String(port), KIND_MEITNER_WEBHOOK_PORT: String(port + 1),
    KIND_MEITNER_TEST_INTERNAL_CAPABILITY_KEY: TEST_KEY,
    FAKE_CLAUDE_MODE: "happy",
    ...(existsSync(STATIC_DIR) ? { KIND_MEITNER_STATIC_DIR: STATIC_DIR } : {}),
  },
...
const minted = await api("POST", "/api/testing/internal-capability", { botId: parker.id, threadId: parker.threadId, kind: "agents", depth: 0 }, { "x-kind-meitner-test-capability": TEST_KEY });
