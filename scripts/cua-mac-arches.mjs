...
export function resolveCuaMacArches(env = process.env) {
  const raw = env.KIND_MEITNER_CUA_ARCHES;
  if (raw === undefined) return [...DEFAULT_MAC_ARCHES];
  const arches = raw.split(",").map((arch) => arch.trim()).filter(Boolean);
  if (arches.length === 0) {
    throw new Error(
      "KIND_MEITNER_CUA_ARCHES is empty; omit the variable to stage arm64 and x64",
    );
  }
  if (env.KIND_MEITNER_CUA_ARCHES_PARTIAL !== "1") {
    const missing = DEFAULT_MAC_ARCHES.filter((arch) => !arches.includes(arch));
    if (missing.length) {
      throw new Error(
        `KIND_MEITNER_CUA_ARCHES omits ${missing.join(", ")} but electron-builder packages both arm64 and x64. Set KIND_MEITNER_CUA_ARCHES_PARTIAL=1 for a one-arch local stage.`,
      );
    }
  }
  return arches;
