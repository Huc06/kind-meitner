...
function requireUpdaterTarget(resources, label) {
  ...
  if (!/^owner: milind-soni$/m.test(update) || !/^repo: kind-meitner$/m.test(update)) {
    fail(`${label} app-update.yml does not point at harrymove-ctrl/kind-meitner`);
  }
}
...
function verifyCompliance(licenses, label) {
  ...
  for (const component of registry) {
    const packageId = component.properties?.find(
      (property) => property.name === "kind-meitner:cargo:package-id",
    )?.value;
...
const executable = path.join(unpacked, "kind-meitner");
...
const extracted = mkdtempSync(path.join(tmpdir(), "kind-meitner-deb-verify-"));
try {
  execFileSync("dpkg-deb", ["--extract", deb, extracted]);
  const debAppRoot = path.join(extracted, "opt", "kind-meitner");
  ...
  for (const expected of [
    "Name=kind-meitner",
    "Exec=/opt/kind-meitner/kind-meitner %U",
    "Icon=kind-meitner",
    "StartupWMClass=com.kind-meitner.app",
    "Categories=Utility;",
  ]) {
...
const appImageExtracted = mkdtempSync(path.join(tmpdir(), "kind-meitner-appimage-verify-"));
try {
