...
try {
  ...
  const failLogoutMarker = join(fixtureHome, ".kind-meitner-fake-codex-logout-fail");
  ...
  writeFileSync(join(fixtureHome, ".kind-meitner-fake-codex-authenticated"), "Offline fixture; not a credential.\n", { mode: 0o600 });
  writeFileSync(wrapper, [
    "#!/usr/bin/env node",
    'import { appendFileSync, existsSync, unlinkSync } from "node:fs";',
    `if (process.env.KIND_MEITNER_DEVICE_AUTH_FIXTURE !== "1" || process.env.HOME !== ${JSON.stringify(fixtureHome)} || process.env.CODEX_HOME !== ${JSON.stringify(codexDir)}) { throw new Error("Disposable Codex fixture environment required"); }`,
    ...
    `  for (const file of ${JSON.stringify([join(fixtureHome, ".kind-meitner-fake-codex-authenticated"), join(fixtureHome, ".kind-meitner-fake-codex-login-approved")])}) { if (existsSync(file)) unlinkSync(file); }`,
  ...
  config.instances.codex = {
    ...
    environment: { HOME: fixtureHome, USERPROFILE: fixtureHome, CODEX_HOME: codexDir, KIND_MEITNER_DEVICE_AUTH_FIXTURE: "1", FAKE_CODEX_ACCOUNT_EMAIL: "ada@example.test" },
  };
  writeFileSync(configPath, JSON.stringify(config, null, 2), { mode: 0o600 });
  await api("PUT", "/api/config", { defaultModelSelection: { instanceId: "codex", model: "gpt-6-astra" } });
  await runControlKindMeitner(["new-bot", "--name", "Codex Account Fixture", "--url", fixture.info.url]);
