...
const staging = mkdtempSync(join(tmpdir(), "kind-meitner-smoke-"));
const home = mkdtempSync(join(tmpdir(), "kind-meitner-smoke-home-"));
const port = 21000 + Math.floor(Math.random() * 9000);

// KIND_MEITNER_SMOKE_DIST lets the release workflow aim this at a packaged app's
// Resources/server tree instead of the repo build.
try {
  cpSync(process.env.KIND_MEITNER_SMOKE_DIST ?? join(root, "dist-server"), join(staging, "server"), { recursive: true });
...
const fixtureEnv = {
  ...
  ...(browserBundle ? {
    KIND_MEITNER_RESOURCES_PATH: staging,
...
if (
  ...
  mcpReport.responses?.find((response) => response.id === 1)?.result?.serverInfo?.name !== "kind-meitner-mcp" ||
