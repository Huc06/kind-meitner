...
try {
  ...
  writeFileSync(wrapper, [
    ...
    'const bot = at < 0 ? null : JSON.parse(readFileSync(process.argv[at + 1], "utf8")).mcpServers?.agents?.env?.KIND_MEITNER_BOT_ID;',
  ...
  const { project: resultsFolder } = await api("POST", `/api/bots/${pepper.id}/projects`, { name: "kind-meitner management", emoji: "🛠️" });
  ...
  ui = await mountPreview(fixture, {
    entry: "/scripts/testing/threads-preview.tsx", route: "/__routines.html", title: "Isolated KindMeitner Routines",
  });
