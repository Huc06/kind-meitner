...
const podman = process.env.KIND_MEITNER_VERIFY_PODMAN;
const machine = process.env.KIND_MEITNER_VERIFY_MACHINE;
const image = process.env.KIND_MEITNER_VERIFY_IMAGE;
if (!podman || !machine || !image) throw new Error("Set explicit KIND_MEITNER_VERIFY_PODMAN, KIND_MEITNER_VERIFY_MACHINE and KIND_MEITNER_VERIFY_IMAGE");
const output = resolve(process.env.KIND_MEITNER_VERIFY_OUTPUT || ".kind-meitner-scratch/japanese");
...
process.env.KIND_MEITNER_DATA_DIR = resolve(runtime, "app-data");
...
async function capture() {
  ...
  for (const attempt of [1, 2]) {
    const workspace = machineRun(["mktemp", "-d", "/tmp/kind-meitner-font-XXXXXXXX"]);
    assert.match(workspace, /^\/tmp\/kind-meitner-font-[A-Za-z0-9]+$/);
    const target = { ...perBotLocalVmTarget(randomUUID()), containerName: `kind-meitner-font-fixture-${randomUUID()}`, workspaceDir: workspace };
