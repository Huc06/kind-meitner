...
const wayland = process.env.KIND_MEITNER_SMOKE_WAYLAND === "1";
const hardDeath = process.env.KIND_MEITNER_SMOKE_HARD_DEATH === "1";
const bundled = process.env.KIND_MEITNER_SMOKE_BUNDLED_CUA === "1";
const sessionBlocked = process.env.KIND_MEITNER_SMOKE_LINUX_CUA_BLOCKED === "1";
const signalShutdown = process.env.KIND_MEITNER_SMOKE_SIGNAL_SHUTDOWN === "1";
...
const executable = path.resolve(
  process.env.KIND_MEITNER_SMOKE_EXECUTABLE ?? path.join(root, "release", "linux-unpacked", "kind-meitner"),
);
if (!existsSync(executable)) throw new Error(`[smoke-linux-package] missing executable: ${executable}`);

const sandbox = mkdtempSync(path.join(tmpdir(), "kind-meitner-linux-smoke-"));
...
mkdirSync(path.join(home, ".kind-meitner"), { recursive: true });
mkdirSync(xdgConfig, { recursive: true });
mkdirSync(xdgRuntime, { recursive: true, mode: 0o700 });
chmodSync(xdgRuntime, 0o700);
writeFileSync(
  path.join(home, ".kind-meitner", "config.json"),
  JSON.stringify({ instances: { ghost: { driver: "not-a-real-driver", displayName: "Ghost" } } }),
);
for (const appName of ["kind-meitner", "kind-meitner"]) {
...
const metadata = {
  ...
  pid: process.pid,
  embedded: true,
  host_bundle_id: "com.kind-meitner.app",
};
...
const desktopEnv = {
  ...
  KIND_MEITNER_COMPOSIO_BROKER_URL: `http://127.0.0.1:${brokerAddress.port}`,
  KIND_MEITNER_SMOKE_TEST: "1",
  KIND_MEITNER_SMOKE_CUA: hardDeath || bundled || sessionBlocked ? "0" : "1",
  KIND_MEITNER_SMOKE_BUNDLED_CUA: bundled ? "1" : "0",
};
if (hardDeath || signalShutdown) desktopEnv.KIND_MEITNER_SMOKE_KEEP_OPEN = "1";
...
try {
  ...
  if (health?.app !== "kind-meitner" || health.static !== true) {
    throw new Error(`unexpected embedded health response: ${JSON.stringify(health)}`);
  }
  if (!String(title).includes("kind-meitner")) throw new Error(`unexpected renderer title: ${title}`);
  ...
  if (sessionBlocked) {
    ...
    const activeUserData = ["kind-meitner", "kind-meitner"]
    ...
    const userData = ["kind-meitner", "kind-meitner"]
      .map((name) => path.join(xdgConfig, name))
      .find((directory) => existsSync(path.join(directory, "cua-connection.json")));
  ...
  if (hardDeath) {
    ...
    const restart = spawn(executable, wayland ? ["--ozone-platform=x11"] : [], {
      cwd: root,
      detached: true,
      env: { ...desktopEnv, KIND_MEITNER_SMOKE_KEEP_OPEN: "0" },
      stdio: ["ignore", "pipe", "pipe"],
    });
    try {
  ...
  if (process.env.KIND_MEITNER_KEEP_SMOKE_DIR !== "1") rmSync(sandbox, { recursive: true, force: true });
