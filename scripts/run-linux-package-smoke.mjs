...
const prefixName = "kind-meitner-linux-smoke-runtime-";
...
const executables = [
  path.join(root, "release", "linux-unpacked", "kind-meitner"),
  path.join(root, "release", appImage),
];
if (process.env.KIND_MEITNER_SMOKE_INSTALLED_DEB === "1") {
  executables.push("/opt/kind-meitner/kind-meitner");
}
  ...
  const bundled = spawnSync(
    ...
    ["--", "xvfb-run", "-a", process.execPath, path.join(root, "scripts", "smoke-linux-package.mjs")],
    {
      ...
      env: {
        ...
        KIND_MEITNER_SMOKE_BUNDLED_CUA: "1",
        KIND_MEITNER_SMOKE_EXECUTABLE: executable,
      },
      stdio: "inherit",
    },
  );
  ...
  const signalShutdown = spawnSync(
    ...
    ["--", "xvfb-run", "-a", process.execPath, path.join(root, "scripts", "smoke-linux-package.mjs")],
    {
      ...
      env: {
        ...
        XDG_RUNTIME_DIR: runtimeDirectory,
        KIND_MEITNER_SMOKE_BUNDLED_CUA: "1",
        KIND_MEITNER_SMOKE_SIGNAL_SHUTDOWN: "1",
        KIND_MEITNER_SMOKE_EXECUTABLE: path.join(root, "release", appImage),
      },
      stdio: "inherit",
    },
  );
  ...
  const hardDeath = spawnSync(
    ...
    ["--", "xvfb-run", "-a", process.execPath, path.join(root, "scripts", "smoke-linux-package.mjs")],
    {
      ...
      env: {
        ...process.env,
        XDG_RUNTIME_DIR: runtimeDirectory,
        KIND_MEITNER_SMOKE_HARD_DEATH: "1",
        KIND_MEITNER_SMOKE_EXECUTABLE: path.join(root, "release", "linux-unpacked", "kind-meitner"),
      },
      stdio: "inherit",
    },
  );
  ...
  const result = spawnSync(
    ...
    ["--", "xvfb-run", "-a", process.execPath, path.join(root, "scripts", "smoke-linux-package.mjs")],
    {
      ...
      env: {
        ...process.env,
        XDG_RUNTIME_DIR: runtimeDirectory,
        KIND_MEITNER_SMOKE_WAYLAND: lane.wayland ? "1" : "0",
        KIND_MEITNER_SMOKE_LINUX_CUA_BLOCKED: lane.blocked ? "1" : "0",
      },
      stdio: "inherit",
    },
  );
