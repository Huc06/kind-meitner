...
const browserPolicy = path.join(root, "build", "linux-kind-meitner-browser.apparmor");
const temporaryDirectories = [];

function fixture() {
  const appRoot = fs.mkdtempSync(path.join(fs.realpathSync(os.tmpdir()), "kind-meitner-deb-upgrade-"));
  ...
  fs.copyFileSync(browserPolicy, path.join(resources, "kind-meitner-browser.apparmor"));
  ...
  fs.writeFileSync(path.join(systemRoot, "apparmor-profiles"), "kind-meitner-browser (unconfined)\n");
...
function runHook(appRoot) {
  return spawnSync("/bin/sh", [hook], {
    encoding: "utf8",
    env: { ...process.env, KIND_MEITNER_POSTINSTALL_TEST_ROOT: appRoot },
  });
}
...
describe("Linux DEB sandbox policy", () => {
  it("allows user namespaces only for the exact installed browser executable", () => {
    ...
    expect(policy).toContain("profile kind-meitner-browser /opt/kind-meitner/resources/browser-engine/chrome/chrome-headless-shell-linux64/chrome-headless-shell flags=(unconfined)");
...
describe.skipIf(process.platform !== "linux")("Linux DEB upgrade hook", () => {
  it("repairs legacy directory and executable modes idempotently", () => {
    ...
    for (let pass = 0; pass < 2; pass += 1) {
      ...
      expect(fs.readFileSync(path.join(apparmorDir, "kind-meitner-browser"), "utf8")).toBe(fs.readFileSync(browserPolicy, "utf8"));
      expect(result.stdout).toContain(`profile operation: -r ${apparmorDir}/kind-meitner-browser`);
    }
  });

  it("refuses to follow a replaced package directory symlink", () => {
    const { appRoot, resources } = fixture();
    const external = fs.mkdtempSync(path.join(fs.realpathSync(os.tmpdir()), "kind-meitner-deb-external-"));
  ...
  it("refuses an AppArmor destination symlink without overwriting its target", () => {
    ...
    fs.symlinkSync(external, path.join(apparmorDir, "kind-meitner-browser"));
    const result = runHook(appRoot);
    expect(result.status).not.toBe(0);
  ...
  it("keeps the profile on upgrade and removes only its own policy on uninstall", () => {
    const { appRoot, apparmorDir } = fixture();
    expect(runHook(appRoot).status).toBe(0);
    const profile = path.join(apparmorDir, "kind-meitner-browser");
  ...
  it("removes an already unloaded profile without asking the kernel again", () => {
    ...
    expect(result.stdout).not.toContain("profile operation");
    expect(fs.existsSync(path.join(apparmorDir, "kind-meitner-browser"))).toBe(false);
  });
  ...
  it("keeps the exact policy available for repair if the kernel refuses to unload it", () => {
    ...
    expect(fs.existsSync(path.join(apparmorDir, "kind-meitner-browser"))).toBe(true);
  });
