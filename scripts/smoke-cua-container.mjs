...
const runtime = process.env.KIND_MEITNER_CUA_RUNTIME || "docker";
const container = process.env.KIND_MEITNER_CUA_CONTAINER;
const socket = process.env.KIND_MEITNER_CUA_SOCKET || "/run/user/1000/kind-meitner-cua.sock";
if (!container || !/^[a-zA-Z0-9_.-]+$/.test(container)) {
  throw new Error("set KIND_MEITNER_CUA_CONTAINER to the explicitly created smoke container name");
}
...
try {
  await rpc("initialize", {
    ...
    capabilities: {},
    clientInfo: { name: "kind-meitner-container-smoke", version: "1" },
  });
