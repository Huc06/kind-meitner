...
// reads or writes your real ~/.kind-meitner. No real name, key or token can end up in a fixture. The
...
async function main() {
  ...
  const harness = start("harness", [join(ROOT, "server", "index.ts")], {
    ...
    KIND_MEITNER_WEBHOOK_PORT: String(base + 1),
  });
  ...
  const sidecar = start("companion", [join(ROOT, "companion", "src", "index.ts")], {
    ...
    KIND_MEITNER_COMPANION_DIR: join(home, "companion"),
  });
  ...
  write("pair-response", { ...paired.body, token: "kind_meitner_REDACTED" });
