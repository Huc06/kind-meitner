...
const fixtureFlag = "--kind-meitner-server-connection-fixture";
...
if (process.versions.electron && process.argv.includes(fixtureFlag)) {
    ...
    const open = async (local) => {
      const win = new BrowserWindow({
        ...
        webPreferences: {
          ...
          additionalArguments: [`--kind-meitner-local-origin=${local ? new URL(url).origin : "https://remote-fixture.invalid"}`],
        },
      });
  ...
  const { launchVerificationServer } = await import("./control-kind-meitner.ts");
  const fixture = await launchVerificationServer();
  const output = mkdtempSync(join(tmpdir(), "kind-meitner-server-connection-"));
