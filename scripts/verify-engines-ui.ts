...
try {
  ...
  ui = await createServer({
    ...
    cacheDir: fileURLToPath(new URL("../.kind-meitner-scratch/engine-preview-vite", import.meta.url)),
    ...
    plugins: [{ name: "isolated-engines", configureServer(server) {
      server.middlewares.use((req, res, next) => {
        ...
        void server.transformIndexHtml(req.url!, '<html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>KindMeitner · Engine preview</title></head><body><div id="root"></div><script type="module" src="/scripts/testing/engines-preview.tsx"></script></body></html>')
