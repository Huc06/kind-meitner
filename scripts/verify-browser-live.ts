...
const binaryPath = process.env.KIND_MEITNER_VERIFY_BROWSER_BINARY;
const executablePath = process.env.KIND_MEITNER_VERIFY_BROWSER_CHROME;
if (!binaryPath || !executablePath) throw new Error("Set KIND_MEITNER_VERIFY_BROWSER_BINARY and KIND_MEITNER_VERIFY_BROWSER_CHROME to explicit installed binaries.");
...
try {
  await runControlKindMeitner(["new-bot", "--name", "Pepper", "--url", fixture.info.url]);
