...
const fixture = await mkdtemp(join(tmpdir(), "kind-meitner-browser-smoke-"));
...
const env = {
  ...
  KIND_MEITNER_DATA_DIR: join(fixture, "kind-meitner"), KIND_MEITNER_RESOURCES_PATH: resolve(values.resources),
  ...(values["engine-candidate"] ? { KIND_MEITNER_AGENT_BROWSER_PATH: enginePath, AGENT_BROWSER_EXECUTABLE_PATH: paths.chrome } : {}),
...
for (const directory of new Set([fixtureHome, env.APPDATA, env.LOCALAPPDATA, env.XDG_CONFIG_HOME,
  env.XDG_CACHE_HOME, env.XDG_DATA_HOME, env.XDG_RUNTIME_DIR, env.TMPDIR, env.KIND_MEITNER_DATA_DIR, env.AGENT_BROWSER_SOCKET_DIR])) {
...
try {
  ...
  const status = browserEngineStatus({ dataDir: env.KIND_MEITNER_DATA_DIR, env });
  ...
  const title = `kind-meitner bundled browser ${randomBytes(6).toString("hex")}`;
  ...
  for (const name of ["alpha", "beta"]) {
    const integration = agentBrowserIntegration({ binaryPath: status.binaryPath,
      session: `kind-meitner-${name[0]}-${randomBytes(4).toString("hex")}`,
    ...
    clients.push(client);
    await client.request("initialize", { protocolVersion: "2025-11-25", capabilities: {}, clientInfo: { name: "kind-meitner-bundle-smoke", version: "1" } });
  ...
  const storage = "({ local: localStorage.getItem('kind-meitner-smoke'), cookie: document.cookie })";
  const setStorage = (value) => `(() => { localStorage.setItem('kind-meitner-smoke', '${value}'); document.cookie = 'kind_meitner_smoke=${value}; Path=/; SameSite=Lax'; return ${storage}; })()`;
  assert.deepEqual((await alpha.tool("agent_browser_eval", { script: setStorage("alpha") })).data.result, { local: "alpha", cookie: "kind_meitner_smoke=alpha" });
  ...
  assert.deepEqual((await beta.tool("agent_browser_eval", { script: setStorage("beta") })).data.result, { local: "beta", cookie: "kind_meitner_smoke=beta" });
  assert.deepEqual((await alpha.tool("agent_browser_eval", { script: storage })).data.result, { local: "alpha", cookie: "kind_meitner_smoke=alpha" }, "First bot's state was overwritten by the second bot");
  ...
  assert.deepEqual((await beta.tool("agent_browser_eval", { script: storage })).data.result, { local: "beta", cookie: "kind_meitner_smoke=beta" }, "Restarting the first bot changed the second bot's state");
