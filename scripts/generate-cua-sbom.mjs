...
function registryComponent(pkg, cargoLockPackages) {
  ...
  return {
    ...
    properties: [
      { name: "kind-meitner:cargo:package-id", value: pkg.id },
      { name: "kind-meitner:cargo:crate-sha256", value: locked.checksum },
    ],
  };
}
...
function interComponent() {
  return {
    ...
    properties: [
      {
        name: "kind-meitner:font:name-table-version",
        value: "Version 4.001;git-66647c0bb",
      },
      { name: "kind-meitner:font:embedded-sha256", value: INTER_FONT_SHA256 },
    ],
  };
}
...
<head>
  ...
  <title>kind-meitner — Cua Driver third-party licenses</title>
...
const bom = {
  ...
  metadata: {
    component: {
      ...
      properties: [
        { name: "kind-meitner:archive:sha256", value: ARCHIVE_SHA256 },
        { name: "kind-meitner:cargo-lock:sha256", value: CARGO_LOCK_SHA256 },
        { name: "kind-meitner:target", value: "x86_64-unknown-linux-gnu" },
        { name: "kind-meitner:feature", value: "cua-driver/portal-input" },
        {
          name: "kind-meitner:registry-component-count",
          value: String(EXPECTED_REGISTRY_UNION_COUNT),
        },
        {
          name: "kind-meitner:trycua-component-count",
          value: String(TRYCUA_COMPONENTS.length),
        },
        {
          name: "kind-meitner:file:cua-driver:sha256",
          value:
            "ed5844fadf07b9b72c4a3b3802e1c47233c166d66d6198608d5991f807aab4ac",
        },
        {
          name: "kind-meitner:file:cua-cursor-theme:sha256",
          value:
...
kind-meitner redistributes two executables from the official Cua Driver ${RELEASE_VERSION} Linux x64 release: \`cua-driver\` and \`cua-cursor-theme\`.
...
The complete MPL-2.0 text and package copyright notices appear in \`THIRD_PARTY_LICENSES.html\`. These files remain under their original licenses; kind-meitner's and Cua's MIT licenses do not replace them.
