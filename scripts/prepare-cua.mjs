...
async function officialBinary() {
  const cache = join(root, "node_modules", ".cache", "kind-meitner", `cua-driver-${release.version}`);
  ...
  const response = await fetch(url, { headers: { "user-agent": "kind-meitner-packager" } });
