...
if (!podman || !isAbsolute(podman) || !machine) {
  throw new Error("Set KIND_MEITNER_VERIFY_PODMAN to an absolute executable path and KIND_MEITNER_VERIFY_MACHINE explicitly");
}
...
try {
  ...
  async function turn(id: string) {
    ...
    const wait = await runControlKindMeitner(["wait", "--channel", group.id, "--timeout", "60", "--url", fixture.info.url]) as { status: string };
  ...
  for (const id of ids) {
    ...
    const gate = await fetch(computer.env.KIND_MEITNER_CONTROL_URL, {
      headers: { authorization: `Bearer ${computer.env.KIND_MEITNER_CONTROL_TOKEN}` }, signal: AbortSignal.timeout(5_000),
    });
...
const evidenceDir = resolve(".kind-meitner-scratch/verification-logs");
