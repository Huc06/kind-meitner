...
try {
  ...
  const created = await runControlKindMeitner(["new-bot", "--name", "Codex Helper Fixture", "--url", fixture.info.url]) as { bot: { id: string } };
  await runControlKindMeitner(["set-model", "--bot", created.bot.id, "--instance", "codex", "--model", "gpt-fake-default", "--url", fixture.info.url]);
  ...
  if (process.env.KIND_MEITNER_HELPER_EVIDENCE) writeFileSync(process.env.KIND_MEITNER_HELPER_EVIDENCE, JSON.stringify(info, null, 2));
