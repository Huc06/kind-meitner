...
const home = mkdtempSync(join(tmpdir(), "kind-meitner-phone-auth-smoke-"));
...
app.whenReady().then(async () => {
  ...
  const env = {
    HOME: home, USERPROFILE: home, KIND_MEITNER_DATA_DIR: home, PATH: "",
    ...(process.env.SystemRoot ? { SystemRoot: process.env.SystemRoot } : {}),
    KIND_MEITNER_PORT: String(harnessPort), KIND_MEITNER_WEBHOOK_PORT: String(webhookPort),
    KIND_MEITNER_COMPANION_PORT: String(phonePort), KIND_MEITNER_CONTROL_PORT: String(controlPort),
    FAKE_CLAUDE_MODE: "happy",
  };
  const harness = fork("dist-server/index.js", { ...env, KIND_MEITNER_DESKTOP_PARENT: "1" });
  await until(() => api(harnessPort, "/api/health").catch(() => null));
  assert.equal((await api(harnessPort, "/api/bots", "POST", {})).status, 403);
  harness.postMessage({ type: "kind-meitner:desktop-mutation-token", token: owner, companionToken: relay });
  ...
  assert.equal((await api(harnessPort, "/api/bots", "POST", {}, {
    "x-kind-meitner-companion": "1", "x-kind-meitner-companion-device": paired.body.device.id,
  })).status, 403);
  sidecar.postMessage({ type: "kind-meitner:companion-mutation-token", token: relay });
  ...
  const created = await phone("/api/bots", "POST", { modelSelection: { instanceId: "claude", model: "claude-sonnet-5" } }, {
    "x-kind-meitner-companion-auth": "forged", "x-kind-meitner-desktop-owner": "forged",
    "x-kind-meitner-companion-device": "forged-device",
  });
  ...
  harness.postMessage({ type: "kind-meitner:desktop-mutation-token", token: owner, companionToken: nextRelay });
  const restarted = fork("dist-companion/index.js", env);
  restarted.once("spawn", () => restarted.postMessage({ type: "kind-meitner:companion-mutation-token", token: nextRelay }));
  ...
  const evidence = join(tmpdir(), `kind-meitner-phone-auth-${Date.now()}.log`);
