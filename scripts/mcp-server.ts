#!/usr/bin/env node
// Model Context Protocol (MCP) Server for kind-meitner
...
export function validateBaseUrl(url: string): string {
  ...
  try {
    ...
    throw new Error(`Invalid kind-meitner URL: '${url}'`);
  }
  if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
    throw new Error("kind-meitner URL must use http:// or https://");
  }
  if (parsed.username || parsed.password) {
    throw new Error("kind-meitner URL must not contain credentials; use KIND_MEITNER_TOKEN instead");
  }
  if ((parsed.pathname !== "/" && parsed.pathname !== "") || parsed.search || parsed.hash) {
    throw new Error("kind-meitner URL must be an origin without a path, query, or fragment");
  }
...
const configuredUrl = process.env.KIND_MEITNER_URL ||
  (process.env.KIND_MEITNER_PORT ? `http://127.0.0.1:${process.env.KIND_MEITNER_PORT}` : undefined);

export const KIND_MEITNER_BASE_URL = validateBaseUrl(configuredUrl || "http://127.0.0.1:8799");
const DISCOVERY_URLS = configuredUrl
  ? [KIND_MEITNER_BASE_URL]
  : [8799, 18799, 28799].map((port) => `http://127.0.0.1:${port}`);
let discoveredBaseUrl: string | undefined;

export function log(msg: string) {
  process.stderr.write(`[kind-meitner-mcp] ${msg}\n`);
}

export const DEFAULT_REQUEST_TIMEOUT_MS = 30_000;

function requestTimeoutMs(): number {
  const raw = Number(process.env.KIND_MEITNER_MCP_TIMEOUT_MS);
...
function requestHeaders(options: RequestInit): NonNullable<RequestInit["headers"]> {
  const token = process.env.KIND_MEITNER_TOKEN?.trim();
...
async function fetchJson(url: string, options: RequestInit = {}): Promise<any> {
  ...
  if (!response.ok) {
    ...
    if (response.status === 403 && !process.env.KIND_MEITNER_TOKEN?.trim()) {
      throw new Error(
        "kind-meitner refused this write because the installed desktop app requires a paired session token. " +
        "Set KIND_MEITNER_TOKEN as described in docs/mcp-server.md.",
      );
    }
    throw new Error(`kind-meitner API error (${response.status}): ${text || response.statusText}`);
  }
  try {
    return await response.json();
  } catch {
    throw new Error(`kind-meitner API returned a non-JSON response from ${url}`);
  }
}
...
export async function probeBaseUrls(candidates: string[]): Promise<string> {
  ...
  for (const unvalidated of candidates) {
    ...
    try {
      ...
      if (health?.app !== "kind-meitner") {
        failures.push(`${candidate} answered, but it was not kind-meitner`);
        continue;
      }
      return candidate;
    } catch (error) {
  ...
  throw new Error(`Could not find a running kind-meitner server. ${failures.join("; ")}`);
}

export async function resolveBaseUrl(): Promise<string> {
  if (discoveredBaseUrl) return discoveredBaseUrl;
  if (process.env.KIND_MEITNER_TOKEN?.trim() && !configuredUrl) {
    throw new Error("Set KIND_MEITNER_URL or KIND_MEITNER_PORT when using KIND_MEITNER_TOKEN so credentials are never sent during port discovery");
  }
...
export const TOOLS: McpToolDefinition[] = [
  {
    ...
    description: "Check whether the kind-meitner server is reachable.",
    inputSchema: {
      type: "object",
      properties: {},
...
function idArg(args: Record<string, unknown>, key: string): string {
  ...
  if (!/^[\w-]+$/.test(value)) throw new ToolInputError(`${key} is not a valid kind-meitner ID`);
  return value;
}
...
export async function handleToolCall(
  ...
  switch (name) {
    case "get_system_health": {
      ...
      if (res?.app !== "kind-meitner") throw new Error("The configured endpoint is not a kind-meitner server");
      return {
        status: "connected",
        endpoint: discoveredBaseUrl ?? KIND_MEITNER_BASE_URL,
        app: "kind-meitner",
    ...
    case "create_bot": {
      ...
      if (!isRecord(created?.bot) || typeof created.bot.id !== "string") {
        throw new Error("kind-meitner did not return the created bot");
      }
    ...
    case "update_bot_profile": {
      ...
      if (!isRecord(result?.bot)) {
        throw new Error("kind-meitner did not return the updated bot");
      }
    ...
    case "create_channel": {
      ...
      if (!isRecord(created?.group) || typeof created.group.id !== "string") {
        throw new Error("kind-meitner did not return the created channel");
      }
    ...
    case "update_channel": {
      ...
      if (!isRecord(result?.group)) {
        throw new Error("kind-meitner did not return the updated channel");
      }
    ...
    case "create_task": {
      ...
      if (!isRecord(result?.task) || typeof result.task.threadId !== "string") {
        throw new Error("kind-meitner did not return the created task");
      }
    ...
    case "rename_task": {
      ...
      if (!isRecord(result?.task)) {
        throw new Error("kind-meitner did not return the renamed task");
      }
      return {
        success: true,
        targetType: args.target_type,
    ...
    case "set_bot_model": {
      ...
      if (args.task_id !== undefined) {
        ...
        if (!isRecord(res?.task)) throw new Error("kind-meitner did not return the updated task");
  ...
  try {
    ...
    if (method === "initialize") {
      ...
      return formatResponse(id, {
        ...
        serverInfo: {
          name: "kind-meitner-mcp",
          version: "1.1.0",
        },
        instructions: "Use bounded read tools before mutating the kind-meitner team. Approval grants, deletion, and computer lifecycle are intentionally unavailable.",
      });
    }
...
if (process.argv[1] && (process.argv[1].endsWith("mcp-server.ts") || process.argv[1].endsWith("mcp-server.js"))) {
  ...
  log("kind-meitner MCP server running on stdio");
}
