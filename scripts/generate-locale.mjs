...
function translationPrompt(source, label, code) {
  return [
    `Translate this JSON UI catalog for kind-meitner, a multi-agent desktop workbench, into ${label} (${code}).`,
    ...
    "Keep placeholders such as {name} exactly, including duplicates. Keep kind-meitner, CLI, and AI unchanged.",
...
function runModel(prompt) {
  const workDir = mkdtempSync(join(tmpdir(), "kind-meitner-locale-"));
  try {
