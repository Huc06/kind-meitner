...
const home = mkdtempSync(join(tmpdir(), "kind-meitner-approval-smoke-"));
...
app.whenReady().then(async () => {
  ...
  if (process.argv.includes("--skill-ui-only")) {
    await require("./testing/skill-approval-ui-smoke.cjs")({ root, home, url: `http://127.0.0.1:${port}`, api, until,
      capability: (botId, threadId) => api("/api/testing/internal-capability", "POST", { botId, threadId, skillAuthoring: true }, { "x-kind-meitner-test-capability": testCapabilityKey }),
    });
    return;
  }
  ...
  const capability = await api("/api/testing/internal-capability", "POST", { botId: id, threadId: created.body.bot.threadId }, { "x-kind-meitner-test-capability": testCapabilityKey });
