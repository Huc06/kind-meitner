...
describe("docs/verification recipes cite things that exist", () => {
  it("use control:kind-meitner verbs that help lists", () => {
    const used = cited(/pnpm control:kind-meitner ([a-z][\w-]*)/g);
    ...
    const direct = cited(/scripts\/control-kind-meitner\.ts ([a-z][\w-]*)/g);
