...
if (!enabled) console.log("skipping bot tools UI e2e: no agent-browser; set KIND_MEITNER_UI_E2E=1 to install the pinned release");
...
describe("bot setup and tools in the real renderer", () => {
  ...
  (enabled ? it : it.skip)("creates roles, configures per-bot MCP access, and recovers a rejected preset", async () => {
    ...
    await runControlKindMeitner(["send", "--bot", created[0].id, "--text", "Reply briefly for the sidebar test.", "--url", info.url]);
    expect((await runControlKindMeitner(["wait", "--bot", created[0].id, "--timeout", "20", "--url", info.url]) as { status: string }).status).toBe("settled");
    await openTools();
