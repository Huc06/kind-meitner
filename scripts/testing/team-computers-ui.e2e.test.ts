...
if (!enabled) console.info("skipping team computers UI e2e: set KIND_MEITNER_UI_E2E=1 to install the pinned browser");
...
(enabled ? it : it.skip)("creates and assigns a shared computer only after explicit UI actions", async () => {
  ...
  try {
    ...
    if (info) {
      try { evidence.finalUi = await runControlKindMeitner(["ui", "snapshot", "--ui", info.ui]); } catch { /* keep original failure */ }
