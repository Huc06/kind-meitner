...
const enabled = process.env.KIND_MEITNER_UI_E2E === "1" || Boolean(binary);
if (!enabled) console.info("skipping team canvas UI e2e: set KIND_MEITNER_UI_E2E=1 to install the pinned browser");
...
(enabled ? it : it.skip)("edits and arranges the team canvas without losing bots or conversations", async () => {
  ...
  try {
    ...
    const ui = (verb: string, ...args: string[]) => runControlKindMeitner(["ui", verb, "--ui", fixture.ui, ...args]) as Promise<Record<string, any>>;
    ...
    const control = (...args: string[]) => runControlKindMeitner([...args, "--url", fixture.url]) as Promise<any>;
    ...
    const layout = await evaluate("Object.fromEntries(Object.entries(localStorage).filter(([key]) => key.startsWith('kind-meitner-team-canvas:')))");
    ...
    expect(await evaluate("Object.fromEntries(Object.entries(localStorage).filter(([key]) => key.startsWith('kind-meitner-team-canvas:')))"))
      .toEqual(layout);
    expect((await savedBot(ben.id)).section).toBe("Delivery");
    expect(await messages(ben.id)).toEqual(transcript);
    ...
    if (info) {
      if (!receipts.screenshots) {
        try {
          receipts.failureSnapshot = await runControlKindMeitner(["ui", "snapshot", "--ui", info.ui]);
          receipts.failureScreenshot = await runControlKindMeitner(["ui", "screenshot", "--ui", info.ui, "--out", `${info.logPath}.team-canvas-failure.png`]);
