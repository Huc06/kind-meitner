...
// The scripted tool outputs below are local synthetic outcomes only; no public
// MCP endpoint, OKX listing, wallet, or deployed application is contacted.
...
const readinessOutput = JSON.stringify({
  ...
  data: {
    ...
    checks: [
      ...
      { id: "host_pitfall_vercel", status: "fail", detail: "host=demo.vercel.app" },
      { id: "tools_list_http", status: "skipped", detail: "not probed after known host pitfall" },
    ],
    remediation: ["Replace *.vercel.app with a custom domain or Railway/Fly HTTPS host. OKX listing test env rejects vercel.app."],
  },
});
const trustOutput = JSON.stringify({
  resource: { access: "free", paymentRequired: false, walletRequired: false, mainnet: false },
  data: {
    agentId: "99999",
    decision: "NO_GO",
    summary: "Synthetic fixture: listing probe failed, so payment is not approved.",
    signals: [
      { id: "listing_page", status: "fail", detail: "HTTP 404 (synthetic fixture)" },
      { id: "endpoint_readiness", status: "skipped", detail: "no endpointUrl supplied" },
    ],
    notChecked: ["on-chain credit score", "historical settlement volume", "OKX official endorsement", "mainnet payment success"],
    remediation: ["Do not call pay/x402 tools. Fix listing or endpoint first."],
