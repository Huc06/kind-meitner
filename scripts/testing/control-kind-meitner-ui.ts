// `control-kind-meitner ui`: drive the real React renderer headlessly against the
...
//
// Imported by scripts/control-kind-meitner.ts, which owns HELP and the MUTATING set;
...
import {
  ControlKindMeitnerError,
  HELP_UI,
  launchVerificationServer,
  parse,
  runControlKindMeitner,
  type VerificationServer,
} from "../control-kind-meitner.ts";
import {
  closeBrowserSession,
  ensureChrome,
...
export const UI_TOOLS_DIR = join(ROOT, ".kind-meitner-scratch", "verify-tools");
...
function agentBrowser(
  ...
  return new Promise((done, fail) => {
    ...
    try {
      ...
      fail(new ControlKindMeitnerError(`could not start agent-browser: ${error instanceof Error ? error.message : String(error)}`));
      return;
    }
    let output = "";
    let timedOut = false;
    ...
    child.stderr?.resume();
    child.on("error", (error) => { clearTimeout(timer); fail(new ControlKindMeitnerError(`could not start agent-browser: ${error.message}`)); });
    child.on("close", (code) => {
      clearTimeout(timer);
      if (timedOut) {
        fail(new ControlKindMeitnerError(`agent-browser ${args[0]} did not finish within ${timeoutMs}ms`, "check the page with `ui screenshot` or `ui console`"));
        return;
      }
      ...
      const reason = typeof result?.error === "string" ? result.error
        : result?.error && typeof result.error === "object" && typeof (result.error as { message?: unknown }).message === "string"
          ...
          : `exit ${code ?? "by signal"}`;
      fail(new ControlKindMeitnerError(`agent-browser ${args[0]} failed: ${reason.slice(0, 300)}`, "take a fresh `ui snapshot`; refs change after the page updates"));
    });
  });
}
...
/** KIND_MEITNER_AGENT_BROWSER_PATH, then the tools directory, then PATH; otherwise the
...
export async function ensureUiBrowser(
  ...
  if (explicit) {
    if (!existsSync(explicit)) throw new ControlKindMeitnerError(`AGENT_BROWSER_EXECUTABLE_PATH does not exist: ${explicit}`, "unset it to use the Chrome agent-browser installs");
    return { binary, chrome: resolve(explicit) };
  }
  let chrome = installedChrome(toolsDir);
  if (!chrome) {
    const started = Date.now();
    await ensureChrome(binary, { env: sessionEnv({ home: toolsDir, session: "kind-meitner-ui-install", chrome: null }, parentEnv), log });
...
function loadHandle(raw: unknown, verb: string): UiHandle {
  if (typeof raw !== "string" || !raw.trim()) {
    throw new ControlKindMeitnerError(`ui ${verb} requires --ui HANDLE`, "run `ui launch`; it prints the handle path (ui.json inside its data directory)");
  }
  ...
  try {
    ...
    throw new ControlKindMeitnerError(`could not read the ui handle ${path}: ${error instanceof Error ? error.message : String(error)}`, "the launch that wrote it may have stopped; run `ui launch` again");
  }
  for (const key of ["url", "previewUrl", "session", "binary", "home", "botId", "logPath"] as const) {
    if (typeof handle[key] !== "string" || !handle[key]) throw new ControlKindMeitnerError(`the ui handle ${path} lacks ${key}`, "run `ui launch` again and use the handle it prints");
  }
  if (handle.chrome !== null && typeof handle.chrome !== "string") throw new ControlKindMeitnerError(`the ui handle ${path} has an invalid chrome entry`);
  if (!existsSync(handle.home!)) throw new ControlKindMeitnerError(`the ui session's data directory is gone: ${handle.home}`, "its launch was stopped; run `ui launch` again");
...
async function requireLiveSession(handle: UiHandle): Promise<void> {
  ...
  if (!sessions.includes(handle.session)) {
    throw new ControlKindMeitnerError(`the ui session ${handle.session} is not running`, "its launch was stopped or crashed; run `ui launch` again and use the new handle");
  }
}
...
async function resolveTarget(handle: UiHandle, values: Record<string, unknown>, verb: string): Promise<{ target: string; name?: string }> {
  ...
  if (Boolean(ref) === Boolean(name)) throw new ControlKindMeitnerError(`ui ${verb} needs exactly one of --ref @eN or --name NAME`);
  if (ref) {
    if (!/^@?e\d+$/.test(ref)) throw new ControlKindMeitnerError(`--ref must look like @e12, got ${JSON.stringify(ref)}`, "refs come from `ui snapshot`");
...
function parseFlagPatch(raw: unknown): { features: Record<string, boolean | number | string> } {
  ...
  if (!assignments.length) throw new ControlKindMeitnerError("ui flag requires --set features.NAME=VALUE", "example: --set features.showToolCalls=true");
  ...
  for (const assignment of assignments) {
    ...
    if (scope !== "features" || !name || rest.length || !/^[A-Za-z][\w-]*$/.test(name) || separator === -1) {
      throw new ControlKindMeitnerError(`--set must be features.NAME=VALUE, got ${JSON.stringify(assignment)}`, "example: --set features.showToolCalls=true");
    }
...
async function waitSettle(handle: UiHandle, timeoutSeconds: number): Promise<Record<string, unknown>> {
  ...
  for (;;) {
    wait = await runControlKindMeitner(["wait", "--bot", handle.botId, "--timeout", String(Math.min(120, remaining())), "--url", handle.url]) as Record<string, unknown>;
...
export async function runControlKindMeitnerUi(args: string[]): Promise<unknown> {
  const [verb = "help", ...rest] = args;
  if (verb === "help" || verb === "--help" || verb === "-h") return HELP_UI;
  if (verb === "launch") {
    throw new ControlKindMeitnerError("ui launch is available only from the executable CLI", "run `node --experimental-strip-types scripts/control-kind-meitner.ts ui launch`");
  }
  ...
  if (verb === "click" || verb === "type") {
    ...
    if (verb === "type" && typeof text !== "string") throw new ControlKindMeitnerError("ui type requires --text TEXT");
    await requireLiveSession(handle);
  ...
  if (verb === "press") {
    ...
    const handle = loadHandle(values.ui, verb);
    if (typeof values.keys !== "string" || !values.keys.trim()) throw new ControlKindMeitnerError("ui press requires --keys KEYS", "example: --keys Enter or --keys Meta+k");
    await requireLiveSession(handle);
  ...
  if (verb === "screenshot") {
    ...
    const handle = loadHandle(values.ui, verb);
    if (typeof values.out !== "string" || extname(values.out).toLowerCase() !== ".png") throw new ControlKindMeitnerError("ui screenshot requires --out PATH.png");
    ...
    if (!bytes) throw new ControlKindMeitnerError(`agent-browser reported a screenshot it did not write: ${out}`);
  ...
  if (verb === "eval") {
    ...
    const handle = loadHandle(values.ui, verb);
    if (typeof values.js !== "string" || !values.js.trim()) throw new ControlKindMeitnerError("ui eval requires --js CODE");
    await requireLiveSession(handle);
  ...
  if (verb === "wait-settle") {
    ...
    if (!Number.isInteger(timeout) || timeout < 1 || timeout > 600) throw new ControlKindMeitnerError("--timeout must be an integer from 1 to 600");
    await requireLiveSession(handle);
    return waitSettle(handle, timeout);
  }

  throw new ControlKindMeitnerError(`unknown ui command ${JSON.stringify(verb)}`, "run control-kind-meitner ui help");
}
...
export async function launchUi(
  ...
  if (!Object.hasOwn(ENTRIES, entryName)) {
    throw new ControlKindMeitnerError(`unknown --entry ${JSON.stringify(entryName)}`, `available entries: ${Object.keys(ENTRIES).join(", ")}`);
  }
  ...
  if (values["tool-calls"] !== undefined) {
    ...
    if (!Array.isArray(calls) || !calls.every((call) => call && typeof call === "object" && typeof (call as { name?: unknown }).name === "string")) {
      throw new ControlKindMeitnerError("--tool-calls must be a JSON array of {name, input?, ok?}", 'example: --tool-calls \'[{"name":"Bash","input":{"command":"echo hi"},"ok":true}]\'');
    }
    fakeEnv.FAKE_CLAUDE_TOOL_CALLS = JSON.stringify(calls);
  }
  if (values.mode !== undefined) {
    if (!FAKE_MODES.includes(String(values.mode))) throw new ControlKindMeitnerError(`unknown --mode ${JSON.stringify(values.mode)}`, `fake engine modes: ${FAKE_MODES.join(", ")}`);
  ...
  const checkpoint = () => { if (stopRequested) throw new ControlKindMeitnerError("ui launch cancelled"); };
  ...
  try {
    ...
    const created = await runControlKindMeitner(["new-bot", "--name", SEEDED_BOT, "--url", fixture.info.url]) as { bot: { id: string } };
    checkpoint();
    ...
    checkpoint();
    const session: SessionEnv = { home: fixture.info.dataDir, session: `kind-meitner-ui-${new URL(fixture.info.url).port}`, chrome };
