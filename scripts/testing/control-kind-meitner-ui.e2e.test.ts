...
// machine. It spawns the real `control-kind-meitner ui launch` (a child it can Ctrl-C),
...
// directory, KIND_MEITNER_AGENT_BROWSER_PATH or PATH) or when KIND_MEITNER_UI_E2E=1 asks for the
...
if (!enabled) {
  console.log(`skipping control-kind-meitner ui e2e: no agent-browser binary resolves (looked in ${UI_TOOLS_DIR}, KIND_MEITNER_AGENT_BROWSER_PATH and PATH); set KIND_MEITNER_UI_E2E=1 to install the pinned release`);
}
...
const TOOL_CALLS = JSON.stringify([
  { name: "Bash", input: { command: "pnpm control:kind-meitner doctor" }, ok: true },
  { name: "Bash", input: { command: "pnpm control:kind-meitner ui click --name Missing" }, ok: false },
  { name: "Bash", input: { command: "pnpm control:kind-meitner ui flag --set features.showToolCalls=true --dry-run" }, ok: true },
]);
...
// KIND_MEITNER_UI_EVIDENCE_DIR keeps the screenshot (CI uploads it); otherwise it is temporary.
const evidenceDir = process.env.KIND_MEITNER_UI_EVIDENCE_DIR ? resolve(ROOT, process.env.KIND_MEITNER_UI_EVIDENCE_DIR) : mkdtempSync(join(tmpdir(), "kind-meitner-ui-evidence-"));
const ownsEvidenceDir = !process.env.KIND_MEITNER_UI_EVIDENCE_DIR;
...
describe("control-kind-meitner ui drives the real renderer", () => {
  ...
  run("sends a turn from the composer and shows the reply and the scripted tool chip", async () => {
    ...
    expect(handle).toMatchObject({ url: info.url, previewUrl: info.previewUrl, botId: info.botId, home: info.dataDir, session: `kind-meitner-ui-${new URL(info.url).port}` });
    ...
    const models = await runControlKindMeitner(["models", "--url", info.url]) as any;
    ...
    expect(await runControlKindMeitner(["doctor", "--url", info.url])).toMatchObject({ ok: true });
    ...
    expect(draft).toContain("✓ doctor — pnpm control:kind-meitner doctor (verified)\n");
    expect(draft).toContain("✗ ui — pnpm control:kind-meitner ui click --name Missing (verified)\n");
    expect(draft).toContain("[dry run] ui — pnpm control:kind-meitner ui flag --set features.showToolCalls=true --dry-run (verified)\n");
    ...
    expect(runLog).toContain("pnpm control:kind-meitner doctor");
    expect(runLog).toContain("pnpm control:kind-meitner ui click --name Missing");
    ...
    expect(title).toMatchObject({ ok: true, result: "Isolated KindMeitner Chat" });
