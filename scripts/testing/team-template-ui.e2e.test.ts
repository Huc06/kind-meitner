...
if (!enabled) console.log("skipping template UI e2e: set KIND_MEITNER_UI_E2E=1 to install the pinned browser");
...
describe("additive template imports in the real renderer", () => {
  ...
  (enabled ? it : it.skip)("adds library and file templates as sections without losing existing chats", async () => {
    ...
    const control = (...args: string[]) => runControlKindMeitner([...args, "--url", info.url]);
    ...
    const manifest = { format: "kind-meitner.team", version: 2, team: { name: "Sales crew", members: [
    ...
    await evaluate(`(() => {
      ...
      window.fetch = (input, init) => {
        ...
        if (path === '/api/team-library/catalog') return reply({ format: 'kind-meitner.catalog', version: 1, repositoryUrl: '', teams: [{ slug: 'sales', name: 'Sales crew', summary: 'Fixture template', category: 'Sales', members: 2, skills: [], requires: { apps: [] } }] });
    ...
    const evidence = join(ROOT, ".kind-meitner-scratch", "verify-evidence", "template-import-sections.png");
