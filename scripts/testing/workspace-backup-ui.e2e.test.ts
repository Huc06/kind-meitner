...
if (!enabled) console.log("skipping workspace backup UI e2e: no agent-browser; set KIND_MEITNER_UI_E2E=1 to install the pinned release");
...
describe("full backup Settings in the real renderer", () => {
  ...
  (enabled ? it : it.skip)("uses passwords, a file preview and explicit replacement without leaking secrets", async () => {
    let stdout = "";
    let stderr = "";
    child = spawn(process.execPath, ["--experimental-strip-types", join(ROOT, "scripts/control-kind-meitner.ts"), "ui", "launch"], {
    ...
    const ui = (verb: string, ...args: string[]) => runControlKindMeitner(["ui", verb, "--ui", info.ui, ...args]) as Promise<Record<string, any>>;
    ...
    await evaluate(`(() => {
      ...
      localStorage.setItem('kind-meitner-drafts', JSON.stringify({fixture: 'private fixture draft'}));
      localStorage.setItem('fixture-auth-token', 'must not export');
      localStorage.setItem('kind-meitner-webhook-credentials', 'fixture private URL must not export');
      return true;
    })()`);
    ...
    await ui("screenshot", "--out", join(ROOT, ".kind-meitner-scratch", "verify-evidence", "workspace-backup-settings.png"));
    ...
    expect(await evaluate("window.backupFixture.calls.find(call => call.path.endsWith('/export')).body.clientState['kind-meitner-webhook-credentials'] ?? null")).toBeNull();
    ...
    const evidence = join(ROOT, ".kind-meitner-scratch", "verify-evidence", "workspace-backup-preview.png");
    await ui("screenshot", "--out", evidence);
    await click("Replace workspace");
    await expect.poll(snapshot, { timeout: 10_000 }).toContain("Fully quit kind-meitner");
    ...
    expect(await evaluate("localStorage.getItem('kind-meitner-pending-workspace-restore')")).toBe("validated-stage");
