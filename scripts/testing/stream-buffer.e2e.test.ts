...
import { ensureUiBrowser, sessionEnv, UI_TOOLS_DIR } from "./control-kind-meitner-ui.ts";
import { fixtureApi, mountPreview, type MountedPreview } from "./preview-fixture.ts";

const enabled = process.env.KIND_MEITNER_UI_E2E === "1" || !!resolveAgentBrowserBinary({ dataDir: UI_TOOLS_DIR, env: process.env });
if (!enabled) console.info("skipping stream-buffer e2e: no agent-browser resolves; set KIND_MEITNER_UI_E2E=1 to install the pinned tools");
...
it.skipIf(!enabled)("drains pending text and reasoning with rAF paused, then settles without duplicated output", async () => {
  ...
  const env = sessionEnv({ home: fixture.info.dataDir, session: `kind-meitner-stream-${new URL(fixture.info.url).port}`, chrome });
  ...
  const control = (...args: string[]) => runControlKindMeitner([...args, "--url", fixture.info.url]) as Promise<any>;
