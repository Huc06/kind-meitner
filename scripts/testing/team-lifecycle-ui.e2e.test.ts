...
if (!enabled) console.log("skipping team lifecycle UI e2e: set KIND_MEITNER_UI_E2E=1 to install the pinned browser");
...
(enabled ? it : it.skip)("creates an empty team, moves bots, and manages shared instructions in the renderer", async () => {
  ...
  try {
    ...
    const screenshot = join(ROOT, ".kind-meitner-scratch", "verify-evidence", "team-lifecycle.png");
    ...
    if (!succeeded && fixtureHandle && fixtureLog) {
      try {
        const result = await runControlKindMeitner(["ui", "snapshot", "--ui", fixtureHandle]);
        writeFileSync(`${fixtureLog}.team-lifecycle-failure.json`, JSON.stringify(result, null, 2));
        await runControlKindMeitner(["ui", "screenshot", "--ui", fixtureHandle, "--out", `${fixtureLog}.team-lifecycle-failure.png`]);
