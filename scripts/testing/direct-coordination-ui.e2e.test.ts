...
(enabled ? it : it.skip)("coordinates from the real composer and opens the exact child task from its existing inline receipt", async () => {
  const temporary = mkdtempSync(join(tmpdir(), "kind-meitner-direct-ui-plan-"));
  const planPath = join(temporary, "plan.json");
  writeFileSync(planPath, "{}");
  const child = spawn(process.execPath, ["--experimental-strip-types", "scripts/control-kind-meitner.ts", "ui", "launch"], {
  ...
  try {
    ...
    const ui = (verb: string, ...args: string[]) => runControlKindMeitner(["ui", verb, "--ui", info.ui, ...args]) as Promise<any>;
