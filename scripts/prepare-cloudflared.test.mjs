...
describe("cloudflared download retries", () => {
  ...
  function mockDownload() {
    vi.useFakeTimers();
    vi.stubEnv("KIND_MEITNER_CLOUDFLARED_ARCHIVE_DIR", "");
...
describe("pinned cloudflared packaging", () => {
  ...
  it("accepts only the documented current-target CLI option", () => {
    ...
    // `kind-meitner serve --tunnel` stages into its data dir
    expect(parsePrepareCloudflaredArgs(["--current", "--root", "/srv/kind-meitner"])).toEqual({ current: true, root: "/srv/kind-meitner" });
