// End-to-end check of a running kind-meitner harness server — the exact
...
// codex turn, and — with --with-box + KIND_MEITNER_E2E_BOX_TOKEN — box provisioning,
...
const PORT = Number(opt("--port", process.env.KIND_MEITNER_PORT ?? 8799));
const BASE = `http://127.0.0.1:${PORT}`;
const WITH_BOX = flag("--with-box");
const KEEP_BOTS = flag("--keep-bots");
const BOX_TOKEN = process.env.KIND_MEITNER_E2E_BOX_TOKEN ?? "";

const tag = Date.now().toString(36).slice(-6);
const marker = (s) => `kind-meitner-e2e-${s}-${tag}`;
...
async function main() {
  ...
  try {
    ...
    if (byKind.claudeAgent?.snapshot.state === "available") {
      ...
      await send(bot.id, `Run exactly one shell command: curl -sS https://example.com -o kind_meitner_${tag}_allow.html — then tell me the first word of the downloaded file.`);
      ...
      await send(denyBot.id, `Run the shell command "curl -sS https://example.org -o kind_meitner_${tag}_deny.html" — nothing else.`);
    ...
    // ── box: cloud computer ──
    if (WITH_BOX) {
      if (!BOX_TOKEN) fail("--with-box needs KIND_MEITNER_E2E_BOX_TOKEN");
