...
async function main() {
  ...
  const workspace = mkdtempSync(path.join(tmpdir(), "kind-meitner-update-smoke-"));
  ...
  const launched = path.join(installDir, "kind-meitner-0.0.1-x86_64.AppImage");
  copyFileSync(packaged, launched);
  const desktopEntry = path.join(applications, "com.kind-meitner.app.desktop");
  writeFileSync(
    desktopEntry,
    `[Desktop Entry]\nName=kind-meitner\nExec=${launched} %U\nType=Application\n`,
  );
