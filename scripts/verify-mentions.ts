...
const scratch = process.argv.includes("--bot-mentions") ? mkdtempSync(join(tmpdir(), "kind-meitner-verify-mentions-")) : undefined;
...
try {
  ...
  await runControlKindMeitner(["new-channel", "--name", "Design review", "--members", bots.map((b) => b.id).join(","), "--url", fixture.info.url]);
