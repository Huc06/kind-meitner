// Assemble the `kind-meitner` npm package: the self-contained server bundle,
// the built UI, the bundled skills and the CLI, with a package.json of its
// own. `npx kind-meitner serve` then needs Node 24+ and nothing else.
//
...
for (const required of ["dist-server/index.js", "dist-server/kind-meitner.js", "dist/index.html"]) {
...
// Source-available under its own license; inert without KIND_MEITNER_LICENSE_KEY.
  ...
  JSON.stringify(
    {
      name: "kind-meitner",
      version: app.version,
      description: "Run the kind-meitner server anywhere and pair your devices to it",
      license: "Apache-2.0",
      type: "module",
      bin: { "kind-meitner": "cli.js" },
      files: ["cli.js", "dist-server", "dist", "skills", "enterprise", "LICENSE", "README.md"],
      engines: { node: ">=24" },
      repository: { type: "git", url: "https://github.com/harrymove-ctrl/kind-meitner.git" },
      homepage: "https://github.com/harrymove-ctrl/kind-meitner#readme",
      keywords: ["kind-meitner", "agents", "self-hosted", "server"],
    },
    null,
    2,
  ) + "\n",
...
Or run \`npx kind-meitner\` without a global install. Use the same command next
time; \`kind-meitner start\` is an alias for the bare command.
...
Managed access uses a separate kind-meitner account and asks permission for
...
kind-meitner iOS app can also scan or accept the full link. This CLI link
...
\`\`\`sh
kind-meitner setup          # reconfigure AI and optional phone access; not a reset
kind-meitner --no-open      # do not open a browser
kind-meitner --local        # ignore saved remote access for this launch
kind-meitner --no-pair      # suppress phone prompts and invitations
kind-meitner pair           # another phone while the HTTPS workspace is running
kind-meitner sessions       # list devices; sessions revoke ID signs one out
kind-meitner serve          # no onboarding prompts; explicit remote flags for services
\`\`\`
...
Ctrl-C before saving AI setup leaves its pending kind-meitner changes unapplied.
...
\`kind-meitner setup\` to continue without deleting your data.
...
\`login\` command signs in to a kind-meitner account, not an AI provider;
it does not start the tunnel itself.

[Setup guide](https://github.com/harrymove-ctrl/kind-meitner/blob/main/docs/cli-onboarding.md)
· [Hosting guide](https://github.com/harrymove-ctrl/kind-meitner/blob/main/docs/self-hosting.md)
`,
);
console.log(`npm package assembled at ${out} (kind-meitner@${app.version})`);
