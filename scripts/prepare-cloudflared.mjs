...
// kind-meitner updates this dependency with an ordinary reviewed app release.
...
export function parsePrepareCloudflaredArgs(args = []) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (argument === "--current" && !options.current) {
      ...
      // `kind-meitner serve --tunnel` stages into its data dir, not a checkout.
      options.root = next;
      index += 1;
    } else {
...
export async function releaseBytes(asset) {
  const cacheDirectory = process.env.KIND_MEITNER_CLOUDFLARED_ARCHIVE_DIR;
...
async function stageTarget(root, target) {
  ...
  const scratch = mkdtempSync(join(tmpdir(), `kind-meitner-cloudflared-${target}-`));
  try {
