...
try {
  const status = execFileSync("dpkg-query", ["-W", "-f=${db:Status-Abbrev}", "kind-meitner"], {
    encoding: "utf8",
    stdio: ["ignore", "pipe", "ignore"],
  }).trim();
  if (status.startsWith("ii")) fail("refusing to replace a pre-existing kind-meitner installation");
} catch (error) {
  if (String(error?.message ?? error).includes("refusing to replace")) throw error;
}

const temporary = fs.mkdtempSync(path.join(path.resolve(runnerTemp), "kind-meitner-deb-upgrade-"));
...
const legacyApp = path.join(legacyRoot, "opt", "kind-meitner");
const legacyResources = path.join(legacyApp, "resources");
const legacyDeb = path.join(temporary, "kind_meitner_0.1.7_amd64.deb");
...
try {
  ...
  fs.writeFileSync(
    ...
    [
      "Package: kind-meitner",
      "Version: 0.1.7",
      "Architecture: amd64",
      "Maintainer: kind-meitner CI <ci@kind-meitner.invalid>",
      "Description: Legacy kind-meitner directory-mode upgrade fixture",
      "",
    ].join("\n"),
    { mode: 0o644 },
  );
  ...
  for (const directory of ["/opt/kind-meitner", "/opt/kind-meitner/resources"]) {
  ...
  for (const directory of [
    "/opt/kind-meitner",
    "/opt/kind-meitner/resources",
    "/opt/kind-meitner/resources/cua-linux-x64",
  ]) {
  ...
  for (const executable of ["cua-driver", "cua-cursor-theme"]) {
    const file = path.join("/opt/kind-meitner/resources/cua-linux-x64", executable);
  ...
  const chromiumSandbox = "/opt/kind-meitner/chrome-sandbox";
  ...
  const installedVersion = execFileSync("dpkg-query", ["-W", "-f=${Version}", "kind-meitner"], {
    encoding: "utf8",
  }).trim();
  console.log(
