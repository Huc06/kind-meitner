...
if (!podman || !machine) throw new Error("Set KIND_MEITNER_VERIFY_PODMAN and KIND_MEITNER_VERIFY_MACHINE to an explicit test engine");
const output = resolve(process.env.KIND_MEITNER_VERIFY_OUTPUT || ".kind-meitner-scratch/firefox");
...
process.env.KIND_MEITNER_DATA_DIR = resolve(fixtureHome, "app-data");
...
for (const mode of ["before", "after"] as const) {
  const workspace = execFileSync(podman, ["machine", "ssh", machine, "mktemp", "-d", "/tmp/kind-meitner-firefox-XXXXXXXX"], { env: engineEnv, encoding: "utf8", timeout: 30_000 }).trim();
  assert.match(workspace, /^\/tmp\/kind-meitner-firefox-[A-Za-z0-9]+$/);
