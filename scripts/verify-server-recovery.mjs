...
const flag = "--kind-meitner-server-recovery-fixture";
...
if (process.versions.electron && process.argv.includes(flag)) {
  ...
  app.whenReady().then(async () => {
    ...
    const leaseFile = join(dataDir, "kind-meitner-server.lease");
    ...
    const start = async () => {
      const proc = utilityProcess.fork(join(output, "server/index.js"), [], {
        ...
        env: {
          ...
          KIND_MEITNER_DATA_DIR: dataDir, KIND_MEITNER_PORT: String(port), KIND_MEITNER_WEBHOOK_PORT: String(webhookPort),
          KIND_MEITNER_STATIC_DIR: join(output, "ui"), KIND_MEITNER_DESKTOP_PARENT: "1",
      ...
      proc.once("spawn", () => {
        ...
        proc.postMessage({ type: "kind-meitner:desktop-mutation-token", token, companionToken: token });
      });
    ...
    if (scenario === "recover") {
      ...
      assert.equal(JSON.parse(readFileSync(join(dataDir, ".kind-meitner-server-child/kind-meitner-server.lease"), "utf8")).pid, replacementPid);
  ...
  const output = mkdtempSync(join(process.platform === "darwin" ? "/tmp" : tmpdir(), "kind-meitner-server-recovery-"));
