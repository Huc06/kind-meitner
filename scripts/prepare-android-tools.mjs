...
const override = process.env.KIND_MEITNER_ANDROID_PLATFORM_TOOLS_SOURCE;
const temporary = mkdtempSync(join(tmpdir(), "kind-meitner-android-tools-"));
const staged = join(temporary, platform);

try {
  if (override) {
    if (!existsSync(join(override, process.platform === "win32" ? "adb.exe" : "adb"))) {
      throw new Error(`KIND_MEITNER_ANDROID_PLATFORM_TOOLS_SOURCE has no adb: ${override}`);
    }
