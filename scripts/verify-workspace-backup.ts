...
import { launchVerificationServer, runControlKindMeitner, type VerificationServer } from "./control-kind-meitner.ts";
...
async function restartFixture(fixture: VerificationServer): Promise<ChildProcess> {
  ...
  Object.assign(env, {
    ...
    KIND_MEITNER_DATA_DIR: dataDir, KIND_MEITNER_PORT: new URL(fixture.info.url).port,
    KIND_MEITNER_WEBHOOK_PORT: String(Number(new URL(fixture.info.url).port) + 1),
  ...
  try {
    ...
    for (;;) {
      ...
      try {
        ...
        if (response.ok && health.app === "kind-meitner" && health.pid === child.pid) return child;
...
export async function verifyWorkspaceBackup(report: (event: unknown) => void = () => {}) {
  ...
  const control = async (fixture: VerificationServer, ...args: string[]) => {
    const result = await runControlKindMeitner([...args, "--url", fixture.info.url]);
  ...
  try {
    ...
    const clientState = { "kind-meitner-skin": "default", "kind-meitner-drafts": JSON.stringify({ [bot.threadId]: "Keep draft" }), "kind-meitner-webhook-credentials": "private-source-webhook-url", "unrelated-auth-token": "must-not-transfer" };
    ...
    assert.deepEqual(preferences.body.clientState, { "kind-meitner-skin": "default", "kind-meitner-drafts": clientState["kind-meitner-drafts"] });
