...
async function officialBinary() {
  const cache = join(root, "node_modules", ".cache", "kind-meitner", `cua-driver-${release.version}-win`);
  ...
  const response = await fetch(url, {
    headers: { "user-agent": "kind-meitner-packager" },
  ...
  await run("powershell.exe", ["-NoProfile", "-NonInteractive", "-Command",
    "Expand-Archive -LiteralPath $env:KIND_MEITNER_CUA_ARCHIVE -DestinationPath $env:KIND_MEITNER_CUA_EXTRACT"], {
    env: { ...process.env, KIND_MEITNER_CUA_ARCHIVE: archive, KIND_MEITNER_CUA_EXTRACT: cache },
    timeout: 60_000,
  });
...
await build({
  stdin: {
    ...
    resolveDir: root,
    sourcefile: "kind-meitner-cua-entry.mjs",
    loader: "js",
  },
  bundle: true,
  platform: "node",
  ...
  format: "esm",
  banner: {
    js: 'import { createRequire as __kindMeitnerCreateRequire } from "node:module"; const require = __kindMeitnerCreateRequire(import.meta.url);',
  },
  outfile: bundle,
  logLevel: "silent",
});
...
// KIND_MEITNER_CUA_SDK_LIBRARY (set by electron/cua.mjs to the staged DLL)
...
await writeFile(
  ...
  bundledSource.replace(
    resolverPattern,
    `${resolvers[0]}\n      if (process.env.KIND_MEITNER_CUA_SDK_LIBRARY) return resolveOverride(opts.crateName, process.env.KIND_MEITNER_CUA_SDK_LIBRARY);`,
  ),
);
