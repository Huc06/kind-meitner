...
export function hostedCompanionUrl(value: string | undefined): string | null {
  ...
  try {
    ...
    throw new Error("KIND_MEITNER_COMPANION_HOSTED_URL must be an absolute HTTPS origin");
  }
  ...
  if (
    ...
    throw new Error("KIND_MEITNER_COMPANION_HOSTED_URL must be an HTTPS origin without a path, credentials, query, or fragment");
  }
