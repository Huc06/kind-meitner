 ...
 * record here:
 *
 * - **Shared records (§10.2).** The PTR of `_kind-meitner._tcp.local` is
...
export interface ServiceInfo {
  ...
  name: string;
  /** e.g. "_kind-meitner._tcp" */
  type: string;
  port: number;
  /** the name our A records claim, e.g. "kind-meitner-1a2b3c4d.local" */
  host: string;
  addresses: string[];
...
export function dnsLabel(text: string, fallback = "kind-meitner"): string {
  let label = text
...
export function defaultHostName(machine = hostname()): string {
  ...
  return `kind-meitner-${digest}.local`;
}
