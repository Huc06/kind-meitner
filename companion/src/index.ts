...
parentPort?.on("message", ({ data }) => {
  ...
  if (message.type !== "kind-meitner:companion-mutation-token") return;
...
const HARNESS_PORT = num(process.env.KIND_MEITNER_PORT, 8799);
const WEBHOOK_PORT = num(process.env.KIND_MEITNER_WEBHOOK_PORT, HARNESS_PORT + 1);
const COMPANION_PORT = num(process.env.KIND_MEITNER_COMPANION_PORT, 8810);
const CONTROL_PORT = num(process.env.KIND_MEITNER_CONTROL_PORT, 8811);
const SERVICE_TYPE = "_kind-meitner._tcp";
let hostedUrl = hostedCompanionUrl(process.env.KIND_MEITNER_COMPANION_HOSTED_URL);
const PRIVATE_ORIGIN = companionOriginSocket(process.env.KIND_MEITNER_COMPANION_INTERNAL_ORIGIN);
const SECRET_PUBLIC_KEY = normalizedPhoneSecretPublicKey(
  process.env.KIND_MEITNER_PHONE_SECRET_PUBLIC_KEY ?? "",
);
...
let cachedName = process.env.KIND_MEITNER_COMPANION_NAME?.trim() || "";

/** What this computer is called on the phone. Never empty. */
const machineName = (): string => cachedName || "kind-meitner";
...
async function refreshMachineName(): Promise<void> {
  ...
  try {
    ...
    /* not up, or no profile — "kind-meitner" is a fine thing to be called */
  }
}
  ...
  new Promise((resolve, reject) => {
    const onError = (error: NodeJS.ErrnoException) => {
      ...
      const hint = ` — another copy of the companion may already be running; ${
        port === COMPANION_PORT ? "KIND_MEITNER_COMPANION_PORT" : "KIND_MEITNER_CONTROL_PORT"
...
async function main(): Promise<void> {
  const clash =
    conflict("KIND_MEITNER_COMPANION_PORT", COMPANION_PORT) ?? conflict("KIND_MEITNER_CONTROL_PORT", CONTROL_PORT);
  ...
  if (COMPANION_PORT === CONTROL_PORT) {
    throw new Error(
      `KIND_MEITNER_COMPANION_PORT and KIND_MEITNER_CONTROL_PORT are both port ${COMPANION_PORT}, and they cannot share one: ` +
