...
const forwardHeaders = (req: IncomingMessage, authenticatedDeviceId?: string, mutationToken?: string): Record<string, string> => {
  ...
  if (authenticatedDeviceId && /^[\w-]{1,128}$/.test(authenticatedDeviceId)) {
    out["x-kind-meitner-companion-device"] = authenticatedDeviceId;
    if (mutationToken) out["x-kind-meitner-companion-auth"] = mutationToken;
  }
  ...
  const handle = function handle(req: IncomingMessage, res: ServerResponse): void {
    ...
    if (isCloudDesktopAccess(method, path) && !device?.cloudDesktopAccess) {
      return sendJson(res, 403, {
        error: "cloud desktop access is off for this device — enable it in kind-meitner → Settings → Remote access",
      });
    }
      ...
      (harness) => {
        ...
        if (method === "GET" && path === "/api/health") {
          ...
          const fail = () => {
            if (finished) return;
            finished = true;
            sendJson(res, 502, { error: "kind-meitner is not ready on this computer" });
          };
          ...
          harness.on("end", () => {
            ...
            if (
              ...
              (identity as { app?: unknown } | null)?.app !== "kind-meitner"
            ) {
              fail();
              return;
            }
            finished = true;
            sendJson(res, 200, { app: "kind-meitner" });
          });
          return;
        }
        ...
        harness.on("data", (chunk: Buffer) => {
          ...
          if (size > MAX_JSON_BODY_BYTES) {
            ...
            else sendJson(res, 502, { error: "the response from kind-meitner was too large" });
            return;
          }
          chunks.push(chunk);
        });
    ...
    upstream.on("error", () => {
      ...
      sendJson(
        ...
        timedOut ? 504 : 502,
        timedOut
          ? { error: "kind-meitner did not respond" }
          : { error: "kind-meitner is not running on this computer" },
      );
    });
    req.pipe(upstream);
  };
