...
const TOKEN = "kind_meitner_test_token";
...
beforeAll(async () => {
  ...
  harness = spawn(process.execPath, [join(ROOT, "server", "index.ts")], {
    ...
    env: {
      ...
      HOME: home,
      USERPROFILE: home,
      KIND_MEITNER_PORT: String(HARNESS_PORT),
    },
...
describe("the sidecar in front of an unmodified harness", () => {
  ...
  it("requires a paired token", async () => {
    ...
    expect((await device("GET", "/api/bots", { token: "kind_meitner_wrong" })).status).toBe(401);
  ...
  it("serves a minimal, non-cacheable companion health identity", async () => {
    ...
    expect(health.body).toEqual({ app: "kind-meitner" });
...
describe("live companion endpoint refresh", () => {
  it("requires a valid paired bearer and reflects hosted add/remove without restart", async () => {
    ...
    try {
      ...
      endpoints = [
        { kind: "hosted", priority: 0, url: "https://c-opaque.kind-meitner.test" },
...
describe("pairing, end to end", () => {
  it("turns a QR credential into a working device token", async () => {
    ...
    const control = createControlServer({
      devices: registry,
      companionPort: port,
      discovery: () => ({ advertising: false, name: "kind-meitner" }),
    ...
    try {
      ...
      expect(opened.token).toMatch(/^kind_meitner_pair_[A-Za-z0-9_-]{43}$/);
      ...
      expect(body.token).toMatch(/^kind_meitner_/);
      ...
      expect(body.hosts).toEqual(["macbook.tail1234.ts.net", "192.168.1.42", "kind-meitner-abcd1234.local"]);
