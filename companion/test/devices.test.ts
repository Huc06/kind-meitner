...
describe("DeviceRegistry", () => {
  ...
  it("issues a token that authenticates, and never stores it", () => {
    ...
    expect(token.startsWith("kind_meitner_")).toBe(true);
  ...
  it("refuses unknown, empty, and near-miss tokens", () => {
    ...
    expect(registry.authenticate("kind_meitner_nope")).toBeNull();
  ...
  it("replays one logical redemption without creating an orphan device", () => {
    ...
    expect(registry.redeem("kind_meitner_pair_wrong", "iPhone", requestId)).toMatchObject({
      error: expect.stringContaining("no pairing"),
    });
  });
  ...
  it("uses a high-entropy QR credential and burns the manual fallback with it", () => {
    ...
    expect(token).toMatch(/^kind_meitner_pair_[A-Za-z0-9_-]{43}$/);
...
describe("bearerToken", () => {
  it("reads only a well-formed Bearer header", () => {
    expect(bearerToken("Bearer kind_meitner_abc")).toBe("kind_meitner_abc");
    expect(bearerToken("  Bearer kind_meitner_abc  ")).toBe("kind_meitner_abc");
    expect(bearerToken("kind_meitner_abc")).toBeUndefined();
    expect(bearerToken("Basic kind_meitner_abc")).toBeUndefined();
  ...
  it("matches the scheme however it is cased", () => {
    ...
    expect(bearerToken("Bearer   ")).toBeUndefined();
    expect(bearerToken("Bearerkind_meitner_abc")).toBeUndefined();
  });
});
