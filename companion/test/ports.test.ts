 ...
 * KIND_MEITNER_COMPANION_DIR that pins the sidecar's own data directory inside it,
  ...
  new Promise((resolve) => {
    const child = spawn(process.execPath, [ENTRY], {
      env: {
        ...
        ...(process.env.KIND_MEITNER_COMPANION_DIR ? { KIND_MEITNER_COMPANION_DIR: process.env.KIND_MEITNER_COMPANION_DIR } : {}),
        ...env,
      },
...
describe("port conflicts with the harness", () => {
  it("refuses the harness's own port, and says so", async () => {
    const { code, err } = await start({ KIND_MEITNER_PORT: "9100", KIND_MEITNER_COMPANION_PORT: "9100" });
    expect(code).toBe(1);
    expect(err).toContain("KIND_MEITNER_COMPANION_PORT");
  ...
  it("refuses the webhook receiver's port, and names it", async () => {
    const { code, err } = await start({ KIND_MEITNER_PORT: "9100", KIND_MEITNER_COMPANION_PORT: "9101" });
    expect(code).toBe(1);
  ...
  it("refuses it for the control port too", async () => {
    const { code, err } = await start({
      KIND_MEITNER_PORT: "9100",
      KIND_MEITNER_COMPANION_PORT: "9200",
      KIND_MEITNER_CONTROL_PORT: "9101",
    });
    expect(code).toBe(1);
    expect(err).toContain("KIND_MEITNER_CONTROL_PORT");
    expect(err).toContain("webhook receiver");
  }, 20_000);
  ...
  it("refuses to put the device port and the control port on one socket", async () => {
    const { code, err } = await start({
      KIND_MEITNER_PORT: "9100",
      KIND_MEITNER_COMPANION_PORT: "9300",
      KIND_MEITNER_CONTROL_PORT: "9300",
    });
    expect(code).toBe(1);
    expect(err).toContain("KIND_MEITNER_COMPANION_PORT");
    expect(err).toContain("KIND_MEITNER_CONTROL_PORT");
  ...
  // An explicit KIND_MEITNER_WEBHOOK_PORT moves the receiver, which frees the port
  // above the harness. Refusing it anyway would be refusing a port nothing
  // is on.
  it("follows KIND_MEITNER_WEBHOOK_PORT rather than assuming the port above", async () => {
    const { code, err } = await start({
      KIND_MEITNER_PORT: "9100",
      KIND_MEITNER_WEBHOOK_PORT: "9500",
      KIND_MEITNER_COMPANION_PORT: "9500",
    });
    expect(code).toBe(1);
    expect(err).toContain("webhook receiver");
    ...
    const moved = await start({
      KIND_MEITNER_PORT: "9100",
      KIND_MEITNER_WEBHOOK_PORT: "9500",
      KIND_MEITNER_COMPANION_PORT: "9101",
      KIND_MEITNER_CONTROL_PORT: "9500",
    });
    expect(moved.code).toBe(1);
    expect(moved.err).toContain("KIND_MEITNER_CONTROL_PORT");
    expect(moved.err).not.toContain("KIND_MEITNER_COMPANION_PORT");
  }, 40_000);
});
