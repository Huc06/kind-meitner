...
it("adds the private capability only after authenticating and authorizing, never from client headers", async () => {
  ...
  const call = async (path: string, token = "paired", method = "POST") => {
    const res = await fetch(`http://127.0.0.1:${port}${path}`, { method, headers: {
      ...
      "x-kind-meitner-companion-auth": "forged",
      "x-kind-meitner-desktop-owner": "forged-owner",
      "x-kind-meitner-companion-device": "forged-phone",
    } });
    await res.text();
    return res.status;
  };
  ...
  expect(received[0]["x-kind-meitner-companion-auth"]).toBe(capability);
  expect(received[0]["x-kind-meitner-companion-device"]).toBe("real-phone");
  expect(received[0]["x-kind-meitner-desktop-owner"]).toBeUndefined();
  expect(received[0].authorization).toBeUndefined();
  await call("/api/health", "unpaired", "GET");
  expect(received[1]["x-kind-meitner-companion-auth"]).toBeUndefined();
});
