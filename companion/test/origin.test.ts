...
describe("private managed origin", () => {
  it("accepts only the one-generation UDS or named-pipe shape", () => {
    ...
    expect(
      companionOriginSocket("/tmp/kind-meitner-companion-origin-test/origin.sock", "linux"),
    ).toBe("/tmp/kind-meitner-companion-origin-test/origin.sock");
    expect(
      companionOriginSocket(
        "\\\\.\\pipe\\kind-meitner-companion-origin-42-12345678-1234-1234-1234-123456789abc",
        "win32",
      ),
    ).toContain("kind-meitner-companion-origin-42");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const directory = fs.mkdtempSync(path.join(os.tmpdir(), "kind-meitner-companion-origin-test-"));
      directories.push(directory);
      fs.chmodSync(directory, 0o700);
      const socketPath = path.join(directory, "origin.sock");
      const server = createServer((_incoming, response) => {
        response.writeHead(200, { "content-type": "application/json" });
        response.end(JSON.stringify({ app: "kind-meitner" }));
      });
      ...
      expect(JSON.parse(body)).toEqual({ app: "kind-meitner" });
