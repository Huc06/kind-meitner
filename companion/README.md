...
kind-meitner's harness listens on `127.0.0.1` and nothing else, which is the
...
| Environment | Default | |
|---|---|---|
| `KIND_MEITNER_PORT` | `8799` | where the harness is |
| `KIND_MEITNER_WEBHOOK_PORT` | `KIND_MEITNER_PORT` + 1 | the harness's webhook receiver — refused, not used |
| `KIND_MEITNER_COMPANION_PORT` | `8810` | where devices connect |
| `KIND_MEITNER_CONTROL_PORT` | `8811` | the pairing page, loopback only |
| `KIND_MEITNER_COMPANION_DIR` | `~/.kind-meitner-companion` | paired devices live here |
| `KIND_MEITNER_COMPANION_NAME` | your name, from the harness | what the phone calls this computer |

`KIND_MEITNER_COMPANION_NAME` overrides a name the sidecar otherwise asks the harness
for at startup — the profile from onboarding, as *"Ada's computer"*. It falls
back to `kind-meitner` when the harness is not up or has no profile. Read once
...
it (`KIND_MEITNER_WEBHOOK_PORT`). The companion refuses to start on either and says
