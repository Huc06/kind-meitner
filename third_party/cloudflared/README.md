...
kind-meitner stages the official `cloudflared` 2026.8.2 executable as a separate
...
| kind-meitner target | Release asset | Release asset SHA-256 | Extracted executable SHA-256 |
| --- | --- | --- | --- |
...
checked into git. Set `KIND_MEITNER_CLOUDFLARED_ARCHIVE_DIR` to a directory containing
...
The macOS release process applies kind-meitner's Developer ID signature to the
