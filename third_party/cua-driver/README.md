...
These files accompany the exact Cua Driver `0.19.3` Linux x64 runtime bundled by kind-meitner. The upstream release archive omits its own license, the embedded Inter font license, a dependency notice, an SBOM, and artifact provenance, so they are preserved here and verified during packaging.
...
cargo-about generate \
  ...
  --config /path/to/kind-meitner/third_party/cua-driver/about.toml \
  --format json \
...
node /path/to/kind-meitner/scripts/generate-cua-sbom.mjs \
  cua-driver.cargo-about.json \
  cursor-theme.cargo-about.json \
  Cargo.lock \
  /path/to/kind-meitner/third_party/cua-driver
```
...
kind-meitner ships only the CLI and cursor-theme sidecar. The Linux SDK `.so`, Node `.node`, ABI header, and GNOME helper are not included in Phase 5 because the current runtime does not load them and must not silently install a Shell extension.
