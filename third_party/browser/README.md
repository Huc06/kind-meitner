...
- **Windows exception: agent-browser 0.36.0-kind-meitner.1** is a kind-meitner vendor
...
Set `KIND_MEITNER_BROWSER_ARCHIVE_DIR` to a directory of reviewed vendor assets for an
offline build; cached bytes are still verified. Browser dependencies are
updated through reviewed kind-meitner releases, not a silent browser updater.

On macOS, release signing applies kind-meitner's Developer ID to native
...
imply endorsement of kind-meitner. This provenance record is not legal advice
