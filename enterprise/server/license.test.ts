...
describe("license keys", () => {
  ...
  it("accepts a key signed by any listed public key, so rotation is append-only", () => {
    ...
    expect(() => verifyLicenseKey(key, { publicKeys: [fresh.x], now: new Date("2026-12-01") })).toThrow(/does not match any kind-meitner signing key/);
  ...
  it("register() turns a key into the layer contract and passes verification errors through", () => {
    ...
    expect(() => register({ licenseKey: "omb1.bad.key" })).toThrow(/KIND_MEITNER_LICENSE_KEY/);
  });
});
