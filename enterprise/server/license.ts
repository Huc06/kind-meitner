...
// by a kind-meitner signing key. The public keys are baked in below, so one
...
export function verifyLicenseKey(
  ...
  if (parts.length !== 3 || parts[0] !== PREFIX || !parts[1] || !parts[2]) {
    throw new Error(`KIND_MEITNER_LICENSE_KEY is not a kind-meitner license key (expected "${PREFIX}.<claims>.<signature>")`);
  }
  ...
  if (!trusted) {
    throw new Error(
      "KIND_MEITNER_LICENSE_KEY signature does not match any kind-meitner signing key: the key was altered, or it was issued for a different build",
    );
  }
  let parsed: unknown;
  try {
    ...
    throw new Error("KIND_MEITNER_LICENSE_KEY claims are not JSON; ask for the key to be reissued");
  }
  const result = claimsSchema.safeParse(parsed);
  if (!result.success) {
    throw new Error(`KIND_MEITNER_LICENSE_KEY claims are malformed (${result.error.issues[0]?.message ?? "invalid"}); ask for the key to be reissued`);
  }
  const claims = result.data;
  if (claims.expires !== null) {
    const expires = new Date(claims.expires);
    if ((options.now ?? new Date()) >= expires) {
      throw new Error(`KIND_MEITNER_LICENSE_KEY expired on ${claims.expires}; renew it to keep enterprise features`);
    }
  }
  return claims;
}
