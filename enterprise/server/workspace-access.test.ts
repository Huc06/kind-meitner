...
beforeEach(async () => {
  home = mkdtempSync(join(tmpdir(), "kind-meitner-hosted-access-"));
  ...
  remote.mockReset().mockImplementation(async (url, init) => {
    expect(new URL(String(url)).origin).toBe(env.KIND_MEITNER_ADMIN_URL);
  ...
  server = createServer(async (req, res) => {
    const url = new URL(req.url!, env.KIND_MEITNER_PUBLIC_URL);
...
async function begin(workspace = "acme") {
  ...
  expect(target.origin + target.pathname).toBe(`${env.KIND_MEITNER_ADMIN_URL}/connect`);
...
describe("hosted workspace handoff and continuous access", () => {
  ...
  it("lets only verified portal grants use portal membership and still enforces outage, demotion and stream revocation", async () => {
    const portalEnv = { ...env, KIND_MEITNER_ADMIN_MEMBERSHIP: "portal" };
    localScopes = null;
  ...
  it.each([
    { KIND_MEITNER_ADMIN_URL: "http://admin.example.test" }, { KIND_MEITNER_ADMIN_URL: "https://admin.example.test/path/.." },
    { KIND_MEITNER_ADMIN_URL: "https://user:secret@admin.example.test" }, { KIND_MEITNER_ADMIN_WORKSPACE: "" },
    { KIND_MEITNER_PUBLIC_URL: "https://acme.example.test/?redirect=evil" },
    { KIND_MEITNER_ADMIN_MEMBERSHIP: "invalid" },
