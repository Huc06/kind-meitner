...
Set `KIND_MEITNER_LICENSE_KEY` on the server. The key is `omb1.<claims>.<signature>`:
...
{ "edition": "oss", "features": [], "notice": "KIND_MEITNER_LICENSE_KEY expired on 2027-09-02; renew it to keep enterprise features" }
```
...
`~/.config/kind-meitner-enterprise/signing-key.json`). Rotate by generating a
...
Operators opt a workspace into this protocol using `KIND_MEITNER_ADMIN_URL`,
`KIND_MEITNER_ADMIN_WORKSPACE` and `KIND_MEITNER_PUBLIC_URL`; a valid `admin` entitlement is
...
portal sessions with `KIND_MEITNER_ADMIN_MEMBERSHIP=portal`; this never exempts ordinary
email or pairing sessions. See the [protocol and isolated verification recipe](../docs/verification/hosted-workspaces.md).

## White-label (`whitelabel`)

Put a `brand.json` in the server's data dir (`KIND_MEITNER_DATA_DIR`, the `/data`
volume in Docker) or point `KIND_MEITNER_BRAND_FILE` at one:
