...
description: Build and verify the Windows desktop build (NSIS installer + latest.yml) for the canonical kind-meitner release and its legacy updater mirror. Use when cutting a release, shipping a new version to Windows users, or when a Windows user reports they are stuck on an old version. Windows only — does not cover the macOS dmg/notarization flow.
---

# Windows release

Ships `kind-meitner-<version>-setup.exe` and its update feed to
[harrymove-ctrl/kind-meitner](https://github.com/harrymove-ctrl/kind-meitner/releases).
...
| File | Purpose |
|---|---|
| `kind-meitner-<version>-setup.exe` | the installer |
| `latest.yml` | **the update feed** — see step 4 |
| `kind-meitner-<version>-setup.exe.blockmap` | differential updates |
| `kind-meitner-<version>-x64.zip` | portable, not used by the updater |
...
- `app-update.yml` must point at `harrymove-ctrl/kind-meitner` and, while the
...
2. The chat window renders (not the error page). Server logs land in
   `%APPDATA%\kind-meitner\logs\server.log`.
...
```powershell
Copy-Item release/kind-meitner-<version>-setup.exe release/kind-meitner-setup.exe
gh release upload v<version> --repo harrymove-ctrl/kind-meitner `
  release/kind-meitner-<version>-setup.exe `
  release/kind-meitner-setup.exe `
  release/kind-meitner-<version>-setup.exe.blockmap `
  release/latest.yml
```
...
to the matching draft in `harrymove-ctrl/kind-meitner-releases`; never replace the
bytes of an already-published asset.

Both names are required, for different consumers:

- **`kind-meitner-<version>-setup.exe`** is what `latest.yml` references by name and
  sha512. The auto-updater downloads exactly this.
- **`kind-meitner-setup.exe`** is a byte-identical copy that gives the README's
  `/releases/latest/download/kind-meitner-setup.exe` button a stable URL. This
  mirrors `kind-meitner.dmg` sitting beside `kind-meitner-<version>.dmg`.
