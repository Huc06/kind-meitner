...
kind-meitner is open source under the [Apache License 2.0](LICENSE), with one
carve-out and a few notes.

## The carve-out: `enterprise/`

Everything under `enterprise/` is source-available under the
[kind-meitner Enterprise License](enterprise/LICENSE), not Apache 2.0. You may
...
[`third_party/`](third_party/). The kind-meitner name and mascot are trademarks
of Milind Soni; the Apache License does not grant trademark rights (section 6),
so a product built on kind-meitner needs its own name unless a partner agreement
says otherwise.
