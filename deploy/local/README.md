...
`KIND_MEITNER_HTTP_PORT` changes the host port and the default public URL.
...
For Tailscale Serve, set `KIND_MEITNER_PUBLIC_URL` to your HTTPS URL and
`KIND_MEITNER_HTTPS_HOST` to its hostname without scheme or path. Configure Tailscale
Serve on the host to forward to the chosen localhost HTTP port.
Keep the default `KIND_MEITNER_BIND_ADDRESS=127.0.0.1`. Caddy refuses to start when
`KIND_MEITNER_HTTPS_HOST` is set with any other bind address, so direct remote HTTP
...
Without `KIND_MEITNER_HTTPS_HOST`, the bind address can be changed for HTTP access.
Only use HTTPS hostname mapping with a trusted local TLS-terminating proxy.

Sign in and pair a browser:

```sh
docker compose exec kind-meitner codex login --device-auth
docker compose exec kind-meitner node dist-server/kind-meitner.js pair
```
...
volume, set `KIND_MEITNER_DATA_VOLUME` to its name and `KIND_MEITNER_DATA_EXTERNAL=true`.
