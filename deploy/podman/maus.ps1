...
if (-not $args.Count -or $args[0] -in @('--help', '-h', 'help')) {
    ...
    Write-Output 'Options: KIND_MEITNER_PODMAN_MACHINE (default kind-meitner), KIND_MEITNER_PODMAN_ENV_FILE (default .env)'
    exit 0
}
...
$machine = if ($env:KIND_MEITNER_PODMAN_MACHINE) { $env:KIND_MEITNER_PODMAN_MACHINE } else { 'kind-meitner' }
...
$envFile = if ($env:KIND_MEITNER_PODMAN_ENV_FILE) { $env:KIND_MEITNER_PODMAN_ENV_FILE } else { '.env' }
