...
- stable `kind-meitner-amd64.deb` and `kind-meitner.AppImage` copies used by the latest-download links;
...
[kind-meitner releases](https://github.com/harrymove-ctrl/kind-meitner/releases). Then verify the checksum
...
Data lives in `~/.kind-meitner/` (bots, transcripts, per-thread NDJSON event logs, config with keys).
...
- **Never touch the real `~/.kind-meitner`.** The setup file points `HOME` at a temp dir; keep it
  that way.
...
API keys are write-only: they land in `~/.kind-meitner/config.json` via `PUT /api/config` and the API
...
kind-meitner release repository, and never change the upstream publish target in a feature PR unless
