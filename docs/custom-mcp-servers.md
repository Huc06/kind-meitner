...
kind-meitner saves a new server switched off. Use **Test** to start it briefly,
...
`KIND_MEITNER_CLAUDE_INHERIT_USER_CONFIG=1` on the kind-meitner process; it restores
...
session's compaction window with `--autocompact` (2.1.122+). kind-meitner reads
...
The same registry lives in `~/.kind-meitner/config.json`:
...
If you edit the file by hand, restart kind-meitner. Every bot whose engine can
...
- **Testing is bounded.** The test command is stopped after the handshake (or
  ...
  It inherits none of kind-meitner's workspace or provider credentials; only
