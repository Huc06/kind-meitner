...
Cursor is an optional kind-meitner engine. kind-meitner runs the official
...
3. Confirm `cursor-agent --version` works. The binary installs to `~/.local/bin` by
   default; kind-meitner already looks there when launched from a GUI.
...
CLI also implements ACP `session/set_model`, kind-meitner pins the same id over
...
does not provide a bot approval level. kind-meitner app turns always provide
one: both **Ask for approval** and **Approve for me** launch Cursor without
`--force`, then kind-meitner handles its permission requests according to the
bot's current level.
