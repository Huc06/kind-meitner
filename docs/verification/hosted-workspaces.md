...
An operator configures the workspace with an HTTPS `KIND_MEITNER_ADMIN_URL` origin,
its `KIND_MEITNER_ADMIN_WORKSPACE` slug, its exact HTTPS `KIND_MEITNER_PUBLIC_URL`, and an
...
endpoints; requests use the configured `KIND_MEITNER_ADMIN_URL`, not the tenant origin.
...
An operator may explicitly set `KIND_MEITNER_ADMIN_MEMBERSHIP=portal` when the identity
...
includes `{ok:true,service:"kind-meitner",membershipAuthority:"portal",workspace:"<slug>"}`
plus the contract metadata above and the `X-kind-meitner-Hosted-Contract-Version: 1`
