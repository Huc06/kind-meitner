...
kind-meitner uses Qwen's native ACP model selector and requires the CLI to confirm the
...
up on completion. This proves kind-meitner's integration contract, not real provider auth
