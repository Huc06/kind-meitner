...
1. Follow the [fixture launcher](README.md#launch), retaining its printed URL,
   ...
   explicit URL.
2. Start Vite on a separate free port with `KIND_MEITNER_PORT` set to the printed harness
...
3. For domain UI, open Settings → Remote access → Connect your domain. Enter
   ...
   **Advanced server setup**. The launcher deliberately strips operator
   ...
   detection and the `KIND_MEITNER_PUBLIC_IPV4` override are covered by the helper tests.
...
4. To exercise Codex, add an instance to **only the fixture's config.json**:
   ...
   `KIND_MEITNER_DEVICE_AUTH_FIXTURE=1`, `HOME` set to the fixture's temporary directory,
...
6. Create the empty `.kind-meitner-fake-codex-login-approved` marker in the fixture's
