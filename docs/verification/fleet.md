...
privileges, read-only system, the `${KIND_MEITNER_PORT}`-style expansion), the
...
```sh
kind-meitner fleet init --domain example.test --dry-run
kind-meitner fleet create acme --admin owner@acme.test --dry-run
```
...
1. Fresh Ubuntu 24.04 or newer, Caddy from apt, `npm install -g kind-meitner`,
   Claude Code installed once as root, a wildcard DNS record at the server.
2. `kind-meitner fleet init --domain <domain>`; check `systemctl status
   kind-meitner-fence` and `nft list table inet kind-meitner`.
3. `kind-meitner fleet create alpha --admin you@example.test --cap 1` and the
   same for `beta`; both `https://alpha.<domain>` and `https://beta.<domain>`
   must show the pair page with a certificate.
4. Isolation: as `kind-meitner-alpha` (`runuser -u kind-meitner-alpha -- curl -s
   http://127.0.0.1:<beta port>/api/health`) the connection must be refused;
   as root it must answer. `ls /var/lib/kind-meitner/beta` as `kind-meitner-alpha` must
   be denied.
