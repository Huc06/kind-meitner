...
uses the operator's browser profiles, kind-meitner home, provider logins, or API keys.

```sh
KIND_MEITNER_VERIFY_BROWSER_BINARY=/absolute/path/to/agent-browser \
KIND_MEITNER_VERIFY_BROWSER_CHROME=/absolute/path/to/chrome-headless-shell \
