...
```sh
pnpm control:kind-meitner new-channel --name Review --members BOT_A_ID,BOT_B_ID --url http://127.0.0.1:PORT
# Copy channel.id from the JSON above as CHANNEL_ID.
pnpm control:kind-meitner send-channel --channel CHANNEL_ID --text "Reply once" --url http://127.0.0.1:PORT
pnpm control:kind-meitner wait --channel CHANNEL_ID --timeout 60 --url http://127.0.0.1:PORT
pnpm control:kind-meitner messages --channel CHANNEL_ID --limit 20 --url http://127.0.0.1:PORT
```
