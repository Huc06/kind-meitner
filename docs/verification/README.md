# Verifying kind-meitner

kind-meitner has one development control surface: `pnpm control:kind-meitner`. It is a
...
that exact child. The parent shell and the user's kind-meitner data are
...
```sh
pnpm control:kind-meitner doctor --url http://127.0.0.1:PORT
```
...
`control-kind-meitner ui` ([Chat UI, driven headlessly](chat-ui.md)) drives the real
...
service. It does not drive the People settings UI through `control-kind-meitner`.
