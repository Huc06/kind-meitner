...
```sh
pnpm control:kind-meitner new-bot --name Probe --url http://127.0.0.1:PORT
# Copy bot.id from the JSON above as BOT_ID.
pnpm control:kind-meitner send --bot BOT_ID --text "hello" --url http://127.0.0.1:PORT
pnpm control:kind-meitner wait --bot BOT_ID --timeout 30 --url http://127.0.0.1:PORT
pnpm control:kind-meitner messages --bot BOT_ID --limit 10 --url http://127.0.0.1:PORT
```
...
- `set-model --bot BOT_ID --task THREAD_ID --instance INSTANCE_ID --model MODEL_ID`
  changes only that idle task. Get exact available IDs from `control:kind-meitner models`.
...
pnpm control:kind-meitner new-bot --name Orbit --url http://127.0.0.1:PORT
pnpm control:kind-meitner send --bot BOT_ID --text "list my calendar events" --url http://127.0.0.1:PORT
pnpm control:kind-meitner wait --bot BOT_ID --timeout 40 --url http://127.0.0.1:PORT
...
pnpm control:kind-meitner send --bot BOT_ID --text "open a browser and check my calendar" --url http://127.0.0.1:PORT
pnpm control:kind-meitner wait --bot BOT_ID --timeout 40 --url http://127.0.0.1:PORT
```
...
This test launches a fresh `control-kind-meitner` fixture for each case, wraps only its
