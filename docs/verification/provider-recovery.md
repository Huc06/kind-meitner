...
provider charge, deployment, or running kind-meitner workspace is involved.
...
- Grok must deliver the **exact** synthetic PNG bytes to a loopback model.
  ...
  blocks. kind-meitner enables that verified compatibility path only for official
...
- Claude must complete **two** turns using the selected account's personal
  ...
  personal SessionStart hook must not run. kind-meitner retains project settings but
  projects only account authentication from personal settings. Explicit kind-meitner
...
and the account selected in kind-meitner; never request the user's keys or tokens.
...
to `.kind-meitner-scratch/verify-evidence/provider-fixes/`. The fixture exits and removes
