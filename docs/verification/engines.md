...
- Confirm the endpoint identifies itself as kind-meitner.
...
pnpm control:kind-meitner models --url http://127.0.0.1:PORT
```

`doctor.ok` is true only when the endpoint is kind-meitner and at least one
