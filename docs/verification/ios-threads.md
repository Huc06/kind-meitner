...
iPhone or iPad simulator, then run the `KindMeitnerCompanion` scheme's UI tests
against that explicit simulator ID. For example, from `ios/`:

```sh
xcodebuild -project KindMeitnerCompanion.xcodeproj -scheme KindMeitnerCompanion \
  -configuration Debug -destination 'platform=iOS Simulator,id=SIMULATOR_ID' \
  -derivedDataPath /tmp/kind-meitner-ios-threads-build CODE_SIGNING_ALLOWED=NO test
```
...
the [iOS end-to-end runbook](https://github.com/milind-soni/OpenMausBot/blob/main/ios/TESTING.md) against an isolated companion
...
- Retained results: `/tmp/kind-meitner-ios-threads-iphone-acceptance.xcresult` and
  `/tmp/kind-meitner-ios-threads-ipad-clean.xcresult`, including screenshots.
