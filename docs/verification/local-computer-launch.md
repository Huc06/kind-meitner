...
and focuses the existing kind-meitner window instead of answering MCP requests.
...
distinguishes window delivery from full-desktop input. kind-meitner retains its existing
...
Do not use the operator's kind-meitner data or authenticated browser profiles as fixtures.
