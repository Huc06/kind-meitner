...
`/mnt/c/Projects/kind-meitner`:

```sh
cd "$repo"
podman build -f deploy/podman/Containerfile -t localhost/kind-meitner-podman:verify .
fixture=$(mktemp -d /tmp/kind-meitner-podman-verify-XXXXXXXX)
mkdir -m 700 "$fixture/.kind-meitner"
...
cat > "$fixture/.kind-meitner/config.json" <<EOF
...
umask 077
cat > .env.fixture <<EOF
COMPOSE_PROJECT_NAME=kind-meitner-podman-verify
KIND_MEITNER_IMAGE_TAG=verify
KIND_MEITNER_DATA_ROOT=$fixture
PODMAN_SOCKET=/run/user/$(id -u)/podman/podman.sock
KIND_MEITNER_PORT=28799
KIND_MEITNER_WEBHOOK_PORT=28800
KIND_MEITNER_HTTP_PORT=28880
KIND_MEITNER_PUBLIC_URL=http://localhost:28880
...
Use the candidate checkout's `control-kind-meitner.ts` on the host. The explicit fixture
