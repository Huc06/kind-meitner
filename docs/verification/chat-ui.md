...
The `ui` group of `control-kind-meitner` drives the real React renderer — the same
...
`scripts/testing/control-kind-meitner-ui.ts`). Everything it touches is disposable: the
...
8799 and `~/.kind-meitner` are never involved.

## Launch

```sh
node --experimental-strip-types scripts/control-kind-meitner.ts ui launch \
...
Testing into `.kind-meitner-scratch/verify-tools` (gitignored); later launches reuse
them. `KIND_MEITNER_AGENT_BROWSER_PATH` and `AGENT_BROWSER_EXECUTABLE_PATH` take
...
session named `kind-meitner-ui-<port>`, and prints a handle:

```json
{
  "ok": true,
  "ui": "/tmp/kind-meitner-verify-data-XXXXXX/ui.json",
...
```sh
H=/tmp/kind-meitner-verify-data-XXXXXX/ui.json
pnpm control:kind-meitner ui flag --ui $H --set features.showToolCalls=true --dry-run
pnpm control:kind-meitner ui flag --ui $H --set features.showToolCalls=true
pnpm control:kind-meitner ui snapshot --ui $H --interactive
pnpm control:kind-meitner ui type --ui $H --name "Message Pepper" --text hello
pnpm control:kind-meitner ui press --ui $H --keys Enter
pnpm control:kind-meitner ui wait-settle --ui $H --timeout 60
pnpm control:kind-meitner ui snapshot --ui $H
pnpm control:kind-meitner ui click --ui $H --name "Save as skill"
pnpm control:kind-meitner ui eval --ui $H --js "document.querySelector('textarea[aria-label=\"Message Pepper\"]').value"
```
...
```sh
pnpm control:kind-meitner ui screenshot --ui $H --out .kind-meitner-scratch/verify-evidence/chat-ui.png
pnpm control:kind-meitner ui console --ui $H
pnpm control:kind-meitner ui eval --ui $H --js "document.title"
```
...
The permanent form of this recipe is `scripts/testing/control-kind-meitner-ui.e2e.test.ts`:

```sh
KIND_MEITNER_UI_E2E=1 pnpm exec vitest run scripts/testing/control-kind-meitner-ui.e2e.test.ts
```
...
reason otherwise; `KIND_MEITNER_UI_E2E=1` forces the verified download. The `ui-smoke`
...
```sh
KIND_MEITNER_UI_E2E=1 pnpm exec vitest run scripts/testing/stream-buffer.e2e.test.ts
...
optional `KIND_MEITNER_UI_E2E=1` gate. It verifies profile-only role creation, closing
