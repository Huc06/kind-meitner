...
`KIND_MEITNER_VERIFY_PODMAN`, `KIND_MEITNER_VERIFY_MACHINE`, and `KIND_MEITNER_VERIFY_IMAGE`, then run:
...
engine-host temporary workspaces. Use `KIND_MEITNER_VERIFY_OUTPUT` for a separate evidence
...
Each container has a unique `kind-meitner-font-fixture-` name. The script resolves the
