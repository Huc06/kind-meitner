...
granting `budgets` and `billing`) and launches the `control-kind-meitner` fixture with
...
For the same by hand, launch a fixture with `KIND_MEITNER_ENTERPRISE_DIR` pointing at a
folder whose `server/index.js` exports such a `register()`, and
`KIND_MEITNER_LICENSE_KEY` set to any value, then use the normal chat-turn commands.
