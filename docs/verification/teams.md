...
KIND_MEITNER_UI_E2E=1 pnpm exec vitest run scripts/testing/team-lifecycle-ui.e2e.test.ts scripts/testing/team-template-ui.e2e.test.ts --maxWorkers=1
KIND_MEITNER_UI_E2E=1 pnpm exec vitest run scripts/testing/team-canvas-ui.e2e.test.ts --maxWorkers=1
KIND_MEITNER_UI_E2E=1 pnpm exec vitest run scripts/testing/team-computers-ui.e2e.test.ts --maxWorkers=1
...
explicit empty-team deletion. API requests and `control-kind-meitner` wait/messages
...
The renderer test uses `control-kind-meitner ui launch`, opens **Create team**, leaves it
...
`.kind-meitner-scratch/verify-evidence/team-lifecycle.png` before deletion.
...
The separate `team-computers-ui` test uses a real kind-meitner server and renderer with
