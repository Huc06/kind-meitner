...
KIND_MEITNER_UI_E2E=1 pnpm exec vitest run scripts/testing/cron-routines-ui.e2e.test.ts
```
...
- **Manual inbox check** is created through the routine API and run twice for
  ...
  the **kind-meitner management** folder. Its next scheduled occurrence is an hour later.
...
8. Expand Pepper's threads. **kind-meitner management** contains one **Fleet health
...
```sh
pnpm control:kind-meitner wait --bot BOT_ID --task THREAD_ID --timeout 30 --url http://127.0.0.1:PORT
pnpm control:kind-meitner messages --bot BOT_ID --task THREAD_ID --limit 20 --url http://127.0.0.1:PORT
```
