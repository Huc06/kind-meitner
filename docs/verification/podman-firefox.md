...
Set `KIND_MEITNER_VERIFY_PODMAN` and `KIND_MEITNER_VERIFY_MACHINE` explicitly, with the managed
...
`.kind-meitner-scratch/firefox` (override with `KIND_MEITNER_VERIFY_OUTPUT`).
