...
```sh
KIND_MEITNER_UI_E2E=1 pnpm exec vitest run scripts/testing/team-template-ui.e2e.test.ts --silent=false
```

This owns a disposable `control-kind-meitner ui` app. It sends a fixture conversation,
...
`.kind-meitner-scratch/verify-evidence/template-import-sections.png`.
