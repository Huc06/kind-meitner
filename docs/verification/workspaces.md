...
On a real server, after `kind-meitner fleet init --domain <domain> --operator
<user>` as root: `systemctl status kind-meitner-fleet`, then `ls -l
/run/kind-meitner/fleet.sock` must show `root:<user>` and mode 660. Sign in to
the operator workspace and open Settings → Workspaces. Create one, add a
member, suspend and resume it, and confirm `/var/log/kind-meitner/fleet.jsonl`
