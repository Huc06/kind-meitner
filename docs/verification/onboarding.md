...
```sh
node --experimental-strip-types scripts/verify-onboarding-ui.ts /tmp/kind-meitner-verify-data-XXXXXX/ui.json
```
...
are retained in `.kind-meitner-scratch/verify-evidence/onboarding/` with a PASS line for
