...
kind-meitner can turn a Linux server you already own into a bot's computer. The agent process stays on your
...
only surface exposed: kind-meitner never opens a public port on the VPS, never stores your SSH key or
...
kind-meitner connects only through a named alias in your `~/.ssh/config` — you type the alias into
...
**Host key first, by hand.** Connect once manually before pointing kind-meitner at the alias:
...
- **No public ports.** The managed container is created with no published ports, and kind-meitner refuses to
...
- **Firewall the VPS to SSH only**, ideally from your IP. Nothing kind-meitner does needs any other inbound
  port open, so anything else open is pure attack surface.
- **Nothing sensitive is stored.** The only thing kind-meitner persists is the alias name itself
  (`~/.kind-meitner/config.json`); keys, passphrases, and agent state stay with SSH. The alias is also kept
...
Each bot owns one container on the VPS, named `kind-meitner-vps-<bot>-<hash>` — stable across restarts and
...
- **Remove** is yours, done by hand when a bot no longer needs the server:
  `docker -H ssh://my-vps rm -f <container>`. kind-meitner never deletes a container on its own.
