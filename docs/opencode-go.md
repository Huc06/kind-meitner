...
OpenCode is an optional kind-meitner engine. kind-meitner runs the maintained
...
2. Connect the providers you want in the OpenCode app, or run
   `opencode auth login`.
3. Restart kind-meitner. It reuses OpenCode's existing connections and model
...
kind-meitner does not copy or rewrite `auth.json`. The OpenCode CLI remains the
owner of provider authentication, and the same Zen or Go connection used by
the OpenCode desktop/TUI is used by kind-meitner.
