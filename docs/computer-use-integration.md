# Computer use & browser use in kind-meitner

Decision doc, 2026-08-12. How bots in kind-meitner get local computer use and
...
supply-chain contract tracked in [#113](https://github.com/harrymove-ctrl/kind-meitner/issues/113): pinned archive
...
receives the validated MCP proxy contract. Xorg is tracked in [#79](https://github.com/harrymove-ctrl/kind-meitner/issues/79);
GNOME/Wayland additionally requires WinRects v8 plus the exact Cua health-report contract tracked in
[#109](https://github.com/harrymove-ctrl/kind-meitner/issues/109).
...
1. **Spawn from the Electron main process, never from the server/gateway
   ...
   Spawned from Electron main → the grant is kind-meitner's, users see ONE
   prompt named kind-meitner, and the bundled driver inherits it. Spawned from
...
- Ship the binary at `kind-meitner.app/Contents/Resources/cua-driver`,
