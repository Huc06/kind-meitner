# Self-hosting the kind-meitner server
...
- a browser for bots, once the engine is installed on the server
  (`npx kind-meitner browser install`, or nothing to do in the Docker image,
...
It then starts the server and keeps your data in `~/.kind-meitner`. If you
...
and creates no pairing invitation. Use `npx kind-meitner setup` to configure without starting, or
`npx kind-meitner serve` to start non-interactively with your existing config
...
directories, not all under `.kind-meitner`.
...
`npx kind-meitner browser install` as the service user so that user's browser
is present. Three ways to make the server reachable from elsewhere:

- **On your Tailscale network, no domain needed:**
  `npx kind-meitner serve --tailscale`. Tailscale terminates HTTPS with its
...
- **A public address, no domain, no proxy, no open port:**
  ...
  ```sh
  npx kind-meitner login          # once: an emailed code signs this machine in
  npx kind-meitner serve --tunnel
  ```

  `login` reserves an address like `https://c-….kind-meitner.com` for this
  ...
  the server treats as "through a proxy", never as the owner. `npx kind-meitner
  logout` releases the address. The account credentials live in
  `~/.kind-meitner/tunnel-account.json` (mode 0600).
  Starting it from a fleet or a container, where nobody can type an emailed
  code? Set `KIND_MEITNER_INSTALLATION_CREDENTIAL` to the installation credential the
...
- **Your own domain, still one command:**
  ...
  ```sh
  npx kind-meitner serve --domain maus.example.com
  ```
...
- **Behind your own proxy or domain:** `npx kind-meitner serve --public-url
  https://maus.example.com`, with the proxy rules from "Putting a proxy in
  front".

Later: `npx kind-meitner pair --label "Kitchen iPad"` for another device
(`--client` for one that may chat but not change settings), and
`npx kind-meitner sessions` to see or revoke them. `kind-meitner serve` is a
...
terminal: **Settings → Engines → Codex → Connect ChatGPT**. kind-meitner starts
...
with your own account. kind-meitner checks for completion and refreshes the model list.
...
user as kind-meitner. Your password never goes through kind-meitner; Codex stores its credentials
...
and choose **Sign out of ChatGPT**: kind-meitner runs `codex logout` on the server as
...
set `KIND_MEITNER_PUBLIC_IPV4` to the public IPv4 of the HTTPS proxy and restart kind-meitner.
...
2. Configure HTTPS with a reverse proxy such as Caddy. The Settings example uses
   ...
   is the reference. Keep kind-meitner listening on loopback, forward the original Host
...
3. Enter `bots.yourcompany.com` (or its bare `https://` origin) and choose
   **Connect domain**. kind-meitner checks HTTPS and the workspace identity at that
...
`KIND_MEITNER_WEBHOOK_PUBLIC_URL`, existing webhook URLs, or the desktop companion's
...
(`ghcr.io/harrymove-ctrl/kind-meitner`, tagged `latest`, `sha-…` and `v…`).
...
```sh
docker compose exec kind-meitner claude                       # each CLI you listed in ENGINES
docker compose exec kind-meitner node dist-server/kind-meitner.js pair # prints a code, a link and a QR
```
...
`KIND_MEITNER_WEBHOOK_PUBLIC_URL`.
...
Upgrade with `docker compose pull kind-meitner && docker compose up -d` (or
...
```sh
git clone https://github.com/harrymove-ctrl/kind-meitner && cd kind-meitner
pnpm install

# choose where data lives and start the server
KIND_MEITNER_DATA_DIR="$HOME/.kind-meitner" KIND_MEITNER_PORT=8799 \
...
```sh
npx kind-meitner service install --domain maus.example.com   # or --tunnel, --tailscale, or nothing
```

It renders a systemd unit (Linux) or a launchd agent (macOS) that runs the
same `kind-meitner serve …` with your options, restarts it if it stops, and,
...
`kind-meitner service uninstall` prints the reverse. Install the package
permanently first (`npm install -g kind-meitner`): a service must not point
...
the copy kind-meitner installed is the one bots run. The package name comes from the
...
- **Anthropic API key**: while one is saved, every Claude bot runs on it and
  ...
  page, `config.json`, or `KIND_MEITNER_ANTHROPIC_API_KEY`.
...
`kind-meitner fleet` runs one workspace per client on a single Linux server,
each as its own OS user, its own `kind-meitner@<name>` service on its own
...
```sh
kind-meitner fleet init --domain example.com
```

That writes the template unit, the fence and its unit, the workspace folders,
and adds `import /etc/caddy/kind-meitner.d/*.caddy` to `/etc/caddy/Caddyfile`. Then per
client:

```sh
kind-meitner fleet create acme --admin owner@acme.test --member @acme.test \
  --brand /root/acme-brand.json --anthropic-key-file /root/acme-anthropic.key \
  --cap 50 --memory 1G
kind-meitner fleet users acme add bob@acme.test --chat-only
kind-meitner fleet list
kind-meitner fleet suspend acme      # 503 page, service stopped; resume undoes it
kind-meitner fleet upgrade           # new release, then every running workspace restarted in turn
kind-meitner fleet delete acme --yes # add --keep-data to keep the home folder
```
...
agent's audit log at `/var/log/kind-meitner/fleet.jsonl`.

`https://acme.example.com` is up when `create` returns; the first admin signs
in with an emailed code. `KIND_MEITNER_LICENSE_KEY` in the environment (or
...
```sh
npx kind-meitner pair                         # npm install
pnpm kind-meitner pair                                # from a checkout
docker compose exec kind-meitner node dist-server/kind-meitner.js pair   # Docker
```

It prints a 12-character code (single use, five minutes) and, when the
server knows its public address (`KIND_MEITNER_PUBLIC_URL`, set by the Docker stack),
...
link for each device: `npx kind-meitner pair --label "My desktop"` creates an
...
`GET /.well-known/kind-meitner/environment` is public and tells a client what
...
```sh
KIND_MEITNER_SIGNIN_EMAILS="her@yourcompany.com, @yourcompany.com"   # full access
KIND_MEITNER_SIGNIN_MEMBER_EMAILS="freelancer@example.com"          # chat and approvals only
```
...
```sh
npx kind-meitner access add her@yourcompany.com
npx kind-meitner access add freelancer@example.com --chat-only
npx kind-meitner access list
```

An entry is an address or `@domain` (everyone at that domain). Admins get
the same access as a pairing code from `kind-meitner serve`; members get the
chat-only scope, the same as `kind-meitner pair --client`. The same lists live
...
The code itself comes from `accounts.kind-meitner.com`, the kind-meitner
...
`kind-meitner sessions` and can be revoked the same way.
...
Plus one convenience: set `KIND_MEITNER_PUBLIC_URL=https://your.domain` so pairing
links, and `KIND_MEITNER_WEBHOOK_PUBLIC_URL=https://your.domain` so hook URLs, are
...
code that `kind-meitner serve` (or `kind-meitner pair`) prints, paste the
...
its own, listed and revocable with `kind-meitner sessions`. What it may do is
the code's scope: a code from `kind-meitner pair` carries `admin` and the app
shows everything; a code from `kind-meitner pair --client` (also what the
...
For a foreground invocation, `npx --yes kind-meitner@X.Y.Z serve --tunnel`
selects a particular published release; replace `X.Y.Z` with that version.

```sh
docker compose -f deploy/docker-compose.yml pull kind-meitner && docker compose -f deploy/docker-compose.yml up -d   # Docker
git pull && pnpm install && sudo systemctl restart kind-meitner          # from source
```
