...
Add OpenCode Go as an optional first-class kind-meitner engine by launching the maintained OpenCode CLI through its ACP stdio interface. The integration must preserve the existing session, streaming, tool, permission, cancellation, MCP, credential, and model-selection contracts.
...
4. ACP events stream text and tool activity through existing kind-meitner event handling.
