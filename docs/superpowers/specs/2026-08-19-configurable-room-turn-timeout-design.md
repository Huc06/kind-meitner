...
kind-meitner currently stops every room member turn after five minutes, even when the engine is still producing output. The duration and the error message are hard-coded in `server/index.ts`. This behavior is separate from the activity-based turn stall watchdog controlled by `KIND_MEITNER_TURN_STALL_MS`.
...
- Changing `KIND_MEITNER_TURN_STALL_MS` or the semantics of the stall watchdog.
