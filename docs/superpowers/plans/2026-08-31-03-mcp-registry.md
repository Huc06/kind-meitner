...
- Produces: a process spawned as `node mcp-user-entry.ts <serverName>`, reading its definition and its allowlist from env — `KIND_MEITNER_MCP_SERVER` (JSON definition) and `KIND_MEITNER_MCP_ALLOW` (comma-separated tool names, or `*`).

- [ ] Write a failing test: with `KIND_MEITNER_MCP_ALLOW="read_file"`, a `tools/call` for `read_file` reaches the fake child and a call for `write_file` is refused with a message naming the tool and saying it is not granted to this bot.
- [ ] Write a failing test that `KIND_MEITNER_MCP_ALLOW="*"` forwards everything, and that an **absent** `KIND_MEITNER_MCP_ALLOW` refuses everything. Default-deny: a bridge started without a policy must not become a bridge with no policy.
...
- [ ] Run; expect FAIL.
- [ ] Add the contract fields. In `server/index.ts`, build `integrations.custom` from `bot.mcpGrants` × the registry, spawning each through `mcp-user-entry.ts` with `KIND_MEITNER_MCP_SERVER` and `KIND_MEITNER_MCP_ALLOW` set. Declare `customMcp: true` on claude, antigravity, and pi; leave it absent everywhere else.
