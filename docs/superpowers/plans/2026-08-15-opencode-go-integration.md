...
**Goal:** Add OpenCode Go as an optional ACP-backed kind-meitner engine with dynamic models, write-only credentials, setup UX, and comprehensive tests.
...
- [ ] Add cross-platform setup commands, docs URL, and a login note that never asks kind-meitner to edit OpenCode auth files.
