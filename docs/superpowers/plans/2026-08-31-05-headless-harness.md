...
**Goal:** `kind-meitner serve` — a daemon that runs the harness, the companion sidecar, the webhook receiver, and the managed tunnel with no Electron and no desktop session, so routines and webhooks keep running when the laptop is closed, and the desktop and phone become clients that can list more than one machine.
...
- **Superseded (Sep 2026):** the harness now authenticates remote clients itself (`docs/plans/remote-workspace.md`: pairing codes, sessions, and a "through a proxy" rule for forwarded requests), and `kind-meitner serve --tunnel` (`server/tunnel.ts`) opens a second, IPC listener for the tunnel gateway on which every request is remote by construction. The loopback bind and its owner trust are unchanged; the sidecar is no longer the only door.
...
Status section, and it is the single biggest gap between kind-meitner and an "AI employee".
...
- Create `server/serve.ts`: the `kind-meitner serve` entry point — flags, logging, signal handling.
...
- Create `docs/headless.md`, `build/kind-meitner.service`, `build/com.kind-meitner.serve.plist`.
...
- Modify: `package.json` (`"serve"` script, `"bin": { "kind-meitner": "..." }`)
...
- [ ] Run; expect PASS.
- [ ] Commit `feat(headless): kind-meitner serve`.
...
- [ ] Run; expect FAIL.
- [ ] Implement. `name` defaults to the OS hostname and is overridable by `KIND_MEITNER_MACHINE_NAME`, because three VPS boxes all called `localhost` is not a roster.
- [ ] Run; expect PASS.
...
- [ ] Write the systemd unit: `Type=simple`, `Restart=on-failure`, `RestartSec=5`, a dedicated non-root user, `StateDirectory=kind-meitner`, and `ProtectSystem=strict` with `~/.kind-meitner` as the only writable path.
