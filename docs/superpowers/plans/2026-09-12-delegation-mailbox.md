...
- Work only in the worktree `~/Desktop/kind-meitner/OpenGrokBot-mailbox`, on branch `feat/delegation-mailbox`. Never touch `~/Desktop/kind-meitner/OpenGrokBot`.
...
```
"List the other bots (agents) in your kind-meitner section, with their model and whether they're busy.
```

to:

```
"List the other bots (agents) in your kind-meitner section, with their model and what each is doing right now (available, working, waiting on the user, not responding, or unavailable).
```
