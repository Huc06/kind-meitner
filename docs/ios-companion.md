...
The iOS app is a thin, native client for the kind-meitner instance running on
...
a kind-meitner account of its own.
...
The Mac must be running kind-meitner and must not be asleep. Desktop
...
The sidecar keeps its device registry in `~/.kind-meitner/devices.json`. That is
...
to reach. kind-meitner then places the computer's MagicDNS name in that
...
Tailscale is optional. The direct path does not use a kind-meitner-operated
...
The kind-meitner control plane stores account and installation metadata plus
...
The Mac must remain awake with kind-meitner running for chats, approvals, and
...
A kind-meitner account is not required for nearby, manual, or Tailscale
...
Passwords or any enabled third-party AutoFill provider. kind-meitner-mobile does
...
```text
kind-meitner-phone-credential-v1
<key id>
...
swift test
xcodegen generate
xcodebuild -project KindMeitnerCompanion.xcodeproj \
  -scheme KindMeitnerCompanion \
  -sdk iphonesimulator \
