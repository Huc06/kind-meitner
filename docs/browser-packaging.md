...
The server receives `KIND_MEITNER_RESOURCES_PATH` from Electron. It resolves an explicit
`KIND_MEITNER_AGENT_BROWSER_PATH` override first, then the complete bundled engine and
...
There is no recording control in kind-meitner's browser panel.

## Linux sandbox

Use the `.deb` on Ubuntu 24.04. Its package hooks install a narrowly scoped
AppArmor policy for the root-owned browser executable under `/opt/kind-meitner`.
...
On macOS, use `/absolute/kind-meitner.app/Contents/Resources`. On Linux, run as
an unprivileged user against the installed `.deb` at
`/opt/kind-meitner/resources`. This check creates its own empty home and local
...
Windows uses an explicitly identified kind-meitner build of agent-browser
...
native-verified `0.36.0-kind-meitner.1` revision and does not yet get the new recording
