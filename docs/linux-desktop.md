...
kind-meitner has an Ubuntu 24.04 LTS x86_64 desktop beta. The Electron package embeds the harness server, so
...
progress in [issue #29](https://github.com/harrymove-ctrl/kind-meitner/issues/29) and the safety hold in
[issue #345](https://github.com/harrymove-ctrl/kind-meitner/issues/345). Bundled
CUA supply-chain work is tracked in [issue #113](https://github.com/harrymove-ctrl/kind-meitner/issues/113). Xorg is tracked in
[issue #79](https://github.com/harrymove-ctrl/kind-meitner/issues/79), and guarded GNOME/Wayland support in
[issue #109](https://github.com/harrymove-ctrl/kind-meitner/issues/109).

## Download packages

Choose one Ubuntu 24.04 x86_64 package from the latest release:

- [Debian package (`kind-meitner-amd64.deb`)](https://github.com/harrymove-ctrl/kind-meitner/releases/latest/download/kind-meitner-amd64.deb) — recommended; APT installs its desktop dependencies and configures the bundled bot browser's sandbox.
- [Portable AppImage (`kind-meitner.AppImage`)](https://github.com/harrymove-ctrl/kind-meitner/releases/latest/download/kind-meitner.AppImage) — does not install system files.
- [SHA-256 checksums](https://github.com/harrymove-ctrl/kind-meitner/releases/latest/download/SHA256SUMS-ubuntu-x64.txt)

Versioned packages and previous releases remain available on the
[releases page](https://github.com/harrymove-ctrl/kind-meitner/releases).
...
- `release/kind-meitner-<version>-amd64.deb`
- `release/kind-meitner-<version>-x86_64.AppImage`
...
```sh
sudo apt install ./kind-meitner-amd64.deb
```

Then open **kind-meitner** from the GNOME application launcher. To remove it:

```sh
sudo apt remove kind-meitner
```

The portable AppImage does not install system files:

```sh
chmod +x release/kind-meitner-*-x86_64.AppImage
./release/kind-meitner-*-x86_64.AppImage
```

For a downloaded release AppImage, use `kind-meitner.AppImage` in place of the versioned path above.
...
Application data remains local in `~/.kind-meitner`. Electron browser data and window state use the normal XDG
configuration directory (`~/.config/kind-meitner` unless the environment overrides it).
...
./release/linux-unpacked/kind-meitner
```

## Agent CLI discovery

Applications launched from GNOME do not inherit the same interactive shell `PATH` as a terminal. kind-meitner
...
```sh
KIND_MEITNER_EXTRA_PATH=/your/custom/bin ./release/kind-meitner-*-x86_64.AppImage
```

Restart kind-meitner after installing or signing in to a CLI.
...
24.04 GNOME Wayland session. kind-meitner detects Wayland before XWayland when both `WAYLAND_DISPLAY` and
...
automatically. kind-meitner does not capture screen audio, remember the selected monitor after restart, or
...
starts with `--no-overlay`, so Cua's decorative full-screen X11 cursor surface is never created. kind-meitner also
...
The Xorg runtime uses private sockets, standard permission mode, per-action kind-meitner approvals,
...
then runs the same smoke against `/opt/kind-meitner/kind-meitner`. These lanes prove the embedded server and UI are
...
Run the CLI directly in a terminal, finish its sign-in flow, then restart kind-meitner. If it lives outside the
common directories above, use `KIND_MEITNER_EXTRA_PATH` while testing and report the install location so it can be
...
```sh
chmod +x kind-meitner-*-x86_64.AppImage
file kind-meitner-*-x86_64.AppImage
```
