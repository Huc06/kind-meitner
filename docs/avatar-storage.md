...
- **Custom** accepts a compatible Images API. Enter its **Base URL**, its
  **Image model** ID, and an optional API key, then choose **Save connection**.
  ...
  it requires. kind-meitner sends only the custom key to this connection.
...
`http://127.0.0.1:20128/v1`. kind-meitner appends `/images/generations`; pasting
that complete endpoint is also accepted. `localhost` and `127.0.0.1` refer to
the machine running the kind-meitner server, including when you open the app
