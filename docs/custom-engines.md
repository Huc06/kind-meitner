...
Two zero-code ways to run kind-meitner bots on an engine the app doesn't ship.
Both live in `~/.kind-meitner/config.json` under `"instances"`; restart the app
...
- **Sign in first.** The driver has no auth flow of its own — run the CLI once
  in a terminal and log in there; kind-meitner spawns it with your login intact.
