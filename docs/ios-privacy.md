# kind-meitner-mobile privacy

kind-meitner-mobile is a companion for a kind-meitner service chosen and operated
by the user. Local Wi-Fi and Tailscale connections work without a kind-meitner
...
- The computer remains the source of bots, transcripts, approvals, credentials,
  SQLite data, and screen images. kind-meitner's hosted control plane does not
...
- When a user completes a supported credential request on the phone, the value
  ...
  written to
  iOS preferences or Keychain by kind-meitner-mobile, chat, SQLite, logs, or the
...
- If the desktop user enables optional hosted access, kind-meitner stores the
...
- The optional HTTPS route is proxied by Cloudflare to an outbound-only
  ...
  but are not written to the kind-meitner control-plane database. Cloudflare may
  process IP addresses and connection/request metadata as kind-meitner's service
...
- Connector tokens stay in the desktop operating system's encrypted credential
  ...
  database.
- When a user explicitly chooses kind-meitner from another app's Share sheet, or
  ...
  Share sheet session removes the abandoned copy immediately; a kind-meitner-mobile
...
Revoking the phone in kind-meitner's Companion settings invalidates that device
credential. Transcript deletion is controlled by the kind-meitner installation
...
[kind-meitner Support](https://github.com/harrymove-ctrl/kind-meitner/issues) request
...
[kind-meitner Support](https://github.com/harrymove-ctrl/kind-meitner/issues).
