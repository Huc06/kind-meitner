...
[kind-meitner releases](https://github.com/harrymove-ctrl/kind-meitner/releases).
...
0.1.46 point to `harrymove-ctrl/kind-meitner-releases`; newer builds point to
`harrymove-ctrl/kind-meitner`. For that reason:

1. Every new release is published byte-for-byte to both repositories during
   the bridge period.
2. `kind-meitner-releases` must stay public. Do not delete its final bridge
...
Set these in **kind-meitner → Settings → Secrets and variables → Actions**.
...
Fine-grained tokens** → repository access: only `kind-meitner-releases` →
