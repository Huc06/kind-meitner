...
Install [Node.js](https://nodejs.org/) 24 or newer, then choose either way to run kind-meitner:

```sh
# Install once, then use the short command:
npm install -g kind-meitner
kind-meitner
```
...
Use the same command next time. The first launch guides you through setup; later launches reuse your saved AI connection and phone-access choice. `kind-meitner start` is the same as the bare command. If that workspace is already running, kind-meitner opens it instead of starting a second server.

## First launch

The setup wizard uses Clack. Use ↑/↓ and Enter to select an option. Plain terminals show numbered choices instead.

1. Choose Claude Code. An existing Claude connection is also listed.
2. Sign in with Claude. Setup asks before installing a missing Claude CLI.
...
Claude Code supports agent tools. A Claude subscription does not include separately billed API usage. Changing the saved default affects **new bots only**; existing bots and conversations keep their settings. Grok is available in the app's engine library, not in this terminal wizard.
...
- **Managed HTTPS address:** setup asks explicit permission for a public endpoint through Cloudflare and a possible connector download. Device pairing protects chat and settings; the pairing page and basic server identity remain publicly reachable. Sign in to an **kind-meitner account** using an emailed code, or reuse this machine's saved account. This account is separate from ChatGPT, Claude, or an API-provider account. The connection stays active while kind-meitner runs.
...
- **iPhone/iPad:** scan the QR with Camera to open Safari. If you already have the kind-meitner iOS app, use its pairing scanner or paste the full link there.
...
If the workspace starts but its HTTPS check fails, the local workspace remains usable and no phone code is created. Fix the connection and run `kind-meitner pair` in another terminal to try again. A missing account, connector, or Tailscale prerequisite can prevent startup; follow the printed error, or use `kind-meitner --local`. To add phone access after skipping it, stop the server, run `kind-meitner setup`, then start it again.
...
| Command | Use |
| --- | --- |
| `kind-meitner` | Set up once, then start with saved settings. |
| `kind-meitner setup` | Revisit AI and optional phone setup, save, and exit without starting. This is not a reset. |
| `kind-meitner --no-open` | Start without opening a browser. |
| `kind-meitner --local` | Ignore saved remote access for this launch; keep the saved choice for next time. |
| `kind-meitner --no-pair` | Suppress phone setup prompts and pairing invitations. This does **not** turn off saved remote access; use `--local` for that. |
| `kind-meitner pair` | Create another phone invitation while the configured workspace and HTTPS connection are running. |
| `kind-meitner sessions` | List paired devices; `kind-meitner sessions revoke ID` signs one out. |
| `kind-meitner serve` | Start without onboarding prompts or automatic browser opening; specify remote-access flags explicitly for a service. |
| `kind-meitner login` | Sign in to a kind-meitner account for `--tunnel`; this does not sign in to an AI provider or start the tunnel. |
...
```sh
kind-meitner setup --data-dir /path/to/kind-meitner-data --port 8799
kind-meitner --data-dir /path/to/kind-meitner-data --port 8799
```
...
Ctrl-C during AI setup leaves unsaved kind-meitner changes unapplied. Installations and provider sign-ins already completed remain available. Ctrl-C during the later phone step keeps the AI setup you already saved, exits without starting a server, and does not undo an account sign-in already completed. Run `kind-meitner setup` to continue; no destructive reset is needed.
