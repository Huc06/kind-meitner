...
## ASP #13837 listing follow-up and submission fallback

**Observed 2026-09-21:** the public page for [ASP #13837](https://www.okx.ai/agents/13837) reports **“This ASP is under review”**. It does not expose an approved-listing title or description while that review state is active.

Use this exact submission product link until the review completes:

```text
https://kind-meitner-production.up.railway.app/api/okx/free-mcp
```

Accompany it with: **“ASP #13837 is under review; the product link is the live Railway Free A2MCP endpoint.”** The endpoint was checked with a read-only JSON-RPC `tools/list` request on 2026-09-21 and returned `HTTP 200` with only free, read-only tools.

When the marketplace record is editable, use this truthful copy:

- **Title:** `Kind Meitner Markets — Free A2MCP tools`
- **Description:** `Free, read-only A2MCP tools for OKX.AI agent-service discovery, launch guidance, and locally indexed market research. Price 0; no wallet, payment, mainnet access, or settlement.`
- **Endpoint:** the Railway URL above, with price `0`.

Do not claim listing-readiness scanning or pre-spend trust capabilities until the corresponding tools are deployed. After those tools ship, update the description to name them explicitly and retain the same free/read-only, no-wallet boundary.
