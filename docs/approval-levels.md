...
provider's own permission modes, passed through. kind-meitner does not judge an
...
Access, kind-meitner also applies its own configuration tools without another approval.
...
their existing kind-meitner configuration review cards. Full actions return an applied
...
ends. kind-meitner keeps no standing grant for a provider's tool. It is not
...
Native modes require a CLI version that supports them; kind-meitner does not
silently substitute unrestricted access when a mode is rejected.

### Read-only integration tools

kind-meitner's built-in agents MCP server describes nine scoped reads with
...
provider's session. Its quieter default is Full access; kind-meitner keeps its
