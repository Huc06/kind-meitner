# kind-meitner harness implementation plan
...
(pi, agent-orchestrator, deepseek-harness) and asking what kind-meitner lacked by
...
**The reframe that reorders everything.** kind-meitner is itself a harness. The
...
**On-disk forward compatibility.** `~/.kind-meitner` is shared across app versions.
