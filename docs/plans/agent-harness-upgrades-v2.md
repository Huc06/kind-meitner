# kind-meitner Harness Upgrades v2 Implementation Plan
...
**Architecture:** kind-meitner is itself a harness; `server/store.ts` is the
...
- `~/.kind-meitner` is shared across app versions — every new field or message kind must be ignorable by an older build.
...
messages **per thread** to `join(home, ".kind-meitner", "native",
