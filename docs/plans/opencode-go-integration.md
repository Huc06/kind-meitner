...
Add OpenCode Go as a first-class kind-meitner engine by running the maintained
OpenCode CLI over its official Agent Client Protocol (ACP) interface and selecting
models from the `opencode-go/*` provider.

The first implementation should reuse kind-meitner's ACP runtime rather than call
...
provide inference; kind-meitner does not currently have a provider-independent
...
- OpenCode accepts an `OPENCODE_API_KEY` environment variable. kind-meitner should
...
the process lifecycle kind-meitner uses for Grok and Gemini.
...
- refresh on the existing instance re-probe path so upstream additions do not
  require restarting kind-meitner.
...
- load that session on the next kind-meitner turn;
...
- concurrent kind-meitner threads without shared ACP or credential state.
...
- OpenCode Go is shown as unavailable until both its CLI and credential are
  present; kind-meitner never selects it as a runnable default otherwise.
- The setup flow works without restarting kind-meitner and never silently executes
  an installer.
...
6. Can the existing ACP usage metadata distinguish cumulative session totals from
   per-turn usage so kind-meitner does not double-count it?
