...
Run the kind-meitner server anywhere; connect from the desktop app, any
...
1. **Stable server identity.** The server generates `environmentId` once
   (`KIND_MEITNER_DATA_DIR/environment-id`) and serves a descriptor at
   `/.well-known/kind-meitner/environment` — id, label, platform, version,
...
2. **Private network helper**: detect a Tailscale MagicDNS name and offer
   ...
   line as `kind-meitner serve --tailscale`.
3. **Managed tunnel**: the cloudflared managed-tunnel channel the companion
   ...
   Shipped for the command line as `kind-meitner login` + `serve --tunnel`
