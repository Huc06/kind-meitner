# Deploy kind-meitner on a VPS

From a blank Linux server to kind-meitner running on it around the clock, reachable from your laptop, the desktop app and your phone, with your bots working while every laptop is closed. It assumes nothing beyond being able to open a terminal and paste commands. About twenty minutes, most of it waiting.
...
|---|---|---|---|
| **A. Public address, no domain** (`serve --tunnel`) | a kind-meitner account (email code) | anyone with a pairing code, over HTTPS | the fastest path; a phone on cellular |
...
Whichever you pick, the login is the same: you **pair** each device once with a short code and it stays signed in. A session lasts 30 days; using it with half that or less left renews it to a full 30, up to 180 days from pairing (`KIND_MEITNER_SESSION_TTL_DAYS` and `KIND_MEITNER_SESSION_MAX_DAYS` change both numbers). There is no password.
...
No domain, no proxy, no open port. The server gets an address like `https://c-7f3a9c.kind-meitner.com` through a Cloudflare tunnel; only traffic through the tunnel reaches it, and that traffic still has to pair.

```sh
npx kind-meitner setup          # once: choose AI access, connect, and choose a model
npx kind-meitner login          # once: an emailed code signs this machine in and reserves its address
npx kind-meitner serve --tunnel # runs the server there and prints the pairing link with a QR code
```

`setup` connects an AI provider; it is separate from the kind-meitner account.
...
4. **Start it:**
   ...
   ```sh
   docker compose pull kind-meitner && docker compose up -d
   docker compose ps        # kind-meitner "healthy", caddy "running"
   ```
...
In this path, every `npx kind-meitner …` command below is run inside the container instead:

```sh
docker compose exec kind-meitner node dist-server/kind-meitner.js pair --label "My MacBook"
```
...
```sh
npx kind-meitner serve --tailscale
```
...
```sh
sudo -H npx --yes kind-meitner browser install --with-deps
```

Then install the browser in the service account's home, from the `maus` shell:

```sh
npx --yes kind-meitner browser install
```
...
The npm kind-meitner package does not install model engine CLIs. For paths A and C,
...
example `docker compose exec kind-meitner claude`.

Engine logins belong to the service user's home (for example `~/.codex` and
`~/.claude`), separately from kind-meitner's `~/.kind-meitner`. Keep that home when
...
```sh
npx kind-meitner pair --label "Kitchen iPad"
```

```
pairing code:  RR8Y-BLR6-H939
expires:       10:59:45 AM (single use)
open or scan:  https://c-7f3a9c.kind-meitner.com/pair#code=RR8Y-BLR6-H939
```
...
```sh
npx kind-meitner sessions              # id, device, scope, last seen, expires
npx kind-meitner sessions revoke ID    # signs that device out and closes its stream at once
npx kind-meitner status                # what the server says about itself
```

## Keep it running

`npx kind-meitner serve` is a plain foreground process. For systemd (paths A and C),
install a chosen release first, from the `maus` shell. Replace `X.Y.Z` with the
published version you want to run:

```sh
npm install --global --prefix "$HOME/.local" kind-meitner@X.Y.Z
```

Stop the foreground server with Ctrl-C before enabling the service. From the
administrator shell, save this as `/etc/systemd/system/kind-meitner.service`:

```ini
# /etc/systemd/system/kind-meitner.service
[Unit]
Description=kind-meitner server
...
ExecStart=/home/maus/.local/bin/kind-meitner serve --tunnel --no-pair
Restart=always
RestartSec=5
...
```sh
sudo systemctl daemon-reload && sudo systemctl enable --now kind-meitner
journalctl -u kind-meitner -f            # the server's log, including "tunnel: live at …"
```

Use `--tailscale` instead of `--tunnel` for path C. `--no-pair` skips printing a code at every restart; mint one with `npx kind-meitner pair` when you need it. Docker (path B) restarts on its own (`restart: unless-stopped`).
...
- **Paths A and C with the service above:** choose the new published version,
  ...
  ```sh
  sudo systemctl stop kind-meitner
  sudo -iu maus npm install --global --prefix /home/maus/.local kind-meitner@X.Y.Z
  sudo systemctl start kind-meitner
  ```
  ...
  desired release as `npx --yes kind-meitner@X.Y.Z serve --tunnel`.
- **Path B:** `cd kind-meitner/deploy && docker compose pull kind-meitner && docker compose up -d`.
...
process, or `sudo systemctl stop kind-meitner` from the administrator shell for
...
```sh
umask 077
backup_dir=$(mktemp -d "$PWD/kind-meitner-backup.XXXXXX")
tar czf "$backup_dir/kind-meitner-data.tgz" -C "$HOME" .kind-meitner
```
...
`kind-meitner/deploy`, stop the app before archiving; `deploy_data` is the default
volume name, so use your actual volume name if you changed the Compose project:

```sh
docker compose stop kind-meitner
backup_dir=$(mktemp -d "$PWD/kind-meitner-backup.XXXXXX")
docker run --rm -v deploy_data:/data:ro -v "$backup_dir":/b alpine sh -c 'umask 077; tar czf /b/kind-meitner-data.tgz -C /data .'
```
...
service with `sudo systemctl start kind-meitner`, or `docker compose start kind-meitner`.
...
- The one thing a stranger can read is `/.well-known/kind-meitner/environment` (the server's id, label, version, capabilities) and `/api/health` (only the app name). Everything else answers "pair this device".

## Troubleshooting

**`serve --tunnel` says "no account on this machine yet".** Run `npx kind-meitner login` on this machine first; the credentials are per machine.

**The tunnel stays on "retrying".** The server is running and usable locally; the public hop is not verified yet. Wait a minute (Cloudflare needs a moment on a fresh address), then check `journalctl`/the terminal for the reason. If it never comes up, `npx kind-meitner logout && npx kind-meitner login` issues a fresh address.
...
**What does the server think it is?** `https://<address>/.well-known/kind-meitner/environment` is public and shows its id, label, version and capabilities; `npx kind-meitner status` prints the same on the server.

**Something else.** `journalctl -u kind-meitner --since -10m` (or `docker compose logs kind-meitner --tail 100`) shows the server's startup lines. Paste them with your question in the community channel.
...
```sh
sudo install -d -o root -g root -m 0755 /opt/kind-meitner-browser
sudo cp -R /home/maus/.agent-browser/browsers/chrome-VERSION /opt/kind-meitner-browser/
sudo chown -R root:root /opt/kind-meitner-browser/chrome-VERSION
sudo chmod -R go-w /opt/kind-meitner-browser/chrome-VERSION
```

Save this as the root-owned `/etc/apparmor.d/kind-meitner-chrome`, replacing
...
profile kind-meitner-chrome /opt/kind-meitner-browser/chrome-VERSION/chrome flags=(unconfined) {
  userns,
}
```
...
```sh
sudo apparmor_parser -r /etc/apparmor.d/kind-meitner-chrome
```
...
`sudo systemctl restart kind-meitner`:

```ini
Environment=AGENT_BROWSER_EXECUTABLE_PATH=/opt/kind-meitner-browser/chrome-VERSION/chrome
```
