...
folder on the computer running kind-meitner — nothing is stored anywhere else,
...
disk, those buttons only work from the computer running kind-meitner — a
...
`~/.kind-meitner/memory-journal/<botId>.ndjson`. It lives *outside* the
