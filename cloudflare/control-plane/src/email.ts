...
const SUBJECTS = {
  "sign-in": "Your kind-meitner sign-in code",
  "email-verification": "Verify your kind-meitner email",
  "forget-password": "Reset your kind-meitner password",
  "change-email": "Confirm your kind-meitner email change",
...
export function buildOTPEmail(from: string, input: OTPEmailInput) {
  ...
  return {
    to: input.email,
    from: { email: from, name: "kind-meitner" },
    subject,
    html,
    text,
  };
