...
async function canonicalAuthResponse(response: Response): Promise<Response> {
  ...
  // `message`). Publish only kind-meitner's stable, lowercase error contract.
  ...
  return {
    async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
      ...
      if (request.method === "GET" && url.pathname === "/healthz") {
        ...
        return secureResponse(json({ ok: true, service: "kind-meitner-control-plane" }), request, null, requestId);
      }
