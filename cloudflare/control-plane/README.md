...
- Owner-scoped desktop installations and independently revocable
  `kind_meitner_install_…` credentials. Account bearer tokens are never accepted as
...
All three endpoint methods require `Authorization: Bearer <kind_meitner_install_…>`.
...
```sh
pnpm --filter @kind-meitner/control-plane exec wrangler d1 migrations apply DB --local --config wrangler.jsonc
pnpm --filter @kind-meitner/control-plane exec wrangler dev --config wrangler.jsonc
```
...
8. Deploy the Worker and verify that `GET <BETTER_AUTH_URL>/healthz` returns
   exactly `{ "ok": true, "service": "kind-meitner-control-plane" }` over
