...
const BASE_URL = "https://auth.kind-meitner.test";
...
describe("control-plane migrations and health", () => {
  ...
  it("serves a no-store health response without CORS wildcards", async () => {
    ...
    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({ ok: true, service: "kind-meitner-control-plane" });
...
describe("Better Auth email OTP and bearer boundary", () => {
  ...
  it("requires account bearers and never confuses installation credentials", async () => {
    ...
    expect((await call("/v1/me", { token: "kind_meitner_install_AAAAAAAAAAAAAAAAAAAAAA.AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA" })).status).toBe(401);
...
describe("installation lifecycle", () => {
  it("registers once, stores no raw credential, and serves installation self", async () => {
    ...
    expect(created.credential).toMatch(/^kind_meitner_install_[A-Za-z0-9_-]{22}\.[A-Za-z0-9_-]{43}$/);
...
describe("HTTP boundary hardening", () => {
  ...
  it("defaults CORS to deny and never emits a wildcard", async () => {
    ...
    const allowed = await call("/v1/me", { origin: "https://app.kind-meitner.test" });
    expect(allowed.status).toBe(401);
    expect(allowed.headers.get("access-control-allow-origin")).toBe("https://app.kind-meitner.test");
    expect(allowed.headers.get("cache-control")).toBe("no-store");

    const deniedPreflight = await call("/v1/installations", {
      method: "OPTIONS",
      origin: "https://app.kind-meitner.test",
      headers: {
    ...
    expect(deniedPreflight.headers.get("access-control-allow-origin")).toBe("https://app.kind-meitner.test");
  });
});
