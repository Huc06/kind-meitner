...
describe("connected-apps broker boundaries", () => {
  ...
  it("creates Sessions with explicit multi-account selection", async () => {
    ...
    vi.stubGlobal("fetch", async (input: string | URL | Request, init?: RequestInit) => {
      ...
      return Response.json(session("trs_new", "kind_meitner_user"), { status: 201 });
    });

    await expect(createSession(env as never, "kind_meitner_user")).resolves.toMatchObject({
      sessionId: "trs_new",
      multiAccountConfigured: true,
    });
    expect(JSON.parse(String(fetchCalls[0].init?.body))).toMatchObject({
      user_id: "kind_meitner_user",
  ...
  it("upgrades a legacy Session without changing the installation's Composio user", async () => {
    ...
    vi.stubGlobal("fetch", async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (init?.method === "POST") return Response.json(session("trs_new", "kind_meitner_stable"), { status: 201 });
      return Response.json(session("trs_legacy", "kind_meitner_stable", false));
    });

    await expect(ensureSession({
      id: "install-1",
      composio_user_id: "kind_meitner_stable",
    ...
    expect(JSON.parse(String(creation?.init?.body))).toMatchObject({ user_id: "kind_meitner_stable", multi_account: multiAccount });
  ...
  it("returns every account and deletes only an owned account ID", async () => {
    ...
    vi.stubGlobal("fetch", async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url.includes("/tool_router/session/trs_multi")) return Response.json(session("trs_multi", "kind_meitner_stable"));
