...
async function register(request: Request, env: Env) {
  ...
  ).bind(installationId, await sha256(token), `kind_meitner_${installationId.replaceAll("-", "")}`, now, now).run();
...
async function route(request: Request, env: Env, ctx: ExecutionContext) {
  ...
  if (request.method === "GET" && url.pathname === "/health") return json({ service: "kind-meitner-composio", ready: Boolean(env.COMPOSIO_API_KEY) });
