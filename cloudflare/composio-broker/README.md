# kind-meitner connected-apps broker
...
1. `pnpm broker:types`
2. `pnpm exec wrangler d1 migrations apply kind-meitner-composio --remote --config cloudflare/composio-broker/wrangler.jsonc`
...
`KIND_MEITNER_COMPOSIO_BROKER_URL` in their packaged build. Running only the local
