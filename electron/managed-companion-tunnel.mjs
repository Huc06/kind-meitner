...
export function resolveCloudflaredBinary({
  ...
  const override = environment.KIND_MEITNER_CLOUDFLARED_PATH?.trim();
...
/** Remove only token files created by a dead kind-meitner process. Suspicious
...
async function verifyHostedEndpoint(
  ...
  try {
    return JSON.parse(text)?.app === "kind-meitner";
  } catch {
    return false;
  }
}
