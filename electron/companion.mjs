...
// sidecar's ~/.kind-meitner-companion, which is the child process's directory,
...
async function start({ resourcesPath, harnessPort, mutationToken, hostedUrl = null, secretPublicKey = null, log }) {
  ...
  delete childEnvironment.KIND_MEITNER_COMPANION_HOSTED_URL;
  delete childEnvironment.KIND_MEITNER_COMPANION_INTERNAL_ORIGIN;
  delete childEnvironment.KIND_MEITNER_PHONE_SECRET_PUBLIC_KEY;
  if (hostedUrl) childEnvironment.KIND_MEITNER_COMPANION_HOSTED_URL = hostedUrl;
  childEnvironment.KIND_MEITNER_COMPANION_INTERNAL_ORIGIN = allocatedOrigin.socketPath;
  if (/^[A-Za-z0-9_-]{87}$/.test(String(secretPublicKey ?? ""))) {
    childEnvironment.KIND_MEITNER_PHONE_SECRET_PUBLIC_KEY = secretPublicKey;
  }
  ...
  try {
    child = utilityProcess.fork(resolved.entry, [], {
      env: {
        ...childEnvironment,
        KIND_MEITNER_PORT: String(harnessPort),
        KIND_MEITNER_COMPANION_PORT: String(COMPANION_PORT),
        KIND_MEITNER_CONTROL_PORT: String(CONTROL_PORT),
      },
  ...
  child.once("spawn", () => {
    ...
    try {
      child.postMessage({ type: "kind-meitner:companion-mutation-token", token: mutationToken });
    } catch {
 ...
 * phones. Tailscale may be installed, signed in, or enabled after kind-meitner
...
export async function companionPairing(open, expectedToken) {
  ...
  const token = /^kind_meitner_pair_[A-Za-z0-9_-]{43}$/.test(candidate)
    ? candidate
