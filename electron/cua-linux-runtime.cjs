...
function createLinuxCuaRuntime({
  ...
  const runtimeRoot = () => {
    ...
    if (configured && path.isAbsolute(configured)) {
      try {
        ...
        if (
          ...
          const root = ensurePrivateDirectory(path.join(configured, "kind-meitner-cua"));
    ...
    const currentUid = process.getuid?.() ?? os.userInfo().uid;
    const root = ensurePrivateDirectory(path.join(os.tmpdir(), `kind-meitner-cua-${currentUid}`));
    cleanupStaleRuntimeDirectories(root);
    return root;
  };
