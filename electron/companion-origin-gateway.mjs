...
const RUNTIME_PREFIX = "kind-meitner-companion-origin-";
...
export function validCompanionOriginTarget(target, platform = process.platform) {
  ...
  if (platform === "win32") {
    return /^\\\\\.\\pipe\\kind-meitner-companion-origin-[1-9][0-9]*-[0-9a-f-]{36}$/i.test(
      target.socketPath,
    );
  }
  return (
...
export function createCompanionOriginEndpoint({
  ...
  if (platform === "win32") {
    return Object.freeze({
      pid: processId,
      socketPath: `\\\\.\\pipe\\kind-meitner-companion-origin-${processId}-${identifier()}`,
      directory: null,
    });
  }
    ...
    const outgoing = request(
      ...
      (response) => {
        ...
        response.on("end", () => {
          ...
          try {
            finish(JSON.parse(Buffer.concat(chunks).toString("utf8"))?.app === "kind-meitner");
          } catch {
            finish(false);
          }
        });
