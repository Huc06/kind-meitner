...
export function createComputerSharing({ file, fetch: fetchImpl, environments, cuaConnection, hostControl, protectedPaths = [], enabled = async () => false }) {
  ...
  const request = async (env, route, body, signal, secret) => {
    ...
    const response = await fetchImpl(`${env.origin}${route}`, {
      ...
      headers: { origin: env.origin, ...(body === undefined ? {} : { "content-type": "application/json" }), ...(secret ? { "x-kind-meitner-computer-secret": secret } : {}) },
    ...
    let json; try { json = JSON.parse(Buffer.concat(chunks).toString("utf8")); } catch { throw new Error("This server does not support computer sharing. Update its kind-meitner installation."); }
  ...
  const identity = async env => {
    ...
    const [auth, descriptor] = await Promise.all([request(env, "/api/auth/session"), request(env, "/.well-known/kind-meitner/environment")]);
    await requireEnabled();
