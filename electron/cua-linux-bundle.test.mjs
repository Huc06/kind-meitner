...
function fixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "kind-meitner-appimage-bundle-test-"));
...
describe.skipIf(process.platform === "win32")("AppImage CUA private staging", () => {
  it("copies exact bytes to a private executable stage and removes only that stage", () => {
    ...
    expect(path.basename(stage.directory)).toMatch(/^kind-meitner-cua-linux-x64-4242-/);
  ...
  it("rejects bytes that do not match the pinned runtime", () => {
    ...
    expect(fs.readdirSync(root).filter((name) => name.startsWith("kind-meitner-cua"))).toEqual([]);
  });
  ...
  it("gives legacy stages a race-safe grace period and retains active ones", () => {
    ...
    const createLegacy = (suffix) => {
      const directory = path.join(root, `kind-meitner-cua-linux-x64-${suffix}`);
