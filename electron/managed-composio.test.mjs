  ...
  it("accepts HTTPS and loopback development brokers but rejects insecure remote URLs", async () => {
    ...
    expect(
      managedComposioChildEnvironment("http://broker.example", credentials, {
        PATH: "/usr/bin",
        KIND_MEITNER_COMPOSIO_BROKER_URL: "http://attacker.example",
        KIND_MEITNER_COMPOSIO_BROKER_TOKEN: "attacker-controlled",
      }),
    ...
    expect(
      ...
      PATH: "/usr/bin",
      KIND_MEITNER_COMPOSIO_BROKER_URL: "http://[::1]:8787",
      KIND_MEITNER_COMPOSIO_BROKER_TOKEN: TOKEN,
    });
  });
