...
const DEFAULT_COMPOSIO_BROKER_URL = "https://kind-meitner-composio.milindsoni201.workers.dev";
...
if (process.platform === "linux") {
  ...
  app.setDesktopName("com.kind-meitner.app.desktop");
}
...
if (!app.requestSingleInstanceLock()) {
  console.log("[desktop] kind-meitner is already running — focusing that window");
  process.exit(0);
}
...
const serverSupervisor = createServerSupervisor({
  ...
  onExhausted() {
    ...
    dialog.showErrorBox(
      ...
      "Automatic recovery could not restart the background server. Quit and reopen kind-meitner to try again. Interrupted chat turns were not resent.\n\n" +
...
function desktopDataDir() {
  ...
  // intentionally treats an empty KIND_MEITNER_DATA_DIR differently, so inheriting it
  // without normalization would lease one directory and write another.
  return process.env.KIND_MEITNER_DATA_DIR || path.join(app.getPath("home"), ".kind-meitner");
}
...
function composioBrokerUrl() {
  const configured = process.env.KIND_MEITNER_COMPOSIO_BROKER_URL?.trim();
...
// goes to server.log in the OS log dir (~/Library/Logs/kind-meitner on macOS,
// Console.app-visible; %APPDATA%\kind-meitner\logs on Windows), which is also
...
function syncDesktopMutationToken(proc) {
  try {
    proc.postMessage({
      type: "kind-meitner:desktop-mutation-token",
...
async function startServerOn(port) {
  ...
  const childEnv = managedComposioChildEnvironment(composioBrokerUrl(), secureCredentials, {
    ...
    KIND_MEITNER_DATA_DIR: desktopDataDir(),
    ...
    KIND_MEITNER_DESKTOP_PARENT: "1",
    KIND_MEITNER_STATIC_DIR: path.join(process.resourcesPath, "ui"),
    KIND_MEITNER_RESOURCES_PATH: process.resourcesPath,
    KIND_MEITNER_SKILLS_DIR: path.join(process.resourcesPath, "skills"),
    KIND_MEITNER_PORT: String(port),
    // the server advertises this to remote clients so version skew is visible
    KIND_MEITNER_APP_VERSION: app.getVersion(),
    KIND_MEITNER_USER_DATA: app.getPath("userData"),
    ...(secureCredentials.composioApiKey
      ? { COMPOSIO_API_KEY: secureCredentials.composioApiKey }
      : {}),
    // "we could not read your keys" must not reach the UI as "you have none"
    KIND_MEITNER_CREDENTIAL_STORE: credentialStoreUnavailable ? "unavailable" : "ok",
  ...
  delete childEnv.KIND_MEITNER_BROWSER_CONNECTION;
...
function syncManagedComposioCredentials() {
  ...
  try {
    serverProc.postMessage({
      type: "kind-meitner:managed-composio",
...
function buildErrorPage({ allPortsOccupied }) {
  ...
  const reason = allPortsOccupied
    ? "Every kind-meitner port answered health checks from another process — likely a second copy of the app, or another program on ports 8799–28799. Quit that program, then quit and reopen kind-meitner."
    : "The background server didn't come up in time — this is usually slow startup, not a port conflict. Quit and reopen kind-meitner.";
  return (
...
function openDesktopViewer(owner, rawUrl, rawTitle, contextId) {
  if (!owner || owner.isDestroyed()) throw new Error("The kind-meitner window is unavailable");
  ...
  const viewer = new BrowserWindow({
    ...
    webPreferences: {
      ...
      partition: "kind-meitner-desktop-viewer",
    },
  });
...
function ensureDesktopWorkspace(owner) {
  ...
  const manager = createDesktopWorkspaceManager({
    ...
    partitionPrefix: `kind-meitner-desktop-workspace-${randomUUID()}`,
    notify: (state) => {
...
function createWindow() {
  ...
  if (process.env.KIND_MEITNER_SMOKE_TEST === "1") {
      ...
      try {
        const result = await win.webContents.executeJavaScript(`
          (async () => {
            ...
            if (${JSON.stringify(process.env.KIND_MEITNER_SMOKE_CUA === "1")}) {
        ...
        if (process.env.KIND_MEITNER_SMOKE_BUNDLED_CUA === "1") {
        ...
        if (process.env.KIND_MEITNER_SMOKE_KEEP_OPEN !== "1") win.close();
      }
    });
  }
...
// Bots hand users files as markdown links to paths inside the kind-meitner
...
// renderer-controlled, so it must resolve inside ~/.kind-meitner and be a
...
// live desktop inside kind-meitner instead of sending the person to a browser.
...
app.whenReady().then(async () => {
  if (app.isPackaged) {
    try {
      ...
      dialog.showErrorBox(
        "kind-meitner could not start safely",
        error?.message ?? "Another process is using this kind-meitner data folder.",
      );
      app.quit();
      return;
    }
  ...
  if (app.isPackaged) {
    app.setAsDefaultProtocolClient("kind-meitner");
