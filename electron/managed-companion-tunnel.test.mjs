...
const ENDPOINT = "https://c-installation.kind-meitner.com";
...
const ORIGIN_TARGET =
  process.platform === "win32"
    ? {
        pid: 31337,
        socketPath:
          "\\\\.\\pipe\\kind-meitner-companion-origin-31337-12345678-1234-1234-1234-123456789abc",
      }
    : { pid: 31337, socketPath: "/tmp/kind-meitner-companion-origin-test/origin.sock" };
const temporaryDirectories = [];

function temporaryDirectory() {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "kind-meitner-managed-tunnel-"));
...
function healthyResponse() {
  return {
    ok: true,
    text: async () => JSON.stringify({ app: "kind-meitner" }),
  };
}
  ...
  it("rejects a relative development override instead of searching it on PATH", () => {
    expect(
      resolveCloudflaredBinary({
        ...
        platform: "linux",
        arch: "x64",
        environment: { KIND_MEITNER_CLOUDFLARED_PATH: "./untrusted-cloudflared" },
        exists: () => true,
      }),
    ).toBeNull();
  });
