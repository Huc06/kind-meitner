...
const INSTALL = `kind_meitner_install_${"a".repeat(22)}.${"b".repeat(43)}`;
...
describe("control-plane desktop client", () => {
  it("accepts exact HTTPS and loopback development origins only", () => {
    expect(normalizeControlPlaneURL("https://accounts.kind-meitner.com/")).toBe(
      "https://accounts.kind-meitner.com",
    );
    expect(normalizeControlPlaneURL("http://127.0.0.1:8787/")).toBe("http://127.0.0.1:8787");
    expect(normalizeControlPlaneURL("http://accounts.kind-meitner.com")).toBe("");
    expect(normalizeControlPlaneURL("https://accounts.kind-meitner.com/api")).toBe("");
    expect(normalizeControlPlaneURL("https://user:secret@accounts.kind-meitner.com")).toBe("");
  });
  ...
  it("normalizes an email without accepting malformed input", () => {
    ...
    expect(normalizeControlPlaneURL({ toString: () => "https://accounts.kind-meitner.com" })).toBe("");
  });
  ...
  it("accepts plain cross-realm response records", async () => {
    ...
    const client = createControlPlaneClient({
      baseURL: "https://accounts.kind-meitner.com",
  ...
  it("requires the exact healthy control-plane identity before onboarding", async () => {
    ...
    const fetchImpl = vi
      .fn()
      .mockResolvedValueOnce(jsonResponse({ ok: true, service: "kind-meitner-control-plane" }))
    ...
    expect(fetchImpl.mock.calls[0][0]).toBe("https://accounts.kind-meitner.com/healthz");
  ...
  it("identifies native Better Auth mutations with the trusted control-plane origin", async () => {
    const fetchImpl = vi.fn(async (_url, init) => {
      ...
      if (wireHeaders.has("sec-fetch-mode") && wireHeaders.get("origin") !== "https://accounts.kind-meitner.com") {
  ...
  it("keeps a valid installation credential without rotating it", async () => {
    const fetchImpl = vi.fn(async (url) => {
      expect(url).toBe("https://accounts.kind-meitner.com/v1/installations/self");
      return jsonResponse({
        installation: {
          id: INSTALL_ID,
    ...
    const client = createControlPlaneClient({ baseURL: "https://accounts.kind-meitner.com", fetchImpl });
  ...
  it("recovers a lost installation credential by rotating the matching identity", async () => {
    const rotated = `kind_meitner_install_${"c".repeat(22)}.${"d".repeat(43)}`;
  ...
  it("lists validated active installations for response-loss cleanup", async () => {
    const fetchImpl = vi.fn(async (url, init) => {
      expect(url).toBe("https://accounts.kind-meitner.com/v1/installations");
  ...
  it("validates endpoint material without leaking the connector token into the URL", async () => {
    ...
    const fetchImpl = vi.fn(async (url, init) => {
      expect(url).toBe("https://accounts.kind-meitner.com/v1/installations/self/endpoint");
      expect(init.headers.get("authorization")).toBe(`Bearer ${INSTALL}`);
      expect(url).not.toContain(connectorToken);
      return jsonResponse({ endpoint: { url: "https://c-opaque.kind-meitner.com" }, connectorToken });
    });
    const client = createControlPlaneClient({ baseURL: "https://accounts.kind-meitner.com", fetchImpl });
    await expect(client.ensureEndpoint(INSTALL)).resolves.toEqual({
      endpoint: { url: "https://c-opaque.kind-meitner.com" },
      connectorToken,
    });
  });
