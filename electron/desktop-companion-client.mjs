...
const DEVICE_TOKEN = /^kind_meitner_[A-Za-z0-9_-]{43}$/;
...
export function desktopCompanionRendererArguments(localOrigin, remoteAccess) {
  return [
    `--kind-meitner-local-origin=${localOrigin}`,
    ...(remoteAccess ? ["--kind-meitner-remote-client"] : []),
  ];
}
...
/** Pairing tokens may travel through a verified kind-meitner HTTPS endpoint or
...
export function normalizeDesktopCompanionEndpoint(value) {
  ...
  const managedHttps =
    parsed.protocol === "https:" && hostname.endsWith(".kind-meitner.com");
  if (
...
export async function pairDesktopCompanion({
  ...
  if (!endpoint) {
    throw new Error("Enter the kind-meitner HTTPS companion address or full Tailscale name ending in .ts.net");
  }
