...
const INSTALLATION_CREDENTIAL = `kind_meitner_install_${"b".repeat(22)}.${"c".repeat(43)}`;
const CONNECTOR_TOKEN = `eyJ${"d".repeat(100)}`;
const ENDPOINT = "https://c-opaque.kind-meitner.com";
...
describe("Companion account service", () => {
  it("uses the packaged hosted default and only explicit safe development origins", () => {
    ...
    expect(resolveCompanionControlPlaneURL({
      isPackaged: false,
      environment: { KIND_MEITNER_CONTROL_PLANE_URL: "http://127.0.0.1:8787/" },
    })).toBe("http://127.0.0.1:8787");
    expect(resolveCompanionControlPlaneURL({
      isPackaged: true,
      environment: { KIND_MEITNER_CONTROL_PLANE_URL: "http://accounts.kind-meitner.com" },
    })).toBe("");
    expect(resolveCompanionControlPlaneURL({
      isPackaged: true,
      environment: { KIND_MEITNER_CONTROL_PLANE_URL: new String("https://accounts.kind-meitner.com") },
    })).toBe("");
