...
async function downloadInto(h, files = ["/tmp/kind-meitner-2.0.0-amd64.deb"]) {
...
test("the hand-off install opens the staged package instead of quitting", async () => {
  ...
  assert.deepEqual(received, [["/tmp/kind-meitner-2.0.0-amd64.deb"]]);
...
test("without a hand-off the install still quits and installs", async () => {
  ...
  await downloadInto(h, ["/tmp/kind-meitner-2.0.0.AppImage"]);
  h.coordinator.install();
