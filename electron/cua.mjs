...
//    EmbeddedCuaDriverHost so TCC grants attribute to kind-meitner and the
//    driver inherits them. One prompt, named kind-meitner, out of the box.
...
const HOST_BUNDLE_ID = "com.kind-meitner.app";
...
async function loadEmbeddedSdk() {
  ...
  process.env.KIND_MEITNER_CUA_SDK_LIBRARY = path.join(
...
async function attachStandalone(signal) {
  ...
  if (!(await socketAlive(STANDALONE_SOCKET))) {
    ...
    // machine already granted — instead of the freshly signed kind-meitner.
...
async function startEmbedded(binary, signal) {
  ...
  // kind-meitner rather than to a terminal or helper process.
  if (process.platform === "darwin") {
    ...
    if (!sdk.hasRequiredMacOSPermissions(permissionStatus)) {
      ...
      throw new Error(`${missing || "macOS permissions"} required; grant access in System Settings and restart kind-meitner`);
    }
  }
...
export async function startCua() {
  ...
  const wantEmbedded =
    process.platform === "win32" || app.isPackaged || process.env.KIND_MEITNER_CUA_EMBEDDED === "1";
  let nextConnection;
