...
test("the Ubuntu command resolves dependencies", () => {
  const command = packageInstallCommand("deb", "/home/u/.cache/kind-meitner-updater/pending/x.deb");

  assert.equal(
    command,
    "sudo apt-get install -y '/home/u/.cache/kind-meitner-updater/pending/x.deb'",
  );
...
test("a staged path that is gone is not used", () => {
  const workspace = mkdtempSync(join(tmpdir(), "kind-meitner-staged-"));
  try {
    const present = join(workspace, "kind-meitner.deb");
...
test("the quoted path survives a shell round-trip", { skip: posixShell }, () => {
  const workspace = mkdtempSync(join(tmpdir(), "kind-meitner-quote-"));
  try {
...
test("the whole command parses into the arguments apt-get would receive", { skip: posixShell }, () => {
  const file = "/home/o'brien/.cache/kind-meitner-updater/pending/kind-meitner-0.1.44-amd64.deb";
...
test("the package marker decides the install path", () => {
  ...
  assert.equal(
    linuxPackageType({
      ...
      appImage: "/home/u/kind-meitner.AppImage",
