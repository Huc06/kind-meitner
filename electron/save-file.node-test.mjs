...
const canSymlink = (() => {
  const probe = fs.mkdtempSync(path.join(os.tmpdir(), "kind-meitner-symlink-probe-"));
  try {
...
before(() => {
  home = fs.mkdtempSync(path.join(os.tmpdir(), "kind-meitner-save-file-"));
  botHome = path.join(home, ".kind-meitner");
...
describe("save-file path validation", () => {
  ...
  it("accepts a file under a symlinked bot home", { skip: !canSymlink }, async () => {
    const realHome = fs.mkdtempSync(path.join(os.tmpdir(), "kind-meitner-real-home-"));
    const linkedHome = fs.mkdtempSync(path.join(os.tmpdir(), "kind-meitner-linked-home-"));
    ...
    fs.symlinkSync(realBotHome, path.join(linkedHome, ".kind-meitner"));

    const viaLink = path.join(linkedHome, ".kind-meitner", "report.docx");
...
describe("save-file dialog default name", () => {
  it("suggests a name that does not overwrite an existing file", async () => {
    const downloads = fs.mkdtempSync(path.join(os.tmpdir(), "kind-meitner-downloads-"));
  ...
  it("keeps the extension on the suggestion", async () => {
    const downloads = fs.mkdtempSync(path.join(os.tmpdir(), "kind-meitner-downloads-ext-"));
    const source = path.join(botHome, "workspaces", "bot", "report.docx");
    fs.writeFileSync(path.join(downloads, "report.docx"), "");
