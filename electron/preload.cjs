...
const desktopRemoteClient = process.argv.includes("--kind-meitner-remote-client");
...
const localOrigin = process.argv.find((arg) => arg.startsWith("--kind-meitner-local-origin="))?.slice("--kind-meitner-local-origin=".length) ?? null;
...
const bridge = {
  ...
  /** Pair this desktop app to another kind-meitner host. The bearer remains in
  ...
  /** A reviewed BotMRR package opened through kind-meitner://install. */
  onPackageInstall: (cb) => {
  ...
  /** Ask where to save a bot-created file (inside ~/.kind-meitner), copy it
