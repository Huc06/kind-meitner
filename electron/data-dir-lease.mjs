...
const LEASE_NAME = "kind-meitner-server.lease";
const DELEGATED_CHILD_DIR = ".kind-meitner-server-child";
...
const CHILD_LEASE_ENV = "KIND_MEITNER_INTERNAL_DATA_DIR_LEASE";
...
function parseRecord(path, invalidMessage, validate) {
  ...
  try {
    ...
    throw leaseError("kind-meitner cannot read the data-directory lease; refusing to start to protect its state.", error);
  }
  let record;
  try {
...
function readOwner(path) {
  return parseRecord(
    path,
    "The kind-meitner data-directory lease is invalid; refusing to start to protect its state.",
    isLeaseOwner,
  );
}
...
function readReaper(path, targetToken) {
  return parseRecord(
    path,
    "The kind-meitner stale-lease recovery record is invalid; refusing to start to protect its state.",
...
function processIsAlive(pid) {
  try {
    ...
    throw leaseError("kind-meitner could not verify the data-directory lease owner; refusing to start.", error);
  }
}
...
function publishRecord(path, record, prepareMessage, acquireMessage) {
  ...
  try {
    ...
    unlinkExact(candidatePath, "kind-meitner could not remove its lease candidate.");
  }
}
...
function claimReaperAuthority(leasePath, expected) {
  ...
  for (let generation = 0; generation < MAX_REAPER_GENERATIONS; generation += 1) {
    ...
    if (publishRecord(
      reaperPath,
      candidate,
      "kind-meitner could not prepare stale-lease recovery.",
      "kind-meitner could not safely recover the stale data-directory lease.",
    )) return true;
    ...
    if (current.host !== candidate.host) {
      throw leaseError(
        `A stale kind-meitner data-directory lease is being recovered on another machine. Recovery record: ${JSON.stringify(reaperPath)}.`,
      );
    }
    if (ownerIsAlive(current)) return false;
    reaperPath = successorReaperPath(leasePath, expected.token, current.token);
  }
  throw leaseError("kind-meitner could not recover the stale data-directory lease after repeated interrupted attempts.");
}
...
function retireDeadOwner(leasePath, expected) {
  ...
  if (current.host !== hostname()) {
    throw leaseError(
      `The stale kind-meitner data-directory lease changed ownership to another machine. Lease record: ${JSON.stringify(leasePath)}.`,
    );
  }
  if (ownerIsAlive(current)) return false;
  unlinkExact(leasePath, "kind-meitner could not retire the stale data-directory lease.");
  return true;
}

function validateDataDir(dataDir) {
  if (typeof dataDir !== "string" || dataDir.trim().length === 0 || /[\r\n\0]/.test(dataDir)) {
    throw leaseError("kind-meitner cannot lease an invalid data directory.");
  }
  return dataDir;
}
...
function prepareDataDir(dataDir, legacyDataDir) {
  ...
  try {
    ...
    throw leaseError("kind-meitner cannot create its data directory.", error);
  }
...
function assertNoLiveDelegatedChild(dataDir) {
  ...
  if (child.host !== hostname()) {
    throw leaseError(
      `This kind-meitner data directory still has a delegated server on another machine. Delegated server lease: ${JSON.stringify(childLeasePath)}.`,
    );
  }
  if (ownerIsAlive(child)) {
    throw leaseError(
      `kind-meitner's previous server process ${child.pid} is still shutting down. Try again shortly.`,
    );
  }
}
...
function consumeChildCapability(environment) {
  ...
  try {
    ...
    throw leaseError("kind-meitner could not consume its private desktop lease delegation.", error);
  }
  if (environment[CHILD_LEASE_ENV] !== undefined) {
    throw leaseError("kind-meitner could not consume its private desktop lease delegation.");
  }
  return value;
}
...
function validateChildDelegation(dataDir, encoded) {
  ...
  const invalid = () => leaseError("The kind-meitner desktop lease delegation is invalid; refusing to start to protect its state.");
...
function acquireDataDirLeaseInternal(dataDir, options = {}) {
  ...
  try {
    ...
    throw leaseError("kind-meitner cannot prepare its data-directory lease.", error);
  }
    ...
    for (;;) {
      try {
        ...
        if (error?.code !== "EEXIST") {
          throw leaseError("kind-meitner cannot acquire its data-directory lease.", error);
        }
      }
      ...
      if (current.host !== owner.host) {
        throw leaseError(
          `This kind-meitner data directory is already owned by a process on another machine. Lease record: ${JSON.stringify(leasePath)}.`,
        );
      }
      if (ownerIsAlive(current)) {
        throw leaseError(
          `kind-meitner is already using this data directory (process ${current.pid}). Close the other instance first. If no kind-meitner is running, delete this lease record and start again: ${JSON.stringify(leasePath)}.`,
        );
      }
      if (!retireDeadOwner(leasePath, current)) {
        throw leaseError("A stale kind-meitner data-directory lease is already being recovered; try again shortly.");
      }
    }
  } finally {
    unlinkExact(candidatePath, "kind-meitner could not remove its lease candidate.");
  ...
  if (!acquired) throw leaseError("kind-meitner could not acquire its data-directory lease.");
  let released = false;
  return Object.freeze({
    ownerPid: owner.pid,
    delegated: false,
    release() {
      ...
      if (!current || current.pid !== owner.pid || current.host !== owner.host || current.token !== owner.token) {
        throw leaseError("kind-meitner will not release a data-directory lease owned by another process.");
      }
      try {
        unlinkSync(leasePath);
      } catch (error) {
        throw leaseError("kind-meitner could not release its data-directory lease.", error);
      }
      released = true;
      return true;
    },
    utilityServerLeaseEnvironment() {
      if (released) throw leaseError("kind-meitner cannot delegate a released data-directory lease.");
      return Object.freeze({ [CHILD_LEASE_ENV]: capabilityFor(owner) });
    },
  });
}

/** Claim exclusive ownership of one persistent kind-meitner data directory. */
