  ...
  const routeError = (manual, error) => {
    ...
    if (nativeStagingStarted) {
      ...
      setState({
        status: "error",
        retryable: false,
        message: `${updateErrorMessage(error)} Quit and reopen kind-meitner before trying the update again.`,
      });
      return;
    }
    if (!manual) {
  ...
  function install() {
    ...
    if (installOperation === operation) {
      operation.timer = setTimeout(() => {
        ...
        setState({ status: "installing", message: "Restart is taking longer than expected. Quit and reopen kind-meitner to finish the update." });
      }, 2 * 60 * 1000);
