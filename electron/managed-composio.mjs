...
export function managedComposioChildEnvironment(brokerUrl, credentials, environment) {
  ...
  delete next.KIND_MEITNER_COMPOSIO_BROKER_URL;
  delete next.KIND_MEITNER_COMPOSIO_BROKER_TOKEN;
  const access = managedComposioAccess(brokerUrl, credentials);
  if (access) {
    next.KIND_MEITNER_COMPOSIO_BROKER_URL = access.url;
    next.KIND_MEITNER_COMPOSIO_BROKER_TOKEN = access.token;
  }
  return next;
}
