...
test("the shipped installer replaces the AppImage and queues its original path for relaunch", (t) => {
  ...
  const workspace = mkdtempSync(join(tmpdir(), "kind-meitner-appimage-install-"));
  ...
  const launched = join(workspace, "kind-meitner-0.1.43-x86_64.AppImage");
  const staged = join(workspace, "pending", "kind-meitner-0.1.44-x86_64.AppImage");
...
test("a rejected AppImage relaunch is reported without quitting the desktop", (t) => {
  ...
  const workspace = mkdtempSync(join(tmpdir(), "kind-meitner-appimage-relaunch-failed-"));
  t.after(() => rmSync(workspace, { recursive: true, force: true }));
  const launched = join(workspace, "kind-meitner.AppImage");
...
test("the running AppImage is never removed before its replacement is in place", (t) => {
  ...
  const { AppImageUpdater } = createRequire(import.meta.url)("./vendor/electron-updater.cjs");
  const workspace = mkdtempSync(join(tmpdir(), "kind-meitner-appimage-failed-"));
  t.after(() => rmSync(workspace, { recursive: true, force: true }));
...
test("Electron relaunch waits for deferred cleanup and the replacement acquires the same data directory", {
  ...
  const workspace = mkdtempSync(join(tmpdir(), "kind-meitner-appimage-relaunch-"));
  t.after(() => rmSync(workspace, { recursive: true, force: true }));
  const launched = join(workspace, "kind-meitner-1.0.0.AppImage");
  const staged = join(workspace, "kind-meitner-2.0.0.AppImage");
  ...
  const replacement = `
    ...
    (async () => {
      ...
      try { process.kill(Number(process.env.KIND_MEITNER_UPDATER_TEST_PARENT), 0); }
  ...
  writeFileSync(fixture, `
    ...
    app.whenReady().then(async () => {
      ...
      process.env.KIND_MEITNER_UPDATER_TEST_PARENT = String(process.pid);
