...
async function fixture(t) {
  const dir = await realpath(await mkdtemp(path.join(tmpdir(), "kind-meitner-shared-access-")));
...
function stubWorkspace() {
  ...
  const fetchImpl = async (url, init) => {
    ...
    if (route === "/.well-known/kind-meitner/environment") return json({ environmentId, capabilities: { sharedComputers: true } });
...
test("a protected directory spelled in another case is still refused", async t => {
  ...
  await mkdir(path.join(dir, "kind-meitner"));
  await writeFile(path.join(dir, "kind-meitner", "credentials.bin"), "credential blob");
  grant.protectedPaths = [path.join(dir, "kind-meitner")];
  folder.write = true;
...
test("a harness data directory that does not exist yet still saves and connects", async t => {
  ...
  const sharing = createComputerSharing({
    file: path.join(dir, "profile", "computer-sharing.json"), fetch: stub.fetchImpl,
    environments: () => [stub.env], enabled: async () => true, cuaConnection: async () => null, protectedPaths: [path.join(dir, "never-installed", ".kind-meitner")],
  });
  t.after(() => sharing.close());
  const info = await sharing.identity(stub.env);
