...
3. Restart kind-meitner after installation.
...
Run the provider's own status or login command in a terminal. kind-meitner uses the CLI's authenticated session; installing the binary alone is not enough.

A `401 Unauthorized` response from `api.openai.com` means the launched Codex process did not receive valid authentication. Confirm `codex login status`, sign in again if needed, then restart kind-meitner. Do not paste a ChatGPT password into kind-meitner.
...
- Try the provider CLI directly with the same model to separate an account problem from a kind-meitner integration problem.
