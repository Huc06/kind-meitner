...
Install and sign in to at least one supported agent CLI. kind-meitner detects installed engines during onboarding and again from **App settings → Agent engines**.

<ProductScreenshot name="docs-engine-detection" alt="kind-meitner detecting installed agent engines during onboarding" caption="Detection is automatic. Engines that need installation or sign-in include the exact next step." />
...
<Callout type="warn" title="Authentication belongs to the CLI">
  kind-meitner launches agent tools you installed. If an engine returns a 401 or asks you to sign in, finish that engine's own CLI login, then refresh detection in App settings.
</Callout>
