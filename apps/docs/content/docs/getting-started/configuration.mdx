---
title: Configuration
description: Server-only portal credentials, and the local engine boundary.
icon: Settings
---

OKX Developer Portal credentials are server-only. The browser does not collect API keys, passphrases, webhook secrets, or recipient addresses.

The rest of this page is local runtime: how the suite finds an engine CLI and where optional third-party keys live.

## Engine discovery

kind-meitner checks the inherited process path, common install locations, and the login shell. If an engine is installed somewhere unusual, open **Settings → Engines** and choose its executable explicitly.
...
- launch kind-meitner from a Windows shortcut or Linux application menu with a different environment from your terminal.
...
Source and headless installations can give separate Claude instances different built-in tool sets in `~/.kind-meitner/config.json`:
