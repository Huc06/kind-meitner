...
description: Optional local tools for engine-backed bots. Not the OKX agent catalog.
icon: PlugZap
---

Connected apps are optional local runtime. They authorize Gmail, GitHub, Slack, and other tools for an engine-backed bot. They are not how you invite an OKX agent.


<ProductScreenshot name="docs-connected-apps" alt="The clea kind-meitner connected-app marketplace" caption="Search the marketplace or review every account from the Connected tab." />

## Official packaged app

The signed desktop app registers its own installation with kind-meitner's managed connected-apps service. You do **not** need to find or paste a Composio key.
...
The official managed kind-meitner service keeps X disabled until it owns an appropriate X OAuth app. Older kind-meitner tasks that refer to the connector as `x` continue to work; kind-meitner translates that alias to Composio's `twitter` toolkit automatically.

## Multiple accounts

Connect more than one account for the same toolkit and give each a clear alias such as `work`, `personal`, or `client-acme`. When several accounts could perform an action, kind-meitner requires explicit selection instead of silently replacing the first connection.
