...
description: A map of the capabilities currently available in kind-meitner.
icon: Sparkles
---

kind-meitner turns agent CLIs into a messaging-style workspace. The harness normalizes each provider into one event stream, so the interface can treat different agents consistently while preserving their native strengths.
