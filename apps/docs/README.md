...
```bash
pnpm docs:build
pnpm --filter @kind-meitner/docs types:check
pnpm --filter @kind-meitner/docs lint
```

The changelog reads published releases from `harrymove-ctrl/kind-meitner` plus the
...
Create a second Vercel project beside the existing `kind-meitner.com` project:

1. Import the `harrymove-ctrl/kind-meitner` repository.
...
5. Add `docs.kind-meitner.com` under **Settings → Domains**.
...
preview URLs for documentation pull requests. Keep `kind-meitner.com` on the
