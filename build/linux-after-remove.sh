...
if [ -n "${KIND_MEITNER_POSTINSTALL_TEST_ROOT:-}" ]; then
  ...
  case "$TEST_ROOT" in
    /tmp/*) ;;
    *) echo "kind-meitner test install root must stay under /tmp" >&2; exit 1 ;;
  esac
...
if [ "$TEST_MODE" -eq 0 ]; then
  if command -v update-alternatives >/dev/null 2>&1; then
    update-alternatives --remove kind-meitner /opt/kind-meitner/kind-meitner
  elif [ -L /usr/bin/kind-meitner ] && [ "$(readlink /usr/bin/kind-meitner)" = /opt/kind-meitner/kind-meitner ]; then
    rm -- /usr/bin/kind-meitner
  fi
fi

profile=$APPARMOR_DIR/kind-meitner-browser
if [ ! -e "$profile" ] && [ ! -L "$profile" ]; then exit 0; fi
if [ -L "$APPARMOR_DIR" ] || [ -L "$profile" ] || [ ! -f "$profile" ]; then
  echo "kind-meitner AppArmor profile is unsafe; refusing to remove it: $profile" >&2
  exit 1
fi
if [ "$TEST_MODE" -eq 0 ] && [ -x /usr/bin/ischroot ] && /usr/bin/ischroot; then
...
elif [ -r "$APPARMOR_PROFILES" ] && ! grep -q '^kind-meitner-browser ' "$APPARMOR_PROFILES"; then
  ...
  if ! "$APPARMOR_PARSER" -R "$profile"; then
    echo "kind-meitner could not unload its browser AppArmor profile: $profile" >&2
    exit 1
  fi
elif [ -r "$APPARMOR_PROFILES" ]; then
  echo "kind-meitner needs apparmor_parser to unload its browser profile: $profile" >&2
  exit 1
fi
rm -- "$profile"
