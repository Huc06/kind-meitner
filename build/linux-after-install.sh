...
# kind-meitner 0.1.7 installed the application ancestors as 0775, which makes
...
if [ -n "${KIND_MEITNER_POSTINSTALL_TEST_ROOT:-}" ]; then
  TEST_ROOT="$(realpath -e -- "$KIND_MEITNER_POSTINSTALL_TEST_ROOT")"
  case "$TEST_ROOT" in
    /tmp/*) APP_ROOT=$TEST_ROOT ;;
    *)
      echo "kind-meitner test install root must stay under /tmp" >&2
      exit 1
      ;;
  esac
  ...
  if [ -L /opt ] || [ "$(stat -c '%U:%G:%a' -- /opt)" != root:root:755 ]; then
    echo "kind-meitner package ancestor is unsafe: /opt must be root:root 0755" >&2
    exit 1
  fi
fi
...
repair_directory() {
  ...
  if [ -L "$target" ] || [ ! -d "$target" ]; then
    echo "kind-meitner package directory is missing or unsafe: $target" >&2
    exit 1
  fi
  ...
  if [ "$actual" != "$EXPECTED_OWNER:755" ]; then
    echo "kind-meitner could not secure package directory: $target ($actual)" >&2
    exit 1
  fi
}
...
repair_executable() {
  ...
  if [ -L "$target" ] || [ ! -f "$target" ]; then
    echo "kind-meitner package executable is missing or unsafe: $target" >&2
    exit 1
  fi
  if [ "$TEST_MODE" -eq 0 ]; then chown root:root -- "$target"; fi
  chmod 0755 -- "$target"
  actual="$(stat -c '%U:%G:%a' -- "$target")"
  if [ "$actual" != "$EXPECTED_OWNER:755" ]; then
    echo "kind-meitner could not secure package executable: $target ($actual)" >&2
    exit 1
  fi
}
...
repair_chromium_sandbox() {
  target=$1
  if [ -L "$target" ] || [ ! -f "$target" ]; then
    echo "kind-meitner Chromium sandbox is missing or unsafe: $target" >&2
    exit 1
  fi
  if [ "$TEST_MODE" -eq 0 ]; then chown root:root -- "$target"; fi
  chmod 4755 -- "$target"
  ...
  if [ "$actual" != "$EXPECTED_OWNER:4755" ]; then
    echo "kind-meitner could not secure Chromium sandbox: $target ($actual)" >&2
    exit 1
  fi
}
...
secure_browser_tree() {
  ...
  if [ -n "$unsafe" ]; then
    echo "kind-meitner browser resource is missing or unsafe: $unsafe" >&2
    exit 1
  fi
  unsafe=$(find "$BROWSER_ROOT" -type f -links +1 -print -quit)
  if [ -n "$unsafe" ]; then
    echo "kind-meitner browser resource has an unsafe hard link: $unsafe" >&2
    exit 1
  fi
...
install_browser_apparmor_profile() {
  profile=$APP_ROOT/resources/kind-meitner-browser.apparmor
  if [ -L "$profile" ] || [ ! -f "$profile" ] || [ "$(stat -c '%h' -- "$profile")" != 1 ]; then
    echo "kind-meitner browser AppArmor profile is missing or unsafe: $profile" >&2
    exit 1
  fi
  ...
  if [ -f "$USERNS_RESTRICTION" ]; then
    ...
    if ! restricted=$(cat -- "$USERNS_RESTRICTION"); then
      echo "kind-meitner could not read the browser user-namespace restriction." >&2
      exit 1
    fi
    case "$restricted" in
      0|1) ;;
      *)
        echo "kind-meitner browser user-namespace restriction is invalid; refusing unsafe sandbox setup." >&2
        exit 1
        ;;
    esac
  fi
  ...
  if [ ! -d "$APPARMOR_DIR" ] || [ ! -x "$APPARMOR_PARSER" ]; then
    if [ "$restricted" = 1 ]; then
      echo "kind-meitner needs apparmor and apparmor_parser to configure the browser sandbox on this host." >&2
      exit 1
    fi
    return
  fi
  ...
  if [ -L "$APPARMOR_DIR" ] || [ "$(stat -c '%U:%G:%a' -- "$APPARMOR_DIR")" != "$EXPECTED_OWNER:755" ]; then
    echo "kind-meitner AppArmor policy directory is unsafe: $APPARMOR_DIR" >&2
    exit 1
  fi
  target=$APPARMOR_DIR/kind-meitner-browser
  if [ -L "$target" ] || { [ -e "$target" ] && [ ! -f "$target" ]; }; then
    echo "kind-meitner AppArmor profile target is unsafe: $target" >&2
    exit 1
  fi
  if [ "$TEST_MODE" -eq 0 ]; then
  ...
  if [ ! -x "$APPARMOR_STATUS" ] || ! "$APPARMOR_STATUS" --enabled >/dev/null 2>&1; then
    if [ "$restricted" = 1 ]; then
      echo "kind-meitner cannot load its browser sandbox policy while AppArmor restrictions are active but AppArmor is unavailable." >&2
      exit 1
    fi
    return
  fi
